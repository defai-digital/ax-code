...
export const globalBin = (name: string, platform = process.platform) =>
  path.join(codeIntelHost().binDir(), name + (platform === "win32" ? ".exe" : ""))

export const globalPath = () => (process.env["PATH"] ?? "") + path.delimiter + codeIntelHost().binDir()
...
export const resolveTypescriptServer = (input: { directory?: string; resolve?: ModuleResolver } = {}) =>
  (input.resolve ?? Module.resolve)(TYPESCRIPT_SERVER_MODULE, input.directory ?? codeIntelHost().projectRoot())
...
export const ensureTool = async (input: {
  ...
  if (codeIntelHost().flags().disableLspDownload) return
...
export const bunSpawnInfo = (
  ...
  const handle = {
    process: spawn(codeIntelHost().runtime.executable(), [script, ...args], {
      cwd: root,
      env: bunEnv(),
    }),
  }
...
export const toolSpawnInfo = (
  ...
  const runner = codeIntelHost().runtime.toolRunner()
...
export const resolveManagedToolBin = async (input: {
  ...
  if (input.installedBin && !Filesystem.contains(codeIntelHost().binDir(), input.installedBin))
...
// would try to execute a file named "run"). The bin is codeIntelHost().runtime.executable(), which
...
export const bunServer = async (input: {
  ...
  if (!bin) {
    if (!(await Filesystem.exists(input.script))) {
      if (codeIntelHost().flags().disableLspDownload) return
      // On the Node runtime codeIntelHost().runtime.executable() is `node`, so `node install` is
      // meaningless — install with npm (bundled with Node) instead.
      const installCmd =
        codeIntelHost().runtime.kind() === "node"
          ? [codeIntelHost().runtime.npmExecutable(), "install", "--prefix", codeIntelHost().binDir(), input.pkg]
          : [codeIntelHost().runtime.executable(), "install", input.pkg]
    ...
    bin = codeIntelHost().runtime.executable() ?? null
    if (!bin) return
