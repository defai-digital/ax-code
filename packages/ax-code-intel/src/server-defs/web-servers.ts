...
export const Deno: Info = {
  ...
  root: async (file) => {
    const files = Filesystem.up({
      ...
      stop: codeIntelHost().projectRoot(),
    })
...
export const Vue: Info = {
  id: "vue",
  extensions: [".vue"],
  root: NearestRoot([...JS_LOCKFILE_ROOT_MARKERS]),
  async spawn(root) {
...
export const ESLint: Info = {
  ...
  async spawn(root) {
    ...
    const eslint = Module.resolve("eslint", codeIntelHost().projectRoot())
    if (!eslint) return
    ...
    const legacyServer = path.join(codeIntelHost().binDir(), "vscode-eslint", "server", "out", "eslintServer.js")
...
export const Oxlint: Info = {
  ...
  async spawn(root) {
    ...
    const resolveBin = async (target: string) => {
      ...
      const candidates = Filesystem.up({ targets: [target], start: root, stop: codeIntelHost().worktreeRoot() })
...
export const Biome: Info = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      if (!resolved) return
      const tool = codeIntelHost().runtime.toolRunner()
...
export const Ty: Info = {
  ...
  async spawn(root) {
    if (!codeIntelHost().flags().experimentalLspTy) return undefined
    let binary = which("ty")
    const initialization: Record<string, string> = {}
    const python = await venvPython(root)
    if (python) initialization["pythonPath"] = python
...
export const BashLS: Info = {
  ...
  root: async () => codeIntelHost().projectRoot(),
  async spawn(root) {
    return bunServerHandle({
      root,
