...
const keepSandbox = process.env.AX_CODE_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  AX_CODE_DISABLE_SHARE: process.env.AX_CODE_DISABLE_SHARE ?? "true",
  AX_CODE_DISABLE_LSP_DOWNLOAD: "true",
  AX_CODE_DISABLE_DEFAULT_PLUGINS: "true",
  AX_CODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  AX_CODE_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  AX_CODE_E2E_PROJECT_DIR: repoDir,
  AX_CODE_E2E_SESSION_TITLE: "E2E Session",
  AX_CODE_E2E_MESSAGE: "Seeded for UI e2e",
  AX_CODE_E2E_MODEL: "opencode/gpt-5-nano",
  AX_CODE_CLIENT: "app",
  AX_CODE_STRICT_CONFIG_DEPS: "true",
...
const runnerEnv = {
  ...
  VITE_AX_CODE_SERVER_HOST: "127.0.0.1",
  VITE_AX_CODE_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    console.log(`ax-code server listening on http://127.0.0.1:${serverPort}`)
