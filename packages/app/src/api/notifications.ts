import type { NotificationPayload, NotificationsAPI } from '@ax-code/app-ui/lib/api/types';
...
const notifyWithServiceWorker = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    await registration.showNotification(payload?.title ?? 'ax-code', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
...
const notifyWithWebAPI = async (payload?: NotificationPayload): Promise<boolean> => {
  ...
  try {
    ...
    new Notification(payload?.title ?? 'ax-code', {
      body: payload?.body,
      tag: payload?.tag,
    });
    return true;
