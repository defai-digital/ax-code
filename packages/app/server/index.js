...
import { registerAxCodeRoutes } from './lib/opencode/ax-code-routes.js';
...
const DESKTOP_NOTIFY_PREFIX = '[AxCodeDesktopNotify] ';
const uiNotificationClients = new Set();
const uiNotificationWsClients = new Set();
const uiAxCodeEventClients = new Set();
...
const AX_CODE_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'ax-code');
const OPENCHAMBER_USER_THEMES_DIR = path.join(AX_CODE_USER_CONFIG_ROOT, 'themes');
const OPENCHAMBER_PROJECTS_CONFIG_DIR = path.join(AX_CODE_USER_CONFIG_ROOT, 'projects');
...
const AX_CODE_DATA_DIR = process.env.AX_CODE_DATA_DIR
  ? path.resolve(process.env.AX_CODE_DATA_DIR)
  : path.join(os.homedir(), '.config', 'ax-code');
const SETTINGS_FILE_PATH = path.join(AX_CODE_DATA_DIR, 'settings.json');
const PUSH_SUBSCRIPTIONS_FILE_PATH = path.join(AX_CODE_DATA_DIR, 'push-subscriptions.json');
const CLOUDFLARE_MANAGED_REMOTE_TUNNELS_FILE_PATH = path.join(AX_CODE_DATA_DIR, 'cloudflare-managed-remote-tunnels.json');
const CLOUDFLARE_LEGACY_NAMED_TUNNELS_FILE_PATH = path.join(AX_CODE_DATA_DIR, 'cloudflare-named-tunnels.json');
...
const ENV_DESKTOP_NOTIFY = (() => {
  ...
  return /ax-code-server/i.test(argv0) || /ax-code-server/i.test(argv1);
})();
...
const scheduledTasksRuntime = createScheduledTasksRuntime({
  ...
  emitTaskRunEvent: (event) => {
    for (const client of uiAxCodeEventClients) {
      try {
        writeSseEvent(client, {
          type: 'ax-code:scheduled-task-ran',
          properties: {
        ...
        uiAxCodeEventClients.delete(client);
      }
    }
  },
  logger: console,
...
async function main(options = {}) {
  ...
  console.log(`Starting ax-code on port ${port === 0 ? 'auto' : port}`);
  ...
  const bootstrapResult = bootstrapRuntime.setupBaseRoutes(app, {
    ...
    server,
    __dirname,
    axCodeDataDir: AX_CODE_DATA_DIR,
  ...
  await featureRoutesRuntime.registerRoutes(app, {
    ...
    axCodeDataDir: AX_CODE_DATA_DIR,
    axCodeUserConfigRoot: AX_CODE_USER_CONFIG_ROOT,
    normalizeDirectoryPath,
    resolveProjectDirectory,
    ...
    projectConfigRuntime,
    scheduledTasksRuntime,
    getAxCodeEventClients: () => uiAxCodeEventClients,
    writeSseEvent,
  });
