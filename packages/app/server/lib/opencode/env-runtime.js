...
export const createOpenCodeEnvRuntime = (deps) => {
  ...
  const createConfiguredOpencodeBinaryError = (raw, normalized) => {
    ...
    const messageSuffix = 'ax-code needs the standalone opencode CLI. Install it and set settings.opencodeBinary to the CLI path, for example ~/.opencode/bin/opencode, or leave the setting empty to use PATH lookup.';
    const error = (() => {
  ...
  const createConfiguredWslOpencodeError = (raw) => new Error(
    `Configured settings.opencodeBinary uses WSL but ax-code could not resolve a WSL OpenCode command: ${raw}. Ensure WSL is available and opencode is installed in the configured distro.`
  );
