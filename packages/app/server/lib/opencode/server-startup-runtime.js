        ...
        try {
          ...
          if (typeof process.send === 'function') {
            if (!process.connected) {
              throw new Error('ax-code startup IPC channel disconnected before ready notification');
            }

            await new Promise((resolveReadyNotification, rejectReadyNotification) => {
              try {
                process.send({ type: 'ax-code:ready', port: activePort }, (error) => {
                  if (error) {
          ...
          console.log(`ax-code server listening on ${bindHost}:${activePort}`);
