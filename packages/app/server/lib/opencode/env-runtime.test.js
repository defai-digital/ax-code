...
const originalax-codeWslBinary = process.env.OPENCHAMBER_WSL_BINARY;
...
afterEach(() => {
  ...
  if (typeof originalax-codeWslBinary === 'string') {
    process.env.OPENCHAMBER_WSL_BINARY = originalax-codeWslBinary;
  } else {
...
describe('OpenCode env runtime', () => {
  ...
  it('throws a specific error for a configured directory without an executable CLI in strict mode', async () => {
    const dir = createTempDir('ax-code-opencode-dir-');
  ...
  it('applies a valid configured executable OpenCode binary', async () => {
    const dir = createTempDir('ax-code-opencode-bin-');
  ...
  it('does not classify WSL settings as a native invalid configured binary in strict mode', async () => {
    setPlatform('win32');
    const dir = createTempDir('ax-code-no-wsl-');
    process.env.PATH = dir;
    process.env.SystemRoot = dir;
    process.env.WSL_BINARY = path.join(dir, 'missing-wsl.exe');
    process.env.OPENCHAMBER_WSL_BINARY = path.join(dir, 'missing-ax-code-wsl.exe');
  ...
  it('launches Windows cmd shims through cmd call without embedded quotes', () => {
    ...
    const dir = createTempDir('ax-code-opencode-cmd-');
  ...
  it('resolves npm OpenCode cmd shims to the packaged Windows executable', () => {
    setPlatform('win32');
    const npmDir = createTempDir('ax-code-opencode-npm-');
