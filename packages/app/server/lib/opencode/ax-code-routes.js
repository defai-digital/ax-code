export const registerAxCodeRoutes = (app, dependencies) => {
  ...
  app.get('/api/ax-code/update-check', async (req, res) => {
    try {
  ...
  app.post('/api/ax-code/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(axCodeDataDir, 'run', `ax-code-${currentPort}.json`);
      ...
      let restartCmdFallback = `ax-code serve --port ${storedOptions.port}`;
      ...
      const updateLogPath = path.join(axCodeDataDir, 'update-install.log');
      const logPreamble = [
        '',
        `=== ax-code update ${new Date().toISOString()} ===`,
      ...
      res.json({
        ...
        setTimeout(() => {
          ...
          const script = isWindows
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting ax-code...
              ${restartCmd || 'echo Service manager will restart ax-code.'}
            ) else (
              echo Update failed
              exit /b 1
            )
  ...
  app.get('/api/ax-code/models-metadata', async (_req, res) => {
    const now = Date.now();
