...
export const registerScheduledTaskRoutes = (app, dependencies) => {
  ...
  app.get('/api/ax-code/scheduled-tasks/status', async (_req, res) => {
    try {
  ...
  app.get('/api/ax-code/events', (req, res) => {
    ...
    const clients = getAxCodeEventClients();
    clients.add(res);

    try {
      writeSseEvent(res, {
        type: 'ax-code:event-stream-ready',
        properties: {
