export const createTunnelRoutesRuntime = (dependencies) => {
  ...
  const registerRoutes = (app) => {
    app.get('/api/ax-code/tunnel/check', async (req, res) => {
      try {
    ...
    app.post('/api/ax-code/tunnel/doctor', handleTunnelDoctor);
    app.get('/api/ax-code/tunnel/doctor', handleTunnelDoctor);

    app.get('/api/ax-code/tunnel/providers', (_req, res) => {
      const providers = tunnelProviderRegistry.listCapabilities();
      return res.json({ providers });
    });

    app.get('/api/ax-code/tunnel/status', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.put('/api/ax-code/tunnel/managed-remote-token', async (req, res) => {
      try {
    ...
    app.post('/api/ax-code/tunnel/start', async (_req, res) => {
      try {
        const settings = await readSettingsFromDiskMigrated();
    ...
    app.post('/api/ax-code/tunnel/stop', (_req, res) => {
      let revokedBootstrapCount = 0;
      let invalidatedSessionCount = 0;
      const activeTunnelId = tunnelAuthController.getActiveTunnelId();
