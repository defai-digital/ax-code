...
const PREVIEW_BRIDGE_SCRIPT_ID = 'ax-code-preview-bridge';
...
const PREVIEW_BRIDGE_SCRIPT = String.raw`(() => {
  if (window.__axCodePreviewBridgeInstalled) return;
  window.__axCodePreviewBridgeInstalled = true;

  const SOURCE = 'ax-code-preview-bridge';
  const VERSION = 1;
  const MAX_TEXT = 500;
  const MAX_ARG = 1000;
  ...
  const installColorSchemeMatchMediaPatch = () => {
    if (window.__axCodePreviewColorSchemePatched || typeof window.matchMedia !== 'function') return;
    window.__axCodePreviewColorSchemePatched = true;
  ...
  const applyPreviewColorScheme = (scheme) => {
    ...
    try {
      ...
      root.dataset.axCodePreviewColorScheme = next;
  ...
  const installViteHmrProxyPatch = () => {
    if (window.__axCodeViteHmrProxyPatched || typeof window.WebSocket !== 'function') return;
    window.__axCodeViteHmrProxyPatched = true;
    ...
    function AxCodePreviewWebSocket(url, protocols) {
      const protocolList = Array.isArray(protocols) ? protocols : [protocols];
    ...
    AxCodePreviewWebSocket.prototype = NativeWebSocket.prototype;
    Object.setPrototypeOf(AxCodePreviewWebSocket, NativeWebSocket);
    Object.defineProperty(AxCodePreviewWebSocket, 'name', { value: 'WebSocket' });
    window.WebSocket = AxCodePreviewWebSocket;
  };

  const installAppRequestProxyPatch = () => {
    if (window.__axCodeAppRequestProxyPatched) return;
    window.__axCodeAppRequestProxyPatched = true;
    const proxyMatch = window.location.pathname.match(/^(\/api\/preview\/proxy\/[a-f0-9]{16,64})(?:\/|$)/i);
    if (!proxyMatch) return;
    const proxyBase = proxyMatch[1];
    ...
    if (typeof window.EventSource === 'function') {
      ...
      function AxCodePreviewEventSource(url, eventSourceInitDict) {
        return new NativeEventSource(proxiedUrl(String(url)), eventSourceInitDict);
      }
      AxCodePreviewEventSource.prototype = NativeEventSource.prototype;
      Object.setPrototypeOf(AxCodePreviewEventSource, NativeEventSource);
      Object.defineProperty(AxCodePreviewEventSource, 'name', { value: 'EventSource' });
      window.EventSource = AxCodePreviewEventSource;
    }

    if (typeof window.WebSocket === 'function') {
      const NativeWebSocket = window.WebSocket;
      function AxCodePreviewAppWebSocket(url, protocols) {
        const nextUrl = proxiedWebSocketUrl(String(url));
        return arguments.length === 1
          ? new NativeWebSocket(nextUrl)
          : new NativeWebSocket(nextUrl, protocols);
      }
      AxCodePreviewAppWebSocket.prototype = NativeWebSocket.prototype;
      Object.setPrototypeOf(AxCodePreviewAppWebSocket, NativeWebSocket);
      Object.defineProperty(AxCodePreviewAppWebSocket, 'name', { value: 'WebSocket' });
      window.WebSocket = AxCodePreviewAppWebSocket;
    }
  };
  ...
  const sendHover = (event) => {
    ...
    pendingHover = event;
    if (window.__axCodePreviewHoverFrame) return;
    window.__axCodePreviewHoverFrame = window.requestAnimationFrame(() => {
      window.__axCodePreviewHoverFrame = 0;
  ...
  window.addEventListener('message', (event) => {
    ...
    if (!data || data.source !== 'ax-code-preview-parent' || data.version !== VERSION) return;
...
const normalizeLoopbackUrl = (rawUrl) => {
  ...
  // Only keep origin here; the proxy path is preserved on the ax-code side.
...
export const createPreviewProxyRuntime = ({
  ...
  const attach = (app, {
    ...
    const proxy = createProxyMiddleware({
      ...
      on: {
        proxyReq: (proxyReq) => {
          // Keep local dev servers from receiving ax-code credentials.
          proxyReq.removeHeader('cookie');
          proxyReq.removeHeader('authorization');
          proxyReq.removeHeader('x-ax-code-ui-session');
          proxyReq.setHeader('accept-encoding', 'identity');
        },
        proxyRes: responseInterceptor(async (responseBuffer, proxyRes, req) => {
          // Allow the dev server response to be framed inside ax-code even
