...
const PACKAGE_NAME = '@ax-code/app';
...
const UPDATE_CHECK_URL = process.env.AX_CODE_UPDATE_API_URL || 'https://api.ax-code.dev/v1/update/check';

function getAxCodeConfigDir() {
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA;
    if (appData) return path.join(appData, 'ax-code');
  }

  return path.join(os.homedir(), '.config', 'ax-code');
}
...
function getOrCreateInstallId(scope = 'web') {
  const configDir = getAxCodeConfigDir();
...
function getOwnedPackagePathsFromGlobalBins(pm) {
  ...
  for (const binDir of getGlobalBinDirs(pm)) {
    const binaryName = process.platform === 'win32' ? 'ax-code-app.cmd' : 'ax-code-app';
...
function isPackageInstalledWith(pm) {
  try {
    ...
    return result.stdout.includes(PACKAGE_NAME) || result.stdout.includes('ax-code-app');
  } catch {
    return false;
  }
}
