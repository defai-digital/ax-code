...
describe('cli entry detection', () => {
  const modulePath = '/tmp/ax-code/bin/cli.js';
  const moduleUrl = pathToFileURL(modulePath).href;

  it('resolves symlinked entry paths before comparing', () => {
    const symlinkPath = '/usr/local/bin/ax-code-app';
  ...
  it('accepts wrapper binary name fallback when requested', () => {
    const wrapperPath = '/home/user/.local/bin/ax-code-app';
    expect(isModuleCliExecution(wrapperPath, moduleUrl, undefined, 'ax-code-app')).toBe(true);
  });
