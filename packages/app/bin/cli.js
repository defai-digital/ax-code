...
const STARTUP_SERVICE_ID = 'dev.ax-code.web';
...
function formatUnsafePortWarning(port) {
  return `Port ${port} is browser-unsafe (ERR_UNSAFE_PORT) and is not supported for ax-code UI at ${buildLocalUrl(port, '/')}.`;
}
...
function buildTunnelStartReplayCommand({
  ...
  const parts = ['ax-code-app', 'tunnel', 'start'];
...
function showHelp() {
  console.log(`
 ax-code - Web interface for the OpenCode AI coding agent
...
ENVIRONMENT:
  ...
  AX_CODE_DATA_DIR         Override ax-code data directory
...
EXAMPLES:
  ...
  ax-code-app startup enable     # Start ax-code at user login
...
COMMON OPTIONS:
  -p, --port              Target ax-code instance port
  --json                  Output machine-readable JSON
...
BEHAVIOR NOTES:
  - One active tunnel per ax-code instance.
...
complete -F _ax_code_tunnel ax-code-app
`;
  }

  if (normalized === 'zsh') {
    return `#compdef ax-code-app
# Zsh completion for ax-code-app tunnel
...
compdef _ax_code_app ax-code-app
`;
  }
...
function getDataDir() {
  if (typeof process.env.AX_CODE_DATA_DIR === 'string' && process.env.AX_CODE_DATA_DIR.trim().length > 0) {
    return path.resolve(process.env.AX_CODE_DATA_DIR.trim());
  }
  return path.join(os.homedir(), '.config', 'ax-code-app');
}
...
async function resolveAvailablePort(desiredPort, explicitPort = false, onNotice) {
  ...
  if (occupant?.runtime === 'desktop') {
    ...
    message = `Port ${startPort} is used by an existing ax-code instance; using a free port`;
  } else {
...
function getStartupServicePaths() {
  ...
  if (process.platform === 'linux') {
    return {
      platform: 'linux',
      servicePath: path.join(os.homedir(), '.config', 'systemd', 'user', 'ax-code-app.service'),
    };
  }
  if (process.platform === 'win32') {
...
function getMacosStartupWrapperPath() {
  return path.join(getDataDir(), 'bin', 'ax-code');
}
...
function collectStartupEnv(options = {}) {
  ...
  if (typeof process.env.AX_CODE_DATA_DIR === 'string' && process.env.AX_CODE_DATA_DIR.trim().length > 0) {
    env.AX_CODE_DATA_DIR = path.resolve(process.env.AX_CODE_DATA_DIR.trim());
  }
  return env;
}
...
function buildMacosLaunchAgent(options = {}) {
  ...
  const env = collectStartupEnv(options);
  const logDir = path.join(os.homedir(), 'Library', 'Logs', 'ax-code');
...
function buildSystemdUserService(options = {}) {
  ...
  const envFilePath = getStartupEnvFilePath();
  return `[Unit]
Description=ax-code web server
...
function getStartupStatus() {
  ...
  if (paths.platform === 'linux') {
    const enabledResult = runStartupCommand('systemctl', ['--user', 'is-enabled', 'ax-code-app.service'], { allowFailure: true });
    const activeResult = runStartupCommand('systemctl', ['--user', 'is-active', 'ax-code-app.service'], { allowFailure: true });
...
function enableStartupService(options = {}) {
  ...
  if (paths.platform === 'macos') {
    ...
    fs.mkdirSync(path.join(os.homedir(), 'Library', 'Logs', 'ax-code'), { recursive: true, mode: 0o700 });
  ...
  if (paths.platform === 'linux') {
    ...
    runStartupCommand('systemctl', ['--user', 'enable', '--now', 'ax-code-app.service']);
    return getStartupStatus();
  }
...
function disableStartupService() {
  ...
  if (paths.platform === 'linux') {
    runStartupCommand('systemctl', ['--user', 'disable', '--now', 'ax-code-app.service'], { allowFailure: true });
    try { fs.unlinkSync(paths.servicePath); } catch {}
...
async function discoverRunningInstances() {
  ...
  try {
    ...
    const pidFiles = files.filter((file) => file.startsWith('ax-code-') && file.endsWith('.pid'));
    for (const file of pidFiles) {
      const port = parseInt(file.replace('ax-code-', '').replace('.pid', ''), 10);
      ...
      const instanceFilePath = path.join(runDir, `ax-code-app-${port}.json`);
      let mtime = 0;
      let startedAt = 0;
      try {
...
async function fetchTunnelProvidersFromPort(port, fetchImpl = globalThis.fetch) {
  ...
  try {
    const response = await fetchImpl(buildLocalUrl(port, '/api/ax-code/tunnel/providers'));
...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running ax-code instance found. Start one with `ax-code-app serve`.');
    }
    return running;
  }
  ...
  if (options.explicitPort) {
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by ax-code Desktop app. Tunnel attach requires a CLI instance from \`ax-code-app serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable ax-code tunnel instance. Ensure it is healthy and running ax-code CLI runtime.`
          );
        }
      }
      return found;
    ...
    throw new Error(`No running ax-code instance found on port ${options.port}.`);
  }
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable ax-code instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only ax-code Desktop instance(s) detected. Tunnel attach requires a CLI instance from `ax-code-app serve`.');
    }

    throw new Error('No attachable ax-code instance found. Start one with `ax-code-app serve`.');
  }
  ...
  throw new Error(`Multiple ax-code instances found: ${ports}. Use --port <port> or --all.`);
}
...
const commands = {
  async serve(options) {
    ...
    if (targetPort !== 0 && !options.suppressUnsafePortWarning) {
      assertSafeBrowserPort(targetPort, { context: 'ax-code serve' });
    }
    ...
    if (targetPort !== 0) {
      ...
      if (existingPid && isProcessRunning(existingPid)) {
        throw new Error(`ax-code is already running on port ${targetPort} (PID: ${existingPid})`);
      }
      ...
      if (explicitPort && !(await isPortAvailable(targetPort, options.host))) {
        ...
        if (systemInfo?.runtime === 'desktop') {
          throw new Error(
            `Port ${targetPort} is used by ax-code Desktop app. Choose another port or stop the desktop app.`
          );
        }
        if (systemInfo?.runtime) {
          throw new Error(`ax-code is already running on port ${targetPort}. Use \`ax-code-app status\` or \`ax-code-app stop --port ${targetPort}\`.`);
        }
    ...
    if (options.foreground) {
      ...
      if (!isQuietMode(options)) {
        console.log(`Starting ax-code on port ${targetPort === 0 ? 'auto' : targetPort} (foreground)`);
      }
    ...
    child.unref();
    serveSpin?.start(`Starting ax-code on port ${targetPort === 0 ? 'auto' : targetPort}...`);
    ...
    try {
      resolvedPort = await new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          if (settled) return;
          settled = true;
          reject(new Error(`ax-code daemon did not report ready within ${DAEMON_READY_TIMEOUT_MS / 1000}s`));
        }, DAEMON_READY_TIMEOUT_MS);

        child.on('message', (msg) => {
          if (settled) return;
          if (msg && msg.type === 'ax-code:ready' && typeof msg.port === 'number') {
            settled = true;
            clearTimeout(timeout);
            resolve(msg.port);
          }
        ...
        child.on('exit', (code, signal) => {
          ...
          settled = true;
          clearTimeout(timeout);
          reject(new Error(`ax-code daemon exited before reporting ready${signal ? ` (${signal})` : ` (code ${code ?? 'unknown'})`}`));
        });
      });
    } catch (error) {
    ...
    if (!isProcessRunning(child.pid)) {
      serveSpin?.error('Failed to start ax-code');
  ...
  async stop(options) {
    ...
    if (runningInstances.length === 0) {
      ...
      if (showOutput) {
        logStatus('info', 'No running ax-code instances found');
      ...
      if (runningInstances.length === 0) {
        ...
        if (systemInfo?.runtime) {
          ...
          if (showOutput && !unmanagedStopSpin) {
            logStatus('info', `found unmanaged ax-code instance on port ${options.port}`, 'attempting shutdown');
          }
          unmanagedStopSpin?.start(`Stopping unmanaged ax-code on port ${options.port}...`);
          ...
          if (stopped) {
            unmanagedStopSpin?.stop(`Stopped unmanaged ax-code on port ${options.port}`);
            jsonResults.push({ port: options.port, runtime: 'unmanaged', stopped: true });
            if (isJsonMode(options)) {
              printJson({ stoppedCount: 1, results: jsonResults });
            }
            if (showOutput && !unmanagedStopSpin) {
              logStatus('success', `stopped ax-code on port ${options.port}`);
            ...
            unmanagedStopSpin?.error(`Could not stop ax-code on port ${options.port}`);
            ...
            if (isJsonMode(options)) {
              printJson({
                ...
                stoppedCount: 0,
                results: jsonResults,
                messages: [{ level: 'error', code: 'STOP_FAILED', message: `Could not stop ax-code on port ${options.port}.` }],
              });
            }
            if (showOutput && !unmanagedStopSpin) {
              logStatus('error', `could not stop ax-code on port ${options.port}`);
              finish('failed');
            }
            printQuietStopResults();
          }
        ...
        if (showOutput) {
          logStatus('info', `no ax-code instance found on port ${options.port}`);
          finish('nothing to stop');
        }
        printQuietStopResults();
        return;
    ...
    for (const instance of runningInstances) {
      ...
      stopSpin?.start(`Stopping ax-code on port ${instance.port}...`);
      try {
        ...
        stopSpin?.stop(`Stopped ax-code on port ${instance.port}`);
        jsonResults.push({ port: instance.port, pid: instance.pid, stopped: true });
        if (showOutput && !stopSpin) {
          logStatus('success', `stopped port ${instance.port}`);
        }
      } catch (error) {
        stopSpin?.error(`Failed to stop ax-code on port ${instance.port}`);
  ...
  async restart(options) {
    ...
    if (runningInstances.length === 0) {
      ...
      if (showOutput) {
        logStatus('info', 'No running ax-code instances to restart');
    ...
    if (options.explicitPort) {
      ...
      if (runningInstances.length === 0) {
        ...
        if (showOutput) {
          logStatus('warning', `no ax-code instance found on port ${options.port}`);
          clackOutro('nothing to restart');
        } else if (isQuietMode(options)) {
          process.stdout.write('restarted 0\n');
        }
    ...
    for (const instance of runningInstances) {
      ...
      restartSpin?.start(`Restarting ax-code on port ${instance.port}...`);
      try {
        await this.stop({
          explicitPort: true,
          port: instance.port,
        ...
        restartSpin?.stop(`Restarted ax-code on port ${restartedPort}`);
        if (showOutput && !restartSpin) {
          logStatus('success', `port ${restartedPort} restarted`, `mode: ${launchMode}`);
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        restartSpin?.error(`Failed to restart ax-code on port ${instance.port}`);
        if (showOutput && !restartSpin) {
    ...
    switch (subcommand) {
      ...
      case 'ready': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, `/api/ax-code/tunnel/check?provider=${encodeURIComponent(provider)}`);
            if (!response.ok) {
      ...
      case 'status': {
        ...
        for (const entry of entries) {
          try {
            const { response, body } = await requestJson(entry.port, '/api/ax-code/tunnel/status');
            if (!response.ok) {
          ...
          for (const diagnosticsEntry of diagnosticsEntries) {
            try {
              ...
              const { response, body } = await requestJson(
                ...
                `/api/ax-code/tunnel/doctor?${query.toString()}`,
                doctorFetchOptions,
              );
          ...
          if (isReady) {
            ...
            if (isManagedRemote && (hasPortOrOriginIssue || hasTokenIssue)) {
              troubleshootingHints.push({
                ...
                code: '[PORT_MISMATCH]',
                lines: [
                  'Cloudflare target must match the active ax-code CLI port.',
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered ax-code instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable ax-code CLI instances found on safe ports. Start one with `ax-code-app serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select ax-code instance port',
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for ax-code on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for ax-code health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('ax-code is still starting');
            throw new Error(
              `ax-code on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
        ...
        if (selectedProfile && mode === 'managed-remote') {
          ...
          const { response: presetResponse, body: presetBody } = await requestJson(instance.port, '/api/ax-code/tunnel/managed-remote-token', {
            method: 'PUT',
        ...
        let body;
        try {
          ({ response, body } = await requestJson(instance.port, '/api/ax-code/tunnel/start', {
            method: 'POST',
            body: JSON.stringify(payload),
            timeoutMs: 60000,
          }));
        } catch (error) {
          if (error instanceof Error && /\/api\/ax-code-app\/tunnel\/start/.test(error.message) && /timed out/.test(error.message)) {
      ...
      case 'stop': {
        ...
        for (const entry of entries) {
          ...
          try {
            const { response, body } = await requestJson(entry.port, '/api/ax-code/tunnel/stop', {
              method: 'POST',
            });
            if (!response.ok) {
  ...
  async logs(options) {
    ...
    if (options.all) {
      targets = running;
      if (targets.length === 0) {
        throw new Error('No running ax-code instance found.');
      }
