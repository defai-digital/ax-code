...
const pwaDevEnabled = process.env.AX_CODE_APP_DISABLE_PWA_DEV !== '1';
...
export default defineConfig({
  ...
  resolve: {
    alias: [
      ...
      { find: '@ax-code/app-ui', replacement: path.resolve(__dirname, '../app-ui/src') },
      { find: '@web', replacement: path.resolve(__dirname, './src') },
      { find: '@', replacement: path.resolve(__dirname, '../app-ui/src') },
    ],
  },
  worker: {
    format: 'es',
  ...
  server: {
    ...
    proxy: {
      '/auth': {
        target: `http://127.0.0.1:${process.env.AX_CODE_APP_PORT || 3001}`,
        changeOrigin: true,
      },
      '/health': {
        target: `http://127.0.0.1:${process.env.AX_CODE_APP_PORT || 3001}`,
