import { isNonEmptyRecord } from "./internal/record"
import { codeIntelHost, type LspConfigSection } from "./host"
...
export namespace LSPServerConfig {
  ...
  function filterExperimentalServers(servers: Record<string, LSPServer.Info>) {
    if (codeIntelHost().flags().experimentalLspTy) {
      if (servers["pyright"]) {
  ...
  export function buildEnabledServers(cfg: { lsp?: LspConfigSection }): Record<string, LSPServer.Info> {
    ...
    for (const [name, item] of Object.entries(cfg.lsp ?? {})) {
      ...
      servers[name] = {
        ...
        root: existing?.root ?? (async () => codeIntelHost().projectRoot()),
