...
export const Gopls: Info = {
  ...
  async spawn(root) {
    return toolServer(root, {
      ...
      env: { ...Env.sanitize(), GOBIN: codeIntelHost().binDir() },
      require: ["go"],
    })
  },
}
...
export const Zls: Info = {
  ...
  async spawn(root) {
    ...
    if (!codeIntelHost().flags().disableLspDownload) {
...
export const CSharp: Info = {
  ...
  async spawn(root) {
    return toolServer(root, {
      name: "csharp-ls",
      install: ["dotnet", "tool", "install", "csharp-ls", "--tool-path", codeIntelHost().binDir()],
      require: ["dotnet"],
...
export const FSharp: Info = {
  ...
  async spawn(root) {
    return toolServer(root, {
      name: "fsautocomplete",
      install: ["dotnet", "tool", "install", "fsautocomplete", "--tool-path", codeIntelHost().binDir()],
      require: ["dotnet"],
...
export const RustAnalyzer: Info = {
  ...
  root: async (root) => {
    ...
    while (currentDir !== path.dirname(currentDir)) {
      ...
      if (!currentDir.startsWith(codeIntelHost().worktreeRoot())) break
    }
    return crateRoot
  },
  extensions: [".rs"],
...
export const Clangd: Info = {
  ...
  async spawn(root) {
    ...
    const entries = await fs.readdir(codeIntelHost().binDir(), { withFileTypes: true }).catch(() => [])
    for (const entry of entries) {
      if (!entry.isDirectory()) continue
      if (!entry.name.startsWith("clangd_")) continue
      const candidate = path.join(codeIntelHost().binDir(), entry.name, "bin", "clangd" + ext)
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    if (!(await pathExists(launcherDir))) {
      const legacyDistPath = path.join(codeIntelHost().binDir(), "jdtls")
...
export const KotlinLS: Info = {
  ...
  async spawn(root) {
    ...
    const legacyLauncher = path.join(codeIntelHost().binDir(), "kotlin-ls", launcherName)
