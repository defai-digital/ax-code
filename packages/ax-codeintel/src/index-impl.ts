...
export namespace LSP {
  ...
  async function stopLSPProcess(proc: ChildProcessWithoutNullStreams) {
    await codeIntelHost().killTree(proc, {
  ...
  // responsibilities (see CodeIntelHost.publishUpdated). The ax-code glue
  // registers "lsp.updated" with its BusEvent registry so the event stays
  // part of the SSE/OpenAPI contract.
  export const Event = {
    Updated: { type: "lsp.updated" as const, properties: z.object({}) },
  }
  ...
  // Lazily created per-workspace state holder. The host is only required on
  // first use, never at module load, so importing this package does not
  // require the host to be configured yet.
  let stateHolder: ReturnType<typeof createLspState> | undefined
  function state() {
    return (stateHolder ??= createLspState())()
  }

  function createLspState() {
    return codeIntelHost().state(
      async () => {
        const clients: LSPClient.Info[] = []
        const cfg = await codeIntelHost().lspConfig()
        ...
        s.rootCacheUnsubscribe = codeIntelHost().subscribeRootMarkerChange((file) => {
          if (!isRootMarkerFile(file)) return
        ...
        s.healthCheck = setInterval(() => {
          ...
          for (const client of dead) {
            ...
            client.shutdown().catch((e) => {
              log.debug("failed to shutdown dead LSP client", {
      ...
      async (state) => {
        ...
        await Promise.allSettled(
          [...state.spawningProcesses].map((process) =>
            stopLSPProcessBestEffort(process, { phase: "instance-dispose" }),
          ),
        )
    ...
    return state().then((x) => {
      ...
      for (const client of x.clients) {
        ...
        result.push({
          id: client.serverID,
          name: server.id,
          root: path.relative(codeIntelHost().projectRoot(), client.root),
          status: "connected",
        })
      }
      return result
  ...
  function evictExcessClients(s: State, serverID: string) {
    ...
    if (evictions.length > 0) codeIntelHost().publishUpdated()
  }
  ...
  async function collectPendingClients(pending: PendingClientSpawn[], result: LSPClient.Info[]): Promise<void> {
    ...
    for (let i = 0; i < resolved.length; i++) {
      ...
      if (!client) continue
      result.push(client)
      if (pending[i].fresh) codeIntelHost().publishUpdated()
    }
  }
  ...
  async function getWorkspaceClientsDetailed(opts: ClientOptions = {}): Promise<ClientSelection> {
    ...
    for await (const rel of codeIntelHost().listFiles({
