...
import { Ssrf } from "./internal/ssrf"
...
// These pins are intentionally explicit so runtime installs stay reproducible.
// Update them deliberately after verifying upstream release compatibility.
...
// Keep auto-managed zls installs deterministic. Nightly/dev Zig builds must
// provide their own zls because there is no stable pinned match.
...
export const managedToolDir = (name: string, version: string, platform = process.platform, arch = process.arch) =>
  path.join(codeIntelHost().binDir(), ".managed", name, version, `${platform}-${arch}`)
