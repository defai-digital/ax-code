...
const localOrigin = readArgValue('--ax-code-local-origin');
const homeDirectory = readArgValue('--ax-code-home');
const macosMajorRaw = readArgValue('--ax-code-macos-major');
...
ipcRenderer.on('ax-code:emit', (_evt, payload) => {
...
// ipcMain.handle('ax-code:invoke') decides per-command what is safe
// for non-local callers (window/host-switcher ops yes, file/shell ops
// no). See COMMANDS_SAFE_FOR_REMOTE in main.mjs.
contextBridge.exposeInMainWorld('__TAURI__', {
  core: {
    invoke: (cmd, args) => ipcRenderer.invoke('ax-code:invoke', cmd, args || {}),
  },
  dialog: {
    open: (options) => ipcRenderer.invoke('ax-code:dialog:open', options || {}),
  },
  shell: {
    open: (url) => ipcRenderer.invoke('ax-code:invoke', 'desktop_open_external_url', { url }),
  },
  event: {
