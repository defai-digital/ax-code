...
const SSH_STATUS_EVENT = 'ax-code:ssh-instance-status';
...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded ax-code health');
};
...
export class ElectronSshManager {
  ...
  sanitizeInstance(instance) {
    ...
    return {
      ...
      remoteAxCode: {
        mode: instance?.remoteAxCode?.mode === 'external' ? 'external' : 'managed',
        keepRunning: instance?.remoteAxCode?.keepRunning !== false,
        ...(Number.isFinite(instance?.remoteAxCode?.preferredPort) ? { preferredPort: Number(instance.remoteAxCode.preferredPort) } : {}),
        installMethod: ['npm', 'bun', 'download_release', 'upload_bundle'].includes(instance?.remoteAxCode?.installMethod)
          ? instance.remoteAxCode.installMethod
          : 'bun',
        uploadBundleOverSsh: Boolean(instance?.remoteAxCode?.uploadBundleOverSsh),
      },
      localForward: {
      ...
      auth: {
        ...
        ...(this.sanitizeStoredSecret(instance?.auth?.axCodePassword) ? { axCodePassword: this.sanitizeStoredSecret(instance.auth.axCodePassword) } : {}),
      },
      portForwards,
    };
  }
  ...
  configuredAxCodePassword(instance) {
    const secret = instance?.auth?.axCodePassword;
  ...
  async currentRemoteAxCodeVersion(parsed, controlPath) {
    try {
      const output = await runRemoteCommand(parsed, controlPath, 'ax-code --version 2>/dev/null || true');
  ...
  async installAxCodeManaged(parsed, controlPath, version, preferred) {
    ...
    if (preferred === 'bun') {
      if (hasBun) commands.push(`bun add -g @ax-code/app@${version}`);
      if (hasNpm) commands.push(`npm install -g @ax-code/app@${version}`);
    ...
    throw lastError || new Error('Failed to install ax-code on remote host');
  }

  async probeRemoteSystemInfo(parsed, controlPath, port, axCodePassword) {
    const authPayload = axCodePassword ? JSON.stringify({ password: axCodePassword }) : '{}';
    const authEnabled = axCodePassword ? '1' : '0';
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (axCodePassword && authStatus !== 200) {
          throw new Error(`Remote ax-code requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote ax-code requires UI authentication on /api/system/info; configure ax-code UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote ax-code probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async remoteServerRunning(parsed, controlPath, port, axCodePassword) {
    try {
      await this.probeRemoteSystemInfo(parsed, controlPath, port, axCodePassword);
      return true;
    } catch {
      return false;
    }
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    ...
    const secret = this.configuredAxCodePassword(instance);
    if (secret) {
      envPrefix += ` OPENCHAMBER_UI_PASSWORD=${shellQuote(secret)}`;
    }
    const output = await runRemoteCommand(parsed, controlPath, `${envPrefix} ax-code serve --hostname 127.0.0.1 --port ${desiredPort}`);
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteAxCode.mode === 'external') {
      if (!instance.remoteAxCode.preferredPort) {
        throw new Error('External mode requires a preferred remote ax-code port');
      }
      const port = instance.remoteAxCode.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external ax-code server', null, null, port, false, 0, false);
      await this.probeRemoteSystemInfo(parsed, controlPath, port, this.configuredAxCodePassword(instance));
      return { remotePort: port, startedByUs: false };
    }

    this.setStatus(instance.id, 'remote_probe', 'Checking remote ax-code installation');
    const installedVersion = await this.currentRemoteAxCodeVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing ax-code on remote host');
      await this.installAxCodeManaged(parsed, controlPath, this.appVersion, instance.remoteAxCode.installMethod);
    } else if (installedVersion !== this.appVersion) {
      this.setStatus(instance.id, 'updating', `Updating remote ax-code from ${installedVersion} to ${this.appVersion}`);
      await this.installAxCodeManaged(parsed, controlPath, this.appVersion, instance.remoteAxCode.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed ax-code server');
    let remotePort = instance.remoteAxCode.preferredPort || null;
    let startedByUs = false;
    if (remotePort && !(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredAxCodePassword(instance)))) {
      remotePort = null;
    }
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed ax-code server');
      const desiredPort = instance.remoteAxCode.preferredPort || randomPortCandidate(instance.id);
      remotePort = await this.startRemoteServerManaged(parsed, controlPath, instance, desiredPort);
      startedByUs = true;
    }
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredAxCodePassword(instance)))) {
      throw new Error('Managed ax-code server failed to become reachable');
    }
  ...
  async disconnectInternal(id, reportIdle) {
    ...
    if (session) {
      if (session.startedByUs && session.instance.remoteAxCode.mode === 'managed' && !session.instance.remoteAxCode.keepRunning) {
