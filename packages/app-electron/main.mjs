...
const isDev = process.env.AX_CODE_ELECTRON_DEV === '1' || !app.isPackaged;

const DEEP_LINK_PROTOCOL = 'ax-code';
const PACKAGED_APP_USER_MODEL_ID = 'dev.ax-code.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.ax-code.desktop.dev';
...
// ~/Library/Logs/ax-code/ (not ~/Library/Logs/@ax-code/app-electron/).
app.setName('ax-code');
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'ax-code Dev'));
}
...
// that output never lands in ~/Library/Logs/ax-code/main.log and we
...
const readAppMetadata = () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed?.name === '@ax-code/app-electron' && typeof parsed.version === 'string') {
        return { name: parsed.name, version: parsed.version };
      }
    } catch {
    }
  }
  return { name: '@ax-code/app-electron', version: app.getVersion() };
};
...
const CHANGELOG_URL = 'https://raw.githubusercontent.com/automatosx/ax-code/main/CHANGELOG.md';
const UPDATE_METADATA_URL = 'https://github.com/automatosx/ax-code/releases/latest/download/latest.json';
const GITHUB_BUG_REPORT_URL = 'https://github.com/automatosx/ax-code/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/automatosx/ax-code/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `ax-code detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const refreshQuitRiskFlags = async () => {
  ...
  const scheduledUrl = `${base}/api/ax-code/scheduled-tasks/status`;
  const tunnelUrl = `${base}/api/ax-code/tunnel/status`;
...
const settingsFilePath = () => {
  ...
  return path.join(os.homedir(), '.config', 'ax-code', 'settings.json');
};
...
const maybeShowNativeNotification = (rawInput) => {
  ...
  notification.on('click', () => {
    ...
    if (sessionId) {
      emitToAllWindows('ax-code:open-session', { sessionId });
    }
    release();
  });
...
import { pathLooksUserConfigured, mergePathValues } from '@ax-code/app/server/lib/opencode/path-utils.js';
...
const spawnLocalServer = async () => {
  ...
  const { startWebUiServer } = await import('@ax-code/app/server/index.js');
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ax-code loading icon">
...
const emitToWindow = (browserWindow, event, detail) => {
  if (!browserWindow || browserWindow.isDestroyed()) return;
  browserWindow.webContents.send('ax-code:emit', { event, detail });
};
...
const dispatchDeepLink = (link) => {
  ...
  if (link.type === 'session' && link.value) {
    emitToAllWindows('ax-code:open-session', { sessionId: link.value });
    return;
  }
  if (link.type === 'project' && link.value) {
    emitToAllWindows('ax-code:open-project', { projectPath: link.value });
    return;
  }
...
const dispatchMenuAction = (action) => {
  ...
  emitToWindow(target, 'ax-code:menu-action', action);
  dispatchDomEventToWindow(target, 'ax-code:menu-action', action);
};

const dispatchCheckForUpdates = () => {
  emitToAllWindows('ax-code:check-for-updates');
  for (const browserWindow of BrowserWindow.getAllWindows()) {
    dispatchDomEventToWindow(browserWindow, 'ax-code:check-for-updates');
  }
};
...
const createBrowserWindow = ({ label, restoreGeometry, url }) => {
  ...
  const options = {
    ...
    webPreferences: {
      additionalArguments: [
        `--ax-code-local-origin=${desktopLocalOrigin}`,
        `--ax-code-home=${desktopHome}`,
        `--ax-code-macos-major=${desktopMacosMajor}`,
        `--ax-code-boot-outcome=${JSON.stringify(state.bootOutcome || null)}`,
      ],
  ...
  browserWindow.on('resize', () => {
    emitToWindow(browserWindow, 'ax-code:window-resized');
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('maximize', () => {
    emitToWindow(browserWindow, 'ax-code:window-maximized-changed', { maximized: true });
    debounceWindowStatePersist(browserWindow, false);
  });
  browserWindow.on('unmaximize', () => {
    emitToWindow(browserWindow, 'ax-code:window-maximized-changed', { maximized: false });
    debounceWindowStatePersist(browserWindow, false);
  });
...
const parseGithubRepo = () => {
  return { owner: 'automatosx', repo: 'ax-code' };
};
...
const setupAutoUpdater = () => {
  ...
  autoUpdater.on('download-progress', (progress) => {
    ...
    emitToAllWindows('ax-code:update-progress', mapUpdaterProgressEvent({
      event: 'Progress',
      data: {
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_read_file': {
      ...
      const DENIED_SEGMENTS = ['.ssh', '.aws', '.gnupg', '.gpg', '.config/gh', '.config/ax-code/credentials'];
    ...
    case 'desktop_get_installed_apps': {
      ...
      const refresh = async () => {
        ...
        emitToAllWindows('ax-code:installed-apps-updated', apps);
      };
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires ax-code.app to be installed in /Applications');
          }
        } catch (error) {
    ...
    case 'desktop_focus_main_window':
      if (state.mainWindow && !state.mainWindow.isDestroyed()) {
        ...
        if (sessionId) {
          emitToWindow(state.mainWindow, 'ax-code:open-session', { sessionId, directory });
        } else if (mode === 'draft') {
          const projectId = typeof args.projectId === 'string' ? args.projectId.trim() : '';
          emitToWindow(state.mainWindow, 'ax-code:open-draft-session', { directory, projectId });
        }
        return { focused: true };
      }
...
ipcMain.handle('ax-code:invoke', async (event, command, args) => {
...
ipcMain.handle('ax-code:dialog:open', async (event, options) => {
...
app.whenReady().then(async () => {
  ...
  powerMonitor.on('resume', () => {
    emitToAllWindows('ax-code:system-resume', { timestamp: Date.now() });
  });
}).catch((error) => {
