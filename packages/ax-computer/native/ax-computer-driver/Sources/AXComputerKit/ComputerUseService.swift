...
func inputFallbackDebugEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["AX_COMPUTER_DEBUG_INPUT_FALLBACKS"]?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    else {
        return false
...
func globalPointerFallbacksEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS"]?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    else {
        return false
...
public final class ComputerUseService {
    ...
    // CFTypeID-verified cast: malformed AX replies return nil instead of trapping.
    private func axValueValue(_ value: CFTypeRef) -> AXValue? {
    ...
    private func localFrame(of element: AXUIElement, windowBounds: CGRect?) -> CGRect? {
        ...
        guard
            let positionAXValue = axValueValue(positionValue),
            let sizeAXValue = axValueValue(sizeValue)
        else {
            return nil
        }
    ...
    private func debugInputFallback(tool: String, targetDescription: String, snapshot: AppSnapshot) {
        ...
        let appReference = snapshot.app.bundleIdentifier ?? snapshot.app.name
        fputs(
            "[ax-computer-driver] global pointer fallback tool=\(tool) app=\(appReference) target=\(targetDescription)\n",
            stderr
        )
    }
    ...
    private func debugClickDecision(_ message: String) {
        ...
        fputs("[ax-computer-driver] click decision \(message)\n", stderr)
    }
    ...
    private func performNonAXClickFallback(
        ...
        do {
            ...
            guard globalPointerFallbacksEnabled(environment: ProcessInfo.processInfo.environment) else {
                throw ComputerUseError.message(
                    "click could not be handled through accessibility, and global pointer fallback is disabled. Set AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS=1 to allow physical-pointer fallback for this process."
                )
            }
        }
    }
