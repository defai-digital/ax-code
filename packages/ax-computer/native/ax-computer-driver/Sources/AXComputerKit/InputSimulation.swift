...
enum InputSimulation {
    ...
    static func clickGlobally(at point: CGPoint, button: MouseButtonKind, clickCount: Int) throws {
        ...
        // clickState must be the 1-based index of the click within the
        // sequence, otherwise the first click of a double-click is stamped 2.
        for clickIndex in 0..<max(clickCount, 1) {
            let clickState = clickIndex + 1
            try postMouseEvent(type: .mouseMoved, source: source, point: point, button: button.cgButton, clickState: clickState)
            try postMouseEvent(type: button.downEvent, source: source, point: point, button: button.cgButton, clickState: clickState)
            try postMouseEvent(type: button.upEvent, source: source, point: point, button: button.cgButton, clickState: clickState)
        }
    }
    ...
    static func clickTargeted(at point: CGPoint, button: MouseButtonKind, clickCount: Int, pid: pid_t) throws {
        ...
        for clickIndex in 0..<max(clickCount, 1) {
            let clickState = clickIndex + 1
            try postMouseEventToPid(type: .mouseMoved, source: source, point: point, button: button.cgButton, clickState: clickState, pid: pid)
            try postMouseEventToPid(type: button.downEvent, source: source, point: point, button: button.cgButton, clickState: clickState, pid: pid)
            try postMouseEventToPid(type: button.upEvent, source: source, point: point, button: button.cgButton, clickState: clickState, pid: pid)
        }
    }
