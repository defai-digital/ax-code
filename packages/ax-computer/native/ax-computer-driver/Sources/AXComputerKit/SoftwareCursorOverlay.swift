...
func visualCursorEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["AX_COMPUTER_VISUAL_CURSOR"]?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() else {
        return true
    }
...
public func visualCursorObservationFileURL(environment: [String: String]) -> URL? {
    guard
        let rawPath = environment["AX_COMPUTER_VISUAL_CURSOR_OBSERVATION_FILE"]?
...
enum SoftwareCursorOverlay {
    ...
    private static var moveAnimationTimer: Timer?
    private static var moveAnimation: MoveAnimation?
    private static var pulseAnimationTimer: Timer?
    private static var pulseAnimation: ClickPulseAnimation?

    // Animations are timer-driven (see startMoveAnimation/startClickPulseAnimation)
    // so tool calls never block the MCP request thread on cursor visuals.
    private struct MoveAnimation {
        let path: CursorMotionPath
        let duration: CGFloat
        let springTargetDuration: CGFloat
    ...
    private struct ClickPulseAnimation {
        let point: CGPoint
        let clickCount: Int
        let pulseBias: CGFloat
        var pulseIndex = 0
        var phaseStartTime: CFTimeInterval
        var inInterPulsePause = false
    }
    ...
    static func moveCursor(to targetPoint: CGPoint, in targetWindow: CursorTargetWindow?) {
        ...
        if distanceBetween(startPoint, constrainedTarget) > 2 {
            startMoveAnimation(from: startPoint, to: constrainedTarget, relativeTo: targetWindow)
        }
    }
    ...
    static func pulseClick(at targetPoint: CGPoint, clickCount: Int, mouseButton: MouseButtonKind, in targetWindow: CursorTargetWindow?) {
        ...
        startClickPulseAnimation(at: constrainedTarget, clickCount: max(clickCount, 1), mouseButton: mouseButton)
    }
    ...
    private static func startMoveAnimation(from start: CGPoint, to end: CGPoint, relativeTo targetWindow: CursorTargetWindow?) {
        ...
        moveAnimation = MoveAnimation(
            ...
            springTargetDuration: OfficialCursorMotionModel.closeEnoughTime,
            startTime: CACurrentMediaTime(),
            end: end,
            progress: 0,
            springState: CursorMotionSpringState()
        )
        ...
        moveAnimationTimer = timer
    }

    private static func advanceMoveAnimation() {
        guard var animation = moveAnimation else {
            cancelMoveAnimation()
            return
        }
        ...
        let elapsed = CGFloat(CACurrentMediaTime() - animation.startTime)
        let normalizedElapsed = (elapsed / max(animation.duration, 0.001)).clamped(to: 0...1)
        let springTime = normalizedElapsed * animation.springTargetDuration
        (animation.progress, animation.springState) = CursorMotionProgressAnimator.advance(
            current: animation.progress,
            state: animation.springState,
            to: springTime
        )
        moveAnimation = animation

        let sample = animation.path.sample(at: animation.progress)
        placeCursor(
            using: advanceVisualDynamics(
                toward: sample.point,
                at: CACurrentMediaTime()
        ...
        if normalizedElapsed >= 1 || CursorMotionProgressAnimator.isCloseEnough(progress: animation.progress) {
            cancelMoveAnimation()
            placeCursor(
                using: advanceVisualDynamics(
                    toward: animation.end,
                    at: CACurrentMediaTime()
                ),
                clickProgress: 0
            )
    ...
    private static func cancelMoveAnimation() {
    ...
    private static func startClickPulseAnimation(at point: CGPoint, clickCount: Int, mouseButton: MouseButtonKind) {
        pulseAnimation = ClickPulseAnimation(
            point: point,
            clickCount: clickCount,
            pulseBias: mouseButton == .right ? 0.82 : 1,
            phaseStartTime: CACurrentMediaTime()
        )

        let timer = Timer(timeInterval: 1 / 120, repeats: true) { _ in
            MainActor.assumeIsolated {
                advanceClickPulseAnimation()
            }
        }
        RunLoop.main.add(timer, forMode: .common)
    ...
    private static func advanceClickPulseAnimation() {
        guard var animation = pulseAnimation else {
            cancelPulseAnimation()
            return
        }

        let now = CACurrentMediaTime()
        let pulseDuration = 0.16
        let interPulsePause = 0.05

        if animation.inInterPulsePause {
            guard now - animation.phaseStartTime >= interPulsePause else {
                return
            }

            animation.inInterPulsePause = false
            animation.pulseIndex += 1
            animation.phaseStartTime = now
            pulseAnimation = animation
            return
        }

        let rawProgress = min(max((now - animation.phaseStartTime) / pulseDuration, 0), 1)
        let clickProgress = sin(CGFloat(rawProgress) * .pi) * animation.pulseBias
        placeCursor(
            using: advanceVisualDynamics(
                toward: animation.point,
                at: now
            ),
        ...
        if animation.pulseIndex < animation.clickCount - 1 {
            animation.inInterPulsePause = true
            animation.phaseStartTime = now
            pulseAnimation = animation
            return
        }
    ...
    private static func cancelPulseAnimation() {
