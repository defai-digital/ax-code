...
public struct AXComputerCallSpec {
...
public struct AXComputerCallOutput {
...
public typealias AXComputerSleepHandler = (TimeInterval) -> Void

public func runAXComputerCall(
    _ invocation: AXComputerCallInvocation,
    service: ComputerUseService = ComputerUseService(),
    sleepHandler: AXComputerSleepHandler = { Thread.sleep(forTimeInterval: $0) }
) throws -> AXComputerCallOutput {
    ...
    case let .sequence(callsJSON, callsFile, interCallDelay):
        ...
        return AXComputerCallOutput(jsonObject: outputs, hasToolError: hasToolError)
    }
}

public func readAXComputerToolArguments(
    json: String?,
    file: String?
) throws -> [String: Any] {
    guard let source = try readAXComputerJSONSource(json: json, file: file) else {
        return [:]
    }

    let object = try decodeAXComputerJSONObject(source)
    guard let arguments = object as? [String: Any] else {
        throw AXComputerCLIError(message: "--args must be a JSON object", helpCommand: "call")
    }

    return arguments
}

public func readAXComputerCallSequence(
    json: String?,
    file: String?
) throws -> [AXComputerCallSpec] {
    guard let source = try readAXComputerJSONSource(json: json, file: file) else {
        throw AXComputerCLIError(message: "call sequence requires --calls or --calls-file", helpCommand: "call")
    }

    let object = try decodeAXComputerJSONObject(source)
    guard let array = object as? [Any] else {
        throw AXComputerCLIError(message: "--calls must be a JSON array", helpCommand: "call")
    }
    ...
    return try array.enumerated().map { index, item in
        ...
        return AXComputerCallSpec(tool: tool, arguments: arguments)
    }
}

private func readAXComputerJSONSource(json: String?, file: String?) throws -> String? {
    if json != nil, file != nil {
        throw AXComputerCLIError(message: "Use either inline JSON or a JSON file, not both", helpCommand: "call")
    }
...
private func decodeAXComputerJSONObject(_ source: String) throws -> Any {
    guard let data = source.data(using: .utf8) else {
        throw AXComputerCLIError(message: "JSON input must be UTF-8 text", helpCommand: "call")
    }
