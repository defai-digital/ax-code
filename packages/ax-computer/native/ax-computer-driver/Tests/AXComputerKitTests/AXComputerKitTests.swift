...
import ImageIO
import XCTest
@testable import AXComputerKit

final class AXComputerKitTests: XCTestCase {
    func testCLIRecognizesGlobalHelpAndVersionFlags() throws {
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["-h"]), .help(command: nil))
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["--help"]), .help(command: nil))
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["-v"]), .version)
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["--version"]), .version)
    }

    func testCLIRecognizesCommandSpecificHelp() throws {
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["help", "snapshot"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["snapshot", "--help"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["doctor", "-h"]), .help(command: "doctor"))
        XCTAssertEqual(try parseAXComputerCLI(arguments: ["call", "--help"]), .help(command: "call"))
    }

    func testCLIRecognizesSingleToolCallCommand() throws {
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["call", "list_apps"]),
            .call(.single(toolName: "list_apps", argumentsJSON: nil, argumentsFile: nil))
        )

        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["call", "get_app_state", "--args", #"{"app":"TextEdit"}"#]),
    ...
    func testCLIRecognizesJSONSequenceCallCommand() throws {
        ...
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["call", "--calls", calls]),
            .call(.sequence(
                callsJSON: calls,
                callsFile: nil,
                interCallDelay: axComputerDefaultInterCallDelay
            ))
        )
    }
    ...
    func testCLIRecognizesJSONSequenceCallCommandWithCustomSleep() throws {
        ...
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["call", "--calls", calls, "--sleep", "0.5"]),
            .call(.sequence(callsJSON: calls, callsFile: nil, interCallDelay: 0.5))
        )
    }

    func testCLIRequiresSnapshotArgument() {
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["snapshot"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(
    ...
    func testCLIRecognizesSnapshotTextLimitFlag() throws {
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["snapshot", "TextEdit"]),
            .snapshot(app: "TextEdit")
        )
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["snapshot", "--text-limit", "1000", "TextEdit"]),
            .snapshot(app: "TextEdit", textLimit: SnapshotTextLimit(maxCount: 1000))
        )
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["snapshot", "TextEdit", "--text-limit", "max"]),
            .snapshot(app: "TextEdit", textLimit: .max)
        )
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["snapshot", "--max-tree-nodes", "3000", "--max-tree-depth", "96", "TextEdit"]),
            .snapshot(
                app: "TextEdit",
        ...
        XCTAssertEqual(
            try parseAXComputerCLI(arguments: ["snapshot", "--max-tree-nodes", "3000", "TextEdit"]),
            .snapshot(
                app: "TextEdit",
    ...
    func testCLIRejectsOldSnapshotFullTextFlag() {
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["snapshot", "--show-full-text", "TextEdit"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(message: "Unknown snapshot option: --show-full-text", helpCommand: "snapshot")
            )
        }
    }
    ...
    func testCLIRejectsInvalidSnapshotTextLimit() {
        for value in ["0", "-1", "1.5", "full"] {
            XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["snapshot", "--text-limit", value, "TextEdit"])) { error in
                XCTAssertEqual(
                    error as? AXComputerCLIError,
                    AXComputerCLIError(message: "--text-limit must be a positive integer or max", helpCommand: "snapshot")
                )
            }
        }
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["snapshot", "--text-limit"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(message: "--text-limit requires a positive integer or max value", helpCommand: "snapshot")
            )
        }
    }

    func testCLIRejectsInvalidSnapshotTreeBudget() {
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["snapshot", "--max-tree-nodes", "0", "TextEdit"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(message: "--max-tree-nodes must be a positive integer", helpCommand: "snapshot")
            )
        }
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["snapshot", "--max-tree-depth", "1.5", "TextEdit"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(message: "--max-tree-depth must be a positive integer", helpCommand: "snapshot")
            )
        }
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["snapshot", "--max-tree-nodes"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(message: "--max-tree-nodes requires a positive integer value", helpCommand: "snapshot")
            )
        }
    }

    func testCLIRejectsMixedCallSequenceInputs() {
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["call", "list_apps", "--calls", "[]"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(
    ...
    func testCLIRejectsSleepForSingleToolCall() {
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["call", "list_apps", "--sleep", "0.5"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(
    ...
    func testCLIRejectsInvalidSequenceSleepValue() {
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["call", "--calls", "[]", "--sleep", "-1"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(
    ...
    func testCLIRejectsUnknownOption() {
        XCTAssertThrowsError(try parseAXComputerCLI(arguments: ["--verbose"])) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(
    ...
    func testGeneralHelpListsCommandsAndGlobalFlags() {
        let help = axComputerHelpText()

        XCTAssertTrue(help.contains("ax-computer-driver [command] [options]"))
    ...
    func testResolvedVersionFallsBackWhenBundleHasNoVersionMetadata() {
        XCTAssertEqual(resolvedAXComputerVersion(bundle: Bundle(for: Self.self)), axComputerVersion)
    }
    ...
    func testReadToolArgumentsAcceptsJSONObject() throws {
        let arguments = try readAXComputerToolArguments(
    ...
    func testElementIndexAcceptsNumericCallSequenceArgument() throws {
        let calls = try readAXComputerCallSequence(
    ...
    func testReadToolArgumentsRejectsNonObject() {
        XCTAssertThrowsError(try readAXComputerToolArguments(json: #"["TextEdit"]"#, file: nil)) { error in
            XCTAssertEqual(
                error as? AXComputerCLIError,
                AXComputerCLIError(message: "--args must be a JSON object", helpCommand: "call")
            )
        }
    }
    ...
    func testRunCallSequenceStopsAfterFirstToolError() throws {
        let output = try runAXComputerCall(
            .sequence(
    ...
    func testRunCallSequenceSleepsBetweenSuccessfulOperations() throws {
        ...
        XCTAssertEqual(recordedSleeps, [axComputerDefaultInterCallDelay, axComputerDefaultInterCallDelay])
    ...
    func testPreferredPermissionAppBundleURLPrefersInstalledCopyOverTransientRunningCopy() {
        let installed = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/ax-computer-driver/dist/Open Computer Use.app")
    ...
    func testPreferredPermissionAppBundleURLCanPreferRunningReleaseCopyOverStaleInstalledCopy() {
        let staleInstalled = URL(fileURLWithPath: "/Users/example/projects/open-codex-computer-use/dist/npm/ax-computer-driver/dist/Open Computer Use.app")
        let running = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/ax-computer-driver/dist/Open Computer Use.app")
    ...
    func testPreferredInstalledAppBundleURLUsesFirstDiscoveredInstalledCopy() {
        ...
        let npm = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/ax-computer-driver/dist/Open Computer Use.app")
    ...
    func testInitializeResponseContainsToolsCapability() throws {
        ...
        XCTAssertTrue(response!.contains(#""name":"ax-computer-driver""#))
    ...
    // Regression: case-variant bundle identifiers must not bypass the denylist.
    func testAppSafetyPolicyBlocksCaseVariants() {
        XCTAssertTrue(AppSafetyPolicy.isBlocked(bundleIdentifier: "COM.1Password.1Password"))
    ...
    func testVisualCursorEnvFlagDefaultsToEnabled() {
        ...
        XCTAssertTrue(visualCursorEnabled(environment: ["AX_COMPUTER_VISUAL_CURSOR": "1"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["AX_COMPUTER_VISUAL_CURSOR": "0"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["AX_COMPUTER_VISUAL_CURSOR": "false"]))
    }

    func testInputFallbackDebugFlagDefaultsToDisabled() {
        XCTAssertFalse(inputFallbackDebugEnabled(environment: [:]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["AX_COMPUTER_DEBUG_INPUT_FALLBACKS": "1"]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["AX_COMPUTER_DEBUG_INPUT_FALLBACKS": "true"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["AX_COMPUTER_DEBUG_INPUT_FALLBACKS": "0"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["AX_COMPUTER_DEBUG_INPUT_FALLBACKS": "off"]))
    }

    func testGlobalPointerFallbackFlagDefaultsToDisabled() {
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: [:]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS": "1"]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS": "yes"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS": "0"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS": "false"]))
    }
    ...
    func testGlobalClickMethodRequiresExplicitPointerAuthorization() {
        XCTAssertThrowsError(
            ...
            XCTAssertEqual(
                (error as? ComputerUseError)?.errorDescription,
                "click_method 'global' requires AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS=1 because it may move the system pointer and change foreground focus"
            )
        }
        ...
        XCTAssertNoThrow(
            try validateClickMethod(
                ...
                environment: ["AX_COMPUTER_ALLOW_GLOBAL_POINTER_FALLBACKS": "1"]
            )
        )
    }
