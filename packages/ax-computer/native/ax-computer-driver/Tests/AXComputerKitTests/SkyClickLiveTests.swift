...
final class SkyClickLiveTests: XCTestCase {
    ...
    func testCoveredChromeReceivesExactlyOneSkyClickWithoutForegroundSideEffects() throws {
        guard ProcessInfo.processInfo.environment["AX_COMPUTER_RUN_SKY_CLICK_LIVE_TEST"] == "1" else {
            throw XCTSkip("Set AX_COMPUTER_RUN_SKY_CLICK_LIVE_TEST=1 to run the isolated Chrome live test")
        }
        ...
        let coverExecutable = Self.packageRoot
            .appendingPathComponent(".build/debug/AXComputerFixture")
        guard FileManager.default.isExecutableFile(atPath: coverExecutable.path) else {
            throw XCTSkip("Build AXComputerFixture before running the live test")
        }
        ...
        let coverProbeWindow = try waitForWindow(
            ...
            nameContaining: "AXComputerFixture"
        )
