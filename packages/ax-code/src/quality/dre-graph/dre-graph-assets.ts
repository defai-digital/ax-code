...
export function live(input: { sessionID?: string; directory?: string }) {
  return [
    ...
    `const sync = async () => {`,
    `  if (!cfg.poll) return`,
    `  const poll = new URL(cfg.poll, window.location.origin)`,
    `  // R4-1: never stack concurrent fingerprint fetches — skip if a poll is`,
    `  // already in flight, otherwise cancel the prior one so a supersede can`,
    `  // win. The AbortController is also threaded into fetch so a page hide`,
    `  // or a server-side retry storm cannot leave dangling requests.`,
    `  if (_polling) return`,
    ...
    `  _polling = true`,
    `  try {`,
    `    // cfg.poll is generated by this page, but keep the browser-side fetch`,
    `    // origin-bound if future callers ever supply a different value.`,
    `    const urlAllowlist = window.location.origin`,
    `    if (poll.origin !== urlAllowlist) return`,
    `    const res = await fetch(poll, { cache: "no-store", headers: { accept: "application/json" }, signal: ac.signal })`,
    `    if (!res.ok) {`,
    `      // R4-2: a non-OK response used to be silently swallowed, leaving`,
    `      // the badge stuck at "live" while poll-driven refresh was dead.`,
    `      // 4xx is treated as terminal (e.g. oversize directory post-D3);`,
    `      // 5xx is transient and only trips the badge after 3 in a row so a`,
    `      // single hiccup doesn't surface.`,
    `      _pollFails = res.status >= 500 ? _pollFails + 1 : _pollFails + 10`,
    `      if (res.status < 500) {`,
    `        set("poll error", "off")`,
    `        return`,
    `      }`,
    `      if (_pollFails >= 3) set("poll error", "off")`,
    `      return`,
    `    }`,
    `    _pollFails = 0`,
    ...
    `  } finally {`,
    `    _polling = false`,
    `    if (_pollAbort === ac) _pollAbort = null`,
    `  }`,
    `}`,
    `let _liveSrc = null;`,
    ...
    `  src.onmessage = (event) => {`,
    `    // A malformed frame must not silently disable SSE-driven refresh;`,
    `    // the interval poll keeps running but the live line gets stuck`,
    `    // "connecting" without this guard.`,
    `    let data`,
