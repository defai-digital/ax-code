...
export async function isSupportedHost() {
  return (await getPlatformEligibility()).supported
}

export async function requirePlatformEligibility() {
  const eligibility = await getPlatformEligibility()
  if (!eligibility.supported) {
    throw new Error(eligibility.blockers.join("; ") || "ax-engine provider is not supported on this host")
  }
  return eligibility
}
