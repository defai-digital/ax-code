  ...
  function message(e: APICallError) {
    return iife(() => {
      ...
      if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
        if (e.statusCode === 401) {
          return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `ax-code login <your provider URL>` to re-authenticate."
        }
        if (e.statusCode === 403) {
