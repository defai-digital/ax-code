...
export const make = Effect.gen(function* () {
  ...
  const spawn = (command: ChildProcess.StandardCommand, opts: NodeChildProcess.SpawnOptions) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal], PlatformError.PlatformError>((resume) => {
      ...
      const proc = launch(command.command, command.args, opts) as unknown as NodeChildProcess.ChildProcess & {
  ...
  const killGroup = (
    ...
    if (globalThis.process.platform === "win32") {
      return Effect.callback<void, PlatformError.PlatformError>((resume) => {
        if (proc.pid == null) return resume(Effect.void)
        NodeChildProcess.execFile("taskkill", ["/pid", String(proc.pid), "/T", "/F"], { windowsHide: true }, (err) => {
