...
export namespace SkillCandidate {
  ...
  async function cleanRevision() {
    ...
    if (!state.available || !state.commit || state.dirtyDigest !== Hash.fast(""))
      throw new Error("Skill candidates require a clean, committed Git worktree")
    return state.commit
  }

  async function citation(input: Evidence, commit: string) {
    ...
    if (
      ...
      executed.some(
        (item) =>
          ...
          item.sourceState.commit !== commit ||
          item.sourceState.dirtyDigest !== Hash.fast(""),
      )
    )
      throw new Error("Verification must contain executed, successful checks against the current clean revision")
    return { ...ref, commit, digest: Hash.fast(JSON.stringify(part)) }
  }
  ...
  export async function propose(input: z.infer<typeof Proposal>) {
    ...
    const source = await citation(request.evidence, await cleanRevision())
  ...
  export async function validate(name: string, evidence: Evidence) {
    ...
    const commit = await cleanRevision()
    const source = await citation(candidate.source, commit)
    ...
    candidate.validation = await citation(evidence, commit)
  ...
  export async function promote(name: string) {
    ...
    if (!candidate.promotion) {
      ...
      for (const ref of [candidate.source, candidate.validation]) {
        if ((await citation(ref, commit)).digest !== ref.digest) throw new Error("Verification evidence changed")
      }
