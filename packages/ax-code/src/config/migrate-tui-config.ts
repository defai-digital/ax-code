...
const TUI_SCHEMA_URL = "https://ax-code.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from ax-code.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const configs = await axcodeFiles(input)
...
async function axcodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.AX_CODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("ax-code", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "ax-code")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "ax-code"))
  }
  if (Flag.AX_CODE_CONFIG) files.push(Flag.AX_CODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "ax-code"))
