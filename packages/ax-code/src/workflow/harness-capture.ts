    ...
    for (const task of manifest.tasks)
      for (let repetition = 0; repetition < manifest.repetitions; repetition++) {
        ...
        for (const arm of arms) {
          ...
          try {
            for (const file of task.files) {
              // @scan-suppress security_scan - All fixture paths reject absolute paths, separators, traversal and Git metadata above; root is a fresh owned mkdtemp directory.
