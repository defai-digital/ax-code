...
export namespace SessionSemanticDiff {
  export const Kind = z
    .enum(["bug_fix", "refactor", "optimization", "test", "documentation", "configuration", "dependency", "rewrite"])
    .meta({
      ref: "SessionSemanticDiffKind",
    })
  export type Kind = z.output<typeof Kind>

  export const Risk = z.enum(["low", "medium", "high"]).meta({
  ...
  function classify(diff: Snapshot.FileDiff) {
    ...
    if (file.includes("/__tests__/") || file.includes("/test/") || file.includes("/tests/") || file.includes(".test.") || file.includes(".spec."))
      return "test" satisfies Kind
    if (docs.has(ext)) return "documentation" satisfies Kind
    if (dep.has(name(file))) return "dependency" satisfies Kind
    if (cfg.has(ext) || file.includes(".env") || name(file).startsWith("tsconfig") || name(file) === "bunfig.toml")
      return "configuration" satisfies Kind
    if (diff.status === "deleted") return "rewrite" satisfies Kind
    if (code.has(ext) && (churn >= 180 || (Math.max(before, after) >= 80 && (ratio >= 2.5 || ratio <= 0.4))))
      return "rewrite" satisfies Kind
    if (faster > 0 && churn <= 120) return "optimization" satisfies Kind
    if (guards > 0 && churn <= 100) return "bug_fix" satisfies Kind
    if (helpers > 0 || code.has(ext)) return "refactor" satisfies Kind
    return churn >= 120 ? ("rewrite" satisfies Kind) : ("configuration" satisfies Kind)
  }

  function assess(diff: Snapshot.FileDiff, kind: Kind): Risk {
    const file = diff.file.toLowerCase()
    const churn = diff.additions + diff.deletions
    const hot =
