...
export namespace ReplayCompare {
  ...
  export function score(input: {
    ...
    const toolCount = Math.max(input.view.tools.length, input.risk.signals.totalTools)
    const routeCount = input.view.routes.length
    const simplicity = round(
      clamp(
        1 -
          ...
          Math.min(0.15, Math.max(0, toolCount - 2) * 0.05) -
          Math.min(0.15, routeCount * 0.05) -
