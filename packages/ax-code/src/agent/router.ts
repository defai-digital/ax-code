 ...
 * its simplicity meant it actually fired. v3+ piled on intent gates, negative-
 * keyword blockers, an LLM tier, switch/delegate modes, and read-only specialist
 * restrictions — each well-intentioned, but the cumulative effect was a feature
 * that almost never fired and confused users when it did. We removed all of it
...
const RULES: RouteRule[] = [
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(app|service|server|endpoint|api|route|page|screen|feature|flow|command|cli|tool|login|signup|auth|build|script)\b.*\b(?:doesn.t|isn.t|won.t|cannot|can.t|not)\s+(?:work|working|run|running|start|starting|load|loading|function|functioning|pass|passing)\b/i,
      /\b(app|service|server|endpoint|api|route|page|screen|feature|flow|command|cli|tool|login|signup|auth|build|script)\b.*\b(?:fail|fails|failed|failure|broken|failing)\b/i,
      /\broot\s+cause\b/i,
      /\btroubleshoot/i,
      /\bstack\s+trace\b/i,
    ],
  ...
  },
  {
    ...
    patterns: [
      ...
      /\btests?\s+(?:do|does|are|is)\s+not\s+(?:pass|passing)\b/i,
      /\btests?\s+(?:don.t|doesn.t|won.t|cannot|can.t)\s+pass\b/i,
