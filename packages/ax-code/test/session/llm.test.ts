...
describe("session.llm.stream", () => {
  ...
  test("sends OpenRouter bearer auth after config and model header merging", async () => {
    const providerID = "openrouter"
    const modelID = "openrouter/auto"
    const fixture = await loadFixture(providerID, modelID)
    const model = fixture.model
          ...
          JSON.stringify({
            ...
            provider: {
              [providerID]: {
                options: {
                  apiKey: " \n ",
                  baseURL: `${state.server.url.origin}/api/v1`,
                  headers: {
                    authorization: " ",
                  },
                },
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const resolved = await Provider.getModel(ProviderID.make(providerID), ModelID.make(model.id))
        const sessionID = SessionID.make("session-test-openrouter-auth")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const stream = await LLM.stream({
          user: {
            id: MessageID.make("user-openrouter-auth"),
            sessionID,
            role: "user",
            time: { created: Date.now() },
            agent: agent.name,
