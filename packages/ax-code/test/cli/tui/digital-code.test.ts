...
describe("reverse exit rain", () => {
  ...
  test("the /ev and /ov previews stay out of the slash menu", () => {
    const commands = readFileSync(path.join(import.meta.dirname, "../../../src/cli/tui", "app-commands.ts"), "utf8")
    // Typing either name still runs this launch's preview. The slash menu
    // omits them so the palette is not filled with the launch pair.
    expect(commands).toMatch(/slash: \{ name: "ev", hidden: true \}/)
    expect(commands).toMatch(/slash: \{ name: "ov", aliases: \["op"\], hidden: true \}/)
  })
