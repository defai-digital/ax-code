...
describe("tui sync bootstrap task", () => {
  test("normalizes arbitrary promise values before applying them", async () => {
    ...
    await createBootstrapTask(() => Promise.resolve(["a", "b"]), (value) => value.join(","), (value) => {
      applied.push(value)
    })()
  ...
  test("maps response data through the normalize step before applying", async () => {
    ...
    await createBootstrapResponseTask(() => Promise.resolve({ data: [1, 2, 3] }), (value) => (value ?? []).length, (value) => {
      applied.push(value)
    })()
