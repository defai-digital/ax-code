...
describe("footer stickman indicator", () => {
  test("steps an 8x5 matchstick man through walk/punch/kick poses", () => {
    // A braille cell is 2x4: 8 columns become 4 cells, 5 rows become 2 lines.
    expect(FOOTER_STICKMAN_FRAMES).toHaveLength(6)
    for (const frame of FOOTER_STICKMAN_FRAMES) {
      expect([...frame.top]).toHaveLength(4)
      expect([...frame.bottom]).toHaveLength(4)
    }
