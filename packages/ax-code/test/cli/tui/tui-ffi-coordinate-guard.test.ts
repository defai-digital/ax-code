...
// Regression guard for the unstoppable TUI crash where the renderer loop
// spammed `TypeError: Argument N must be a uint32` every frame. The native
...
// against the exact crash inputs. If an upstream renderer sync drops the guard,
