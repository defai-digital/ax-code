...
describe("Fuji Mountain scenes", () => {
  ...
  test("day and night share the sunset composition with distinct sky and celestial colors", () => {
    ...
    expect(day.flat().some((r) => r.color === "#b07a88")).toBe(true)
    expect(night.flat().some((r) => r.color === "#557a85")).toBe(true)
    expect(day.flat().some((r) => r.background === "#432a42")).toBe(true)
    expect(night.flat().some((r) => r.background === "#2d4454")).toBe(true)
    expect(day.flat().some((r) => r.background === "#f4c9b4")).toBe(true)
    expect(day.flat().some((r) => r.background === "#ff96b4")).toBe(true)
    expect(day.flat().some((r) => r.background === "#a86278")).toBe(true)
    ...
    expect(fujiSkyRgb("fuji-day", 1)).toEqual([242, 166, 94])
...
test("daytime renders the sunset composition with petals at their cycle start", () => {
  ...
  expect(rows[12]!.some((run) => run.text.includes("(") && run.color === "#dd6f92")).toBe(true)
  expect(rows[10]!.some((run) => run.background === "#a86278")).toBe(true)
