...
describe("dialog model options", () => {
  ...
  test("does not exempt retired CLI providers from tool-call requirements", () => {
    ...
    expect(dialogModelOptionDisabled("kimi-cli", "kimi-cli", model(false))).toBe(true)
    expect(dialogModelOptionDisabled("minimax-cli", "minimax-cli", model(false))).toBe(true)
