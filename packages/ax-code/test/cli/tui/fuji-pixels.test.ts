...
test.each(["fuji-dawn", "fuji-night"] as const)("%s keeps the sky fixed while the train moves and loops", (style) => {
  ...
  expect([...first.subarray(0, 3)]).toEqual(style === "fuji-dawn" ? [30, 27, 75] : [3, 8, 20])
  expect([...first.subarray((439 * 780 + 0) * 3, (439 * 780 + 1) * 3)]).toEqual(
    style === "fuji-dawn" ? [244, 63, 94] : [29, 53, 87],
  )
...
test("Fuji retains higher resolution without changing Digital Code bounds", () => {
  const fuji = createDigitalCodePixels(3840, 2160, "down", undefined, "fuji-dawn")
...
test("Fuji transport sends the scene, preserves animation time on resize, and deletes its image", () => {
  ...
  player.draw(input)
  expect(decode(writes[0]!)).toEqual(renderFujiPixels(780, 440, "fuji-dawn", 1200))
  player.draw({ ...input, width: 600 })
  expect(writes[1]).toContain("a=d,d=I")
  expect(decode(writes[2]!)).toEqual(renderFujiPixels(600, 440, "fuji-dawn", 1200))
