...
const RENDERABLE_SAFETY_SRC = path.join(TUI_ROOT, "util/renderable-safety.ts")
...
describe("tui OpenTUI stability guardrails", () => {
  ...
  test("keeps OpenTUI renderable operations behind named safety helpers", async () => {
    ...
    const session = await fs.readFile(SESSION_ROUTE_SRC, "utf8")
    const prompt = await fs.readFile(PROMPT_SRC, "utf8")
    const dialog = await fs.readFile(DIALOG_SRC, "utf8")
    const keybind = await fs.readFile(path.join(TUI_ROOT, "context/keybind.tsx"), "utf8")
    const autocomplete = await fs.readFile(AUTOCOMPLETE_SRC, "utf8")
    const dialogSelect = await fs.readFile(DIALOG_SELECT_SRC, "utf8")

    expect(safety).toContain('Log.create({ service: "tui.renderable-safety" })')
    expect(safety).toContain("export function focusRenderable")
    expect(safety).toContain("export function blurRenderable")
    expect(safety).toContain("export function renderableChildren")
    expect(safety).toContain("export function findRenderableChild")

    expect(session).toContain("renderableChildren<ScrollChild>")
    expect(session).toContain("findRenderableChild<ScrollChild>")
    expect(session).toContain("focusRenderable(promptRef.current")
    expect(prompt).toContain("focusRenderable(input")
    expect(prompt).toContain("blurRenderable(input")
    expect(prompt).toContain("focusRenderable(r.target")
    expect(dialog).toContain("renderableChildren<Renderable>")
    expect(dialog).toContain("focusRenderable(focus")
    expect(dialog).toContain("blurRenderable(focus")
    expect(keybind).toContain("focusRenderable(focus")
    expect(keybind).toContain("blurRenderable(focus")
    expect(autocomplete).toContain("findRenderableChild<{")
    expect(dialogSelect).toContain("findRenderableChild<{")
  })
  ...
  test("keeps prompt editing and navigation resilient across duplicate summaries and wide characters", async () => {
    ...
    expect(question).toContain("focusRenderable(val")
  })
  ...
  test("keeps the OpenTUI worker startup bounded with a readiness handshake", async () => {
    ...
    expect(thread).toContain("DEFAULT_TUI_UPGRADE_CHECK_INTERVAL_MS = 24 * 60 * 60 * 1_000")
    ...
    expect(thread).toContain("AX_CODE_TUI_UPGRADE_CHECK_INTERVAL_MS")
    expect(thread).toContain("AX_CODE_TUI_BACKEND_TRANSPORT")
    expect(thread).toContain('path.join(Global.Path.state, "upgrade-check.json")')
    expect(thread).toContain("shouldRunStartupUpgradeCheck()")
