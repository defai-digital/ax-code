...
describe("distribution support guardrails", () => {
  ...
  test("default formula installs the node-bundled distribution", async () => {
    ...
    const nativeManifest = JSON.parse(
      await readFile(path.join(repoRoot, "packages/ax-code/node_modules/ax-tui/vendor/manifest.json"), "utf-8"),
    )
    const dylib = `node_modules/ax-tui/vendor/darwin-arm64/${nativeManifest.targets["darwin-arm64"].lib.file}`
    expect(text).toContain(`AX_TUI_DYLIB = "${dylib}"`)
    expect(text).toContain(`if_path_exists "${dylib}.gz"`)
    expect(text).toContain(`"{{libexec}}/${dylib}.gz"`)
    ...
    expect(text).toContain("export AX_CODE_SYSTEM_NODE=1")
    expect(text).toContain('exec "#{libexec}/bin/ax-code"')
  ...
  test("release workflow verifies draft assets before publishing without package-channel gates", async () => {
    ...
    expect(publishJob![0]).toContain("node script/github-release-assets.mjs inspect")
    expect(publishJob![0]).toContain("node script/extract-changelog-notes.mjs")
    expect(publishJob![0]).toContain("--channel cli")
    expect(publishJob![0]).toContain('--notes-file "$NOTES_FILE"')
    expect(publishJob![0]).not.toContain(".internal/release/release-notes")
    expect(publishJob![0]).toContain("node script/github-release-assets.mjs create")
    expect(publishJob![0]).toContain("node script/github-release-assets.mjs upload")
    expect(publishJob![0]).toContain("node script/github-release-assets.mjs download")
  ...
  test("release workflow signs archives with minisign before uploading assets", async () => {
    ...
    expect(text).toContain("Open draft GitHub release")
    expect(text.indexOf("\n  draft-release:")).toBeGreaterThan(text.indexOf("\n  validate:"))
    expect(text.indexOf("\n  draft-release:")).toBeLessThan(text.indexOf("\n  build:"))
    ...
    expect(job).toContain("node script/github-release-assets.mjs create")
    expect(job).toContain("node script/github-release-assets.mjs upload")
    ...
    expect(job).toContain("release $TAG is already published; refusing to replace its assets")
  ...
  test("install matrix supports macOS installer, Homebrew, Windows, and Linux without npm channels", async () => {
    ...
    expect(text).toContain('SHELL=/bin/bash bash "$RUNNER_TEMP/ax-install" --version "$VERSION"')
  ...
  test("release build matrix smokes each runtime via doctor", async () => {
    ...
    expect(buildJob![0]).toContain('SMOKE_PARENT="$RUNNER_TEMP"')
    expect(buildJob![0]).toContain('cygpath -u "$SMOKE_PARENT"')
    expect(buildJob![0]).toContain('mktemp -d "$SMOKE_PARENT/ax-code-release-smoke.XXXXXX"')
    expect(buildJob![0]).toContain('SMOKE_BIN="$SMOKE_ROOT/bin/$(basename "$BIN")"')
