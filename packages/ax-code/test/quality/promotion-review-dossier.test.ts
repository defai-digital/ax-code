...
function benchmarkBundle(): QualityCalibrationModel.BenchmarkBundle {
  return {
    ...
    model: {
      ...
      source: "review-dossier-model-v1",
    ...
    comparison: {
      ...
      candidateSource: "review-dossier-model-v1",
      overallStatus: "pass",
      dataset: {
  ...
  return QualityPromotionDecisionBundle.build({
    ...
    releasePolicySnapshot: {
      ...
      provenance: QualityPromotionReleasePolicy.PolicyProvenance.parse({
        ...
        policyProjectID: "review-dossier-project-1",
...
async function clearDossiers() {
  const keys = await Storage.list(["quality_model_review_dossier"])
...
describe("QualityPromotionReviewDossier", () => {
  test("creates, persists, verifies, and lists review dossiers", async () => {
    ...
    try {
      ...
      expect(QualityPromotionReviewDossier.verify(bundle, dossier)).toEqual([])

      await QualityPromotionReviewDossier.append(dossier)
      await QualityPromotionReviewDossier.assertPersisted(dossier)

      const listed = await QualityPromotionReviewDossier.list(bundle.source)
      expect(listed).toHaveLength(1)
      expect(listed[0]?.summary.overallStatus).toBe("pass")
      expect(listed[0]?.summary.recommendation).toBe("approve_promotion")
      expect(listed[0]?.submissionBundle.submissionID).toBe(submission.submissionID)
    } finally {
      await clearDossiers()
    }
  })

  test("reports verification failures when the review dossier summary is tampered", () => {
    const bundle = decisionBundle()
    const approval = QualityPromotionApproval.create({
      bundle,
      approver: "reviewer@example.com",
    ...
    const tampered = QualityPromotionReviewDossier.DossierArtifact.parse({
      ...dossier,
      summary: {
        ...dossier.summary,
        recommendation: "hold",
    ...
    const reasons = QualityPromotionReviewDossier.verify(bundle, tampered)
    expect(reasons.some((reason) => reason.includes("review dossier summary mismatch"))).toBe(true)
  })
})
