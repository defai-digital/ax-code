...
function benchmarkBundle(): QualityCalibrationModel.BenchmarkBundle {
  return {
    ...
    model: {
      ...
      source: "handoff-package-model-v1",
    ...
    comparison: {
      ...
      candidateSource: "handoff-package-model-v1",
      overallStatus: "pass",
      dataset: {
  ...
  return QualityPromotionDecisionBundle.build({
    ...
    releasePolicySnapshot: {
      ...
      provenance: QualityPromotionReleasePolicy.PolicyProvenance.parse({
        ...
        policyProjectID: "handoff-package-project-1",
...
async function clearHandoffPackages() {
  for (const prefix of [
...
function buildArchiveManifest() {
  ...
  const auditManifest = QualityPromotionAuditManifest.create({
    ...
    promotion: QualityPromotionAuditManifest.PromotionSnapshot.parse({
      promotionID: "promo_handoff_package_1",
  ...
  return { bundle, archiveManifest }
}

describe("QualityPromotionHandoffPackage", () => {
  test("creates, persists, verifies, and lists handoff packages", async () => {
    const { bundle, archiveManifest } = buildArchiveManifest()
    await clearHandoffPackages()
    try {
      const packet = QualityPromotionHandoffPackage.create({
        archiveManifest,
      })

      expect(QualityPromotionHandoffPackage.verify(packet)).toEqual([])

      await QualityPromotionHandoffPackage.append(packet)
      await QualityPromotionHandoffPackage.assertPersisted(packet)

      const listed = await QualityPromotionHandoffPackage.list(bundle.source)
      expect(listed).toHaveLength(1)
      expect(listed[0]?.packageID).toBe(packet.packageID)
      expect(listed[0]?.summary.documentCount).toBe(11)
      expect(listed[0]?.archiveManifest.exportBundle.auditManifest.promotion.promotionID).toBe(
        "promo_handoff_package_1",
      )
    } finally {
      await clearHandoffPackages()
    }
  ...
  test("reports verification failures when the handoff package documents are tampered", () => {
    const { archiveManifest } = buildArchiveManifest()
    const packet = QualityPromotionHandoffPackage.create({
      archiveManifest,
    })
    const tampered = QualityPromotionHandoffPackage.PackageArtifact.parse({
      ...packet,
      documents: packet.documents.slice(1),
    })

    const reasons = QualityPromotionHandoffPackage.verify(tampered)
    expect(reasons.some((reason) => reason.includes("handoff package documents mismatch"))).toBe(true)
  })
})
