...
function benchmarkBundle(): QualityCalibrationModel.BenchmarkBundle {
  return {
    ...
    model: {
      ...
      source: "archive-manifest-model-v1",
    ...
    comparison: {
      ...
      candidateSource: "archive-manifest-model-v1",
      overallStatus: "pass",
      dataset: {
  ...
  return QualityPromotionDecisionBundle.build({
    ...
    releasePolicySnapshot: {
      ...
      provenance: QualityPromotionReleasePolicy.PolicyProvenance.parse({
        ...
        policyProjectID: "archive-manifest-project-1",
...
function buildExportBundle() {
  ...
  const auditManifest = QualityPromotionAuditManifest.create({
    ...
    promotion: QualityPromotionAuditManifest.PromotionSnapshot.parse({
      promotionID: "promo_archive_manifest_1",
...
describe("QualityPromotionArchiveManifest", () => {
  test("creates, persists, verifies, and lists archive manifests", async () => {
    const { bundle, exportBundle } = buildExportBundle()
    await clearArchives()
    try {
      ...
      expect(QualityPromotionArchiveManifest.verify(archive)).toEqual([])

      await QualityPromotionArchiveManifest.append(archive)
      await QualityPromotionArchiveManifest.assertPersisted(archive)

      const listed = await QualityPromotionArchiveManifest.list(bundle.source)
      expect(listed).toHaveLength(1)
      expect(listed[0]?.archiveID).toBe(archive.archiveID)
      expect(listed[0]?.summary.inventoryCount).toBe(9)
      expect(listed[0]?.exportBundle.auditManifest.promotion.promotionID).toBe("promo_archive_manifest_1")
    } finally {
      await clearArchives()
    }
  })

  test("reports verification failures when the archive manifest inventory is tampered", () => {
    const { exportBundle } = buildExportBundle()
    const archive = QualityPromotionArchiveManifest.create({
      exportBundle,
    })
    const tampered = QualityPromotionArchiveManifest.ArchiveArtifact.parse({
      ...archive,
      inventory: archive.inventory.slice(1),
    })

    const reasons = QualityPromotionArchiveManifest.verify(tampered)
    expect(reasons.some((reason) => reason.includes("archive manifest inventory mismatch"))).toBe(true)
  })
})
