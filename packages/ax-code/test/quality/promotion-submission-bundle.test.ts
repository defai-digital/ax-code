...
function benchmarkBundle(): QualityCalibrationModel.BenchmarkBundle {
  return {
    ...
    model: {
      ...
      source: "submission-bundle-model-v1",
    ...
    comparison: {
      ...
      candidateSource: "submission-bundle-model-v1",
      overallStatus: "pass",
      dataset: {
  ...
  return QualityPromotionDecisionBundle.build({
    ...
    releasePolicySnapshot: {
      ...
      provenance: QualityPromotionReleasePolicy.PolicyProvenance.parse({
        ...
        policyProjectID: "submission-bundle-project-1",
...
async function clearSubmissions() {
  const keys = await Storage.list(["quality_model_submission_bundle"])
...
describe("QualityPromotionSubmissionBundle", () => {
  test("creates, persists, verifies, and lists submission bundles", async () => {
    ...
    try {
      ...
      expect(QualityPromotionSubmissionBundle.verify(bundle, submission)).toEqual([])

      await QualityPromotionSubmissionBundle.append(submission)
      await QualityPromotionSubmissionBundle.assertPersisted(submission)

      const listed = await QualityPromotionSubmissionBundle.list(bundle.source)
      expect(listed).toHaveLength(1)
      expect(listed[0]?.summary.overallStatus).toBe("pass")
      expect(listed[0]?.approvalPacket.packetID).toBe(packet.packetID)
    } finally {
      await clearSubmissions()
    }
  })

  test("reports verification failures when the submission summary is tampered", () => {
    const bundle = decisionBundle()
    const approval = QualityPromotionApproval.create({
      bundle,
      approver: "reviewer@example.com",
    ...
    const tampered = QualityPromotionSubmissionBundle.BundleArtifact.parse({
      ...submission,
      summary: {
        ...submission.summary,
        approvalPacketStatus: "fail",
      },
    })

    const reasons = QualityPromotionSubmissionBundle.verify(bundle, tampered)
    expect(reasons.some((reason) => reason.includes("submission bundle summary mismatch"))).toBe(true)
  })
})
