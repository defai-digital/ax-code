...
import { QualityPromotionAuditManifest } from "../../src/quality/promotion/promotion-audit-manifest"
import { QualityPromotionBoardDecision } from "../../src/quality/promotion/promotion-board-decision"
import { QualityPromotionApprovalPacket } from "../../src/quality/promotion/promotion-approval-packet"
...
import { QualityPromotionEligibility } from "../../src/quality/promotion/promotion-eligibility"
import { QualityPromotionExportBundle } from "../../src/quality/promotion/promotion-export-bundle"
import { QualityPromotionHandoffPackage } from "../../src/quality/promotion/promotion-handoff-package"
import { QualityPromotionPackagedArchive } from "../../src/quality/promotion/promotion-packaged-archive"
import { QualityPromotionPortableExport } from "../../src/quality/promotion/promotion-portable-export"
import { QualityPromotionReleaseDecisionRecord } from "../../src/quality/promotion/promotion-release-decision-record"
import { QualityPromotionReleasePacket } from "../../src/quality/promotion/promotion-release-packet"
import { QualityPromotionReleasePolicy } from "../../src/quality/promotion/promotion-release-policy"
import { QualityPromotionReviewDossier } from "../../src/quality/promotion/promotion-review-dossier"
import { QualityPromotionSubmissionBundle } from "../../src/quality/promotion/promotion-submission-bundle"
...
function benchmarkBundle(): QualityCalibrationModel.BenchmarkBundle {
  return {
    ...
    model: {
      ...
      source: "packaged-archive-model-v1",
    ...
    comparison: {
      ...
      candidateSource: "packaged-archive-model-v1",
      overallStatus: "pass",
      dataset: {
  ...
  return QualityPromotionDecisionBundle.build({
    ...
    releasePolicySnapshot: {
      ...
      provenance: QualityPromotionReleasePolicy.PolicyProvenance.parse({
        ...
        policyProjectID: "packaged-archive-project-1",
...
function buildPortableExport() {
  ...
  const auditManifest = QualityPromotionAuditManifest.create({
    ...
    promotion: QualityPromotionAuditManifest.PromotionSnapshot.parse({
      promotionID: "promo_packaged_archive_1",
  ...
  return { bundle, portableExport }
}

describe("QualityPromotionPackagedArchive", () => {
  test("creates, persists, verifies, lists, materializes, and extracts packaged archives", async () => {
    const { bundle, portableExport } = buildPortableExport()
    ...
    try {
      ...
      expect(QualityPromotionPackagedArchive.verify(archive)).toEqual([])

      await QualityPromotionPackagedArchive.append(archive)
      await QualityPromotionPackagedArchive.assertPersisted(archive)

      const listed = await QualityPromotionPackagedArchive.list(bundle.source)
      expect(listed).toHaveLength(1)
      expect(listed[0]?.archiveID).toBe(archive.archiveID)
      expect(listed[0]?.summary.entryCount).toBe(archive.entries.length)

      const archiveFile = path.join(tmp.path, "bundle", "promotion.bundle.json")
      const writeResult = await QualityPromotionPackagedArchive.materialize(archive, archiveFile)
      expect(writeResult.byteLength).toBeGreaterThan(0)
      const archiveJson = await fs.readFile(archiveFile, "utf8")
      expect(archiveJson).toContain('"kind": "ax-code-quality-promotion-packaged-archive"')

      const outDir = path.join(tmp.path, "extracted")
      const extractResult = await QualityPromotionPackagedArchive.extract(archive, outDir)
      expect(extractResult.entryCount).toBe(archive.entries.length)
  ...
  test("reports verification failures when the packaged archive entries are tampered", () => {
    const { portableExport } = buildPortableExport()
    const archive = QualityPromotionPackagedArchive.create({
      portableExport,
    })
    const tampered = QualityPromotionPackagedArchive.ArchiveArtifact.parse({
      ...archive,
      entries: archive.entries.slice(1),
    })

    const reasons = QualityPromotionPackagedArchive.verify(tampered)
    expect(reasons.some((reason) => reason.includes("packaged archive entries mismatch"))).toBe(true)
  })
})
