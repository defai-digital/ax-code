...
function benchmarkBundle(): QualityCalibrationModel.BenchmarkBundle {
  return {
    ...
    model: {
      ...
      source: "portable-export-model-v1",
    ...
    comparison: {
      ...
      candidateSource: "portable-export-model-v1",
      overallStatus: "pass",
      dataset: {
  ...
  return QualityPromotionDecisionBundle.build({
    ...
    releasePolicySnapshot: {
      ...
      provenance: QualityPromotionReleasePolicy.PolicyProvenance.parse({
        ...
        policyProjectID: "portable-export-project-1",
...
async function clearPortableExports() {
  for (const prefix of [
...
function buildHandoffPackage() {
  ...
  const auditManifest = QualityPromotionAuditManifest.create({
    ...
    promotion: QualityPromotionAuditManifest.PromotionSnapshot.parse({
      promotionID: "promo_portable_export_1",
  ...
  return { bundle, handoffPackage }
}

describe("QualityPromotionPortableExport", () => {
  test("creates, persists, verifies, lists, and materializes portable exports", async () => {
    const { bundle, handoffPackage } = buildHandoffPackage()
    ...
    try {
      ...
      expect(QualityPromotionPortableExport.verify(exportArtifact)).toEqual([])

      await QualityPromotionPortableExport.append(exportArtifact)
      await QualityPromotionPortableExport.assertPersisted(exportArtifact)

      const listed = await QualityPromotionPortableExport.list(bundle.source)
      expect(listed).toHaveLength(1)
      expect(listed[0]?.exportID).toBe(exportArtifact.exportID)
      expect(listed[0]?.summary.fileCount).toBe(exportArtifact.files.length)

      const outDir = path.join(tmp.path, "portable-export")
      const result = await QualityPromotionPortableExport.materialize(exportArtifact, outDir)
      expect(result.fileCount).toBe(exportArtifact.files.length)
  ...
  test("reports verification failures when the portable export files are tampered", () => {
    const { handoffPackage } = buildHandoffPackage()
    const exportArtifact = QualityPromotionPortableExport.create({
      handoffPackage,
    })
    const tampered = QualityPromotionPortableExport.ExportArtifact.parse({
      ...exportArtifact,
      files: exportArtifact.files.slice(1),
    })

    const reasons = QualityPromotionPortableExport.verify(tampered)
    expect(reasons.some((reason) => reason.includes("portable export files mismatch"))).toBe(true)
  })
})
