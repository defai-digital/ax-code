...
async function writeManagedSettings(settings: object, filename = "ax-code.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "ax-code.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "ax-code.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  Account.config = mock(async () => ({
    provider: {
      opencode: {
        options: {
          apiKey: "{env:AX_CODE_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["AX_CODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["AX_CODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("loads agents from .ax-code/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".ax-code")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .ax-code/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".ax-code")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = await Filesystem.readJson(path.join(tmp.path, "ax-code.json"))
...
test("does not try to install dependencies in read-only AX_CODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
...
test("drops unresolved package plugins from untrusted config directories", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ax-code config with different plugins
      await Filesystem.write(
...
test("drops duplicate unresolved package plugins from untrusted config directories", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ax-code config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "ax-code.json"),
        JSON.stringify({
          $schema: "https://raw.githubusercontent.com/defai-digital/ax-code/main/packages/ax-code/config.schema.json",
...
// Note: preload.ts sets AX_CODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .ax-code config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ax-code directory config enables it
      const opencodeDir = path.join(dir, ".ax-code")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "ax-code.json"),
...
describe("getPluginName", () => {
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@ax-code/plugin@2.0.0")).toBe("@ax-code/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.ax-code/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.ax-code/plugin/oh-my-opencode.js")
  })
...
describe("AX_CODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["AX_CODE_DISABLE_PROJECT_CONFIG"]
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["AX_CODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["AX_CODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .ax-code/ directories when flag is set", async () => {
    const originalEnv = process.env["AX_CODE_DISABLE_PROJECT_CONFIG"]
    process.env["AX_CODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .ax-code directory with a command
          const opencodeDir = path.join(dir, ".ax-code", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .ax-code should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["AX_CODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AX_CODE_CONFIG_DIR"]
    ...
    try {
      ...
      if (originalDisable === undefined) {
        ...
        process.env["AX_CODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
  ...
  test("AX_CODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["AX_CODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["AX_CODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["AX_CODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["AX_CODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from AX_CODE_CONFIG_DIR, not project
...
describe("AX_CODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in AX_CODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["AX_CODE_CONFIG_CONTENT"]
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["AX_CODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["AX_CODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in AX_CODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["AX_CODE_CONFIG_CONTENT"]
