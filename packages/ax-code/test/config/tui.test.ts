...
test("migrates tui-specific keys from ax-code.json when tui.json does not exist", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "ax-code.json")))
      expect(server.theme).toBeUndefined()
      expect(server.keybinds).toBeUndefined()
      expect(server.tui).toBeUndefined()
      expect(await Filesystem.exists(path.join(tmp.path, "ax-code.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when ax-code.jsonc is syntactically invalid", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "ax-code.jsonc.tui-migration.bak"))).toBe(false)
      const source = await Filesystem.readText(path.join(tmp.path, "ax-code.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "ax-code.json.tui-migration.bak"))).toBe(false)
    },
  })
})
...
test("migration backup preserves JSONC comments", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await TuiConfig.get()
      const backup = await Filesystem.readText(path.join(tmp.path, "ax-code.jsonc.tui-migration.bak"))
...
test("AX_CODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from AX_CODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
test("loads .ax-code/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".ax-code"), { recursive: true })
