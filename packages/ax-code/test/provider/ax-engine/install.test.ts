...
describe("resolveInstallableRelease", () => {
  ...
  test("pins a self-contained darwin-arm64 release and refuses other hosts", () => {
    expect(AX_ENGINE_BINARY_RELEASE.version).toBe("7.5.7")
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'if [ "$1" = "doctor" ]; then echo \'{"install":{"version":"7.5.7"}}\'; exit 0; fi',
          "exit 1",
          "",
        ].join("\n"),
        { mode: 0o755 },
      ...
      expect(status).toMatchObject({ available: true, mode: "configured", version: "7.5.7", blockers: [] })
    } finally {
      await fs.rm(dir, { recursive: true, force: true })
    }
  })

  test.each(["6.6.0", "7.5.6"])("blocks configured AX Engine version %s below 7.5.7", async (version) => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "axe-version-"))
    try {
      const binary = path.join(dir, "ax-engine")
      ...
      expect(status.blockers.join(" ")).toContain("7.5.7")
    } finally {
      await fs.rm(dir, { recursive: true, force: true })
    }
  })
