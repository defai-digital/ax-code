...
describe("cloud API providers: DeepSeek + Meta Muse Spark", () => {
  test("deepseek and meta are native setup/login providers", () => {
    ...
    expect(DEFAULT_SETUP_PROVIDER_IDS).not.toContain("kimi-cli")
    expect(DEFAULT_SETUP_PROVIDER_IDS).not.toContain("minimax-cli")
    expect(DEFAULT_SETUP_PROVIDER_IDS).not.toContain("qoder-cli")
    expect(
  ...
  test("catalog exposes Meta Muse Spark models and MODEL_API_KEY aliases", async () => {
    ...
    expect(meta?.models["muse-spark-1.2"]).toBeUndefined()
    expect(meta?.models["muse-spark-1.1"]).toBeUndefined()
    expect(meta?.models["muse-spark-1.3"]?.tool_call).toBe(true)
    expect(meta?.models["muse-spark-1.3"]?.limit?.context).toBeGreaterThanOrEqual(1_000_000)
  })
