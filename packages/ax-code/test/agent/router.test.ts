...
describe("v2-style keyword route", () => {
  ...
  test("substring matching is intentional — accepts plural / suffixed forms", () => {
    // v2 used .includes() on lowercased text. We keep that simplicity.
  ...
  test("multi-rule message picks the highest-confidence specialist", () => {
    // "performance" + "bottleneck" weighs more for perf than "test" alone for test.
    const result = route("the test suite has a performance bottleneck", "build")
