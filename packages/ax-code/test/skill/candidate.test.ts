...
test("requires the current content fingerprint instead of a legacy clean-status digest", async () => {
  await using tmp = await tmpdir({ git: true })
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const first = await verification((await Session.create({})).id)
      if (first.part.state.status !== "completed") throw new Error("Expected completed verification")
      const envelope = first.part.state.metadata.verificationEnvelopes[0]
      expect(envelope.sourceState.dirtyDigest).toMatch(/^content-v1:/)
      for (const dirtyDigest of [Hash.fast(""), "content-v1:stale"]) {
        await Session.updatePart({
          ...first.part,
          state: {
            ...first.part.state,
            metadata: {
              ...first.part.state.metadata,
              verificationEnvelopes: [{ ...envelope, sourceState: { ...envelope.sourceState, dirtyDigest } }],
            },
          },
        })
        await expect(SkillCandidate.propose(proposal(first.evidence))).rejects.toThrow("current clean revision")
      }
      await Session.updatePart(first.part)
      expect((await SkillCandidate.propose(proposal(first.evidence))).source.commit).toBe(envelope.sourceState.commit)
    },
  })
})
