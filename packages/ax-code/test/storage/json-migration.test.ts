...
// Bun.write (which these fixtures originally used) creates missing parent
// directories; node's fs.writeFile does not. Wrap it so fixture writes into
// per-id subdirs (message/<sid>, part/<mid>) don't ENOENT after the port.
...
async function setupStorageDir() {
  ...
  await writeFile(path.join(storageDir, "migration"), "1")
  return storageDir
}

async function writeProject(storageDir: string, project: Record<string, unknown>) {
  await writeFile(path.join(storageDir, "project", `${project.id}.json`), JSON.stringify(project))
}

async function writeSession(storageDir: string, projectID: string, session: Record<string, unknown>) {
  await writeFile(path.join(storageDir, "session", projectID, `${session.id}.json`), JSON.stringify(session))
}
...
describe("JSON to SQLite migration", () => {
  ...
  test("migrates permissions", async () => {
    ...
    await writeFile(path.join(storageDir, "permission", "proj_test123abc.json"), JSON.stringify(permissionData))
  ...
  test("continues when a JSON file is unreadable and records an error", async () => {
    ...
    await writeFile(path.join(storageDir, "project", "broken.json"), "{ invalid json")
  ...
  test("continues when a JSON read failure cannot be stringified", async () => {
    ...
    await writeFile(path.join(storageDir, "project", "unprintable.json"), JSON.stringify({ worktree: "/" }))
  ...
  test("handles mixed corruption and partial validity in one migration run", async () => {
    ...
    await writeFile(path.join(storageDir, "project", "proj_broken.json"), "{ nope")
    ...
    await writeFile(path.join(storageDir, "message", "ses_test456def", "msg_broken.json"), "{ nope")
    await writeFile(
    ...
    await writeFile(path.join(storageDir, "part", "msg_ok", "part_broken.json"), "{ nope")
    ...
    await writeFile(path.join(storageDir, "todo", "ses_broken.json"), "{ nope")
    ...
    await writeFile(path.join(storageDir, "permission", "proj_broken.json"), "{ nope")
    ...
    await writeFile(path.join(storageDir, "session_share", "ses_broken.json"), "{ nope")
