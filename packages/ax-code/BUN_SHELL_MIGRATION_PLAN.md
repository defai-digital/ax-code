...
Replace runtime Bun shell template-tag usage in `packages/ax-code/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/ax-code/src` is removed except:
