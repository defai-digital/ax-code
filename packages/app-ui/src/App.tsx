...
function App({ apis }: AppProps) {
  ...
  React.useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'ax-code:embedded-visibility') {
        return;
      }
    ...
    const scopedWindow = window as unknown as {
      __axCodeSetEmbeddedVisibility?: (payload?: EmbeddedVisibilityPayload) => void;
    };

    scopedWindow.__axCodeSetEmbeddedVisibility = applyVisibility;
    window.addEventListener('message', handleMessage);

    return () => {
      window.removeEventListener('message', handleMessage);
      if (scopedWindow.__axCodeSetEmbeddedVisibility === applyVisibility) {
        delete scopedWindow.__axCodeSetEmbeddedVisibility;
      }
    };
  }, [embeddedSessionChat]);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ax-code:open-session', handler as EventListener);
    return () => window.removeEventListener('ax-code:open-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ax-code:open-draft-session', handler as EventListener);
    return () => window.removeEventListener('ax-code:open-draft-session', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ax-code:open-project', handler as EventListener);
    return () => window.removeEventListener('ax-code:open-project', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    (window as unknown as { __axCodeAppReady?: boolean }).__axCodeAppReady = true;
    window.dispatchEvent(new Event('ax-code:app-ready'));
