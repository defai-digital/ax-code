...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('ax-code:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('ax-code:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    const scopedWindow = window as unknown as {
      __axCodeApplyThemeSync?: (payload: ThemeSyncPayload) => void;
    };

    scopedWindow.__axCodeApplyThemeSync = applyIncomingThemeSync;

    return () => {
      if (scopedWindow.__axCodeApplyThemeSync === applyIncomingThemeSync) {
        delete scopedWindow.__axCodeApplyThemeSync;
      }
    };
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data?.type !== 'ax-code:theme-sync' || !data.payload) {
        return;
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('ax-code:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('ax-code:settings-synced', handleSettingsSynced);
  }, []);
