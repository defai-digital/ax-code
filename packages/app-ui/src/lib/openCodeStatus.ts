...
type ax-codeOpencodeResolution = {
  configured?: unknown;
  resolved?: unknown;
  resolvedDir?: unknown;
  source?: unknown;
...
export const buildOpenCodeStatusReport = async (): Promise<string> => {
  ...
  const openChamberHealth: ax-codeHealthSnapshot | null = await (async () => {
    ...
    try {
      ...
      return json as ax-codeHealthSnapshot;
    } catch {
      return null;
    } finally {
      clearTimeout(timeout);
  ...
  const openChamberOpencodeResolutionResult: {
    data: ax-codeOpencodeResolution | null;
    status: number | null;
    error: string | null;
  } = await (async () => {
    if (!origin) return null;
    ...
    try {
      ...
      return { data: json as ax-codeOpencodeResolution, status: resp.status, error: null };
    } catch (error) {
      return {
        data: null,
        status: null,
