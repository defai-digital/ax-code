/**
 * ax-code project-level configuration service.
 * Stores per-project settings in ~/.config/ax-code/<projectId>.json.
 * Migrates from legacy <project>/.ax-code/ax-code.json.
 */
...
const CONFIG_FILENAME = 'ax-code.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.ax-code';
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'ax-code', 'projects'];
...
export interface ax-codeConfig {
  projectPath?: string;
  'setup-worktree'?: string[];
  projectNotes?: string;
  projectTodos?: ax-codeProjectTodoItem[];
  projectPlanFiles?: ax-codeProjectPlanFileLink[];
  projectActions?: ax-codeProjectAction[];
  projectActionsPrimaryId?: string;
}

export type ax-codeProjectActionPlatform = 'macos' | 'linux' | 'windows';

export interface ax-codeProjectAction {
  id: string;
  name: string;
  command: string;
  icon?: string | null;
  platforms?: ax-codeProjectActionPlatform[];
  autoOpenUrl?: boolean;
  openUrl?: string;
  desktopOpenSshForward?: string;
}

export interface ax-codeProjectActionsState {
  actions: ax-codeProjectAction[];
  primaryActionId: string | null;
}

export interface ax-codeProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface ax-codeProjectPlanFileLink {
  id: string;
  path: string;
  createdAt: number;
}

export interface ax-codeProjectPlanFile {
  title: string;
  body: string;
  raw: string;
  path: string;
...
export interface ax-codeProjectNotesTodos {
  notes: string;
  todos: ax-codeProjectTodoItem[];
}

export interface ax-codeProjectContextData extends ax-codeProjectNotesTodos {
  plans: ax-codeProjectPlanFileLink[];
}
...
const OPENCHAMBER_ACTION_PLATFORM_SET = new Set<ax-codeProjectActionPlatform>(['macos', 'linux', 'windows']);
...
const sanitizeProjectTodoItems = (value: unknown): ax-codeProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: ax-codeProjectTodoItem[] = [];
...
const sanitizeProjectPlanFileLinks = (value: unknown): ax-codeProjectPlanFileLink[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: ax-codeProjectPlanFileLink[] = [];
...
const sanitizeProjectActionPlatforms = (value: unknown): ax-codeProjectActionPlatform[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const unique: ax-codeProjectActionPlatform[] = [];
  const seen = new Set<ax-codeProjectActionPlatform>();
  for (const entry of value) {
    if (typeof entry !== 'string') {
      continue;
    }
    const normalized = entry.trim().toLowerCase() as ax-codeProjectActionPlatform;
...
const sanitizeProjectActions = (value: unknown): ax-codeProjectAction[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: ax-codeProjectAction[] = [];
  const seenIds = new Set<string>();
...
} | null | undefined): ax-codeProjectActionsState => {
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): ax-codeProjectNotesTodos => {
  return {
...
const sanitizeProjectContextData = (value: {
  ...
  todos?: unknown;
  plans?: unknown;
} | null | undefined): ax-codeProjectContextData => {
 ...
 */
export async function readax-codeConfig(project: ProjectRef): Promise<ax-codeConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): ax-codeConfig | null => {
    ...
    try {
      ...
      return parsed as ax-codeConfig;
    } catch {
      return null;
    }
  };
  ...
  // 2) Migrate legacy <project>/.ax-code/ax-code.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local ax-code.json -> ~/.config/ax-code/projects/<projectId>.json
  ...
  try {
    const wrote = await writeax-codeConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyax-codeConfig(projectDirectory);
    }
  } catch {
 ...
 */
export async function writeax-codeConfig(
  project: ProjectRef,
  config: ax-codeConfig
): Promise<boolean> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return false;
  ...
  try {
    ...
    console.error('Failed to write ax-code config:', error);
    return false;
  }
}
 ...
 */
export async function updateax-codeConfig(
  project: ProjectRef,
  updates: Partial<ax-codeConfig>
): Promise<boolean> {
  const existing = await readax-codeConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeax-codeConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readax-codeConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateax-codeConfig(project, { 'setup-worktree': filtered });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<ax-codeProjectNotesTodos> {
  const config = await readax-codeConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: ax-codeProjectNotesTodos
): Promise<boolean> {
  ...
  return updateax-codeConfig(project, {
    projectNotes: sanitized.notes,
    projectTodos: sanitized.todos,
  });
}

export async function getProjectContextData(project: ProjectRef): Promise<ax-codeProjectContextData> {
  const config = await readax-codeConfig(project);
...
export async function getProjectPlanFiles(project: ProjectRef): Promise<ax-codeProjectPlanFileLink[]> {
  const config = await readax-codeConfig(project);
...
export async function saveProjectPlanFiles(
  project: ProjectRef,
  value: ax-codeProjectPlanFileLink[]
): Promise<boolean> {
...
export async function readProjectPlanFile(path: string): Promise<ax-codeProjectPlanFile | null> {
...
): Promise<ax-codeProjectPlanFileLink | null> {
...
export async function getProjectActionsState(project: ProjectRef): Promise<ax-codeProjectActionsState> {
  const config = await readax-codeConfig(project);
...
export async function saveProjectActionsState(
  project: ProjectRef,
  value: ax-codeProjectActionsState
): Promise<boolean> {
...
async function deleteLegacyax-codeConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
