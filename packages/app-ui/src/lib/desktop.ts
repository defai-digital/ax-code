...
export type DesktopSettings = {
  ...
  // User-added skills catalogs (persisted to ~/.config/ax-code/settings.json)
...
export const downloadDesktopUpdate = async (
  ...
  try {
    if (typeof onProgress === 'function' && tauri?.event?.listen) {
      unlisten = await tauri.event.listen('ax-code:update-progress', (evt) => {
