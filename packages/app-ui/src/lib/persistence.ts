...
const persistToLocalStorage = (settings: DesktopSettings) => {
  ...
  if (typeof settings.pwaAppName === 'string') {
    ...
    if (normalized.length > 0) {
      localStorage.setItem('ax-code.pwaName', normalized);
    } else {
      localStorage.removeItem('ax-code.pwaName');
    }
  }
...
const dispatchSettingsSynced = (settings: DesktopSettings): void => {
  ...
  window.dispatchEvent(new CustomEvent<DesktopSettings>('ax-code:settings-synced', { detail: settings }));
};
