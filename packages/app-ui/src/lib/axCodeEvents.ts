...
type AxCodeEvent = ScheduledTaskRanEvent;
type Listener = (event: AxCodeEvent) => void;
...
const dispatchFromEnvelope = (envelope: { type: string; properties: unknown }) => {
  if (envelope.type === 'ax-code:event-stream-ready') {
    reconnectAttempt = 0;
    return;
  }

  if (envelope.type === 'ax-code:heartbeat') {
    return;
  }

  if (envelope.type !== 'ax-code:scheduled-task-ran') {
    return;
  }
...
const connect = () => {
  ...
  const source = new EventSource('/api/ax-code/events');
  source.onopen = () => {
    resetHeartbeatTimer();
  };
