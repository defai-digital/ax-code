...
export type DesktopSshInstance = {
  ...
  auth: {
    ...
    axCodePassword?: DesktopSshStoredSecret;
  };
...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const remoteRaw = isRecord(value.remoteOpenchamber)
    ...
    : isRecord(value.remote_ax-code)
      ? value.remote_ax-code
      : {};
  ...
  const axCodePassword = parseStoredSecret(authRaw.axCodePassword || authRaw.ax_code_password);
...
export const listenDesktopSshStatus = async (
  ...
  const unlisten = await listen('ax-code:ssh-instance-status', (event) => {
