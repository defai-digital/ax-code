...
const useExternalLinkInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (anchor.getAttribute('data-ax-code-file-link') === 'true') {
        return;
      }
...
const FILE_LINK_SELECTOR = '[data-ax-code-file-link="true"]';
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const clearFileLinkAttributes = (candidate: HTMLElement) => {
      candidate.removeAttribute('data-ax-code-file-link');
      candidate.removeAttribute('data-ax-code-file-ref');
      candidate.removeAttribute('data-ax-code-file-path');
    ...
    const annotateFileLinks = () => {
      ...
      for (const candidate of Array.from(candidates)) {
        ...
        void fileReferenceExists(resolved.resolvedPath).then((exists) => {
          ...
          candidate.setAttribute('data-ax-code-file-link', 'true');
          candidate.setAttribute('data-ax-code-file-ref', latestRawCandidate);
          candidate.setAttribute('data-ax-code-file-path', latestResolved.resolvedPath);
    ...
    const openFileReference = (sourceElement: HTMLElement) => {
      const raw = sourceElement.getAttribute('data-ax-code-file-ref') || extractPathCandidateFromElement(sourceElement);
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const target = event.target;
      if (!(target instanceof HTMLElement) || target.getAttribute('data-ax-code-file-link') !== 'true') {
        return;
      }
