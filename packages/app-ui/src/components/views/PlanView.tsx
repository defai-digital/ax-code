...
import { parseProjectPlanMarkdown } from '@/lib/axCodeConfig';
...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'ax-code:plan:md-viewer-mode';
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewax-codeSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, currentProjectRef, initializeNewax-codeSession, pendingPlanSend, resolvedPath, routeToChat, sendMessage, sendPromptTitle, setCurrentSession]
  );
