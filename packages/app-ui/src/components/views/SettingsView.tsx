...
import type { ax-codeSection } from '@/components/sections/ax-code/types';
import { ax-codePage } from '@/components/sections/ax-code/ax-codePage';
...
const SETTINGS_DETAIL_HISTORY_KEY = '__axCodeSettingsDetail';
...
export const SettingsView: React.FC<SettingsViewProps> = ({ onClose, forceMobile, isWindowed }) => {
  ...
  const openChamberSectionBySlug: Partial<Record<SettingsPageSlug, ax-codeSection>> = React.useMemo(() => ({
    appearance: 'visual',
    chat: 'chat',
    shortcuts: 'shortcuts',
    sessions: 'sessions',
  ...
  const renderPageContent = React.useCallback((slug: SettingsPageSlug) => {
    ...
    switch (slug) {
      ...
      case 'tunnel': {
        ...
        return <ax-codePage section={section} />;
      }
      default:
        return <SettingsHome onOpen={openPage} />;
    }
