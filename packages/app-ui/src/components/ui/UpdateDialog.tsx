...
const GITHUB_RELEASES_URL = 'https://github.com/btriapitsyn/ax-code/releases';
...
async function installWebUpdate(): Promise<InstallWebUpdateResult> {
  try {
    const response = await fetch('/api/ax-code/update-install', {
      method: 'POST',
...
async function waitForUpdateApplied(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      const response = await fetch('/api/ax-code/update-check', {
        method: 'GET',
        headers: { Accept: 'application/json' },
      });
      if (response.ok) {
...
export const UpdateDialog: React.FC<UpdateDialogProps> = ({
  ...
  const updateCommand = info?.updateCommand || 'ax-code update';
