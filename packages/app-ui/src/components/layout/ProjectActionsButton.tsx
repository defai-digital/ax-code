...
const AUTO_DISCOVER_ACTION_ID = '__ax_code_auto_discover_preview__';
...
export const ProjectActionsButton = ({
  ...
  const [actions, setActions] = React.useState<ax-codeProjectAction[]>([]);
  ...
  const autoDiscoverAction = React.useMemo<ax-codeProjectAction>(() => ({
  ...
  const getOrCreateActionTab = React.useCallback(async (action: ax-codeProjectAction, options: { revealTerminal?: boolean } = {}) => {
  ...
  const runAction = React.useCallback(async (action: ax-codeProjectAction) => {
    ...
    try {
      const discovered = action.id === AUTO_DISCOVER_ACTION_ID
        ? await (async (): Promise<ax-codeProjectAction> => {
  ...
  const stopAction = React.useCallback(async (action: ax-codeProjectAction) => {
    const runKey = toProjectActionRunKey(normalizedDirectory, action.id);
  ...
  const handleSelectAction = React.useCallback((action: ax-codeProjectAction, toggleStopIfRunning = false) => {
