...
import { ax-codeLogo } from "@/components/ui/ax-codeLogo";
...
const DESKTOP_BROWSER_INSPECT_SCRIPT = `new Promise((resolve) => {
  const existing = document.getElementById('__ax_code_desktop_browser_overlay');
  if (existing) existing.remove();
  if (typeof window.__axCodeDesktopBrowserCancelInspect === 'function') {
    try { window.__axCodeDesktopBrowserCancelInspect(); } catch { /* webview not ready */ }
  }
  const overlay = document.createElement('div');
  overlay.id = '__ax_code_desktop_browser_overlay';
  ...
  const cleanup = () => {
    ...
    if (window.__axCodeDesktopBrowserCancelInspect === cancel) {
      delete window.__axCodeDesktopBrowserCancelInspect;
    }
  };
  const cancel = () => {
    cleanup();
  ...
  window.__axCodeDesktopBrowserCancelInspect = cancel;
...
const DESKTOP_BROWSER_CANCEL_INSPECT_SCRIPT = `(() => {
  if (typeof window.__axCodeDesktopBrowserCancelInspect === 'function') {
    window.__axCodeDesktopBrowserCancelInspect();
    return;
  }
  const overlay = document.getElementById('__ax_code_desktop_browser_overlay');
...
const PreviewPane: React.FC<PreviewPaneProps> = ({ rawUrl, onNavigate }) => {
  ...
  React.useEffect(() => {
    ...
    const handler = (event: MessageEvent<PreviewBridgeMessage>) => {
      ...
      if (!data || data.source !== 'ax-code-preview-bridge' || data.version !== 1) {
        return;
      }
    ...
    <div className="absolute inset-0 flex flex-col bg-background">
      ...
      <div className="relative min-h-0 flex-1 bg-background">
        <webview
          ...
          partition="persist:ax-code-browser"
        ...
        {(!currentUrl || currentUrl === 'about:blank') && !isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 bg-background p-6 text-center">
            <ax-codeLogo width={140} height={140} className="opacity-20" />
  ...
  const postThemeSyncToEmbeddedChat = React.useCallback(() => {
    ...
    for (const frame of chatFrameRefs.current.values()) {
      ...
      const directThemeSync = (frameWindow as unknown as {
        __axCodeApplyThemeSync?: (themePayload: typeof payload) => void;
      }).__axCodeApplyThemeSync;
  ...
  const postEmbeddedVisibilityToChats = React.useCallback(() => {
    ...
    for (const [tabID, frame] of chatFrameRefs.current.entries()) {
      ...
      const directVisibilitySync = (frameWindow as unknown as {
        __axCodeSetEmbeddedVisibility?: (visibilityPayload: typeof payload) => void;
      }).__axCodeSetEmbeddedVisibility;
      ...
      frameWindow.postMessage(
        {
          type: 'ax-code:embedded-visibility',
          payload,
        },
        window.location.origin,
      );
