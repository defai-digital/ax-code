...
export const VSCodeLayout: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('ax-code.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('ax-code.updateSessionEditorTitle', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ax-code:connection-status', handler as EventListener);
    return () => window.removeEventListener('ax-code:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ax-code:navigate', handler as EventListener);
    return () => window.removeEventListener('ax-code:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('ax_code_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[ax-code][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[ax-code][VSCode][bootstrap] post-load', {
