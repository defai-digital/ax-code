...
const WindowsWindowControls = React.memo(function WindowsWindowControls({ visible }: WindowsWindowControlsProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('ax-code:window-maximized-changed', handleMaximizedChange);
    return () => {
      disposed = true;
      window.removeEventListener('ax-code:window-maximized-changed', handleMaximizedChange);
    };
  }, [visible]);
