  ...
  const handleBrowse = React.useCallback(async () => {
    ...
    try {
      const selected = await tauri.dialog.open({
        title: t('settings.ax-code.opencodeCli.dialog.selectBinaryTitle'),
        multiple: false,
        directory: false,
      });
  ...
  const handleSaveAndReload = React.useCallback(async () => {
    ...
    try {
      ...
      await reloadOpenCodeConfiguration({
        message: t('settings.ax-code.opencodeCli.actions.restartingOpenCode'),
        mode: 'projects',
        scopes: ['all'],
      });
    } finally {
      ...
      <div className="mb-1 px-1">
        <div className="flex items-center gap-2">
          <h3 className="typography-ui-header font-medium text-foreground">
            {t('settings.ax-code.opencodeCli.title')}
          </h3>
          <Tooltip>
            ...
            <TooltipContent sideOffset={8} className="max-w-xs">
              {t('settings.ax-code.opencodeCli.tooltipPrefix')}
              {' '}
              <code className="font-mono text-xs">opencode</code>
              {t('settings.ax-code.opencodeCli.tooltipSuffix')}
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      <section className="px-2 pb-2 pt-0 space-y-0.5">
        <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-3">
          <div className="flex min-w-0 flex-col shrink-0">
            <span className="typography-ui-label text-foreground">{t('settings.ax-code.opencodeCli.field.binaryPath')}</span>
          </div>
          <div className="flex min-w-0 items-center gap-2 sm:w-[20rem]">
            <Input
              value={value}
              onChange={(e) => setValue(e.target.value)}
              placeholder={t('settings.ax-code.opencodeCli.field.binaryPathPlaceholder')}
            ...
            <Button
              ...
              aria-label={t('settings.ax-code.opencodeCli.actions.browseAria')}
              title={t('settings.ax-code.opencodeCli.actions.browse')}
            >
        ...
        <div className="py-1.5">
          <div className="typography-micro text-muted-foreground/70">
            {t('settings.ax-code.opencodeCli.tipPrefix')}
            {' '}
            <span className="font-mono">OPENCODE_BINARY</span>
            {' '}
            {t('settings.ax-code.opencodeCli.tipMiddle')}
            {' '}
            <span className="font-mono">~/.config/ax-code/settings.json</span>
            {'.'}
          </div>
        </div>
        ...
        <label className="flex cursor-pointer items-center gap-2 py-1.5">
          <Checkbox
            ...
            ariaLabel={t('settings.ax-code.opencodeCli.field.showUpdateNotificationsAria')}
          />
          <span className="typography-ui-label text-foreground">
            {t('settings.ax-code.opencodeCli.field.showUpdateNotifications')}
          </span>
        </label>
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? t('settings.common.actions.saving') : t('settings.ax-code.opencodeCli.actions.saveAndReload')}
          </Button>
        </div>
      </section>
    </div>
