...
const phaseLabelKey = (phase?: string): I18nKey => {
  switch (phase) {
    ...
    case 'installing':
      return 'settings.remoteInstances.page.phase.installingax-code';
    case 'updating':
      return 'settings.remoteInstances.page.phase.updatingax-code';
...
export const RemoteInstancesPage: React.FC = () => {
  ...
  const handleSave = React.useCallback(async () => {
    ...
    if (
      normalized.auth.axCodePassword?.enabled &&
      normalized.auth.axCodePassword.value?.trim() &&
      normalized.auth.axCodePassword.store !== 'settings'
    ) {
      const store = window.confirm(t('settings.remoteInstances.page.confirm.storeUiPasswordPlaintext'));
      normalized.auth.axCodePassword.store = store ? 'settings' : 'never';
      if (!store) {
        normalized.auth.axCodePassword.value = undefined;
      }
    }
        ...
        <section className="px-2 pb-2 pt-0 space-y-3">
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            ...
            <Input
              className="h-7 md:max-w-sm"
              type="password"
              value={draft.auth.axCodePassword?.value || ''}
              onChange={(event) =>
                updateDraft((current) => ({
                  ...current,
                  auth: {
                    ...
                    axCodePassword: {
                      enabled: event.target.value.trim().length > 0,
                      value: event.target.value,
                      store: current.auth.axCodePassword?.store || 'never',
                    },
                  },
                }))
              }
