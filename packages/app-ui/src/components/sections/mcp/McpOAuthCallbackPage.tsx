...
export const McpOAuthCallbackPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        if (!code) {
          throw new Error('Missing OAuth authorization code. Start authorization again from MCP Settings or paste the returned code into ax-code manually.');
        }
        ...
        if (!pendingContext?.name) {
          throw new Error('Authorization session details were not available. Start authorization again from MCP Settings or paste the returned code into ax-code manually.');
        }
