...
import {
  ...
  type ax-codeProjectPlanFileLink,
  type ax-codeProjectTodoItem,
  type ProjectRef,
} from '@/lib/axCodeConfig';
...
type ProjectPlanListItem = ax-codeProjectPlanFileLink & {
  title: string;
};

const toPlanListItem = async (
  plan: ax-codeProjectPlanFileLink,
...
const sortTodosWithCompletedLast = (items: ax-codeProjectTodoItem[]): ax-codeProjectTodoItem[] => [
  ...items.filter((todo) => !todo.completed),
  ...items.filter((todo) => todo.completed),
];

const insertTodoBeforeCompleted = (items: ax-codeProjectTodoItem[], item: ax-codeProjectTodoItem): ax-codeProjectTodoItem[] => {
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<ax-codeProjectTodoItem[]>([]);
  ...
  const initializeNewax-codeSession = useSessionUIStore((state) => state.initializeNewax-codeSession);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: ax-codeProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ax-code:project-plan-saved', handleProjectContextRefresh);
    window.addEventListener('ax-code:project-notes-updated', handleProjectContextRefresh);
    return () => {
      window.removeEventListener('ax-code:project-plan-saved', handleProjectContextRefresh);
      window.removeEventListener('ax-code:project-notes-updated', handleProjectContextRefresh);
    };
  }, [projectRef]);
  ...
  const handleConfirmSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingSendTarget.kind === 'worktree') {
          ...
          initializeNewax-codeSession(session.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, initializeNewax-codeSession, onActionComplete, pendingSendTarget, projectRef, routeToChat, sendMessage, setCurrentSession, t]
  );
  ...
  const handleDeletePlan = React.useCallback(
    async (planId: string) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent('ax-code:project-plan-saved', {
