...
export type SessionUIState = {
  ...
  clearError: () => void
  markSessionAsax-codeCreated: (sessionId: string) => void
  isax-codeCreatedSession: (sessionId: string) => boolean
  getContextUsage: (contextLimit: number, outputLimit: number) => SessionContextUsage | null
  initializeNewax-codeSession: (sessionId: string, agents: unknown[]) => void
...
export const useSessionUIStore = create<SessionUIState>()((set, get) => ({
  ...
  markSessionAsax-codeCreated: (sessionId) =>
    set((s) => {
  ...
  isax-codeCreatedSession: (sessionId) => get().webUICreatedSessions.has(sessionId),
  ...
  sendMessage: async (
    ...
    if (!options?.sessionId && draft?.open) {
      ...
      get().initializeNewax-codeSession(created.id, configState.agents ?? [])
