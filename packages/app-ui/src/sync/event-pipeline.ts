...
import type { Event, OpencodeClient, SessionStatus } from "@ax-code/sdk/v2/client"
...
const normalizeax-codeSessionStatus = (payload: Event): Event | null => {
  ...
  if (record.type !== "ax-code:session-status") return null
  ...
  return {
    id: typeof record.id === "string" && record.id.length > 0
      ? record.id
      : `ax-code-status-${sessionID}-${Date.now()}`,
    type: "session.status",
    properties: {
      sessionID,
      status,
...
const normalizeEventType = (payload: Event): Event => {
  const normalizedax-codeStatus = normalizeax-codeSessionStatus(payload)
  if (normalizedax-codeStatus) {
    return normalizedax-codeStatus
  }
...
export function createEventPipeline(input: EventPipelineInput): EventPipeline {
  ...
  if (typeof globalThis.window !== "undefined") {
    globalThis.window.addEventListener("ax-code:system-resume", onSystemResume)
  ...
  const cleanup = () => {
    ...
    if (typeof globalThis.window !== "undefined") {
      globalThis.window.removeEventListener("ax-code:system-resume", onSystemResume)
