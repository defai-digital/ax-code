...
import type { Provider, Agent } from "@ax-code/sdk/v2";
...
const MODELS_DEV_PROXY_URL = "/api/ax-code/models-metadata";
...
const fetchax-codeDefaults = async (): Promise<ax-codeDefaults> => {
    try {
...
interface ConfigStore {
    ...
    // ax-code settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and ax-code settings in parallel
                            ...
                            // 1. Check ax-code settings for default agent
                            if (openChamberDefaults.defaultAgent) {
                             ...
                             // 1. Check ax-code settings for default model
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName) {
                        ...
                        if (currentSessionId && useSessionUIStore.getState().isax-codeCreatedSession(currentSessionId)) {
                            const existingAgentModel = selState.getAgentModelForSession(currentSessionId, agentName);
                            if (!existingAgentModel) {
                                useSessionUIStore.getState().initializeNewax-codeSession(currentSessionId, agents);
                            }
                        }
                    }
