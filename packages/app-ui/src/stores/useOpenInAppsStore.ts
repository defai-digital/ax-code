...
export const useOpenInAppsStore = create<OpenInAppsState>()((set, get) => ({
  ...
  initialize: () => {
    ...
    window.addEventListener('ax-code:settings-synced', settingsHandler);
    window.addEventListener('ax-code:app-ready', appReadyHandler);
    window.addEventListener('ax-code:installed-apps-updated', updateHandler);

    const appReady = (window as unknown as { __axCodeAppReady?: boolean }).__axCodeAppReady;
    if (appReady) {
      void loadInstalledApps();
    }
