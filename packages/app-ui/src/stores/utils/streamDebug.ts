...
export const sessionStatusDebugEnabled = (): boolean => {
    if (typeof window === 'undefined') return false;
    try {
        return window.localStorage.getItem('ax_code_session_status_debug') === '1';
    } catch {
        return false;
    }
};

const STREAM_PERF_STORAGE_KEY = 'ax_code_stream_perf';
...
declare global {
    interface Window {
        __axCodeStreamPerfState?: StreamPerfState;
        __axCodeVsCodeStreamPerfState?: {
            counters: Map<string, PerfCounter>;
            lastReportAt?: number;
            lastUpdatedAt?: number;
...
const ensureStreamPerfState = (): StreamPerfState | null => {
    ...
    if (!window.__axCodeStreamPerfState) {
        const startedAt = Date.now();
        window.__axCodeStreamPerfState = {
    ...
    return window.__axCodeStreamPerfState;
};
...
export const setStreamPerfEnabled = (enabled: boolean): void => {
    ...
    try {
        ...
        delete window.__axCodeStreamPerfState;
        delete window.__axCodeVsCodeStreamPerfState;
    } catch {
...
export const resetStreamPerf = (): void => {
    ...
    if (window.__axCodeVsCodeStreamPerfState) {
        window.__axCodeVsCodeStreamPerfState = {
            ...window.__axCodeVsCodeStreamPerfState,
            counters: new Map<string, PerfCounter>(),
            startedAt: Date.now(),
            lastUpdatedAt: Date.now(),
...
export const getStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__axCodeStreamPerfState;
...
export const getVsCodeStreamPerfSnapshot = (): StreamPerfSnapshot => {
    ...
    const state = window.__axCodeVsCodeStreamPerfState;
    if (!streamPerfEnabled() || !state) {
        return {
            enabled: false,
            startedAt: null,
