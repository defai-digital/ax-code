...
// src/Renderable.ts
var BrandedRenderable = Symbol.for("@ax-code/tui/Renderable")
var LayoutEvents
;((LayoutEvents2) => {
...
var BrandedVNode = Symbol.for("@ax-code/tui/VNode")
...
class CodeRenderable extends TextBufferRenderable {
  ...
  getConcealLinesSourceMap(content, highlights) {
    if (!this._conceal || content.length === 0) return
    const concealLineRanges = CodeRenderable.getMergedConcealLineRanges(highlights)
    if (concealLineRanges.length === 0) return
    const lineSources = []
    let sourceLine = 0
    let lineStart = 0
    let rangeIndex = 0
...
`,
    ...
    if (lineSources.length === 0 || CodeRenderable.isIdentityLineSources(lineSources)) return
    return lineSources
  }
  getLineHighlights(lineIdx) {
...
var BrandedTextNodeRenderable = Symbol.for("@ax-code/tui/TextNodeRenderable")
...
var canThrowAcrossNativeCallback =
...
class TerminalConsole extends EventEmitter4 {
  ...
  _rgbaSelection
  _rgbaCopyButton
  _positions = ["top" /* TOP */, "right" /* RIGHT */, "bottom" /* BOTTOM */, "left" /* LEFT */]
...
`
  ...
  getCachedLogs() {
    return terminalConsoleCache.cachedLogs.map(
      (logEntry) => logEntry[0].toISOString() + " " + logEntry.slice(1).join(" "),
    ).join(`
`)
  }
  updateFrameBuffer() {
...
var BrandedEditBufferRenderable = Symbol.for("@ax-code/tui/EditBufferRenderable")
...
function resolveModes(config) {
  ...
  if (process.env.AX_CODE_TUI_USE_ALTERNATE_SCREEN !== undefined) {
  ...
  if (process.env.AX_CODE_TUI_OVERRIDE_STDOUT !== undefined) {
    externalOutputMode =
      env.AX_CODE_TUI_OVERRIDE_STDOUT && screenMode === "split-footer" ? "capture-stdout" : "passthrough"
  }
...
`
  ...
  canFlushSplitOutputBeforeTransition(allowSuspended = false, allowUnsetup = false) {
    return (
      ...
      (allowSuspended || this._controlState !== "explicit_suspended") /* EXPLICIT_SUSPENDED */ &&
  ...
  applyScreenMode(screenMode, emitResize = true, requestRender = true) {
    ...
    const terminalWritable =
      this._terminalIsSetup && this._controlState !== "explicit_suspended" /* EXPLICIT_SUSPENDED */
...
`.repeat(space)
  ...
  resume() {
    ...
    if (
      ...
      this._previousControlState === "explicit_started" /* EXPLICIT_STARTED */
    ) {
      this.internalStart()
    } else {
      this.requestRender()
  ...
  async loop() {
    ...
    try {
      ...
      if (this.consecutiveFrameFailures <= 3 || this.consecutiveFrameFailures % 100 === 0) {
        console.error(`[CliRenderer] frame render failed (x${this.consecutiveFrameFailures}); dropping frame:`, error)
      }
  ...
  refreshPalette() {
    ...
    this.getPalette({ size: NATIVE_PALETTE_QUERY_SIZE })
      .then(() => {
        if (this._isDestroyed) return
