...
function assertCompatibleInstall(install, options) {
    for (const specifier of Object.keys(options.additional ?? {})) {
        if (!install.additionalSpecifiers.has(specifier)) {
            throw new Error(`AX Code TUI runtime plugin support is already installed without ${specifier}. Call ensureRuntimePluginSupport({ additional }) from @ax-code/tui/runtime-plugin-support/configure before importing @ax-code/tui/runtime-plugin-support.`);
        }
    }
