...
} from "@ax-code/tui/runtime-plugin"
...
function assertCompatibleInstall(install, modules, options) {
  for (const specifier of Object.keys(modules)) {
    if (!install.specifiers.has(specifier)) {
      throw new Error(
        `AX Code TUI Solid runtime plugin support is already installed without ${specifier}. Call ensureRuntimePluginSupport({ additional }) from @ax-code/tui/solid/runtime-plugin-support/configure before importing @ax-code/tui/solid/runtime-plugin-support.`,
      )
    }
  }
  if (options?.core && options.core !== install.core) {
    throw new Error(
      "AX Code TUI Solid runtime plugin support is already installed with a different core runtime module.",
    )
  }
