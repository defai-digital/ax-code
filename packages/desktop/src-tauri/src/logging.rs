...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("ax-code-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("ax_code_lib=debug,ax_code_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("ax_code_lib=info,ax_code_desktop=info,sidecar=info")
        }
    });
