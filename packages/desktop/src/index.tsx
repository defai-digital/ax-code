...
const deepLinkEvent = "ax-code:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__AX_CODE__ ??= window.__OPENCODE__ ?? {}
  window.__OPENCODE__ = window.__AX_CODE__
  const pending = window.__AX_CODE__.deepLinks ?? []
  window.__AX_CODE__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !(window.__AX_CODE__?.wsl ?? window.__OPENCODE__?.wsl)) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !(window.__AX_CODE__?.wsl ?? window.__OPENCODE__?.wsl)) return result
  ...
  return {
    ...
    getWslEnabled: async () => {
      ...
      return window.__AX_CODE__?.wsl ?? window.__OPENCODE__?.wsl ?? false
    },
