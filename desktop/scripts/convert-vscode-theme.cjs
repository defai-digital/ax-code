...
const THEMES_DIR = path.join(REPO_ROOT, "packages", "ui", "src", "lib", "theme", "themes")
...
function extractStatusColors(colors, tokenColors, semanticTokenColors) {
  const error =
    colors["editorError.foreground"] || semanticTokenColors["variable.defaultLibrary"]?.foreground || "#D14D41"
...
function registerThemeInPresets(themeId) {
  ...
  if (!arrayEntryExists) {
    ...
    const arrayEndIndex = lines.findIndex((line, index) => index > arrayStartIndex && /^\s*\](?:\.map\(|;)/.test(line))
    if (arrayEndIndex === -1) {
