...
const normalizeInnerSvg = (iconId, value) => {
  ...
  for (const sourceId of idsBySpecificity) {
    ...
    inner = inner.replace(urlPattern, (_full, quote) =>
      quote ? `url(${quote}#${targetId}${quote})` : `url(#${targetId})`,
    )
  }
...
console.log(
  `Generated ${path.relative(ROOT, SPRITE_FILE)} with ${symbols.length} symbols (${files.length} source SVG files)`,
)
