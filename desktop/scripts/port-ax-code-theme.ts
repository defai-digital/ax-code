...
function isDesktopTheme(value: ThemeSource): value is DesktopTheme {
  return Boolean(
    value && typeof value === "object" && "light" in value && "dark" in value && "id" in value && "name" in value,
  )
}

function isContextTheme(value: ThemeSource): value is ContextThemeJson {
  return Boolean(
    value && typeof value === "object" && "theme" in value && !("light" in value) && !("metadata" in value),
  )
}
...
function sourceVariantColor(
  variant: ThemeVariant,
  key: keyof ThemePaletteColors | keyof ThemeSeedColors,
): string | undefined {
  if (variant.palette) {
...
function syntheticTokensFromContextTheme(resolved: ResolvedContextTheme, mode: ContextThemeMode): ResolvedTheme {
  ...
  const subtleBorder =
    resolved.borderSubtle ?? resolved.border ?? withAlpha(resolved.textMuted ?? resolved.text ?? "#000000", 0.2)
...
function syntheticTokensFromOpenChamberTheme(theme: OpenChamberTheme): ResolvedTheme {
  ...
  return {
    ...
    "surface-raised-base-hover":
      interactive.hover ?? surface.elevated ?? surface.muted ?? surface.background ?? "#1C1B1A",
    "surface-raised-base-active":
      interactive.active ?? interactive.hover ?? surface.elevated ?? surface.muted ?? surface.background ?? "#1C1B1A",
    ...
    "surface-diff-add-weak":
      syntax.highlights?.diffAddedBackground ??
      status.successBackground ??
      withAlpha(status.success ?? "#66800B", 0.12),
    "surface-diff-delete-weak":
      syntax.highlights?.diffRemovedBackground ?? status.errorBackground ?? withAlpha(status.error ?? "#AF3029", 0.12),
    "surface-diff-hidden-weak":
      syntax.highlights?.diffModifiedBackground ?? status.infoBackground ?? withAlpha(status.info ?? "#205EA6", 0.12),
...
function buildTheme(
  ...
  const interactiveSeed =
    sourceVariantColor(variant, "interactive") ?? sourceVariantColor(variant, "primary") ?? borderFocus
  ...
  const primaryForeground = accessibleForeground(
    primaryBase,
    token(resolved, "text-on-brand-base", isDark ? "#151313" : "#ffffff"),
  )
    ...
    colors: {
      ...
      syntax: {
        ...
        highlights: {
          ...
          diffRemovedBackground: token(
            resolved,
            "surface-diff-delete-weak",
            withAlpha(diffDeleteBase, isDark ? 0.18 : 0.14),
          ),
          diffModified: token(resolved, "icon-diff-modified-base", warningBase),
          diffModifiedBackground: token(
            resolved,
            "surface-diff-hidden-weak",
            withAlpha(infoBase, isDark ? 0.18 : 0.14),
          ),
      ...
      tools: {
        ...
        edit: {
          ...
          removedBackground: token(
            resolved,
            "surface-diff-delete-weak",
            withAlpha(diffDeleteBase, isDark ? 0.16 : 0.12),
          ),
