...
function toPangoMarkup(sentence) {
  ...
  const withHighlights = escaped.replace(
    /\*([^*]+)\*/g,
    (_, phrase) => `<span foreground="${AMBER}" font_family="Instrument Serif" font_style="italic">${phrase}</span>`,
  )
...
async function main() {
  ...
  const titleBuf = await sharp({
    text: {
      text: `<span foreground="${AMBER}" letter_spacing="2048">${escapeMarkup(title)}</span>`,
  ...
  const outPath = opt.out
    ...
    : path.resolve(process.cwd(), `changelog-${String(title).replace(/[^\w.-]+/g, "-")}.png`)
