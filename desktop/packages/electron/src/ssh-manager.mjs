...
const expandSshIncludeToken = (token, baseDir) => {
  ...
  const expandedHome = trimmed.startsWith("~/")
    ...
    : trimmed === "~"
      ? os.homedir()
      : trimmed
  const resolved = path.isAbsolute(expandedHome) ? expandedHome : path.resolve(baseDir, expandedHome)
...
const hasDisallowedOOption = (value) => {
  ...
  return ["controlmaster", "controlpath", "controlpersist", "batchmode", "proxycommand"].some((prefix) =>
...
const runRemoteCommand = async (parsed, controlPath, script, timeoutSec = DEFAULT_CONNECTION_TIMEOUT_SEC) => {
  const args = buildSshArgs(
    parsed,
    ["-o", "ControlMaster=no", "-o", `ControlPath=${controlPath}`, "-o", `ConnectTimeout=${timeoutSec}`, "-T"],
...
export class ElectronSshManager {
  ...
  sanitizeInstance(instance) {
    ...
    return {
      id,
      ...(typeof instance?.nickname === "string" && instance.nickname.trim()
        ? { nickname: instance.nickname.trim() }
        : {}),
      sshCommand,
      sshParsed: parsed,
      connectionTimeoutSec:
        Number.isFinite(instance?.connectionTimeoutSec) && Number(instance.connectionTimeoutSec) > 0
      ...
      auth: {
        ...(this.sanitizeStoredSecret(instance?.auth?.sshPassword)
          ? { sshPassword: this.sanitizeStoredSecret(instance.auth.sshPassword) }
          : {}),
        ...(this.sanitizeStoredSecret(instance?.auth?.openchamberPassword)
  ...
  async spawnMasterProcess(parsed, controlPath, askpassPath, sshPassword) {
    const child = spawn(
      ...
      buildSshArgs(parsed, [
        ...
        `ControlPath=${controlPath}`,
        "-o",
        `ControlPersist=${DEFAULT_CONTROL_PERSIST_SEC}`,
        "-N",
      ]),
      {
  ...
  async waitForMasterReady(parsed, controlPath, timeoutSec, master) {
    ...
    let pollMs = 250
    while (Date.now() < deadline) {
      const { code } = await runOutput(
        "ssh",
        buildSshArgs(parsed, ["-o", "ControlMaster=no", "-o", `ControlPath=${controlPath}`, "-O", "check"]),
      )
      if (code === 0) return
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (openchamberPassword && authStatus !== 200) {
          throw new Error(
            `Remote OpenChamber requires UI authentication and configured password was rejected (auth status ${authStatus})`,
          )
        }
  ...
  async startRemoteServerManaged(parsed, controlPath, instance, desiredPort) {
    ...
    const output = await runRemoteCommand(
      parsed,
      controlPath,
      `${envPrefix} ax-code-desktop serve --daemon --hostname 127.0.0.1 --port ${desiredPort} || ${envPrefix} openchamber serve --daemon --hostname 127.0.0.1 --port ${desiredPort}`,
    )
    const port = output
      .split(/\s+/)
  ...
  async spawnMainForward(parsed, controlPath, bindHost, localPort, remotePort) {
    return spawn(
      ...
      buildSshArgs(parsed, [
        ...
        "-N",
        "-L",
        `${bindHost}:${localPort}:127.0.0.1:${remotePort}`,
      ]),
      {
  ...
  async spawnExtraForward(parsed, controlPath, forward) {
    const args = ["-o", "ControlMaster=no", "-o", `ControlPath=${controlPath}`, "-O", "forward"]
    if (forward.type === "local") {
      args.push(
        "-L",
        `${forward.localHost || "127.0.0.1"}:${forward.localPort}:${forward.remoteHost || "127.0.0.1"}:${forward.remotePort}`,
      )
    } else if (forward.type === "remote") {
      args.push(
        "-R",
        `${forward.remoteHost || "127.0.0.1"}:${forward.remotePort}:${forward.localHost || "127.0.0.1"}:${forward.localPort}`,
      )
    } else {
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteOpenchamber.mode === "external") {
      ...
      this.setStatus(
        instance.id,
        "server_detecting",
        "Probing external OpenChamber server",
        null,
        null,
        port,
        false,
    ...
    if (!installedVersion) {
      ...
      this.setStatus(
        instance.id,
        "updating",
        `Updating remote OpenChamber from ${installedVersion} to ${this.appVersion}`,
      )
      await this.installOpenChamberManaged(
        parsed,
        controlPath,
  ...
  async connectBlocking(instance) {
    ...
    await this.waitForMasterReady(
      parsed,
      controlPath,
      instance.connectionTimeoutSec || DEFAULT_CONNECTION_TIMEOUT_SEC,
      master,
    )
    ...
    this.setStatus(
      ...
      extraErrors.length === 0
        ? "SSH instance is ready"
        : `SSH instance is ready with forward warnings: ${extraErrors.join("; ")}`,
      localUrl,
      localPort,
      remotePort,
      startedByUs,
  ...
  async statusesWithDefaults(id) {
    ...
    return this.readInstances()
      .instances.map((instance) => this.statusSnapshotForInstance(instance.id))
