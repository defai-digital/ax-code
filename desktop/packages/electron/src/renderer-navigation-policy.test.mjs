...
describe("renderer navigation policy", () => {
  test("normalizes loopback dev renderer URLs", () => {
    ...
    expect(normalizeDevRendererUrl("http://0.0.0.0:5180/")).toBeNull()
    expect(normalizeDevRendererUrl("http://[::]:5180/")).toBeNull()
  })
  ...
  test("trusts all loopback server URLs on the active desktop server port", () => {
    ...
    expect(isTrustedRendererNavigationUrl("http://0.0.0.0:3910/projects", { serverPort: 3910 })).toBe(false)
    expect(isTrustedRendererNavigationUrl("http://[::]:3910/projects", { serverPort: 3910 })).toBe(false)
  })
