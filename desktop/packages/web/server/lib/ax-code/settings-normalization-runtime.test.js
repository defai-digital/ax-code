...
describe("settings normalization runtime - symlink resolution", () => {
  describe("normalizePathForPersistence", () => {
    it("resolves symlinks via realpathSync", () => {
      const runtime = createTestRuntime({
        realpathSync: (p) => (p === "/home/user/workplace" ? "/workplace/user" : p),
      })
  ...
  describe("sanitizeProjects", () => {
    it("resolves symlinks in project paths", () => {
      const runtime = createTestRuntime({
        realpathSync: (p) => (p === "/home/user/workplace/MyProject" ? "/workplace/user/MyProject" : p),
      })

      const projects = [
        {
          id: "proj1",
          path: "/home/user/workplace/MyProject",
          label: "MyProject",
          color: "primary",
          addedAt: 1000,
          lastOpenedAt: 1000,
  ...
  describe("normalizeSettingsPaths", () => {
    it("resolves symlinks in lastDirectory", () => {
      const runtime = createTestRuntime({
        realpathSync: (p) => (p === "/home/user/workplace/LyraRefactoring" ? "/workplace/user/LyraRefactoring" : p),
      })
