...
import {
  isExactSemver as defaultIsExactSemver,
  isPathSpec as defaultIsPathSpec,
  parseNpmSpec as defaultParseNpmSpec,
  parsePathSpec as defaultParsePathSpec,
} from "./plugin-spec.js"
...
export const registerPluginRoutes = (app, dependencies) => {
  ...
  const handlePluginError = (res, error, fallbackMessage, context, existsKind = null) => {
    ...
    if (
      (existsKind === "entry" && ENTRY_EXISTS_CODES.has(code)) ||
      (existsKind === "file" && FILE_EXISTS_CODES.has(code))
    ) {
  ...
  app.get("/api/config/plugins/registry", async (req, res) => {
    try {
      ...
      await Promise.all(
        Array.from(npmJobs.keys()).map(async (name) => {
      ...
      for (const spec of uniqueSpecs) {
        ...
        if (!info.ok) {
          ...
          results.push({
            kind: "npm-network",
            spec,
            error: info.status === "network" ? info.error : `Registry returned ${info.status}`,
          })
          continue
        }
