...
export const registerSkillRoutes = (app, dependencies) => {
  ...
  const fetchAxCodeDiscoveredSkills = async (workingDirectory) => {
    ...
    try {
      ...
      const response = await client.app.skills(workingDirectory ? { directory: workingDirectory } : undefined)
  ...
  app.get("/api/config/skills/catalog", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ ok: false, error: { kind: "unknown", message: error.message || "Failed to load catalog" } })
    }
  })
  ...
  app.post("/api/config/skills/scan", async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ ok: false, error: { kind: "unknown", message: error.message || "Failed to scan repository" } })
    }
  })

  app.post("/api/config/skills/install", async (req, res) => {
    try {
      const { source, subpath, gitIdentityId, scope, targetSource, selections, conflictPolicy, conflictDecisions } =
        req.body || {}
      ...
      res
        .status(500)
        .json({ ok: false, error: { kind: "unknown", message: error.message || "Failed to install skills" } })
    }
  })
  ...
  app.get("/api/config/skills/:name", async (req, res) => {
    try {
      ...
      const discoveredSkill =
        (await fetchAxCodeDiscoveredSkills(directory)).find((skill) => skill.name === skillName) || null
  ...
  app.post("/api/config/skills/:name", async (req, res) => {
    try {
      ...
      const { directory, error } =
        scope === SKILL_SCOPE.PROJECT ? await resolveProjectDirectory(req) : await resolveOptionalProjectDirectory(req)
