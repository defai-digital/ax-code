...
export const waitForSseDrain = (res, signal) =>
  new Promise((resolve) => {
...
export const createSseProxyMetrics = (options = {}) => {
  const { now = () => Date.now(), maxRecentStreams = 20 } = options
  ...
  const finish = (id, reason = "complete", details = {}) => {
    ...
    const entry = {
      ...
      upstreamFirstChunkLatencyMs:
        stream.firstChunkAtEpochMs && stream.upstreamConnectedAtEpochMs
          ? Math.max(0, stream.firstChunkAtEpochMs - stream.upstreamConnectedAtEpochMs)
          : null,
...
export const registerAxCodeProxy = (app, deps) => {
  ...
  app.use("/api", (req, _res, next) => {
    if (
      req.url === "/config/provider" ||
      req.url.startsWith("/config/provider?") ||
      req.url.startsWith("/config/provider/")
    ) {
  ...
  app.use("/api", (req, res, next) => {
    ...
    const stillWaiting =
      (!runtimeState.isAxCodeReady &&
        (runtimeState.axCodeNotReadySince === 0 || waitElapsed < OPEN_CODE_READY_GRACE_MS)) ||
  ...
  if (process.platform === "win32") {
    app.get("/api/session", async (req, res, next) => {
      ...
      try {
        ...
        console.log(
          `[SessionMerge] ${globalSessions.length} global + ${extraSessions.length} extra = ${merged.length} total`,
        )
