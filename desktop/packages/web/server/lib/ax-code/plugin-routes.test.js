...
async function createEntry(spec = "a") {
  return request(app).post("/api/config/plugins/entry").send({ spec, scope: "user" }).expect(200)
}

async function createFile(fileName = "test.js", content = "//x") {
  return request(app).post("/api/config/plugins/file").send({ fileName, content, scope: "user" }).expect(200)
}
...
describe("ax-code plugin routes", () => {
  ...
  test("GET /registry reports update for exact npm version behind latest", async () => {
    createRegistryApp(
      vi.fn(async () => ({ ok: true, latest: "2.0.0", versions: ["1.0.0", "2.0.0"], distTags: { latest: "2.0.0" } })),
    )
  ...
  test("GET /registry reports no update when exact npm version matches latest", async () => {
    createRegistryApp(
      vi.fn(async () => ({ ok: true, latest: "1.0.0", versions: ["1.0.0"], distTags: { latest: "1.0.0" } })),
    )
  ...
  test("GET /registry reports existing path plugin ok", async () => {
    ...
    const response = await request(app)
      .get(`/api/config/plugins/registry?specs=${encodeURIComponent(tmpFile)}`)
      .expect(200)
  ...
  test("GET /registry reports missing path plugin", async () => {
    ...
    const response = await request(app)
      .get("/api/config/plugins/registry?specs=%2Fnonexistent%2F__path%2Fxyz.js")
      .expect(200)

    expect(response.body.results[0]).toEqual({
      kind: "path-missing",
      spec: "/nonexistent/__path/xyz.js",
      absolutePath: "/nonexistent/__path/xyz.js",
    })
  })
  ...
  test("POST duplicate entry returns 409", async () => {
    ...
    const response = await request(app).post("/api/config/plugins/entry").send({ spec: "a", scope: "user" }).expect(409)
  ...
  test("DELETE /entry/:id removes entry and prunes plugin key", async () => {
    ...
    await request(app)
      .delete(`/api/config/plugins/entry/${encodeURIComponent(id)}`)
      .expect(200)
  ...
  test("DELETE /file/:id unlinks file", async () => {
    ...
    await request(app)
      .delete(`/api/config/plugins/file/${encodeURIComponent(id)}`)
      .expect(200)
