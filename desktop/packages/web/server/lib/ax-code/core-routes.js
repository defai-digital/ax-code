...
export const registerServerStatusRoutes = (app, dependencies) => {
  ...
  app.post("/api/system/dev-shutdown", express.json({ limit: "64kb" }), async (req, res) => {
    ...
    try {
      ...
      const previewPorts = Array.from(
        new Set(rawPreviewUrls.map((value) => parseLoopbackPort(value)).filter((port) => typeof port === "number")),
      )
...
export const registerSettingsUtilityRoutes = (app, dependencies) => {
  const { readCustomThemesFromDisk, refreshAxCodeAfterConfigChange, clientReloadDelayMs, backgroundAxCodeReloader } =
    dependencies
  const axCodeReloader =
    backgroundAxCodeReloader ??
    createBackgroundAxCodeReloader({
