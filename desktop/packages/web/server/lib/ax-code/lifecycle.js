...
export const createAxCodeLifecycleRuntime = (deps) => {
  ...
  const waitForChildProcessClose = (child, timeoutMs) =>
    new Promise((resolve) => {
  ...
  const waitForPortRelease = (port, timeoutMs, hostname = env.ENV_CONFIGURED_AX_CODE_HOSTNAME) => {
    ...
    const probeHost =
      !hostname || hostname === "0.0.0.0" || hostname === "::" || hostname === "[::]" ? "127.0.0.1" : hostname
  ...
  const createManagedAxCodeServerProcessLegacy = async ({
    ...
    const url = await new Promise((resolve, reject) => {
      ...
      const onExit = (code, signal) => {
        ...
        const appBundleHint =
          process.platform === "darwin" && /\/AX Code\.app\/Contents\/MacOS\/(?:AX Code|ax-code)$/i.test(binary)
            ? " The configured binary appears to point at the macOS desktop app bundle; AX Code Desktop needs the standalone ax-code CLI."
            : ""
        finish(
          reject,
          new Error(
            `AX Code process exited before serving with ${reason}. Binary used: ${binary}.${appBundleHint} ${formatCapturedOutput({ stdout, stderr })}`,
          ),
        )
      }
  ...
  const createManagedAxCodeServerProcess = async ({
    hostname,
    port,
    timeout,
    cwd,
    ...
    // shapes. The workspace SDK accepts explicit binary/args, so wrapper
  ...
  const recordAxCodeRuntimeHealth = (body) => {
    ...
    const taskSummary =
      body.runtime?.taskSummary && typeof body.runtime.taskSummary === "object" ? body.runtime.taskSummary : null
    state.axCodeRuntimeHealth =
      readiness || startup || taskSummary
  ...
  const startAxCodeOnce = async () => {
    ...
    const envPath =
      typeof buildManagedAxCodePath === "function"
        ? buildManagedAxCodePath()
  ...
  const refreshAxCodeAfterConfigChange = async (reason, options = {}) => {
    ...
    const effectiveReadyTimeoutMs = Number.isFinite(readyTimeoutMs) && readyTimeoutMs > 0 ? readyTimeoutMs : undefined
  ...
  const probeAxCodeHealth = async () => {
    ...
    healthProbePromise = managedAxCodeRuntime
      .probeProcessHealth()
      .then((healthy) => {
  ...
  const shouldSkipRestartForBusySessions = () => {
    ...
    if (now - lastUnhealthyWithBusySessionsAt >= STALE_BUSY_GRACE_MS) {
      console.warn(`[lifecycle] ax-code unhealthy with ${activeCount} busy session(s) for > 2 min — forcing restart`)
      lastUnhealthyWithBusySessionsAt = 0
      return false
    }
