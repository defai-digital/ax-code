...
describe("ax-code plugins data layer", () => {
  ...
  test("rejects invalid specs and file names", () => {
    ...
    expect(() => plugins.writePluginDirFile({ fileName: "", content: "", scope: "project" }, projectDir)).toThrow(
      "Plugin file name",
    )
    expect(() =>
      plugins.writePluginDirFile({ fileName: "../bad.js", content: "", scope: "project" }, projectDir),
    ).toThrow("Plugin file name")
    expect(() => plugins.writePluginDirFile({ fileName: "a/b.js", content: "", scope: "project" }, projectDir)).toThrow(
      "Plugin file name",
    )
    expect(() => plugins.writePluginDirFile({ fileName: "A.js", content: "", scope: "project" }, projectDir)).toThrow(
      "Plugin file name",
    )
    expect(() =>
      plugins.writePluginDirFile({ fileName: "foo.txt", content: "", scope: "project" }, projectDir),
    ).toThrow("Plugin file name")
  })
  ...
  test("creates string and tuple entries with duplicate rejection", () => {
    ...
    expect(() => plugins.createPluginEntry({ spec: "plain-plugin", scope: "user" }, projectDir)).toThrow(
      "already exists",
    )
    expect(
      thrownBy(() => plugins.createPluginEntry({ spec: "plain-plugin", scope: "user" }, projectDir)),
    ).toHaveProperty("code", "ENTRY_EXISTS")
  })
  ...
  test("updates entries in place and transitions between string and tuple", () => {
    ...
    plugins.updatePluginEntry(
      plugins.encodePluginId("config", "user:second"),
      { spec: "second-new", options: {} },
      projectDir,
    )
    expect(readJson(userConfigPath).plugin).toEqual(["first", "second-new", "third"])

    plugins.updatePluginEntry(
      plugins.encodePluginId("config", "user:first"),
      { spec: "first-new", options: { b: 2 } },
      projectDir,
    )
  ...
  test("lists entries from user and project layers with scopes and parsed kinds", () => {
    ...
    expect(entries).toEqual([
      ...
      expect.objectContaining({
        spec: "./local-plugin.js",
        scope: "project",
        kind: "config",
        parsedKind: "path",
        sourcePath: path.join(projectDir, ".ax-code", "ax-code.json"),
      }),
    ])
    fs.rmSync(userConfigPath, { force: true })
  ...
  test("round-trips plugin dir files", () => {
    ...
    expect(plugins.readPluginDirFile(file.id, projectDir)).toEqual({
  ...
  test("rejects duplicate plugin dir files unless overwrite is true", () => {
    ...
    expect(() =>
      plugins.writePluginDirFile({ fileName: "dup.js", content: "two", scope: "project" }, projectDir),
    ).toThrow("already exists")
    expect(
      thrownBy(() => plugins.writePluginDirFile({ fileName: "dup.js", content: "two", scope: "project" }, projectDir)),
    ).toHaveProperty("code", "FILE_EXISTS")
