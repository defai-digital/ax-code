  ...
  const recoverOrphanProjectFiles = async (canonicalProjects) => {
    ...
    const extractRootRelPaths = (orphan) => {
      const commands = [
        ...
        ...(Array.isArray(orphan.content.projectActions)
          ? orphan.content.projectActions.map((a) => (typeof a?.command === "string" ? a.command : ""))
          : []),
    ...
    const orphanMatchesProject = async (orphan, project) => {
      ...
      const haystacks = [
        ...(Array.isArray(orphan.content["setup-worktree"]) ? orphan.content["setup-worktree"] : []),
        ...(Array.isArray(orphan.content.projectActions)
          ? orphan.content.projectActions.map((a) => `${a?.name || ""} ${a?.command || ""}`)
          : []),
      ]
        .join(" ")
        .toLowerCase()
    ...
    if (remaining.length > 0) {
      console.warn(
        `[projects] ${remaining.length} orphan project file(s) could not be auto-matched: ${remaining.map((o) => o.orphanId).join(", ")}`,
      )
    }
  }
