...
const extractSessionStatusUpdate = (payload) => {
  ...
  return {
    ...
    attempt:
      typeof status.attempt === "number" ? status.attempt : typeof info.attempt === "number" ? info.attempt : undefined,
    message:
      typeof status.message === "string" ? status.message : typeof info.message === "string" ? info.message : undefined,
    next: typeof status.next === "number" ? status.next : typeof info.next === "number" ? info.next : undefined,
  }
}
...
export const createSessionRuntime = ({ writeSseEvent, getNotificationClients, broadcastEvent }) => {
  ...
  const updateSessionState = (sessionId, status, eventId, metadata = {}) => {
    ...
    const attentionChanged =
      !!attentionState && existingAttentionState?.needsAttention !== attentionState.needsAttention
