...
describe("terminal websocket protocol", () => {
  ...
  it("rejects malformed control json", () => {
    const frame = Buffer.concat([Buffer.from([TERMINAL_WS_CONTROL_TAG_JSON]), Buffer.from("{not json", "utf8")])
    expect(readTerminalWsControlFrame(frame)).toBeNull()
  })
  ...
  it("rejects control json that is not object", () => {
    const frame = Buffer.concat([Buffer.from([TERMINAL_WS_CONTROL_TAG_JSON]), Buffer.from('"str"', "utf8")])
    expect(readTerminalWsControlFrame(frame)).toBeNull()
  })
  ...
  it("normalizes chunk array payloads", () => {
    const normalized = normalizeTerminalWsMessageToBuffer([Buffer.from("ab", "utf8"), Buffer.from("c", "utf8")])
    expect(normalized.toString("utf8")).toBe("abc")
  })
