...
const gitReadEntryBytes = (key, result) => key.length + (result?.stdout?.length || 0) + (result?.stderr?.length || 0)
...
const resolveReadPathFromContext = async ({
  req,
  targetPath,
  resolveProjectDirectory,
  readSettingsFromDiskMigrated,
...
export const registerFsRoutes = (app, dependencies) => {
  ...
  app.post("/api/fs/clone", async (req, res) => {
    try {
      ...
      if (sshKeyPath) {
        gitArgs.unshift(
          `core.sshCommand=ssh -i ${escapeCloneSshKeyPath(sshKeyPath)} -o IdentitiesOnly=yes -o BatchMode=yes -o StrictHostKeyChecking=accept-new`,
        )
        gitArgs.unshift("-c")
      }
  ...
  app.get("/api/fs/list", async (req, res) => {
    const rawPath =
      typeof req.query.path === "string" && req.query.path.trim().length > 0 ? req.query.path.trim() : os.homedir()
