...
export function registerGitHubRoutes(app) {
      ...
      if (sha) {
        try {
          ...
          if (checkRuns.length > 0) {
            ...
            const state =
              counts.failure > 0 ? "failure" : counts.pending > 0 ? "pending" : total > 0 ? "success" : "unknown"
  ...
  app.post("/api/github/pr/create", async (req, res) => {
    try {
      ...
      const targetRepo =
        req.body?.targetRepo &&
        typeof req.body.targetRepo.owner === "string" &&
  ...
  app.get("/api/github/pulls/context", async (req, res) => {
    try {
      ...
      const reviewComments = (Array.isArray(reviewCommentsResp?.data) ? reviewCommentsResp.data : []).map(
        (comment) => ({
          id: comment.id,
          url: comment.html_url,
          body: comment.body || "",
