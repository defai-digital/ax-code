...
import { asNonEmptyString, toNumber, toTimestamp, toUsageWindow } from "../../utils/index.js"
...
export const transformQuotaBucket = (bucket, sourceId) => {
  ...
  const scopedName = modelId.startsWith(`${sourceId}/`) ? modelId : `${sourceId}/${modelId}`
...
export const transformModelData = (modelName, modelData, sourceId) => {
  const scopedName = modelName.startsWith(`${sourceId}/`) ? modelName : `${sourceId}/${modelName}`
  ...
  const usedPercent = remainingPercent !== null ? Math.max(0, 100 - remainingPercent) : null
  const resetAt = modelData?.quotaInfo?.resetTime ? new Date(modelData.quotaInfo.resetTime).getTime() : null
  const window = resolveGoogleWindow(sourceId, resetAt)
