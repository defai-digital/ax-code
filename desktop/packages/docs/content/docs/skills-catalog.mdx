...
Catalog entries must use English-only visible metadata. Sources can contain other content, but AX Code Desktop only lists ready-made skills whose name and description metadata are English-only, and it rechecks downloaded packages before installing them.

## Install a skill

1. Open the catalog.
2. Browse the built-in sources — including Anthropic, ClawdHub, and curated engineering skill repositories — or search.
