    ...
    if (runtimeSettings) {
      try {
        ...
        if (data) {
          ...
          const defaultFileViewerPreview =
            typeof data?.defaultFileViewerPreview === "boolean" ? data.defaultFileViewerPreview : undefined
          const zenModel = typeof data?.zenModel === "string" ? data.zenModel.trim() : ""
          const messageStreamTransport =
            data?.messageStreamTransport === "ws" ||
            data?.messageStreamTransport === "sse" ||
            data?.messageStreamTransport === "auto"
...
const hasProviderModel = hasSelectableProviderModel
...
const mapModalities = (
  cap: { text: boolean; audio: boolean; image: boolean; video: boolean; pdf: boolean } | undefined,
): string[] => {
  if (!cap) return []
  const result: string[] = []
...
const transformModelsDevResponse = (payload: unknown): Map<string, ModelMetadata> => {
  ...
  for (const [providerKey, providerValue] of Object.entries(payload)) {
    ...
    const providerId =
      typeof providerValue.id === "string" && providerValue.id.length > 0 ? providerValue.id : providerKey
    ...
    for (const [modelKey, modelValue] of Object.entries(models)) {
      ...
      const resolvedModelId = typeof modelKey === "string" && modelKey.length > 0 ? modelKey : modelValue.id
...
const resolveFirstSelectableSelection = (
  providers: ProviderWithModelList[],
  ...
  const orderedProviders =
    preferredProviderId && providers.some((provider) => provider.id === preferredProviderId)
      ? [
...
const normalizeProviderModels = (provider: { id?: string; models?: unknown }): ProviderModel[] => {
  ...
  if (modelRecord && typeof modelRecord === "object") {
    return Object.keys(modelRecord as Record<string, ProviderModel>).map(
      (modelId) => (modelRecord as Record<string, ProviderModel>)[modelId],
    )
  }
  console.warn(
    `[loadProviders] Provider "${provider.id ?? "unknown"}" has unexpected models shape:`,
    typeof modelRecord,
  )
  return []
}
    ...
    persist(
      (set, get) => ({
          ...
          // to prevent zombie re-polls after switching.
          {
            ...
            if (prevKey && prevKey !== directoryKey) {
              ...
              if (pending) {
                ...
                _providerPollTimer.delete(prevKey)
              }
        ...
        loadProviders: async (options) => {
          ...
          const promise = (async () => {
            set((state) =>
              state.activeDirectoryKey === directoryKey ? { providersLoading: true, providersError: null } : {},
            )
              ...
              try {
                ...
                set((state) => {
                  ...
                  if (state.activeDirectoryKey === directoryKey) {
                    ...
                    const currentSelectionIsSelectable =
                      state.currentProviderId && state.currentModelId
                        ? hasProviderModel(processedProviders, state.currentProviderId, state.currentModelId)
                        : false
                      ...
                      const nextSelection =
                        ...
                        (state.currentProviderId || state.currentModelId
                          ? {
                              ...
                              currentModelId: "",
                              currentVariant: undefined,
                              selectedProviderId: state.selectedProviderId || state.currentProviderId,
                            }
                          : null)
                      if (nextSelection) {
                        Object.assign(nextState, nextSelection)
                        Object.assign(nextSnapshot, nextSelection)
                      }
                    }
                  }
            ...
            set((state) => {
              ...
              const fallbackProviders =
                baseSnapshot.providers.length > 0
              ...
              if (state.activeDirectoryKey === directoryKey) {
                ...
                const currentSelectionIsSelectable =
                  state.currentProviderId && state.currentModelId
                    ? hasProviderModel(fallbackProviders, state.currentProviderId, state.currentModelId)
                    : false
                const resolved = resolveSettingsDefaultSelection(
                  fallbackProviders,
                  state.settingsDefaultModel,
                ...
                set((state) => {
                  ...
                  const nextState: Partial<ConfigStore> = {
                    ...
                    settingsMessageStreamTransport:
                      openChamberDefaults.messageStreamTransport ?? state.settingsMessageStreamTransport ?? "auto",
                    directoryScoped: {
                      ...state.directoryScoped,
                      [directoryKey]: nextSnapshot,
                    },
                ...
                if (openChamberDefaults.defaultModel) {
                  ...
                  if (parsed && defaultModel && isProviderModelSelectable(defaultModel)) {
                    ...
                    if (openChamberDefaults.defaultVariant) {
                      ...
                      if (
                        variants &&
                        Object.prototype.hasOwnProperty.call(variants, openChamberDefaults.defaultVariant)
                      ) {
                ...
                if (Object.keys(invalidSettings).length > 0) {
                  ...
                  set({
                    settingsDefaultModel:
                      invalidSettings.defaultModel !== undefined ? undefined : get().settingsDefaultModel,
                    settingsDefaultVariant:
                      invalidSettings.defaultVariant !== undefined ? undefined : get().settingsDefaultVariant,
                    settingsDefaultAgent:
                      invalidSettings.defaultAgent !== undefined ? undefined : get().settingsDefaultAgent,
                  })
        ...
        setAgent: (agentName: string | undefined) => {
          ...
          if (agentName) {
            ...
            if (currentSessionId) {
              const existingAgentModel = useSelectionStore
                .getState()
                .getAgentModelForSession(currentSessionId, agentName)
              if (
                existingAgentModel &&
                hasProviderModel(providers, existingAgentModel.providerId, existingAgentModel.modelId)
              ) {
                const savedVariant = useSelectionStore
                  .getState()
