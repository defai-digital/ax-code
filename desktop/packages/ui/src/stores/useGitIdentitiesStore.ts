...
import type { DiscoveredGitCredential, GitIdentityAuthType, GitIdentityProfile } from "@/lib/api/types"
import { updateDesktopSettings } from "@/lib/persistence"
import { getRegisteredRuntimeAPIs } from "@/contexts/runtimeAPIRegistry"
import { API_ENDPOINTS } from "@/lib/http"

export type { DiscoveredGitCredential, GitIdentityAuthType, GitIdentityProfile }
      ...
      (set, get) => ({
        ...
        getUnimportedCredentials: () => {
          ...
          return discoveredCredentials.filter((cred) => {
            return !profiles.some((p) => p.authType === "token" && p.host === cred.host)
          })
        },
      }),
      {
