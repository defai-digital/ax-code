...
function resolveProjectRef(): { id: string; path: string } | null {
  ...
  const raw =
    typeof activeProjectPath === "string" && activeProjectPath.trim().length > 0 ? activeProjectPath : currentDirectory
...
function buildGroups(sessions: Session[], metaByPath: Map<string, WorktreeMetadata>): AgentGroup[] {
...
export const useAgentGroupsStore = create<Store>()((set, get) => ({
  groups: [],
      ...
      set({
        ...
        error:
          failedDirectories.size > 0
            ? `Failed to load sessions for ${failedDirectories.size} worktree${failedDirectories.size === 1 ? "" : "s"}`
            : null,
      })
    } catch (err) {
      set({
  ...
  deleteGroupSessions: async (sessions, options) => {
    ...
    if (removeWorktrees) {
      ...
      for (const [path, pathSessions] of worktreesByPath) {
        ...
        const source =
          pathSessions.find((session) => session.worktreeMetadata)?.worktreeMetadata ??
          pathSessions[0]?.worktreeMetadata
        const projectRef =
          pathSessions.map(resolveProjectRefForWorktree).find((value): value is ProjectRef => value !== null) ?? null
