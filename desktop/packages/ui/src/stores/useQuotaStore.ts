...
const parseSettings = (data: Record<string, unknown> | null): QuotaSettingsState => {
  ...
  const autoRefresh = typeof data?.usageAutoRefresh === "boolean" ? data.usageAutoRefresh : false
  ...
  const showPredValues = typeof data?.usageShowPredValues === "boolean" ? data.usageShowPredValues : false
  const rawDropdownProviders = Array.isArray(data?.usageDropdownProviders) ? data?.usageDropdownProviders : null
  const dropdownProviderIds = rawDropdownProviders
    ? rawDropdownProviders.filter(
        (entry): entry is QuotaProviderId =>
      ...
      fetchProviderQuota: async (providerId) => {
        ...
        try {
          const response = await fetch(
            API_ENDPOINTS.quota.byProvider.replace(":providerId", encodeURIComponent(providerId)),
          )
