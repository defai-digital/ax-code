...
interface ProjectsStore {
  ...
  updateProjectMeta: (
    id: string,
    meta: { label?: string; icon?: string | null; color?: string | null; iconBackground?: string | null },
  ) => void
...
const sanitizeProjectIconImage = (value: unknown): ProjectEntry["iconImage"] | undefined => {
  ...
  const updatedAt =
    typeof candidate.updatedAt === "number" && Number.isFinite(candidate.updatedAt)
      ...
      : 0
  const source = candidate.source === "custom" || candidate.source === "auto" ? candidate.source : null
  ...
  devtools(
    (set, get) => ({
      ...
      renameProject: (id: string, label: string) => {
        ...
        const { projects, activeProjectId } = get()
        const nextProjects = projects.map((project) => (project.id === id ? { ...project, label: trimmed } : project))
        set({ projects: nextProjects })
      ...
      uploadProjectIcon: async (id: string, file: File) => {
        ...
        try {
          ...
          const response = await fetch(
            `${replacePathParams(API_ENDPOINTS.projects.icon, {
              projectId: id,
            })}`,
            {
              method: "PUT",
      ...
      discoverProjectIcon: async (id: string, options?: { force?: boolean }) => {
        try {
          const response = await fetch(
            `${replacePathParams(API_ENDPOINTS.projects.iconDiscover, {
              projectId: id,
            })}`,
            {
              method: "POST",
