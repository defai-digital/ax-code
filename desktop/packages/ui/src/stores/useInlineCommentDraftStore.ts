...
interface InlineCommentDraftActions {
  ...
  updateDraft: (
    sessionKey: string,
    draftId: string,
    updates: Partial<Omit<InlineCommentDraft, "id" | "createdAt" | "sessionKey">>,
  ) => void
...
const isValidSource = (value: unknown): value is InlineCommentSource =>
  ...
  value === "plan" ||
  value === "file" ||
  value === "preview-console" ||
  value === "preview-annotation"

const isValidSide = (value: unknown): value is "original" | "modified" => value === "original" || value === "modified"
