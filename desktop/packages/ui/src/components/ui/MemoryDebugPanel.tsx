...
const PerfSection: React.FC<{ title: string; snapshot: StreamPerfSnapshot; emptyLabel: string }> = ({
  title,
  snapshot,
  emptyLabel,
}) => {
  ...
  return (
    <div className="space-y-2 border-t border-[var(--interactive-border)] pt-2 first:border-t-0 first:pt-0">
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <MetricCard
          label={t("memoryDebugPanel.metric.lastUpdate")}
          value={snapshot.lastUpdatedAt ? t("memoryDebugPanel.common.live") : t("memoryDebugPanel.common.notAvailable")}
        />
      </div>
        ...
        <ScrollableOverlay outerClassName="max-h-64" className="pr-1">
          <div className="space-y-1">
            {topEntries.map((entry) => (
              ...
              >
                <div className="typography-meta font-medium text-[var(--surface-foreground)] break-all">
                  {entry.metric}
                </div>
    ...
    >
      <div className="mb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="typography-ui-label font-semibold text-[var(--surface-foreground)]">
      ...
      {activeTab === "memory" ? (
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2 typography-meta">
            ...
            <MetricCard
              label={t("memoryDebugPanel.metric.cachedSessions")}
              value={`${cachedSessionCount} / ${MEMORY_LIMITS.MAX_SESSIONS}`}
            />
          </div>
          ...
          <div className="typography-meta space-y-1 border-t border-[var(--interactive-border)] pt-2">
            <div className="flex justify-between gap-2">
              <span className="text-[var(--surface-muted-foreground)]">
                {t("memoryDebugPanel.metric.viewportWindow")}
              </span>
              <span className="text-[var(--surface-foreground)]">
                {t("memoryDebugPanel.metric.messagesValue", { count: getBackgroundTrimLimit() })}
              </span>
            </div>
            <div className="flex justify-between gap-2">
              <span className="text-[var(--surface-muted-foreground)]">
                {t("memoryDebugPanel.metric.zombieTimeout")}
              </span>
              <span className="text-[var(--surface-foreground)]">
                {t("memoryDebugPanel.metric.minutesValue", { count: MEMORY_LIMITS.ZOMBIE_TIMEOUT / 1000 / 60 })}
              </span>
            </div>
            <div className="flex justify-between gap-2">
              <span className="text-[var(--surface-muted-foreground)]">
                {t("memoryDebugPanel.metric.githubTotalRequests")}
              </span>
          ...
          <div className="border-t border-[var(--interactive-border)] pt-2">
            <div className="mb-1 typography-meta font-semibold text-[var(--surface-foreground)]">
              {t("memoryDebugPanel.section.sessionsInMemory")}
            </div>
              ...
              {sessionStats.map((stat) => (
                ...
                >
                  <div className="flex min-w-0 flex-1 items-center gap-2">
                    ...
                    {stat.isStreaming ? (
                      <Icon name="pulse" className="h-3 w-3 animate-pulse text-[var(--status-info)]" />
                    ) : null}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <MetricCard
              label={t("memoryDebugPanel.metric.messageListRenders")}
              value={streamMetricCounts.messageListRender}
            />
            <MetricCard
              label={t("memoryDebugPanel.metric.messageListStreamRenders")}
              value={streamMetricCounts.messageListRenderStreaming}
            />
            <MetricCard
              label={t("memoryDebugPanel.metric.chatMessageRenders")}
              value={streamMetricCounts.chatMessageRender}
            />
            <MetricCard
              label={t("memoryDebugPanel.metric.chatMessageStreamRenders")}
              value={streamMetricCounts.chatMessageRenderStreaming}
            />
            <MetricCard
              label={t("memoryDebugPanel.metric.chatMessageStaticDuringStream")}
              value={streamMetricCounts.chatMessageRenderStaticDuringStream}
            />
            <MetricCard
