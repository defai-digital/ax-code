...
export const AxCodeStatusDialog: React.FC = () => {
  ...
  const handleCopy = React.useCallback(async () => {
    ...
    if (result.ok) {
      toast.success(t("axCodeStatusDialog.toast.copiedTitle"), {
        description: t("axCodeStatusDialog.toast.copiedDescription"),
      })
      return
    }
    ...
    <Dialog open={isAxCodeStatusDialogOpen} onOpenChange={setAxCodeStatusDialogOpen}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>{t("axCodeStatusDialog.description")}</DialogDescription>
        </DialogHeader>
