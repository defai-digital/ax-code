...
const SortableTabWrapper: React.FC<{ id: string; children: React.ReactNode; className?: string }> = ({
  id,
  children,
  className,
}) => {
...
const StaticTabWrapper: React.FC<{ id: string; children: React.ReactNode; className?: string }> = ({
  id,
  children,
  className,
}) => (
  <div className={cn("h-full", className)} data-sortable-tab-id={id}>
    {children}
  </div>
)
...
export const SortableTabsStrip: React.FC<SortableTabsStripProps> = ({
  ...
  const [pillRect, setPillRect] = React.useState<{ left: number; top: number; width: number; height: number } | null>(
    null,
  )
    ...
    <div className={cn("relative flex h-full min-w-0 flex-1", className)}>
      {isScrollable && overflow.left ? (
        <div
          className={cn(
            ...
            usesActivePillIndicator ? "w-8 from-[var(--surface-background)]" : "w-6 from-background",
          )}
        />
      ) : null}
      ...
      <div
        ...
        className={cn(
          ...
          showPillTrackBackground &&
            "rounded-[10px] [corner-shape:squircle] supports-[corner-shape:squircle]:rounded-[50px] bg-[color-mix(in_srgb,var(--foreground)_2%,transparent)] p-0.5 gap-0.5",
          isScrollable ? "overflow-x-auto scrollbar-none" : "overflow-x-hidden",
        )}
      ...
      >
        ...
        {items.map((item) => {
          ...
          const showInactiveIconOnly =
            inactiveTabsIconOnly && usesActivePillIndicator && !isActive && Boolean(item.icon)
          const shouldShowLabel = !showInactiveIconOnly
          const shouldShowIcon = Boolean(item.icon) && (!iconOnlyActiveTab || isActive)
          const useIntrinsicActiveTab =
            inactiveTabsIconOnly && usesActivePillIndicator && isActive && !isScrollable && !useIntrinsicPillSizing
              ...
              >
                ...
                >
                  {usesActivePillIndicator ? (
                        ...
                        <span className="relative flex h-4 w-4 shrink-0 items-center justify-center">
                          <span
                            className={cn(
                              ...
                              closeReplacesIcon && (alwaysShowCloseControls ? "opacity-0" : "group-hover:opacity-0"),
                            )}
                          >
                            {item.icon}
                          </span>
                          {closeReplacesIcon ? (
                            <span
                              ...
                              tabIndex={-1}
                              className={cn(
                                "absolute inset-0 z-20 flex !min-h-0 !min-w-0 items-center justify-center rounded-sm text-muted-foreground transition-opacity hover:text-foreground",
                                alwaysShowCloseControls ? "opacity-100" : "opacity-0 group-hover:opacity-100",
                              )}
                              onPointerDown={(event) => {
                                event.stopPropagation()
                              }}
                    ...
                    <span
                      className={cn("flex min-w-0 flex-nowrap items-center gap-1.5", !isScrollable && "justify-center")}
                    >
                      {shouldShowIcon ? (
                        <span
                          className={cn(
