...
export const PermissionCard: React.FC<PermissionCardProps> = ({ permission, onResponse }) => {
  ...
  const renderToolContent = () => {
    if (tool === "bash" || tool === "shell" || tool === "shell_command") {
      ...
      return (
        <>
          {description && <div className="typography-meta text-muted-foreground mb-2">{description}</div>}
          {workingDir && (
            <div className="typography-meta text-muted-foreground mb-2">
              <span className="font-semibold">{t("chat.permissionCard.workingDirectory")}</span>{" "}
              <code className="px-1 py-0.5 bg-muted/30 rounded">{workingDir}</code>
            </div>
          )}
          {timeout && (
            <div className="typography-meta text-muted-foreground mb-2">
    ...
    if (tool === "webfetch" || tool === "fetch" || tool === "curl" || tool === "wget") {
      ...
      const headers =
        permission.metadata.headers && typeof permission.metadata.headers === "object"
          ...
          {url && (
            <div className="mb-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <code className="typography-meta px-2 py-1 bg-muted/30 rounded flex-1 break-all">{url}</code>
              </div>
            </div>
          )}
