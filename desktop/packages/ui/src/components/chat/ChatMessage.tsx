...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  const currentSessionAgent = useSelectionStore((state) =>
    isInActiveTurn && sessionId ? state.sessionAgentSelections.get(sessionId) : undefined,
  )
  ...
  const assistantTopPaddingClass =
    !isUser && shouldShowHeader ? (stickyUserHeader ? (isMobile ? "pt-4" : "pt-6") : "pt-0") : "pt-0"
              ...
              >
                <div className={cn("relative flex justify-end", !isMobile ? "group/user-shell" : undefined)}>
                  <div className={cn("max-w-[85%]", showStickyInlineHoverRow ? "pb-5" : undefined)}>
                    <div style={USER_BUBBLE_STYLE} className="px-5 py-3 shadow-none border border-primary/5">
                      <MessageBody
