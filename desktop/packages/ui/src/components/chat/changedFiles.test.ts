          ...
          files: [
            {
              ...
              patch: ["--- a/src/patched.ts", "+++ b/src/patched.ts", "@@", "-old", "+new", "+another"].join("\n"),
            },
          ],
        },
      }),
          ...
          results: [
            {
              filediff: {
                file: "src/result.ts",
                patch: ["--- a/src/result.ts", "+++ b/src/result.ts", "@@", "-removed", "+added"].join("\n"),
              },
            },
          ],
        },
  ...
  test("falls back to input paths and patch stats", () => {
    const files = extractChangedFiles([
      ...
      toolPart({
        ...
        tool: "apply_patch",
        metadata: {
          patch: ["--- a/src/app.ts", "+++ b/src/app.ts", "@@", "-old", "+new", "+another"].join("\n"),
        },
      }),
    ])
