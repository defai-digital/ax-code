...
export const areRenderRelevantMessageInfoEqual = (left: Message, right: Message): boolean => {
  ...
  return (
    ...
    (left as { userMessageMarker?: unknown }).userMessageMarker ===
      (right as { userMessageMarker?: unknown }).userMessageMarker &&
    ((left as { time?: { created?: unknown; completed?: unknown } }).time?.created ?? null) ===
      ((right as { time?: { created?: unknown; completed?: unknown } }).time?.created ?? null) &&
    ((left as { time?: { created?: unknown; completed?: unknown } }).time?.completed ?? null) ===
      ((right as { time?: { created?: unknown; completed?: unknown } }).time?.completed ?? null)
  )
}

export const areRenderRelevantMessagesEqual = (left: SessionMessageRecord, right: SessionMessageRecord): boolean => {
  return (
    areRenderRelevantMessageInfoEqual(left.info, right.info) && areRenderRelevantPartsEqual(left.parts, right.parts)
  )
}

export const areOptionalRenderRelevantMessagesEqual = (
  left?: SessionMessageRecord,
  right?: SessionMessageRecord,
): boolean => {
  if (!left || !right) {
    return left === right
  }
...
const areTurnDiffStatsEqual = (left?: TurnDiffStats, right?: TurnDiffStats): boolean => {
  ...
  return left.additions === right.additions && left.deletions === right.deletions && left.files === right.files
}
...
const areTurnActivityGroupsEqual = (left: TurnActivityGroup, right: TurnActivityGroup): boolean => {
  if (
    ...
    left.afterToolPartId !== right.afterToolPartId
  ) {
    return false
  }
...
export const areRelevantTurnGroupingContextsEqual = (
  ...
  const segmentsRelevant =
    hasRelevantActivitySegments(left.activityGroupSegments, messageId) ||
