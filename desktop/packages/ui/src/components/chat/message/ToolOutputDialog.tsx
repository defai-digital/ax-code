...
const ImagePreviewDialog: React.FC<{
  ...
  const gallery = React.useMemo(() => {
    ...
    const fromPopup = Array.isArray(baseImage.gallery)
      ? baseImage.gallery.filter((item): item is { url: string; mimeType?: string; filename?: string; size?: number } =>
          Boolean(item?.url),
        )
      : []
  ...
  React.useEffect(() => {
    ...
    const matchingIndex = popup.image?.url ? gallery.findIndex((item) => item.url === popup.image?.url) : -1
      ...
      >
        ...
        >
          <div className="flex items-center justify-between gap-2">
            <div
              className="min-w-0 flex-1 text-foreground typography-ui-header font-semibold truncate"
              title={imageTitle}
            >
              {imageTitle}
            </div>
...
const DialogReadContent: React.FC<{
  ...
  const codeLines: CodeLine[] = React.useMemo(() => {
    ...
    for (const line of parsedReadOutput.lines) {
      ...
      const shouldAssignFallback =
        parsedReadOutput.type === "file" && !hasExplicitLineNumbers && line.lineNumber === null && !line.isInfo
      const effectiveLineNumber = line.lineNumber ?? (shouldAssignFallback ? nextLineNumber + 1 : null)
  ...
  return (
    <VirtualizedCodeBlock lines={codeLines} language={detectedLanguage} syntaxTheme={syntaxTheme} maxHeight="70vh" />
  )
})
  ...
  const loadMermaidSource = React.useCallback(async () => {
    ...
    if (target.url.startsWith("data:")) {
      ...
      if (!normalizedPath) {
        ...
        sourcePromise = fetch(`${API_ENDPOINTS.fs.raw}?path=${encodeURIComponent(normalizedPath)}`).then((response) => {
          if (!response.ok) {
      ...
      const canParse = typeof URL.canParse === "function" ? URL.canParse(target.url, window.location.origin) : false
      ...
      >
        <div className="flex-shrink-0 pb-1">
          <div className="flex items-start gap-2 text-foreground typography-ui-header font-semibold">
            {popup.metadata?.tool ? (
              getToolIcon(popup.metadata.tool as string)
            ) : (
            ...
            {popup.isDiff && (
              <DiffViewToggle mode={diffViewMode} onModeChange={setDiffViewMode} className="mr-8 flex-shrink-0" />
            )}
          </div>
        </div>
        <div className="flex-1 min-h-0 rounded-xl border border-border/30 bg-muted/10 overflow-hidden">
          <div className="tool-output-surface h-full max-h-[75vh] overflow-y-auto px-3 pr-4">
            ...
            typeof popup.metadata.input === "object" &&
                            ...
                            file={{
                              ...
                              lang:
                                getLanguageFromExtension(
                                  getInputValue("filePath") || getInputValue("file_path") || "",
                                ) || undefined,
                            }}
                            options={{
                              disableFileHeader: true,
            ...
            {popup.isDiff ? (
              <DialogUnifiedDiff popup={popup} diffViewMode={diffViewMode} pierreThemeConfig={pierreThemeConfig} />
            ) : popup.content ? (
              <div className="p-4">
                {(() => {
                  ...
                  if (jsonResult.isJson) {
                    return <JsonTreeView jsonString={popup.content} initiallyExpandedDepth={3} maxHeight="70vh" />
                  }
