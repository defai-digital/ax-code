...
describe("toolDiffUtils", () => {
  test("treats raw apply_patch envelopes as text, not visual diffs", () => {
    const entries = getDiffPatchEntries(
      undefined,
      ["*** Begin Patch", "*** Update File: src/app.ts", "@@ -1 +1 @@", "-old", "+new", "*** End Patch"].join("\n"),
      identity,
    )
        ...
        files: [
          {
            ...
            patch: ["--- a/src/file.ts", "+++ b/src/file.ts", "@@ -1 +1 @@", "-old", "+new"].join("\n"),
          },
        ],
      },
      "not a diff",
  ...
  test("synthesizes headers for valid headerless hunks", () => {
    const entries = getDiffPatchEntries(undefined, ["@@ -1 +1 @@", "-old", "+new"].join("\n"), identity)
  ...
  test("keeps malformed unified patches as text fallbacks", () => {
    const entries = getDiffPatchEntries(
      undefined,
      ["--- a/src/file.ts", "+++ b/src/file.ts", "@@", "-old", "+new"].join("\n"),
      identity,
    )
