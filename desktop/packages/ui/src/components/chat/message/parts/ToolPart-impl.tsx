    ...
    <>
      {entries.map((entry) => {
        ...
        return (
          <span
            key={entry.path}
            className={cn("inline-flex min-w-0 max-w-full items-center gap-1", TOOL_ROW_DESCRIPTION_CLASS)}
  ...
  if (jsonResult.isJson) {
    return (
      <div className="tool-output-surface p-2 rounded-xl w-full min-w-0">
        <JsonTreeViewer data={jsonResult.data} initiallyExpandedDepth={1} maxHeight="400px" />
      </div>
    )
  }
                    ...
                    {hasLabel ? (
                      status !== "error" && shouldRenderGitPathLabel(toolName, label) ? (
                        ...
                        <Text
                          ...
                          className={cn(
                            "typography-meta flex-1 min-w-0 text-muted-foreground",
                            isMobile ? "whitespace-normal break-words" : "truncate",
                          )}
                          style={{ color: "var(--tools-description)" }}
                          title={label}
...
const renderPathLikeGitChanges = (path: string, grow = true) => {
  ...
  return (
    <span
      className={cn("min-w-0 flex items-baseline overflow-hidden typography-ui-label", grow && "flex-1")}
      title={path}
    >
...
const renderAnimatedPathWithIcon = (path: string, animate = true, grow = true, showFileIcons = true) => {
  ...
  return (
    <span className={cn("min-w-0 inline-flex items-center gap-1 overflow-hidden", grow && "flex-1")} title={path}>
      ...
      <span
        className={cn(
          "min-w-0 inline-flex max-w-full items-baseline overflow-hidden",
          TOOL_ROW_DESCRIPTION_CLASS,
          grow && "flex-1",
        )}
      >
        {hasAbsoluteRoot ? (
          <span className="flex-shrink-0" style={{ color: "var(--tools-description)" }}>
            /
          </span>
        ) : null}
        <span
...
const ToolExpandedContent: React.FC<ToolExpandedContentProps> = React.memo(
  ({ part, state, syntaxTheme, currentDirectory, onShowPopup }) => {
    const { t } = useI18n()
    ...
    const hideToolInputPreview = part.tool === "apply_patch" || part.tool === "edit" || part.tool === "multiedit"
    ...
    const renderResultContent = () => {
            ...
            <div className="space-y-1">
              ...
              <div className="space-y-1">
                {diagnosticSection.diagnostics.map((diagnostic, index) => (
                  <div
                    key={`${diagnosticSection.displayPath}:${diagnostic.line}:${diagnostic.character}:${index}`}
                    className="rounded-md border px-2 py-1"
                    style={{ borderColor: "var(--status-error-border)", backgroundColor: "var(--surface-elevated)" }}
                  >
          ...
          return renderScrollableBlock(
            <div className="space-y-2">
              {questionInput.questions.map((q, index) => (
                <div key={index} className="space-y-0.5">
                  {q.header ? <div className="typography-micro text-muted-foreground">{q.header}</div> : null}
      ...
      if (
        (part.tool === "edit" || part.tool === "multiedit" || part.tool === "apply_patch" || part.tool === "write") &&
        (diffEntries.length > 0 || !!diagnosticSection)
      ) {
        return renderScrollableBlock(
            ...
            {diffEntries.map((entry) => (
              <div key={entry.id} className="w-full min-w-0">
                ...
                {entry.renderMode === "diff" ? (
                  <DiffPreview diff={entry.patch} diffViewMode={diffViewMode} />
                ) : (
      ...
      if (part.tool === "write" && diagnosticSection) {
        return renderScrollableBlock(<div className="space-y-3">{renderDiagnosticsSection()}</div>, {
          className: "p-1",
        })
      }
    ...
    return (
      <div className={cn("relative pr-2 pb-2 pt-2 space-y-2 pl-4")}>
        {part.tool === "question" ? (
          renderResultContent()
        ) : (
          <>
              ...
              <div className="my-1">
                {renderScrollableBlock(
                  part.tool === "bash" ? (
                    ...
                    <DiffPreview diff={writeLikeInputPatch} diffViewMode={diffViewMode} />
                  ) : (
              ...
              <div>
                ...
                hasVisualDiffEntry ? (
                  <div className="mb-1 flex items-center justify-end gap-2">
                    <DiffViewToggle mode={diffViewMode} onModeChange={setDiffViewMode} className="h-5 w-5 p-0" />
                  </div>
                ) : null}
                {renderResultContent()}
              </div>
...
const ToolPartContent: React.FC<ToolPartProps> = ({
  ...
  const effectiveTimeStart = React.useMemo(() => {
    ...
    const candidates = [pinnedTime.start, time?.start].filter((value): value is number => typeof value === "number")
  ...
  const metadataTaskSummaryEntries = React.useMemo<TaskToolSummaryEntry[]>(() => {
    ...
    const candidateSummary = metadata as
      | { summary?: unknown; entries?: unknown; tools?: unknown; calls?: unknown }
      | undefined
  ...
  React.useEffect(() => {
    ...
    const sinceStart = typeof taskSessionResolutionStart === "number" ? Date.now() - taskSessionResolutionStart : 0
  ...
  }, [explicitTaskSessionId, isFinalized, isTaskTool, taskFallbackRetried, taskSessionId, taskSessionResolutionStart])
  ...
  React.useEffect(() => {
    ...
    const childSessionIsActive =
      childSessionActivity.phase === "busy" || childSessionActivity.phase === "retry" || childSessionHasInFlightTools
        ...
        if (Array.isArray(messages) && messages.length > 0) {
          ...
          childStores.update(currentDirectory, (prev) => {
            ...
            return {
              message: {
                ...prev.message,
                [capturedSessionId]: records.map((r) => r.info) as import("@ax-code/sdk/v2").Message[],
              },
              part: partPatch,
            }
          })
  ...
  }, [effectiveTimeStart, isFinalized, pinnedTime.end, time?.end])
      ...
      try {
        ...
        childStores.update(currentDirectory, (prev) => {
          ...
          return {
            message: {
              ...prev.message,
              [taskSessionId]: records.map((r) => r.info) as import("@ax-code/sdk/v2").Message[],
            },
            part: partPatch,
          }
        })
  ...
  const isMultiFileApplyPatch =
    normalizedPartTool === "apply_patch" && Array.isArray(metadata?.files) && (metadata?.files as []).length > 1
  const normalizedPart =
    normalizedPartTool !== part.tool ? ({ ...part, tool: normalizedPartTool } as ToolPartType) : part
  ...
  return (
    <div>
      ...
      >
        <div
          className={cn(
            ...
            isMultiFileApplyPatch
              ? "w-full min-w-0 flex-wrap items-center gap-x-2 gap-y-0.5"
        ...
        {!isMultiFileApplyPatch && (
          <div
            className={cn("flex items-center gap-1 flex-1 min-w-0", TOOL_ROW_DESCRIPTION_CLASS)}
            style={{ color: "var(--tools-description)" }}
          >
