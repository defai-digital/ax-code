...
export const ReasoningTimelineBlock: React.FC<ReasoningTimelineBlockProps> = ({
  ...
  const isExpanded = expansion.source === "auto" ? canAutoExpand && expansion.expanded : expansion.expanded
  ...
  const toggleAriaLabel = isExpanded ? t("chat.reasoningTrace.collapseAria") : t("chat.reasoningTrace.expandAria")
  ...
  return (
    <div data-reasoning-block-id={blockId} data-message-text-export-root="true">
      <div
        ...
        className={cn("group/tool flex gap-1.5 pr-2 pl-px py-1.5 rounded-xl cursor-pointer items-center")}
...
const ReasoningPart = React.memo(({ part, onContentChange, messageId, streamPhase }: ReasoningPartProps) => {
...
export const MergedReasoningPart = React.memo(
  ({ parts, onContentChange, messageId, streamPhase }: MergedReasoningPartProps) => {
    const chatRenderMode = useUIStore((state) => state.chatRenderMode)
    ...
    const isStreaming =
      chatRenderMode === "live" &&
      canBeStreaming &&
      parts.some((part) => typeof (part as TimedTextContentPart).time?.end !== "number")
