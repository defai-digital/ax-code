...
export const VirtualizedCodeBlock: React.FC<VirtualizedCodeBlockProps> = React.memo((props) => {
  const { lines, language, syntaxTheme, maxHeight = "60vh", showLineNumbers = true, lineStyles } = props
  ...
  if (!shouldVirtualize) {
    return (
      ...
      >
        ...
        {lines.map((line, idx) => (
          <Row key={idx} line={line} language={language} showLineNumbers={showLineNumbers} style={lineStyles?.(line)} />
        ))}
      </div>
    )
  }
...
const VirtualizedRows: React.FC<VirtualizedRowsProps> = React.memo(
  ({ lines, language, prismThemeCss, maxHeight, showLineNumbers, lineStyles }) => {
        ...
        >
          {virtualizer.getVirtualItems().map((vItem) => {
            ...
            return (
              ...
              >
                <Row line={line} language={language} showLineNumbers={showLineNumbers} style={lineStyles?.(line)} />
              </div>
            )
          })}
        </div>
...
const Row: React.FC<RowProps> = React.memo(({ line, language, showLineNumbers, style }) => {
  ...
  return (
    <div className="typography-code font-mono flex w-full min-w-0" style={style}>
      {showLineNumbers && (
        <span
