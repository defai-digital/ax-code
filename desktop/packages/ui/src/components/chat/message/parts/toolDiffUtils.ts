...
const getPatchEntriesFromText = (
  ...
  for (const chunk of getPatchChunks(normalized)) {
    ...
    if (!info) {
      if (
        HUNK_HEADER_PATTERN.test(chunk) ||
        GIT_DIFF_FILE_BREAK_TEST.test(chunk) ||
        UNIFIED_DIFF_FILE_BREAK_TEST.test(chunk)
      ) {
        chunkEntries.push({
...
const getFilePatch = (file: unknown): { patch: string; title: string } | null => {
  ...
  const rawPath =
    typeof file.relativePath === "string" ? file.relativePath : typeof file.filePath === "string" ? file.filePath : ""
