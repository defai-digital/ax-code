...
export const formatEditOutput = (output: string, toolName: string, metadata?: Record<string, unknown>): string => {
  ...
  if (
    (toolName === "edit" || toolName === "multiedit" || toolName === "write" || toolName === "apply_patch") &&
    hasLspDiagnostics(cleaned)
  ) {
    ...
    if (detectedType !== "directory") {
      ...
      if (numberedMatch) {
        ...
        const numberedIsInfo =
          (numberedTrimmed.startsWith("(") && numberedTrimmed.endsWith(")")) || isTruncationInfoLine(numberedTrimmed)
        return {
            ...
            <div className="pl-4 space-y-1">
              {matches.map((match, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={cn("flex items-start gap-2 min-w-0", isMobile ? "typography-micro" : "typography-code")}
                  >
                    <div className="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-1.5" style={GREP_DOT_STYLE} />
                    <div className="flex gap-2 min-w-0 flex-1">
                      {match.lineNum && (
                        <span className="text-muted-foreground font-mono whitespace-nowrap">Line {match.lineNum}:</span>
                      )}
                      <span className="text-foreground font-mono break-words flex-1">{match.content || "\u00A0"}</span>
                    </div>
                  </div>
                )
              })}
          ...
          <div key={dir} className="space-y-1">
            ...
            <div className={cn("pl-4 grid gap-1", isMobile ? "grid-cols-1" : "grid-cols-2")}>
              {groups[dir].sort().map((filename) => (
                <div
                  key={filename}
                  className={cn("flex items-center gap-2 min-w-0", isMobile ? "typography-micro" : "typography-code")}
                >
        ...
        {todosByStatus.pending.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="typography-meta font-semibold text-muted-foreground uppercase tracking-wide">
                {labels.pending}
              </span>
            </div>
            <div className="space-y-1.5 pl-4">
        ...
        {todosByStatus.completed.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span
                className="typography-meta font-semibold uppercase tracking-wide"
        ...
        {todosByStatus.cancelled.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="typography-meta font-semibold text-muted-foreground/50 uppercase tracking-wide">
                {labels.cancelled}
              </span>
            </div>
            <div className="space-y-1.5 pl-4">
              {todosByStatus.cancelled.map((todo, idx) => (
                <div key={todo.id || idx} className="flex items-start gap-2">
                  ...
                  <span className="typography-code text-muted-foreground/50 line-through flex-1 leading-relaxed">
                    {todo.content}
                  </span>
                </div>
              ))}
...
export const renderWebSearchOutput = (
  output: string,
  _syntaxTheme: { [key: string]: React.CSSProperties },
  options?: { unstyled?: boolean },
) => {
  try {
    return (
    ...
    if (line.startsWith("@@")) {
      ...
      leftSide.forEach((leftItem, leftIdx) => {
        if (leftItem.type === "context") {
          const rightIdx = rightSide.findIndex(
            (rightItem, rIdx) =>
