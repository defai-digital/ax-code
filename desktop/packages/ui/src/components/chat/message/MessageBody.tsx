...
const UserSubtaskPart: React.FC<{ part: SubtaskPartLike }> = ({ part }) => {
  ...
  return (
    <div className="mt-2">
      ...
      {description ? <div className="typography-ui-label text-foreground/90 mt-1.5">{description}</div> : null}
        ...
        <div className="mt-2 border-t border-border/60 pt-1.5">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              ...
              aria-label={
                copiedOutput ? t("chat.messageBody.shellCommand.copied") : t("chat.messageBody.shellCommand.copyOutput")
              }
              title={
                copiedOutput ? t("chat.messageBody.shellCommand.copied") : t("chat.messageBody.shellCommand.copyOutput")
              }
...
const UserMessageBody = React.memo(
  ({
    messageId,
    parts,
    isMobile,
    ...
    const formattedUserTimestamp = React.useMemo(
      () =>
        typeof messageCreatedAt === "number" && messageCreatedAt > 0
          ? formatTimestampForDisplay(messageCreatedAt, timeFormatPreference)
          : null,
      ...
      >
        <div
          className={cn(
            ...
            useStickyScrollableUserContent ? "overflow-y-auto overscroll-contain scrollbar-none" : "overflow-y-hidden",
          )}
          style={
            useStickyScrollableUserContent ? { maxHeight: "calc(var(--chat-scroll-height, 100dvh) * 0.4)" } : undefined
          }
        >
          {userContentParts.map((part, index) => {
            ...
            return (
              <React.Fragment key={part.id ?? `user-text-${index}`}>
                <UserTextPart part={part} messageId={messageId} isMobile={isMobile} agentMention={mentionForPart} />
              </React.Fragment>
            )
          })}
        </div>
...
const AssistantMessageBody = React.memo(
  ({
    ...
    const currentProjectRef = React.useMemo(() => {
      ...
      const directory = effectiveDirectory ?? (currentSessionId ? getDirectoryForSession(currentSessionId) : null) ?? ""
    ...
    const shouldRenderActivityGroup =
      isSortedRenderMode && isActivityOwnerMessage && hasAnchoredActivitySegments && Boolean(toggleActivityGroup)
    ...
    const renderJustificationActions = React.useCallback(
      (activity: NonNullable<TurnGroupingContext["activityParts"]>[number]) => {
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                onClick={() => {
                  const directory =
                    effectiveDirectory ?? (currentSessionId ? getDirectoryForSession(currentSessionId) : null)
                  if (!directory) {
                    return
                  }
    ...
    return (
      ...
      >
        ...
        <div>
          ...
          {shouldShowTurnFooter && (
            <div className="mt-2 mb-1 flex items-center justify-start gap-1.5" style={MESSAGE_FOOTER_CONTAINER_STYLE}>
              <div className="flex items-center gap-1.5" data-message-action-group="true">
                {messageActionButtons}
              ...
              <div className="flex items-center gap-1.5">
                ...
                {footerTimestamp ? (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className={footerTimestampClassName} aria-label={`Message time: ${footerTimestamp}`}>
