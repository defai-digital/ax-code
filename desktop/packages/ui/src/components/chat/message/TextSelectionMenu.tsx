...
const appendDistilledInsightToNotes = (existingNotes: string, insight: string): string => {
  const trimmedInsight = insight
    .trim()
    .replace(/^[-*+]\s+/, "")
    .slice(0, AX_CODE_DESKTOP_PROJECT_NOTES_MAX_LENGTH)
  if (!trimmedInsight) {
...
export const TextSelectionMenu: React.FC<TextSelectionMenuProps> = ({ containerRef }) => {
  ...
  const showMenu = React.useCallback(() => {
    ...
    const menuX = isMobile ? rect.left + rect.width / 2 : getDesktopClampedX(rect.left + rect.width / 2)
  ...
  React.useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (menuRef.current && !menuRef.current.contains(e.target as Node) && !window.getSelection()?.toString().trim()) {
        hideMenu()
      }
    }
  ...
  const currentProjectRef = React.useMemo(() => {
    const directory =
      effectiveDirectory ?? (typeof currentSession?.directory === "string" ? currentSession.directory : "")
    ...
    return createPortal(
      ...
      >
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            {isAddingToNotes ? (
              <Icon name="loader-4" className="h-5 w-5 flex-shrink-0 animate-spin" />
            ) : (
              <Icon name="booklet" className="h-5 w-5 flex-shrink-0" />
            )}
