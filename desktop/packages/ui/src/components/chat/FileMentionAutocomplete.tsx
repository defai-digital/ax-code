...
export const FileMentionAutocomplete = React.forwardRef<FileMentionHandle, FileMentionAutocompleteProps>(
  ({ searchQuery, onFileSelect, onAgentSelect, onClose, style }, ref) => {
    const { t } = useI18n()
    ...
    const recentFiles = React.useMemo(() => {
      ...
      const ordered = [projectTabs.selectedPath, ...projectTabs.openPaths.slice().reverse()].filter(
    ...
    React.useEffect(() => {
      ...
      const normalizedQueryLower = normalizedQuery.replace(/^\.\//, "").replace(/^\/+/, "").toLowerCase()
    ...
    const handleAgentPick = React.useCallback(
      (agentName: string) => {
    ...
    React.useImperativeHandle(
      ...
      () => ({
        handleKeyDown: (key: string) => {
          ...
          const total =
            visibleAgents.length + visibleDirectories.length + visibleRecentFiles.length + visibleFiles.length
          if (total === 0) {
            return
          }
          ...
          if (key === "Enter" || key === "Tab") {
            ...
            const selectedFile =
              fileIndex < visibleRecentFiles.length
          ...
          {loading ? (
            ...
            <div className="pb-2">
                  ...
                  >
                    ...
                    >
                      ...
                      {isOverflowing && isSelected ? (
                        <span className="inline-block whitespace-nowrap file-mention-marquee">{relativePath}</span>
                      ) : (
              ...
              {visibleFiles.map((file, index) => {
                ...
                return <React.Fragment key={file.path}>{item}</React.Fragment>
              })}
