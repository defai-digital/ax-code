 ...
 */
export function buildHighlightParts(text: string, ranges: HighlightRange[]): HighlightPart[] | null {
  ...
  for (let i = 0; i < sorted.length - 1; i += 1) {
    ...
    const className = bestRange ? (bestRange.className ?? STYLE_CLASS[bestRange.style]) : DEFAULT_CLASS
