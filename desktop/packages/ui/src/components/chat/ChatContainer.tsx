...
const shouldIgnoreChatNavigationForFocus = (
  ...
  ({
    ...
    return (
      <div
        className={cn(
          "relative min-h-0",
          isDesktopExpandedInput ? "absolute inset-0 opacity-0 pointer-events-none" : "flex-1",
        )}
...
export const ChatContainer: React.FC<ChatContainerProps> = ({ autoOpenDraft = true, readOnly = false }) => {
  ...
  const sessionPrefetchInfo = React.useSyncExternalStore(
    React.useCallback(
      (notify) =>
        currentSessionId ? subscribeSessionPrefetch(syncDirectory, currentSessionId, notify) : () => undefined,
  ...
  const activeRetryStatus = React.useMemo(() => {
    ...
    const rawMessage =
      typeof (sessionStatusForCurrent as { message?: string }).message === "string"
  ...
  const isSessionHydrating = Boolean(currentSessionId) && !hasRenderableSessionSnapshot
  ...
  if (isSessionHydrating && sessionMessages.length === 0 && !sessionIsWorking) {
    return (
      <div className="relative flex flex-col h-full bg-background">
        ...
        >
          <div
            className="absolute inset-0 overflow-y-auto overflow-x-hidden bg-background pt-6"
            style={CHAT_SCROLL_STYLE}
          >
  ...
  return (
    <div className="relative flex flex-col h-full bg-background">
      ...
      <div className={cn("relative z-10", isDesktopExpandedInput ? "flex-1 min-h-0 bg-background" : "bg-background")}>
        ...
        {currentSessionId && !isDesktopExpandedInput && <ActivityBreadcrumb sessionId={currentSessionId} />}
        {promptReadOnly ? <ReadOnlyPromptBanner /> : <ChatInput scrollToBottom={resumeToLatestInstant} />}
      </div>
