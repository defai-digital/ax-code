...
export type ChatHashTarget = { kind: "turn"; id: string } | { kind: "message"; id: string }
...
type TurnOffsetTarget = { kind: "noop" } | { kind: "resume" } | { kind: "turn"; turnId: string }
...
export const useChatTurnNavigation = ({
  ...
  const scrollToTurnId = React.useCallback(
    async (turnId: string, options?: { behavior?: ScrollBehavior; updateHash?: boolean }): Promise<boolean> => {
      if (!turnId) {
        return false
      }
  ...
  const scrollToMessageId = React.useCallback(
    async (messageId: string, options?: { behavior?: ScrollBehavior; updateHash?: boolean }): Promise<boolean> => {
      if (!messageId) {
        return false
      }
  ...
  const scrollByTurnOffset = React.useCallback(
    async (offset: number, options?: { resumePastEnd?: boolean }): Promise<boolean> => {
