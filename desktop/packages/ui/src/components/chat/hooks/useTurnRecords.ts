...
export const useTurnRecords = (messages: ChatMessageEntry[], options: UseTurnRecordsOptions): TurnRecordsResult => {
  ...
  const staticTurns = React.useMemo(() => {
    const nextStatic = projection.turns.length <= 1 ? [] : projection.turns.slice(0, -1)
  ...
  const streamingTurn = React.useMemo(() => {
    const nextStreamingTurn = projection.turns.length === 0 ? undefined : projection.turns[projection.turns.length - 1]
