...
export const TurnChangedFilesDropdown: React.FC<TurnChangedFilesDropdownProps> = React.memo(({ activityParts }) => {
  ...
  const syncPortalContainer = () => {
    const container = triggerButtonRef.current?.closest(
    ...
    <Popover.Root open={isExpanded} onOpenChange={setIsExpanded}>
          ...
          <Popover.Trigger
            render={
              <button
                ...
                type="button"
                className="flex items-center gap-1 typography-ui-label text-muted-foreground hover:text-muted-foreground tabular-nums"
      ...
      <Popover.Portal container={portalContainer || undefined}>
        <Popover.Positioner side="top" align="start" sideOffset={4} collisionPadding={8}>
          ...
          >
            <ChangedFilesList files={changedFiles} currentDirectory={currentDirectory} onOpenFile={handleOpenFile} />
          </Popover.Popup>
        </Popover.Positioner>
      </Popover.Portal>
    </Popover.Root>
