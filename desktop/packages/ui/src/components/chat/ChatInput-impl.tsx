...
const withInlineInsertionBoundaries = (content: string, before: string, after: string): string => {
  ...
  const needsLeadingSpace = before.length > 0 && !/\s$/.test(before) && !/^\s/.test(content) && !/[([{]$/.test(before)
  const needsTrailingSpace =
    after.length > 0 && !/\s$/.test(content) && !/^\s/.test(after) && !/^[\])}.,;:!?]/.test(after)
...
const RevertedMessageDock: React.FC<RevertedMessageDockProps> = React.memo(({ sessionId, directory }) => {
  ...
  const sessionMessages = useDirectorySync(
    React.useCallback(
      (state) => (sessionId ? (state.message[sessionId] ?? EMPTY_MESSAGES) : EMPTY_MESSAGES),
      [sessionId],
    ),
    directory,
  )
          ...
          <div className="px-3 pb-3 flex flex-col gap-1.5 max-h-[10.5rem] overflow-y-auto">
            {items.map((item) => (
              <div key={item.id} className="flex min-w-0 items-center gap-2 py-1">
                <span className="min-w-0 flex-1 truncate typography-ui-label text-foreground">{item.text}</span>
                <Button
                  type="button"
                  variant="secondary"
                  size="xs"
...
const ComposerAttachmentControls = React.memo(
  function ComposerAttachmentControls(props: ComposerAttachmentControlsProps) {
    const { t } = useI18n()
    const {
...
const PermissionAutoAcceptButton = React.memo(function PermissionAutoAcceptButton(
  props: PermissionAutoAcceptButtonProps,
) {
  const { t } = useI18n()
  const {
    footerIconButtonClass,
    ...
    <Tooltip>
      ...
      <TooltipContent side="top" sideOffset={8}>
        <div className="flex flex-col gap-0.5 text-center">
          ...
          <span className="font-mono opacity-60">{isMacOS() ? "⌘⇧E" : "Ctrl+Shift+E"}</span>
        </div>
      </TooltipContent>
    </Tooltip>
  )
...
const ComposerActionButtons = React.memo(
  function ComposerActionButtons(props: ComposerActionButtonsProps) {
    const {
      isMobile,
      footerIconButtonClass,
      sendIconSizeClass,
    ...
    const sendButton = (
      <button
        ...
        className={cn(
          footerIconButtonClass,
          canSend && (currentSessionId || newSessionDraftOpen) ? "text-primary hover:text-primary" : "opacity-30",
        )}
...
const ChatInputComponent: React.FC<ChatInputProps> = ({ onOpenSettings, scrollToBottom }) => {
  ...
  const cycleAgentShortcut = React.useMemo(
    () =>
      getEffectiveShortcutCombo(
        "cycle_agent",
        cycleAgentShortcutOverride ? { cycle_agent: cycleAgentShortcutOverride } : undefined,
      ),
  ...
  const knownAgentNames = React.useMemo(() => new Set(agents.map((agent) => agent.name.toLowerCase())), [agents])
  ...
  const [fencedCodeHighlighter, setFencedCodeHighlighter] = React.useState<FencedCodeHighlighter>(
    () => noFencedCodeHighlight,
  )
  ...
  React.useEffect(() => {
    ...
    let cancelled = false
    void importWithChunkRecovery(() => import("./composerCodeHighlight"))
      .then((module) => {
        if (!cancelled) {
  ...
  const sanitizeAttachmentsForSend = React.useCallback(
    (files: AttachedFile[] | undefined): AttachedFile[] =>
      (files ?? []).map((file) => ({
        ...file,
        dataUrl: file.source === "server" && file.serverPath ? toServerFileUrl(file.serverPath) : file.dataUrl,
      })),
    [],
  )
  ...
  const [previewConsoleCount, previewAnnotationCount, reviewCount] = draftSourceKey
    .split(":")
  ...
  const handleSubmit = async (options?: SubmitOptions) => {
    ...
    if (!providerIdToSend || !modelIdToSend) {
      ...
      toast.error(t("chat.chatInput.toast.noModelSelected"), {
        action: {
          label: t("chat.chatInput.toast.noModelSelectedAction"),
    ...
    for (let i = 0; i < queuedMessagesToSend.length; i++) {
      ...
      if (i === 0) {
        ...
        primaryAttachments = [...sanitizeAttachmentsForSend(queuedMsg.attachments), ...mentionAttachments]
      } else {
    ...
    const allAttachments = [...primaryAttachments, ...additionalParts.flatMap((p) => p.attachments ?? [])]
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    const cycleAgentBackwardShortcut =
      cycleAgentShortcut && !cycleAgentShortcut.includes("shift") ? normalizeCombo(`shift+${cycleAgentShortcut}`) : ""
    ...
    const isAnyAutocompleteOpen =
      showCommandAutocomplete || showSkillAutocomplete || showSnippetAutocomplete || showFileMention
    const cursorAtStart = textareaRef.current?.selectionStart === 0 && textareaRef.current?.selectionEnd === 0
    const cursorAtEnd =
      textareaRef.current?.selectionStart === message.length && textareaRef.current?.selectionEnd === message.length
  ...
  const updateAutocompleteOverlayPosition = React.useCallback(() => {
    ...
    const clampedLeft = Math.max(popupMargin, Math.min(caretX - 24, containerRect.width - desiredWidth - popupMargin))
  ...
  const adjustTextareaHeight = React.useCallback(
    (options?: { allowShrink?: boolean }) => {
      ...
      const paddingTotal =
        Number.isNaN(paddingTop) || Number.isNaN(paddingBottom) ? fallbackPadding : paddingTop + paddingBottom
  ...
  const handleSkillSelect = (skillName: string) => {
    ...
    if (lastSlashSymbol !== -1) {
      const newMessage = message.substring(0, lastSlashSymbol) + `/${skillName} ` + message.substring(cursorPosition)
      setMessage(newMessage)
          ...
          if (type === "drop") {
            ...
            for (const path of paths) {
              try {
                ...
                toast.error(
                  t("chat.chatInput.toast.attachNamedFailed", {
  ...
  const selectedDraftProject = React.useMemo(() => {
    ...
    const active = activeProjectId ? (projects.find((project) => project.id === activeProjectId) ?? null) : null
    if (active) {
      return active
    }
  ...
  const draftBranchItems = React.useMemo(() => {
    ...
    return [...baseItems, { value: selectedDraftDirectory, label: formatDirectoryName(selectedDraftDirectory) }]
  ...
  const renderProjectLabelWithIcon = React.useCallback(
    (project: {
      id: string
      path: string
      label?: string
  ...
  }, [abortPromptSessionId, acknowledgeSessionAbort, currentSessionId, showAbortStatus, startAbortIndicator])
    ...
    <>
      ...
      >
        ...
        >
          ...
          <QueuedMessageChips onEditMessage={handleQueuedMessageEdit} onSendMessage={handleQueuedMessageSend} />
              ...
              <div className="flex w-full items-center gap-1.5 typography-ui-label h-5 px-1">
                ...
                >
                  ...
                  <span className="text-muted-foreground flex-shrink-0">
                    ...
                    {linkedIssue.author && (
                      <span className="ml-1">
                        {t("chat.chatInput.linked.byAuthor", { author: linkedIssue.author.login })}
                      </span>
                    )}
                  </span>
              ...
              <div className="flex w-full items-center gap-1.5 typography-ui-label h-5 px-1">
                ...
                >
                  ...
                  <span className="text-muted-foreground flex-shrink-0">
                    ...
                    {linkedPr.author && (
                      <span className="ml-1">
                        {t("chat.chatInput.linked.byAuthor", { author: linkedPr.author.login })}
                      </span>
                    )}
                  </span>
                  <span className="text-foreground truncate">{linkedPr.title}</span>
            ...
            <div className="mb-1.5 flex min-w-0 items-center gap-1.5 px-0.5">
              ...
              {shouldShowDraftBranchSelector ? (
                ...
                >
                  ...
                  >
                    <SelectValue>{selectedDraftBranchLabel ?? t("chat.chatInput.branch")}</SelectValue>
                  </SelectTrigger>
          ...
          <div
            className={cn(
              ...
              "focus-within:ring-1",
              inputMode === "shell" ? "focus-within:ring-[var(--status-info)]" : "focus-within:ring-primary/50",
                ...
                {highlightedComposerContent && (
                  <div
                    ...
                    className={cn(
                      ...
                      isDesktopExpanded ? "h-full min-h-0 py-4" : isMobile ? "py-2.5" : "pt-4 pb-2",
