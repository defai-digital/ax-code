...
const useStableMarkdownBlocks = (text: string, live: boolean, baseKey: string): MarkdownStreamBlock[] => {
  ...
  return React.useMemo(() => {
    ...
    const unchanged =
      stabilized.length === previousBlocks.length && stabilized.every((block, index) => block === previousBlocks[index])
...
const buildMarkdownComponents = ({
  ...
  h1({ children, ...props }) {
    return (
      <h1
        {...props}
        className={cn(
          "typography-markdown-h1 mt-4 mb-2 text-[var(--markdown-heading1,var(--primary))] font-semibold",
          props.className,
        )}
      >
        {children}
  ...
  h2({ children, ...props }) {
    return (
      <h2
        {...props}
        className={cn(
          "typography-markdown-h2 mt-3.5 mb-1.5 text-[var(--markdown-heading2,var(--primary))] font-semibold",
          props.className,
        )}
      >
        {children}
  ...
  h3({ children, ...props }) {
    return (
      <h3
        {...props}
        className={cn(
          "typography-markdown-h3 mt-3 mb-1 text-[var(--markdown-heading3,var(--primary))] font-semibold",
          props.className,
        )}
      >
        {children}
  ...
  h4({ children, ...props }) {
    return (
      <h4
        {...props}
        className={cn(
          "typography-markdown-h4 mt-2.5 mb-1 text-[var(--markdown-heading4,var(--foreground))] font-semibold",
          props.className,
        )}
      >
        {children}
  ...
  p({ children, ...props }) {
    return (
      <p {...props} className={cn("typography-markdown-body my-2 text-foreground/90", props.className)}>
        {children}
      </p>
    )
  },
  thead({ children, ...props }) {
    return (
      <thead {...props} className={cn("[&_tr]:border-b [&_tr]:border-border/80", props.className)}>
        {children}
      </thead>
    )
  },
  ...
  tr({ children, ...props }) {
    return (
      <tr {...props} className={cn("border-b border-border/60", props.className)}>
        {children}
      </tr>
    )
  },
  th({ children, ...props }) {
    return (
      <th
        {...props}
        className={cn(
          "border-r border-border/60 px-4 py-2.5 text-left align-middle font-semibold text-foreground last:border-r-0",
          props.className,
        )}
      >
        {children}
  ...
  td({ children, ...props }) {
    return (
      <td
        {...props}
        className={cn(
          "border-r border-border/60 px-4 py-2.5 align-middle text-foreground/90 last:border-r-0",
          props.className,
        )}
      >
        {children}
  ...
  ul({ children, ...props }) {
    return (
      <ul {...props} className={cn("typography-markdown-body my-2", props.className)}>
        {children}
      </ul>
    )
  },
  ol({ children, ...props }) {
    return (
      <ol {...props} className={cn("typography-markdown-body my-2", props.className)}>
        {children}
      </ol>
    )
  },
  li({ children, ...props }) {
    return (
      <li {...props} className={cn("typography-markdown-body my-0.5 text-foreground/90", props.className)}>
        {children}
      </li>
    )
  },
  blockquote({ children, ...props }) {
    return (
      <blockquote
        {...props}
        className={cn(
          "my-3 border-l-2 border-[var(--markdown-blockquote-border,var(--border))] pl-4 typography-markdown-body text-[var(--markdown-blockquote,var(--muted-foreground))]",
          props.className,
        )}
      >
        {children}
...
const KNOWN_FILE_BASENAMES = new Set(["dockerfile", "makefile", "readme", "license", ".env", ".gitignore", ".npmrc"])
...
const stripTrailingReference = (value: string): string => {
  ...
  const basenameSuffixMatch =
    KNOWN_BASENAME_PATTERN.length > 0
...
const parseFileReference = (value: string): ParsedFileReference | null => {
  ...
  const withoutSemicolonSuffix = semicolonIndex >= 0 ? trimPathCandidate(trimmed.slice(0, semicolonIndex)) : trimmed
...
const useFileReferenceInteractions = ({
  ...
  React.useEffect(() => {
    ...
    const openFileReference = (sourceElement: HTMLElement) => {
      const raw =
        sourceElement.getAttribute("data-openchamber-file-ref") || extractPathCandidateFromElement(sourceElement)
      ...
      if (preferRuntimeEditor && editor) {
        void editor.openFile(
          ...
          Number.isFinite(resolved.line ?? Number.NaN) ? Math.max(1, Math.trunc(resolved.line as number)) : undefined,
      ...
      if (Number.isFinite(resolved.line ?? Number.NaN)) {
        uiStore.openContextFileAtLine(
          ...
          Number.isFinite(resolved.column ?? Number.NaN) ? Math.max(1, Math.trunc(resolved.column as number)) : 1,
        )
      } else {
...
const MarkdownRendererImpl: React.FC<MarkdownRendererProps> = ({
  ...
  const { t } = useI18n()
  const handlePreviewLoopback = React.useCallback(
    (url: string) => {
