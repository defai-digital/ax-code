...
const readTaskSessionId = (toolPart: Part): string | null => {
  ...
  const fromMetadata =
    (typeof metadata?.sessionID === "string" && metadata.sessionID.trim().length > 0
      ...
      : null) ??
    (typeof metadata?.sessionId === "string" && metadata.sessionId.trim().length > 0 ? metadata.sessionId.trim() : null)
...
const isSyntheticSubtaskBridgeAssistant = (
  message: ChatMessageEntry,
...
const TurnBlock = React.memo(
  ({
    ...
    const streamingAssistantMessageId = React.useMemo(() => {
      if (
        activeStreamingMessageId &&
        turn.assistantMessages.some((assistant) => assistant.info.id === activeStreamingMessageId)
      ) {
    ...
    const turnGroupingContextBase = React.useMemo(() => {
      ...
      const userMessageVariant = typeof rawVariant === "string" && rawVariant.trim().length > 0 ? rawVariant : undefined
      return {
        turnId: turn.turnId,
    ...
    const renderMessage = React.useCallback(
      (message: ChatMessageEntry) => {
        ...
        const hasAnchoredActivitySegment = visibleActivitySegments.some(
          (segment) => segment.anchorMessageId === message.info.id,
        )
        const shouldAttachFullTurnContext =
          chatRenderMode === "sorted" ? isAssistantMessage : isActivityOwner || isFirstAssistant || isLastAssistant
    ...
    return <TurnItem turn={renderableTurn} stickyUserHeader={stickyUserHeader} renderMessage={renderMessage} />
  },
)
...
const StaticHistoryList = React.memo(
  ({
    entries,
    shouldVirtualize,
    virtualRows,
    ...
    return (
      <div ref={contentRef} className="relative w-full">
        ...
        {virtualRows.map((virtualRow) => {
          ...
          return (
            <div key={virtualRow.key} ref={measureElement} data-index={virtualRow.index} data-turn-entry={entry.key}>
              {renderEntry(entry)}
            </div>
          )
        })}
...
const MessageList = React.forwardRef<MessageListHandle, MessageListProps>(
  (
    ...
    const baseDisplayMessages = React.useMemo(
      () =>
        streamPerfMeasure("ui.message_list.base_display_ms", () => {
          ...
          for (let index = 0; index < dedupedMessages.length; index += 1) {
            ...
            if (
              hasCompactionPart(current) ||
              current.parts.some((part) => part.type === "text" && getPartText(part).trim() === "/compact")
            ) {
    ...
    const displayMessages = React.useMemo(
      () =>
        streamPerfMeasure("ui.message_list.retry_overlay_ms", () => {
    ...
    const staticEntryUngroupedIds = hasUngroupedStaticEntries
      ? projection.ungroupedMessageIds
      : EMPTY_UNGROUPED_MESSAGE_IDS
    const staticRenderEntries = React.useMemo<RenderEntry[]>(
      () =>
        streamPerfMeasure("ui.message_list.render_entries_ms", () => {
    ...
    const currentUserOrder = React.useMemo(() => {
      return messages.filter((message) => resolveMessageRole(message) === "user").map((message) => message.info.id)
    }, [messages])
    ...
    const findMessageElement = React.useCallback(
    ...
    const scrollHistoryIndexIntoView = React.useCallback(
    ...
    const scrollMessageElementIntoView = React.useCallback(
