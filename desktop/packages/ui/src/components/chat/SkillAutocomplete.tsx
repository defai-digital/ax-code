...
export const SkillAutocomplete = React.forwardRef<SkillAutocompleteHandle, SkillAutocompleteProps>(
  ({ searchQuery, onSkillSelect, onClose, style }, ref) => {
      ...
      () => ({
        handleKeyDown: (key: string) => {
          ...
          if (key === "Enter" || key === "Tab") {
            const safeIndex =
              ((selectedIndexRef.current % filteredSkills.length) + filteredSkills.length) % filteredSkills.length
      ...
      return (
        ...
        >
          <div className="min-w-0 flex-1">
            ...
            {skill.description && (
              <div className="typography-meta text-muted-foreground mt-0.5 truncate">{skill.description}</div>
            )}
          </div>
        </div>
      )
      ...
      >
        <ScrollableOverlay outerClassName="flex-1 min-h-0" className="px-0 pb-2">
          {filteredSkills.length ? (
            ...
            <div className="px-3 py-2 typography-ui-label text-muted-foreground">No skills found</div>
          )}
        </ScrollableOverlay>
