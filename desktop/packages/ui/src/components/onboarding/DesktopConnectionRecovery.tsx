...
import { getDesktopRecoveryConfig, type RecoveryVariant } from "./desktopRecoveryConfig"
...
export function DesktopConnectionRecovery({
  ...
  const retryLabelKey = (config.retryLabelKey ?? "onboarding.desktopRecovery.actions.retryConnection") as Parameters<
    typeof t
  >[0]
    ...
    <div className="flex flex-col items-center justify-center h-full p-8">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="flex flex-col items-center space-y-3 text-center">
          ...
          >
            <div style={{ color: "var(--status-warning)" }}>{getRecoveryIcon(config.iconKey)}</div>
          </div>
        ...
        <div className="flex flex-col gap-2">
          {config.showRetry && onRetry && (
            <Button onClick={onRetry} disabled={isRetrying} className="w-full">
              ...
              {isRetrying ? t("onboarding.desktopRecovery.actions.retrying") : t(retryLabelKey)}
            </Button>
          )}

          <div className="flex gap-2">
            {config.showUseLocal && onUseLocal && (
              <Button variant="outline" onClick={onUseLocal} disabled={isRetrying} className="flex-1">
            ...
            {config.showUseRemote && onUseRemote && (
              <Button variant="outline" onClick={onUseRemote} disabled={isRetrying} className="flex-1">
