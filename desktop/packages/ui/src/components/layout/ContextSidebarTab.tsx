...
const estimatePartChars = (part: Part, role: "user" | "assistant" | "tool" | "other"): ContextBuckets => {
  ...
  if (type === "tool" || role === "tool") {
    const toolInputOutputLength =
      ...
      estimateTextLength(
        (partRecord.call as { input?: unknown; output?: unknown; error?: unknown } | undefined)?.input,
      ) +
      estimateTextLength(
        (partRecord.call as { input?: unknown; output?: unknown; error?: unknown } | undefined)?.output,
      ) +
...
const computeContextBreakdown = (sessionMessages: SessionMessage[], systemPrompt: string): ContextBuckets => {
...
export const ContextPanelContent: React.FC = () => {
  ...
  const viewModel = React.useMemo(() => {
    ...
    const usagePercent =
      contextLimit && contextLimit > 0 ? Math.min(999, (tokenBreakdown.total / contextLimit) * 100) : 0
    ...
    const systemPrompt =
      (
        [...sessionMessages]
          ...
          .find(
            (entry) =>
              deriveMessageRole(entry.info).isUser && typeof (entry.info as { system?: unknown }).system === "string",
    ...
    const lastMessageTs =
      sessionMessages.length > 0 ? sessionMessages[sessionMessages.length - 1]?.info?.time?.created : null
  ...
  const segments: Array<{ key: string; label: string; value: number; color: string }> = [
    {
      key: "user",
      label: t("contextSidebar.breakdown.user"),
      value: viewModel.breakdown.user,
      color: "var(--status-success)",
    },
    {
      key: "assistant",
      label: t("contextSidebar.breakdown.assistant"),
      value: viewModel.breakdown.assistant,
      color: "var(--primary-base)",
    },
    {
      key: "tool",
      label: t("contextSidebar.breakdown.toolCalls"),
      value: viewModel.breakdown.tool,
      color: "var(--status-warning)",
    },
    {
      key: "other",
      label: t("contextSidebar.breakdown.other"),
      value: viewModel.breakdown.other,
        ...
        <div>
          ...
          <div className="mt-2.5 space-y-1">
            {[...sessionMessages].reverse().map((message) => {
              ...
              const jsonValue = isExpanded ? JSON.stringify({ info: message.info, parts: message.parts }, null, 2) : ""

              return (
                <div key={message.info.id} className="overflow-hidden rounded-lg bg-[var(--surface-elevated)]/70">
                  <button
                    type="button"
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 whitespace-nowrap overflow-hidden">
                      ...
                      <span className="typography-micro text-muted-foreground shrink-0">
                        {formatMessageDateMeta(messageCreatedAt)}
                      </span>
                    </div>
                  </button>
                      ...
                      <div className="group relative max-h-[26rem] w-full overflow-auto bg-[var(--surface-background)]">
                        <div className="absolute top-1 right-2 z-10 opacity-0 transition-opacity group-hover:opacity-100">
                          <button
                            ...
                            aria-label={
                              isCopied ? t("contextSidebar.actions.copied") : t("contextSidebar.actions.copyJson")
                            }
