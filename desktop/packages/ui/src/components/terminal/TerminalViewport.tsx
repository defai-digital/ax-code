...
const TerminalViewport = React.forwardRef<TerminalController, TerminalViewportProps>(
  (
    ...
    const scheduleKeyProbe = React.useCallback(
      (target: HTMLElement) => {
        if (typeof window === "undefined") {
          return
        }
    ...
    const setupTouchScroll = React.useCallback(() => {
      ...
      if (supportsPointerEvents) {
        ...
        const handlePointerMove = (event: PointerEvent) => {
          ...
          stateWithPointerId.velocity =
            stateWithPointerId.velocity * (1 - velocityAlpha) + instantVelocity * velocityAlpha
    ...
    const handleHiddenBeforeInput = React.useCallback(
      (event: React.FormEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        lastInputEventAtRef.current = typeof performance !== "undefined" ? performance.now() : Date.now()
    ...
    const handleHiddenKeyDown = React.useCallback(
      ...
      [
        ...
        hasCopyableSelectionInViewport,
        readEditableValue,
        scheduleKeyProbe,
      ],
    )
    ...
    const handleHiddenPaste = React.useCallback(
      (event: React.ClipboardEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        event.preventDefault()
        const terminal = terminalRef.current
        const payload = terminal?.hasBracketedPaste?.() ? `\x1b[200~${text}\x1b[201~` : text
