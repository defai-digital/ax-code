...
const MiniChatHeader: React.FC<{ mode: MiniChatMode }> = ({ mode }) => {
  ...
  const worktreePath = useSessionUIStore((state) =>
    currentSessionId ? (state.worktreeMetadata.get(currentSessionId)?.path ?? "") : "",
  )
  const worktreeMetadataBranch = useSessionUIStore((state) =>
    currentSessionId ? (state.worktreeMetadata.get(currentSessionId)?.branch?.trim() ?? null) : null,
  )
  ...
  const macosHeaderSizeClass = hasMacTrafficLights ? (macosMajor >= 26 ? "h-12" : macosMajor <= 15 ? "h-14" : "") : ""

  const session = React.useMemo(
    () => (currentSessionId ? (sessions.find((entry) => entry.id === currentSessionId) ?? null) : null),
    [currentSessionId, sessions],
  )
  const sessionWorktreeMetadata =
    (
      session as {
        worktreeMetadata?: { path?: string | null; branch?: string | null; projectDirectory?: string | null }
      } | null
  ...
  const sessionDirectory = normalizeMiniChatPath((session as { directory?: string | null } | null)?.directory ?? null)
  const worktreeDirectory = normalizeMiniChatPath(
    worktreePath || sessionWorktreeMetadata?.path || worktreeAttachment?.cwd || worktreeAttachment?.worktreeRoot || "",
  )
  const currentDirectoryNormalized = normalizeMiniChatPath(currentDirectory)
  ...
  const catalogWorktreeBranch = useSessionUIStore((state) => {
    const candidateDirectory = normalizeMiniChatPath(worktreeDirectory || sessionDirectory || "")
    if (!candidateDirectory) return null
    for (const worktrees of state.availableWorktreesByProject.values()) {
      const match = worktrees.find((worktree) => normalizeMiniChatPath(worktree.path) === candidateDirectory)
  ...
  const pathMatchedProject = React.useMemo(() => {
    const projectDirectory = normalizeMiniChatPath(
  ...
  const projectLabel = React.useMemo(() => {
    ...
    if (!project) return directoryLabel || "AX Code"
  ...
  const rawBranchLabel =
    gitBranchForDirectory ||
    worktreeMetadataBranch ||
    sessionWorktreeMetadata?.branch?.trim() ||
    worktreeAttachment?.branch?.trim() ||
  ...
  const limit =
    modelForLimits && typeof modelForLimits.limit === "object" && modelForLimits.limit !== null
  ...
  const displayContextPercentage =
    stableContextUsage && stableContextUsage.contextLimit > 0
