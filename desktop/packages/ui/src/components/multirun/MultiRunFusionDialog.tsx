...
export function MultiRunFusionDialog({
  ...
  React.useEffect(() => {
    ...
    const nextSources = allSessions
      .map((candidate): FusionSource | null => {
        ...
        const directory =
          useSessionUIStore.getState().getDirectoryForSession(candidate.id) ?? resolveGlobalSessionDirectory(candidate)
  ...
  const selectedProviderModel = selectedProvider?.models.find((model) => model.id === modelID) as
  ...
  const selectedModelLabel =
    selectedModelSelection[0]?.displayName ||
    selectedModelSelection[0]?.modelID ||
    t("multirun.fusion.model.placeholder")
      ...
      <DialogContent className="max-w-xl overflow-visible">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="max-w-full">
            <ModelMultiSelect
              ...
              triggerIcon={
                providerID ? <ProviderLogo providerId={providerID} className="h-3.5 w-3.5 mr-1" /> : undefined
              }
            />
          </div>
          ...
          {variantKeys.length > 0 ? (
            <Select
              ...
              onValueChange={(value) => setVariant(value === "__default__" ? "" : value)}
            >
              <SelectTrigger
                size="lg"
                className="h-8 w-fit rounded-[9px] [corner-shape:squircle] supports-[corner-shape:squircle]:rounded-[50px] !border-border/80 !bg-[var(--surface-subtle)] hover:!bg-[var(--interactive-hover)]/70 typography-meta font-medium text-foreground px-3 py-1.5"
              >
                <Icon name="brain-ai-3" className="h-3.5 w-3.5 text-muted-foreground" />
                <SelectValue>
                  {(value) => (value === "__default__" ? t("multirun.modelMultiSelect.variant.default") : value)}
                </SelectValue>
              </SelectTrigger>
          ...
          <AgentSelector
            ...
            onChange={setAgent}
            portalToBody
            className="h-8 rounded-[9px] [corner-shape:squircle] supports-[corner-shape:squircle]:rounded-[50px] px-3 py-1.5"
          />
        </div>
          ...
          <div className="max-h-56 space-y-1 overflow-auto rounded-lg border border-[var(--interactive-border)] p-1">
            {sources.map((source) => (
              <div key={source.session.id} className="flex items-center gap-2 rounded-md px-2 py-1.5 typography-meta">
                <ProviderLogo
                  providerId={parseMultiRunSessionTitle(source.session.title)?.providerID ?? ""}
                  className="h-4 w-4"
                />
                <span className="min-w-0 flex-1 truncate">{source.session.title || source.session.id}</span>
                <button
                  type="button"
                  onClick={() => setSources((prev) => prev.filter((item) => item.session.id !== source.session.id))}
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={() => onOpenChange(false)}>
            {t("multirun.fusion.actions.cancel")}
          </Button>
          <Button onClick={handleStart} disabled={!canStart}>
            {isStarting ? t("multirun.fusion.actions.starting") : t("multirun.fusion.actions.start")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
