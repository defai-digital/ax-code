...
export const ModelChip: React.FC<{
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-1.5 px-2 rounded-md bg-interactive-selection/20 border border-border/30",
        CHIP_HEIGHT_CLASS,
      )}
    >
      <ProviderLogo providerId={model.providerID} className="h-3.5 w-3.5" />
      <span className="typography-meta font-medium truncate max-w-[140px]">{label}</span>
...
export const ModelMultiSelect: React.FC<ModelMultiSelectProps> = ({
  ...
  const getInstanceIndex = React.useCallback(
    (model: ModelSelectionWithId): number => {
      const sameModels = selectedModels.filter((m) => m.providerID === model.providerID && m.modelID === model.modelID)
        ...
        <div className={cn("relative", containerClassName)} ref={dropdownRef}>
          ...
          {isOpen ? (
            <div
              ...
              style={{
                background:
                  "linear-gradient(var(--surface-elevated),var(--surface-elevated)),linear-gradient(var(--surface-background),var(--surface-background))",
              }}
            >
              <ModelPickerList
                providers={providers}
          ...
          <div className="flex flex-col gap-2 w-full">
            {selectedModels.map((model, index) => {
              ...
              const providerModel = provider?.models?.find(
                (m: Record<string, unknown>) => (m as { id?: string }).id === model.modelID,
                    ...
                    >
                      ...
                      >
                        ...
                        <SelectValue placeholder={t("multirun.modelMultiSelect.variant.placeholder")}>
                          {(value) =>
                            value === DEFAULT_VARIANT_VALUE ? t("multirun.modelMultiSelect.variant.default") : value
                          }
                        </SelectValue>
                      </SelectTrigger>
