...
export const AgentSelector: React.FC<MultirunAgentSelectorProps> = ({
  ...
  const selectableAgents = React.useMemo(() => agents.filter((agent) => agent.mode !== "subagent"), [agents])
  ...
  React.useEffect(() => {
    ...
    const candidateDefault =
      typeof defaultAgentName === "string" && defaultAgentName.trim().length > 0 ? defaultAgentName.trim() : null
        ...
        {selectableAgents.length > 0 && (
          <SelectGroup>
            {selectableAgents.map((agent) => (
              <SelectItem key={agent.name} value={agent.name} className="w-auto whitespace-nowrap">
                {agent.name}
              </SelectItem>
            ))}
          </SelectGroup>
