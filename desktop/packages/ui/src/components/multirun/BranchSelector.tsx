  ...
  return (
    <div>
      <Select value={value} onValueChange={onChange} disabled={isDisabled}>
        <SelectTrigger id={id} size="lg" className={className ?? "w-fit typography-meta text-foreground"}>
          <SelectValue
            placeholder={
              isLoading
                ? t("multiRun.branchSelector.status.loadingBranches")
                : t("multiRun.branchSelector.placeholder.selectSourceBranch")
            }
          />
        </SelectTrigger>
          ...
          {isLoading ? (
            ...
            <>
              {localBranches.length > 0 && (
                <SelectGroup>
                  <SelectLabel className="font-semibold text-foreground">
                    {t("multiRun.branchSelector.groups.localBranches")}
                  </SelectLabel>
              ...
              {remoteBranches.length > 0 && (
                <SelectGroup>
                  <SelectLabel className="font-semibold text-foreground">
                    {t("multiRun.branchSelector.groups.remoteBranches")}
                  </SelectLabel>
