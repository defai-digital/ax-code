...
export const MultiRunLauncher: React.FC<MultiRunLauncherProps> = ({
  ...
  const totalRunCount = React.useMemo(() => runGroups.reduce((sum, g) => sum + g.models.length, 0), [runGroups])
          ...
          <div className="flex flex-col gap-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
              <div className="flex flex-col gap-1">
                <FieldLabel htmlFor="multirun-project" required>
                  {t("multirun.launcher.project.label")}
                </FieldLabel>
                {projects.length > 0 ? (
            ...
            <div className="flex flex-col gap-1.5">
              <FieldLabel htmlFor="prompt" required>
...
const RunGroupCard: React.FC<RunGroupCardProps> = ({ group, groupIndex, canRemove, onUpdate, onRemove }) => {
  const { t } = useI18n()
  ...
  const handleUpdateModel = React.useCallback(
    (index: number, model: ModelSelectionWithId) => {
