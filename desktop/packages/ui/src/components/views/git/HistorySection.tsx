...
export const HistorySection: React.FC<HistorySectionProps> = ({
  ...
  const maxLanes = React.useMemo(() => Math.max(1, ...laned.map((l) => l.lane + 1)), [laned])

  const lanedByHash = React.useMemo(() => new Map(laned.map((l) => [l.commit.hash, l])), [laned])
  ...
  const hasDivider =
    branchDivider !== null && branchDivider.insertBeforeIndex > 0 && branchDivider.insertBeforeIndex < log.all.length
    ...
    <ScrollableOverlay outerClassName={`min-h-0 ${contentMaxHeightClassName}`} className="h-full w-full">
      {log.all.length === 0 ? (
        <div className="flex h-full items-center justify-center p-4">
          <p className="typography-ui-label text-muted-foreground">{t("gitView.history.noCommits")}</p>
        </div>
