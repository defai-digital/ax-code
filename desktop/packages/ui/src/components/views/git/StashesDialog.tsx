...
export const StashesDialog: React.FC<StashesDialogProps> = ({
  ...
  React.useEffect(() => {
    ...
    void countGitStashFiles(directory, refs)
      .then((result) => {
  ...
  const filtered = React.useMemo(() => {
    ...
    return stashes.filter((stash) =>
      `${stash.ref} ${stash.message} ${stash.relativeTime}`.toLowerCase().includes(normalized),
    )
  }, [query, stashes])

  const refreshAfterChange = React.useCallback(
    async (change?: { affectsIndex?: boolean }) => {
      await load()
  ...
  const runStashAction = async (stash: GitStashEntry, kind: "apply" | "pop" | "drop") => {
    ...
    try {
      ...
      const successKey =
        kind === "apply"
          ? "gitView.stashes.toast.applySuccess"
          : kind === "pop"
            ? "gitView.stashes.toast.popSuccess"
            : "gitView.stashes.toast.dropSuccess"
      toast.success(t(successKey))
      await refreshAfterChange({ affectsIndex: kind !== "drop" })
    } catch (error) {
      const failedKey =
        kind === "apply"
          ? "gitView.stashes.toast.applyFailed"
          : kind === "pop"
            ? "gitView.stashes.toast.popFailed"
            : "gitView.stashes.toast.dropFailed"
      ...
      <DialogContent className="max-w-2xl max-h-[70vh] flex flex-col">
        ...
        <div className="relative mt-2">
          ...
          <Input
            ...
            placeholder={t("gitView.stashes.searchPlaceholder")}
            className="pl-9"
          />
        </div>
                  ...
                  <p className="typography-meta truncate text-muted-foreground">
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span>
                          {index === 0 && !query.trim()
                            ? t("gitView.stashes.latestLabel")
                            : t("gitView.stashes.itemNumber", { number: index + 1 })}
                        </span>
                      </TooltipTrigger>
                    ...
                    {" · "}
                    {stash.relativeTime} ·{" "}
                    {typeof fileCounts[stash.ref] === "number"
                      ? t("gitView.stashes.fileCount", { count: fileCounts[stash.ref] })
                      : t("gitView.stashes.fileCountLoading")}
                  </p>
                </div>
                <div className="mr-2 flex shrink-0 items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100">
                  <StashIconButton
                    label={t("gitView.stashes.actions.apply")}
                    loading={operation === `apply:${stash.ref}`}
                    onClick={() => runStashAction(stash, "apply")}
                    disabled={isOperating}
                  >
                    <Icon name="inbox-unarchive" className="size-4" />
                  </StashIconButton>
                  <StashIconButton
                    label={t("gitView.stashes.actions.pop")}
                    loading={operation === `pop:${stash.ref}`}
                    onClick={() => runStashAction(stash, "pop")}
                    disabled={isOperating}
                  >
                    <Icon name="inbox-unarchive-fill" className="size-4" />
                  </StashIconButton>
                  <StashIconButton
                    label={t("gitView.stashes.actions.drop")}
                    loading={operation === `drop:${stash.ref}`}
                    onClick={() => runStashAction(stash, "drop")}
                    disabled={isOperating}
                    destructive
                  >
...
const StashIconButton: React.FC<{
  label: string
  loading: boolean
  disabled: boolean
  destructive?: boolean
...
}> = ({ label, loading, disabled, destructive, onClick, children }) => (
  <Tooltip>
    <TooltipTrigger asChild>
      <button
        type="button"
        ...
        className={cn(
          "flex h-6 w-6 items-center justify-center transition-colors disabled:opacity-50",
          destructive
