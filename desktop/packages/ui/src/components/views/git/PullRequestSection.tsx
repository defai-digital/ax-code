...
const getPrVisualState = (
  status: GitHubPullRequestStatus | null,
): "draft" | "open" | "blocked" | "merged" | "closed" | null => {
  const pr = status?.pr
  if (!pr) {
    return null
  }
...
const pickInitialPrRemote = (
  ...
  if (trackingRemoteName) {
    const maybeUpstream = trackingRemoteName === "origin" ? remotes.find((remote) => remote.name === "upstream") : null
    if (maybeUpstream) {
...
const rankRemotesForAutoSelect = (remotes: GitRemote[], trackingBranch?: string): GitRemote[] => {
  ...
  remotes.filter((remote) => !isEphemeralPrRemote(remote.name)).forEach((remote) => pushUnique(remote))
...
export const PullRequestSection: React.FC<{
  ...
  const initialSnapshot = React.useMemo(() => pullRequestDraftSnapshots.get(snapshotKey) ?? null, [snapshotKey])
  ...
  const [targetBaseBranch, setTargetBaseBranch] = React.useState(() => {
    const fromSnapshot =
      typeof initialSnapshot?.targetBaseBranch === "string" ? normalizeBranchRef(initialSnapshot.targetBaseBranch) : ""
    if (fromSnapshot) {
      return fromSnapshot
    }
  ...
  React.useEffect(() => {
    ...
    if (!availableBaseBranches.includes(targetBaseBranch)) {
      const fallback = availableBaseBranches.includes(normalizedBase) ? normalizedBase : availableBaseBranches[0]
      if (fallback) {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false
    void github
      .prContext(directory, pr.number, { includeDiff: false, includeCheckDetails: false })
      .then((ctx) => {
        if (cancelled) {
          return
        }
  ...
  React.useEffect(() => {
    ...
    const numberChanged = lastSyncedPrNumberRef.current !== null && lastSyncedPrNumberRef.current !== pr.number
  ...
  const linkifyMentionsMarkdown = React.useCallback(
    (content: string) => {
  ...
  const resolveChatDispatchTarget = React.useCallback((): ChatDispatchTarget | null => {
    if (!currentSessionId) {
      toast.error(t("gitView.pr.toast.noActiveSession"), {
        description: t("gitView.pr.toast.noActiveSessionDescription"),
      })
      return null
    }
  ...
  const dispatchSyntheticPrompt = React.useCallback(
    (target: ChatDispatchTarget, visibleText: string, instructionsText: string, payloadText: string) => {
  ...
  const renderCheckRunSummary = React.useCallback(
    (run: GitHubCheckRun) => {
      ...
      return (
        <div className="space-y-2">
          ...
          {run.output?.title ? <div className="typography-micro text-foreground">{run.output.title}</div> : null}
          {run.output?.summary ? (
            <div className="typography-micro text-muted-foreground whitespace-pre-wrap">{run.output.summary}</div>
          ) : null}
          {run.output?.text ? (
  ...
  const sendSingleCommentToChat = React.useCallback(
    async (comment: TimelineCommentItem) => {
  ...
  React.useEffect(() => {
    ...
    const candidates = rankRemotesForAutoSelect(remotes, trackingBranch).filter(
      (remote) => remote.name !== selectedRemote?.name,
    )
  ...
  React.useEffect(() => {
    ...
    setTargetBaseBranch(
      snapshot?.targetBaseBranch ? normalizeBranchRef(snapshot.targetBaseBranch) : normalizeBranchRef(baseBranch),
    )
  ...
  const generateDescription = React.useCallback(async () => {
    ...
    try {
      ...
      const baseRef =
        useDetectedUpstream && detectedUpstream?.defaultBranchSha ? detectedUpstream.defaultBranchSha : targetBaseBranch
  ...
  const mergePr = React.useCallback(
    async (pr: GitHubPullRequest) => {
      ...
      try {
        ...
        if (result.merged) {
          ...
          toast.message(t("gitView.pr.toast.prNotMerged"), {
            description: result.message || t("gitView.pr.notMergeable"),
          })
        }
        await refresh({ force: true })
        scheduleActionRefresh()
  ...
  const updatePr = React.useCallback(
    async (pr: GitHubPullRequest) => {
      if (!github?.prUpdate) {
        toast.error(t("gitView.pr.toast.githubApiUnavailable"))
        return
  ...
  const prStatusText = pr
    ? [
        ...
        pr.state === "open" &&
        typeof pr.mergeableState === "string" &&
        pr.mergeableState &&
    ...
    <section className={containerClassName}>
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2">
            {pr ? (
              <Tooltip>
                ...
                <TooltipContent>
                  <p>{t("gitView.pr.actions.openOnGitHub")}</p>
                </TooltipContent>
              </Tooltip>
            ) : (
              <Icon
                name={prStateIconName}
            ...
            {pr ? <span className="typography-meta text-muted-foreground truncate">#{pr.number}</span> : null}
          </div>
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <p>{t("gitView.pr.actions.refresh")}</p>
              </TooltipContent>
            </Tooltip>
          </div>
        </div>
      ...
      <div className={bodyClassName}>
        {shouldShowConnectionNotice ? (
          <div className="space-y-2">
            <div className="typography-meta text-muted-foreground">{t("gitView.pr.githubNotConnected")}</div>
            ...
            <div className="flex flex-col gap-3">
              <div className="min-w-0">
                {isEditingPr ? (
                  ...
                  <>
                    <div className="typography-markdown text-xl font-semibold text-foreground break-words leading-snug">
                      {pr.title}
                    </div>
                    {pr.body?.trim() ? (
                ...
                {canMerge && pr.draft ? (
                  <div className="typography-micro text-muted-foreground">{t("gitView.pr.draftMustBeReady")}</div>
                ) : null}
                {!canMerge ? (
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    isEditingPr ? (
                      <>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>{t("gitView.pr.actions.cancelEditing")}</p>
                          </TooltipContent>
                        </Tooltip>
                        <Tooltip>
                          <TooltipTrigger asChild>
                          ...
                          <TooltipContent>
                            <p>{t("gitView.pr.actions.savePr")}</p>
                          </TooltipContent>
                        </Tooltip>
                      </>
                    ) : (
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{t("gitView.pr.actions.editPr")}</p>
                        </TooltipContent>
                      </Tooltip>
                    )
                  ) : null}
                  ...
                  {checks ? (
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p>{t("gitView.pr.actions.openChecks")}</p>
                      </TooltipContent>
                    </Tooltip>
                  ) : null}
                  ...
                  {checks?.failure ? (
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p>{t("gitView.pr.actions.resolveFailedChecks")}</p>
                      </TooltipContent>
                    </Tooltip>
                  ) : null}
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>{t("gitView.pr.actions.openComments")}</p>
                    </TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p>{t("gitView.pr.actions.shareComments")}</p>
                    </TooltipContent>
                  </Tooltip>
                      ...
                      <TooltipTrigger asChild>
                        ...
                        >
                          {isMarkingReady ? (
                            ...
                            <Icon name="checkbox-circle" className="size-4" />
                          )}
                        </Button>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p>{t("gitView.pr.actions.markReady")}</p>
                      </TooltipContent>
                    </Tooltip>
                  ) : null}
                </div>
                  ...
                  {canMerge ? (
                    <>
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{t("gitView.pr.actions.mergePr")}</p>
                        </TooltipContent>
                      </Tooltip>
                    </>
                  ) : null}
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="typography-micro text-muted-foreground truncate">
                  {branch} <span className="opacity-60">(local)</span> → {targetBaseBranch}{" "}
                  <span className="opacity-60">
                    ({useDetectedUpstream && detectedUpstream ? "upstream" : "remote"})
                  </span>
                </div>
              </div>
              {safeRepoUrl ? (
              ...
              <Collapsible open={isContextOpen} onOpenChange={setIsContextOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between rounded-lg border border-[var(--interactive-border)] bg-[var(--surface-elevated)] px-3 py-2 hover:bg-[var(--interactive-hover)]">
                  ...
                  <span className="typography-micro text-[var(--primary-base)]">
                    {isContextOpen
                      ? t("gitView.pr.actions.hide")
                      : additionalContext.trim()
                        ? t("gitView.pr.actions.edit")
                        : t("gitView.pr.actions.add")}
                  </span>
                </CollapsibleTrigger>
                <CollapsibleContent>
            ...
            <MobileOverlayPanel
              ...
              footer={
                <Button size="sm" onClick={() => setIsContextSheetOpen(false)} className="w-full">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={generateDescription} disabled={isGenerating || isCreating}>
                {isGenerating ? (
                  <Icon name="loader-4" className="size-4 animate-spin" />
                ) : (
              ...
              <Button
                ...
                disabled={
                  isCreating ||
                  !isConnected ||
                  !targetBaseBranch.trim() ||
                  (!useDetectedUpstream && targetBaseBranch.trim() === branch)
                }
              >
            ...
            <DialogTitle className="flex items-center gap-2">
              ...
              {pr ? (
                <span className="typography-meta text-muted-foreground">
                  {t("gitView.pr.numberLabel", { number: pr.number })}
                </span>
              ) : null}
            </DialogTitle>
          </DialogHeader>
                            ...
                            <div className="rounded-lg bg-surface-elevated px-3 pt-0 pb-3 space-y-2">
                              <div className="flex flex-col items-start gap-1 typography-micro text-muted-foreground sm:flex-row sm:flex-wrap sm:items-center sm:gap-x-1 sm:gap-y-1">
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <p>{t("gitView.pr.actions.sendCommentToAgent")}</p>
                                  </TooltipContent>
                                </Tooltip>
                              </div>
                              <div className="typography-micro text-muted-foreground">
