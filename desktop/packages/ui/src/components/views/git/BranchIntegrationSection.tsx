...
export const BranchIntegrationSection: React.FC<BranchIntegrationSectionProps> = ({
  ...
  const operationCompleted =
    operationLogs.length > 0 && operationLogs.every((log) => log.status === "done" || log.status === "error")
  ...
  const filteredLocal = React.useMemo(() => {
    ...
    const remoteBranchNames = new Set(
      remoteBranches.map((branch) => branch.slice(branch.indexOf("/") + 1)).filter(Boolean),
    )
  ...
  const renderOperating = () => (
    <div className="space-y-3">
      <div ref={logContainerRef} className="rounded-lg border border-border bg-muted/30 p-3 max-h-48 overflow-y-auto">
        <div className="space-y-2">
          {operationLogs.map((log, index) => (
            <div key={index} className="flex items-start gap-2">
              <div className="mt-0.5 shrink-0">
                {log.status === "running" && <Icon name="loader-4" className="size-3.5 animate-spin text-primary" />}
                {log.status === "done" && <Icon name="check" className="size-3.5 text-success" />}
                {log.status === "error" && <Icon name="close" className="size-3.5 text-destructive" />}
                {log.status === "pending" && (
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <p className="typography-micro text-muted-foreground">{t("gitView.branch.mergeDescription")}</p>
          </button>
          ...
          >
            ...
            <p className="typography-micro text-muted-foreground">{t("gitView.branch.rebaseDescription")}</p>
          </button>
        </div>
      </div>
      ...
      {selectedBranch ? (
        <div className="rounded-lg bg-muted/50 p-3">
          <p className="typography-meta text-muted-foreground">
            {operation === "merge" ? (
              <>
                {t("gitView.branch.summaryMergePrefix")}{" "}
                <span className="font-mono text-foreground">{selectedBranch}</span>{" "}
                {t("gitView.branch.summaryMergeInfix")}{" "}
                <span className="font-mono text-foreground">{targetBranchLabel}</span>
              </>
            ) : (
              <>
                {t("gitView.branch.summaryRebasePrefix")}{" "}
                <span className="font-mono text-foreground">{targetBranchLabel}</span>{" "}
                {t("gitView.branch.summaryRebaseInfix")}{" "}
                <span className="font-mono text-foreground">{selectedBranch}</span>
              </>
            )}
          </p>
      ...
      {mode === "dialog" ? (
        <DialogFooter className="gap-2 pt-1">
          ...
          <Button variant="default" size="sm" onClick={handleConfirm} disabled={!selectedBranch} className="gap-1.5">
            {operation === "merge" ? (
              <>
    ...
    <>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button variant="outline" size="sm" className="gap-1.5" onClick={handleOpenDialog} disabled={isDisabled}>
            {isOperating ? (
        ...
        <TooltipContent sideOffset={8}>{t("gitView.branch.mergeRebaseTooltip")}</TooltipContent>
      </Tooltip>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {isOperating ? (
                operationCompleted ? (
                  hasError ? (
                    t("gitView.branch.operationFailed")
                  ) : (
                    t("gitView.branch.operationCompleted")
                  )
                ) : operation === "merge" ? (
                  t("gitView.branch.mergingInProgress")
                ) : (
                  t("gitView.branch.rebasingInProgress")
                )
              ) : (
                <>
