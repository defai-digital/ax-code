...
import { clearIntegrateConflictState, readIntegrateConflictState, saveIntegrateConflictState } from "./conflictStorage"
...
export const IntegrateCommitsSection: React.FC<{
  ...
  refreshKey?: number
  onRefresh?: () => void
}> = ({ repoRoot, sourceBranch, worktreeMetadata, localBranches, defaultTargetBranch, refreshKey, onRefresh }) => {
  const { t } = useI18n()
    ...
    void (async () => {
      try {
        ...
        if (plan.commits.length > 0) {
          ...
          const result = await execCommand(`git show -s --format=%H%x09%h%x09%s ${quoted}`, repoRoot)
  ...
  const buildConflictContext = React.useCallback(
    async (payload: { state: IntegrateInProgress; details: IntegrateConflictDetails }) => {
  ...
  const handleResolveWithAi = React.useCallback(
    async (payload: { state: IntegrateInProgress; details: IntegrateConflictDetails }, useNewSession: boolean) => {
      ...
      if (!currentSessionId) {
        toast.error(t("gitView.integrate.noActiveSession"), {
          description: t("gitView.integrate.noActiveSessionDescription"),
        })
        return
      }
    ...
    try {
      ...
      if (result.kind === "success") {
        toast.success(t("gitView.integrate.commitsMovedToast"), {
          description:
            result.moved === 1
              ? t("gitView.integrate.commitsMovedDescriptionSingle", {
                  count: result.moved,
                  branch: ui.plan.targetBranch,
                })
              : t("gitView.integrate.commitsMovedDescriptionPlural", {
                  count: result.moved,
                  branch: ui.plan.targetBranch,
                }),
        })
      ...
      if (result.kind === "conflict") {
        toast.error(t("gitView.integrate.cherryPickConflictToast"), {
          description: t("gitView.integrate.cherryPickConflictDescription"),
        })
      ...
      <div className={bodyClassName}>
        ...
        {ui.kind === "conflict" && (
          <div className="rounded-md border border-border/60 bg-background/60 p-3 space-y-2">
            ...
            <div className="typography-micro text-muted-foreground">
              {t("gitView.integrate.currentCommit")}:{" "}
              <span className="font-mono">{ui.state.currentCommit.slice(0, 7)}</span>
            </div>
