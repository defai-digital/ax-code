...
export const AgentGroupDetail: React.FC<AgentGroupDetailProps> = ({ group, className }) => {
  const { t } = useI18n()
  ...
  const [worktreeDialog, setWorktreeDialog] = React.useState<null | {
    kind: "remove" | "keepOnly"
    path: string
    label: string
  }>(null)
  ...
  return (
    <div className={cn("flex h-full flex-col bg-background", className)}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 mt-1 typography-meta text-muted-foreground">
              ...
              <span className="flex items-center gap-1">
                ...
                {selectedSession?.worktreeMetadata?.label ||
                  selectedSession?.branch ||
                  t("agentManager.detail.header.noBranch")}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="min-w-[220px]">
                <DropdownMenuItem onSelect={handleRemoveSelectedWorktree} closeOnClick={false} variant="destructive">
                  {t("agentManager.detail.actions.removeThisWorktree")}
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={handleKeepOnlySelectedWorktree} closeOnClick={false}>
      ...
      <Dialog
        open={Boolean(worktreeDialog)}
        onOpenChange={(open) => {
          if (!open) setWorktreeDialog(null)
        }}
      >
