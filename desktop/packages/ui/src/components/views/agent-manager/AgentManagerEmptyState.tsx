...
import {
  toMultiRunFileAttachment,
  type CreateMultiRunParams,
  type MultiRunFileAttachment,
  type MultiRunLocalFileAttachment,
} from "@/types/multirun"
...
export const AgentManagerEmptyState: React.FC<AgentManagerEmptyStateProps> = ({
  ...
  const handleAutocompleteFileSelect = React.useCallback(
  ...
  const handleAutocompleteAgentSelect = React.useCallback(
    (agentName: string) => {
      const cursorPosition = textareaRef.current?.selectionStart ?? prompt.length
      const textBeforeCursor = prompt.substring(0, cursorPosition)
      const lastAtSymbol = textBeforeCursor.lastIndexOf("@")
  ...
  const handleAutocompleteCommandSelect = React.useCallback(
    (command: CommandInfo) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const files: MultiRunFileAttachment[] | undefined =
        attachedFiles.length > 0 ? attachedFiles.map(toMultiRunFileAttachment) : undefined
    ...
    <div className={cn("flex flex-col items-center justify-center h-full w-full p-4", className)}>
      <form onSubmit={handleSubmit} className="w-full max-w-2xl space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <BranchSelector directory={currentDirectory} value={baseBranch} onChange={setBaseBranch} />
        ...
        <div className="space-y-1.5">
          ...
          <p className="typography-micro text-muted-foreground">{t("agentManager.empty.agent.description")}</p>
        </div>
              ...
              <div className="flex items-center justify-between px-3 py-2 border-t border-border/40 bg-transparent">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isValid ? "text-primary hover:text-primary" : "opacity-30",
                  )}
