...
const OpenInAppListIcon = ({ label, iconDataUrl }: { label: string; iconDataUrl?: string }) => {
  ...
  if (iconDataUrl && !failed) {
    return <img src={iconDataUrl} alt="" className="size-4 rounded-sm" onError={() => setFailed(true)} />
  }
    ...
    <span ref={containerRef} className="relative block min-w-0 flex-1 overflow-hidden whitespace-nowrap">
      ...
      {overflowing ? (
        <span className="open-file-name-marquee-track">
          ...
          <span className="open-file-name-marquee-item" aria-hidden="true">
            {name}
          </span>
        </span>
      ) : (
...
const FileRow: React.FC<FilesViewFileRowProps> = ({
  ...
  return (
    <div className="group relative flex items-center" onContextMenu={!isMobile ? handleContextMenu : undefined}>
      <button
        type="button"
        ...
        >
          ...
          >
            ...
            <DropdownMenuContent
              ...
              onCloseAutoFocus={() => setContextMenuPath(null)}
            >
              {canRename && (
                <DropdownMenuItem
                  onClick={(e) => {
                    e.stopPropagation()
                    onOpenDialog("rename", node)
                  }}
                >
                ...
                <>
                  ...
                  {canCreateFolder && (
                    <DropdownMenuItem
                      onClick={(e) => {
                        e.stopPropagation()
                        onOpenDialog("createFolder", node)
                      }}
                    >
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {activeDialog === "createFile" &&
              t("filesView.dialog.createFile.description", {
                path: dialogData?.path ?? t("filesView.dialog.rootFallback"),
              })}
            {activeDialog === "createFolder" &&
              t("filesView.dialog.createFolder.description", {
                path: dialogData?.path ?? t("filesView.dialog.rootFallback"),
              })}
        ...
        {activeDialog !== "delete" && (
          <div className="py-4">
            <Input
              ...
              placeholder={
                activeDialog === "rename"
                  ? t("filesView.dialog.rename.placeholder")
                  : t("filesView.dialog.namePlaceholder")
              }
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  void onDialogSubmit()
        ...
        <DialogFooter>
          ...
          >
            {isDialogSubmitting ? (
              ...
              t("filesView.dialog.delete.confirm")
            ) : (
...
export const FilesView: React.FC<FilesViewProps> = ({ mode = "full" }) => {
  ...
  const handleToolbarDropdownOpenChange = React.useCallback((open: boolean) => {
    toolbarDropdownOpenCountRef.current = Math.max(0, toolbarDropdownOpenCountRef.current + (open ? 1 : -1))
  }, [])
  ...
  const [editorTabsOverflow, setEditorTabsOverflow] = React.useState<{ left: boolean; right: boolean }>({
    left: false,
    right: false,
  })
  ...
  const [activeDialog, setActiveDialog] = React.useState<"createFile" | "createFolder" | "rename" | "delete" | null>(
    null,
  )
  ...
  const loadDirectory = React.useCallback(
    async (dirPath: string) => {
      ...
      const listPromise = files.listDirectory
        ? files.listDirectory(normalizedDir, { respectGitignore }).then((result) =>
        ...
        : axCodeClient.listLocalDirectory(normalizedDir, { respectGitignore }).then((result) =>
            result.map((entry) => ({
              name: entry.name,
              path: entry.path,
              isDirectory: entry.isDirectory,
  ...
  const readFile = React.useCallback(
    async (path: string, options?: { allowOutsideWorkspace?: boolean; optional?: boolean }): Promise<string> => {
      if (files.readFile) {
  ...
  const readFileStat = React.useCallback(
    async (path: string, options?: { allowOutsideWorkspace?: boolean }): Promise<FileStatSnapshot | null> => {
      if (files.statFile) {
  ...
  const displayedContent = React.useMemo(
    () =>
      fileContent.length > MAX_VIEW_CHARS ? `${fileContent.slice(0, MAX_VIEW_CHARS)}\n\n… truncated …` : fileContent,
    [fileContent],
  )
        ...
        <li key={node.path} className="relative">
          {depth > 0 && (
            <>
              ...
              {isLast && <span className="absolute top-3.5 bottom-0 left-[-13px] w-[2px] bg-background" />}
            </>
          )}
          <FileRow
            node={node}
      ...
      <div className="pointer-events-auto flex items-center gap-1 rounded-lg border border-[var(--interactive-border)] bg-[var(--surface-elevated)] p-1 shadow-sm">
        {canEdit && textViewMode === "edit" && (
          <>
            {isSaving ? (
              ...
              withTooltip(
                t(autoSaveEnabled ? "filesView.editor.saveNowTitle" : "filesView.editor.saveNowManualTitle", {
                ...
                <Button
                  ...
                  title={t(autoSaveEnabled ? "filesView.editor.saveNowTitle" : "filesView.editor.saveNowManualTitle", {
                    shortcut: `${getModifierLabel()}+S`,
                  })}
            ...
            {withTooltip(
              autoSaveEnabled ? t("filesView.editor.autoSaveOn") : t("filesView.editor.manualSave"),
              <Button
                variant="ghost"
                size="sm"
        ...
        <DropdownMenu onOpenChange={handleToolbarDropdownOpenChange}>
          ...
          <DropdownMenuContent align="end" className="w-56 max-h-[70vh] overflow-y-auto">
            ...
            {openInCacheStale ? (
              <DropdownMenuItem className="flex items-center gap-2" onClick={() => void loadOpenInApps(true)}>
        ...
        {!isSelectedImage && (
          <>
            {withTooltip(
              wrapLines ? t("filesView.editor.disableLineWrap") : t("filesView.editor.enableLineWrap"),
              <Button
                variant="ghost"
                size="sm"
        ...
        {isJson &&
          withTooltip(
            jsonViewMode === "tree" ? t("filesView.editor.switchToTextView") : t("filesView.editor.switchToTreeView"),
            <Button
              variant="ghost"
              size="sm"
        ...
        {canCopyPath &&
          withTooltip(
            t("filesView.editor.copyFilePathTitle", { path: displaySelectedPath }),
            <Button
              variant="ghost"
              size="sm"
              onClick={async () => {
        ...
        {exitFullscreenOnly
          ...
          : !isMobile &&
            mode === "full" &&
            withTooltip(
              isFullscreen ? t("filesView.editor.exitFullscreen") : t("filesView.editor.fullscreen"),
              <Button
                variant="ghost"
                size="sm"
      ...
      >
        <DialogContent showCloseButton={false} className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{t("filesView.unsaved.description")}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
              variant="outline"
        ...
        <ScrollableOverlay outerClassName="h-full min-w-0" className="h-full min-w-0">
          {!selectedFile ? (
            ...
            <div className="h-full overflow-auto p-3">
              ...
              >
                <SimpleMarkdownRenderer content={fileContent} className="typography-markdown-body" stripFrontmatter />
              </ErrorBoundary>
            </div>
            ...
            <TooltipTrigger asChild>
              <span className="inline-flex flex-shrink-0">
                <Button
                  ...
                  className="size-8 p-0 flex-shrink-0"
                  title={t("filesView.tree.actions.refreshTitle")}
                  aria-label={t("filesView.tree.actions.refreshTitle")}
                >
                  <Icon name="refresh" className="size-4" />
                </Button>
              </span>
  ...
  const fullscreenViewer = mode === "full" && isFullscreen && selectedFile && (
    <div className="absolute inset-0 z-50 flex flex-col bg-background">
      {/* Fullscreen content */}
      <div className="flex-1 min-h-0 min-w-0 relative">
        <div className="absolute right-4 top-4 z-30">{renderFloatingFileControls({ exitFullscreenOnly: true })}</div>
        <ScrollableOverlay outerClassName="h-full min-w-0" className="h-full min-w-0">
          {fileLoading ? (
            suppressFileLoadingIndicator ? (
              <div className="p-4" />
    ...
    <div className="flex h-full min-h-0 overflow-hidden bg-background relative">
      ...
      {isMobile ? (
        ...
        <div className="flex flex-1 min-h-0 min-w-0 overflow-hidden">
          <div className="flex-1 min-h-0 min-w-0 overflow-hidden bg-background">{fileViewer}</div>
        </div>
      ) : (
        <div className="flex flex-1 min-h-0 min-w-0 gap-3 px-3 pb-3 pt-2">
          {screenWidth >= 700 && <div className="w-72 flex-shrink-0 min-h-0 overflow-hidden">{treePanel}</div>}
