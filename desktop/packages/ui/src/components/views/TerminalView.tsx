...
export const TerminalView: React.FC = () => {
  ...
  const startStream = React.useCallback(
    (directory: string, tabId: string, terminalId: string, streamOptions = STREAM_OPTIONS) => {
  ...
  React.useEffect(() => {
    ...
    if (!effectiveDirectory) {
      setConnectionError(
        hasActiveContext ? t("terminalView.empty.noWorkingDirectory") : t("terminalView.empty.selectSession"),
      )
      disconnectStream()
      return
    }
    ...
    const ensureSession = async () => {
      ...
      const tabId = enableTabs ? (state.activeTabId ?? state.tabs[0]?.id ?? null) : (state.tabs[0]?.id ?? null)
      if (!tabId) {
        return
      }
      ...
      const isRehydratedSession = Boolean(terminalId) && rehydratedTerminalIdsRef.current.has(terminalId as string)
      ...
      startStream(directory, tabId, terminalId, isRehydratedSession ? REHYDRATED_STREAM_OPTIONS : STREAM_OPTIONS)
    }
  ...
  const handleRestart = React.useCallback(async () => {
    ...
    try {
      ...
      setConnectionError(error instanceof Error ? error.message : t("terminalView.error.restartFailed"))
      setIsFatalError(true)
      setIsReconnectPending(false)
    } finally {
    ...
    (data: string) => {
      ...
      void terminal.sendInput(terminalId, payload).catch((error) => {
        if (!isReconnectPending) {
          setConnectionError(error instanceof Error ? error.message : t("terminalView.error.sendInputFailed"))
        }
      })
  ...
  React.useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const upperFromCode = code.startsWith("Key") && code.length === 4 ? code.slice(3).toUpperCase() : null
      const upperKey = rawKey.length === 1 && /[a-zA-Z]/.test(rawKey) ? rawKey.toUpperCase() : upperFromCode
  ...
  if (!hasActiveContext) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center">
        <Icon name="terminal-box" className="size-8 text-muted-foreground" />
                  ...
                  <Button
                    ...
                    title={
                      isBottomTerminalExpanded
                        ? t("terminalView.bottomDock.restoreTitle")
                        : t("terminalView.bottomDock.expandTitle")
                    }
                    aria-label={
                      isBottomTerminalExpanded
                        ? t("terminalView.bottomDock.restoreAria")
                        : t("terminalView.bottomDock.expandAria")
                    }
                  >
                    {isBottomTerminalExpanded ? (
