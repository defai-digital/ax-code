...
export function GitHubPrPickerDialog({
  ...
  const attachPr = React.useCallback(
    async (prNumber: number, sourceRepo?: GitHubRepoSelector | null) => {
      if (!projectDirectory) {
    ...
    <>
      ...
      <div className={cn(isMobile ? "min-h-0" : "flex-1 overflow-y-auto")}>
        ...
        {filtered.length === 0 && !isLoading && connected && github && projectDirectory ? (
          <div className="text-center text-muted-foreground py-8">
            {query
              ? t("session.githubPrPicker.empty.noPullRequestsFound")
              : t("session.githubPrPicker.empty.noOpenPullRequestsFound")}
          </div>
        ) : null}
          ...
          <div className="py-2 flex justify-center">
            <button
              ...
              className={cn(
                ...
                (isLoadingMore || Boolean(loadingPrNumber)) &&
