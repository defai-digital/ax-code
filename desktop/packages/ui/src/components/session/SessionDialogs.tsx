...
export const SessionDialogs: React.FC = () => {
  ...
  const [deleteDialogSummaries, setDeleteDialogSummaries] = React.useState<
    Array<{ session: Session; metadata: WorktreeMetadata }>
  >([])
  ...
  const projectDirectory = React.useMemo(() => {
    const targetProject = activeProjectId ? (projects.find((project) => project.id === activeProjectId) ?? null) : null
  ...
  const getProjectRefForWorktree = React.useCallback(
  ...
  const canRemoveRemoteBranches = React.useMemo(() => {
    ...
    return (
      ...
      deleteDialogSummaries.every(
        ({ metadata }) => typeof metadata.branch === "string" && metadata.branch.trim().length > 0,
      )
    )
  ...
  const removeRemoteOptionDisabled = isProcessingDelete || !isWorktreeDelete || !canRemoveRemoteBranches
  ...
  const deleteSessionsWithoutDialog = React.useCallback(
    async (payload: { sessions: Session[]; dateLabel?: string }) => {
  ...
  const removeSelectedWorktree = React.useCallback(
    async (worktree: WorktreeMetadata, deleteLocalBranch: boolean): Promise<boolean> => {
      ...
      try {
        await removeProjectWorktree(projectRef, worktree, { deleteRemoteBranch: shouldRemoveRemote, deleteLocalBranch })
  ...
  const handleConfirmDelete = React.useCallback(async () => {
    ...
    try {
      ...
      if (deleteDialog.sessions.length === 1) {
        ...
        const archiveNote =
          !isWorktreeDelete && shouldArchive
            ? removeRemoteBranch
              ? t("sessions.sidebar.sessionDialogs.worktree.removedWithRemote")
        ...
        <div
          className={cn(
            isWorktreeDelete
              ...
              : "space-y-1.5 rounded-xl border border-border/40 bg-sidebar/60 p-3",
          )}
        >
          {isWorktreeDelete && (
            <div className="flex items-center gap-2">
              ...
              <span className="typography-micro text-muted-foreground/70">{deleteDialog.sessions.length}</span>
            </div>
          )}
          <ul className={cn(isWorktreeDelete ? "mt-2 space-y-1" : "space-y-0.5")}>
            {deleteDialog.sessions.slice(0, 5).map((session) => (
              ...
              >
                <span className={cn(!isWorktreeDelete && "hidden")}>•</span>
                <span className="truncate">{session.title || t("sessions.sidebar.session.untitled")}</span>
              </li>
            ))}
      ...
      {isWorktreeDelete ? (
        <div className="space-y-2 rounded-lg bg-muted/30 p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="typography-meta font-medium text-foreground">
              {t("sessions.sidebar.sessionDialogs.worktree.label")}
            </span>
        ...
        <div className="rounded-xl border border-border/40 bg-sidebar/60 p-3">
          <p className="typography-meta text-muted-foreground/80">{t("sessions.sidebar.sessionDialogs.delete.note")}</p>
        </div>
      )}
    </div>
  ) : null
      ...
      {useMobileOverlay ? (
        ...
        >
          ...
          >
            ...
            <div className={cn(isWorktreeDelete && "flex-1 min-h-0 overflow-y-auto space-y-2")}>{deleteDialogBody}</div>
            <DialogFooter className="mt-2 gap-2 pt-1 pb-1">{deleteDialogActions}</DialogFooter>
          </DialogContent>
        </Dialog>
      )}

      <DirectoryExplorerDialog open={isDirectoryDialogOpen} onOpenChange={setIsDirectoryDialogOpen} />
    </>
  )
}
