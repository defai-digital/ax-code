  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="max-w-[220px]">
        ...
        {options.map((option) => (
          <DropdownMenuItem key={option} className="typography-meta" onSelect={() => onChange(option)}>
            <span className={cn("font-medium capitalize", value === option && "text-primary")}>{option}</span>
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
...
export function TodoSendDialog(props: TodoSendDialogProps) {
  ...
  const variantOptions = React.useMemo(() => {
    ...
    const model = provider?.models?.find((item) => item.id === execution.modelID) as
