...
const insertTodoBeforeCompleted = (
  items: OpenChamberProjectTodoItem[],
  item: OpenChamberProjectTodoItem,
): OpenChamberProjectTodoItem[] => {
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const handleTodoPanelResizeStart = React.useCallback(
  ...
  const todoSensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 8 } }))
  ...
  const handleTriggerUploadPlan = React.useCallback(async () => {
    ...
    if (result.success && result.path) {
      ...
      try {
        ...
        const description = error instanceof Error ? error.message : undefined
        toast.error(
          t("rightSidebar.contextNotesTodo.toast.readPlanFileFailed"),
    ...
    <div className={cn("w-full min-w-0 space-y-3 p-3", className)}>
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">
            {notes.length}/{AX_CODE_DESKTOP_PROJECT_NOTES_MAX_LENGTH}
          </span>
        </div>
        <Textarea
          value={notes}
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">
            {todoInputValue.length}/{AX_CODE_DESKTOP_PROJECT_TODO_TEXT_MAX_LENGTH}
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <Input
            ...
            onChange={(event) =>
              setNewTodoText(event.target.value.slice(0, AX_CODE_DESKTOP_PROJECT_TODO_TEXT_MAX_LENGTH))
            }
            onKeyDown={(event) => {
              if (event.key === "Enter") {
                event.preventDefault()
        ...
        >
          {todos.length === 0 ? (
            ...
            <DndContext sensors={todoSensors} collisionDetection={closestCenter} onDragEnd={handleTodoReorder}>
                        ...
                        {(dragHandleProps) => (
                          <div
                            className={cn(
                              ...
                              isExpandedTodo ? "items-start" : "items-center",
                            )}
                          >
                            <button
                              type="button"
                            ...
                            <div className="flex h-6 items-center">
                              <Checkbox
                                ...
                                ariaLabel={t("rightSidebar.contextNotesTodo.todo.actions.markComplete", {
                                  text: todo.text,
                                })}
                              />
                            </div>
                              ...
                              <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                  <button
                                    ...
                                    aria-label={t("rightSidebar.contextNotesTodo.todo.actions.send", {
                                      text: todo.text,
                                    })}
