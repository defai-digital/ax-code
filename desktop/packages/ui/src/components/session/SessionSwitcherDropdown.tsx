...
export function SessionSwitcherDropdown({
  ...
  return (
    <DropdownMenu open={isOpen} onOpenChange={setOpen} modal={false}>
      ...
      >
        {isOpen ? (
          <SwitcherContent onSelect={() => setOpen(false)} variant={variant} scopeProjectId={scopeProjectId} />
        ) : null}
      </DropdownMenuContent>
    </DropdownMenu>
  )
...
function SwitcherNode({
  ...
  toggleParent,
  closeDropdown,
}: SwitcherNodeProps): React.ReactElement {
...
function SwitcherRow({
  ...
  onToggleExpand,
  closeDropdown,
}: SwitcherRowProps): React.ReactElement {
  const { t } = useI18n()
