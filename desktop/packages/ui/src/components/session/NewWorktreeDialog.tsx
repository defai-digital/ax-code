...
const resolvePrWorktreeConfig = (pr: GitHubPullRequestSummary, localBranches: string[], remoteBranches: string[]) => {
  ...
  const ownerFromLabel = String(pr.headLabel || "")
    .split(":")[0]
    ?.trim()
...
export function NewWorktreeDialog({ open, onOpenChange, onWorktreeCreated }: NewWorktreeDialogProps) {
  const { t } = useI18n()
  ...
  const resetDesktopPickerScroll = React.useCallback(
  ...
  const resetMobilePickerScroll = React.useCallback(
  ...
  const applySessionModelAndAgentDefaults = React.useCallback(
    (args: { sessionId: string; providerID: string; modelID: string; agentName?: string; variant?: string }) => {
      const configState = useConfigStore.getState()
      ...
      try {
        useSelectionStore
          .getState()
          .saveAgentModelForSession(args.sessionId, args.agentName, args.providerID, args.modelID)
      } catch {
        // ignore
      }
  ...
  const sendLinkedContextMessage = React.useCallback(
    async (args: {
      sessionId: string
  ...
  React.useEffect(() => {
    ...
    const loadDefaultSourceBranch = async () => {
      try {
        ...
        const defaultSourceBranch =
          savedSourceBranch && branches.all?.includes(savedSourceBranch)
            ? savedSourceBranch
    ...
    try {
      ...
      if (createdSessionId) {
        ...
        void sendLinkedContextMessage({
          ...
          const message =
            error instanceof Error ? error.message : t("session.newWorktree.error.sendGitHubContextFailed")
  ...
  const isFormValid =
    mode === "existing-branch"
      ? !!existingBranchState.selectedBranch &&
        !!existingBranchState.worktreeName &&
        !validation.branchError &&
        !validation.worktreeError
      : !!normalizeBranchName(newBranchState.branchName) &&
    ...
    <div className={cn("flex gap-2", isMobile ? "flex-col w-full" : "flex-row items-center")}>
      ...
      <div
        className={cn(
          ...
          isMobile ? "w-full justify-center order-first" : "mr-auto",
        )}
      >
        {validation.touched && (validation.branchError || validation.worktreeError) && (
          <>
            <Icon name="error-warning" className="h-3.5 w-3.5" />
            <span className="typography-micro">{validation.branchError || validation.worktreeError}</span>
          </>
        )}
      </div>
            ...
            <SortableTabsStrip
              items={[
                ...
                },
                {
                  ...
                  icon: <Icon name="git-repository" className="h-3.5 w-3.5" />,
                },
              ]}
              activeId={mode}
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-1.5">
                  ...
                  {mode === "new-branch" && isGitHubConnected && (
                    ...
                    >
                      ...
                      {newBranchState.linkedIssue || newBranchState.linkedPr
                        ? t("session.newWorktree.actions.change")
                        : t("session.newWorktree.actions.startFromGitHubIssuePr")}
                    </Button>
                  )}
                </div>
                <Input
                ...
                {newBranchState.linkedIssue && !newBranchState.linkedPr && (
                  <div className="flex items-center gap-1.5 text-muted-foreground">
                    <Icon name="check" className="h-3.5 w-3.5 text-status-success" />
                    <span className="typography-micro">
                      {t("session.newWorktree.fromIssue", {
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {mode !== "existing-branch" && (
                  <button
                    ...
                    disabled={
                      !newBranchState.branchName ||
                      newBranchState.worktreeName === slugifyWorktreeName(newBranchState.branchName)
                    }
                    className={cn(
              ...
              <Input
                ...
                className={cn("h-8", validation.touched && validation.worktreeError && "border-destructive")}
              />
            </div>
          ...
          <DialogContent className="max-w-lg max-h-[80vh] flex flex-col">
                  ...
                  <div className="flex items-center gap-2">
                    <DropdownMenu open={existingBranchDropdownOpen} onOpenChange={setExistingBranchDropdownOpen}>
                      <DropdownMenuTrigger asChild>
                        ...
                        >
                          <span
                            className={cn(
                              "truncate",
                              existingBranchState.selectedBranch ? "text-foreground" : "text-muted-foreground",
                            )}
                          >
                            {existingBranchState.selectedBranch || t("session.newWorktree.chooseBranch")}
                          </span>
                      ...
                      <DropdownMenuContent
                        ...
                        sideOffset={6}
                        portalToBody
                        className="w-[min(42rem,calc(100vw-2rem))] p-0 max-h-[min(var(--available-height),24rem)] flex flex-col overflow-hidden"
                        ref={existingBranchDropdownContentRef}
                      >
                ...
                <div className="space-y-1.5">
                  ...
                  <DropdownMenu open={sourceBranchDropdownOpen} onOpenChange={setSourceBranchDropdownOpen}>
                    <DropdownMenuTrigger asChild>
                      ...
                      >
                        <span
                          className={cn(
                            "truncate",
                            newBranchState.sourceBranch ? "text-foreground" : "text-muted-foreground",
                          )}
                        >
                          {newBranchState.sourceBranch || t("session.newWorktree.selectSourceBranchPlaceholder")}
                        </span>
                    ...
                    <DropdownMenuContent
                      ...
                      portalToBody
                      className="w-[min(42rem,calc(100vw-2rem))] p-0 max-h-[min(var(--available-height),24rem)] flex flex-col overflow-hidden"
                      ref={sourceBranchDropdownContentRef}
                    >
                      <Command shouldFilter={false}>
                        <CommandInput
                          placeholder={t("session.newWorktree.searchBranches")}
            ...
            <DialogFooter className="mt-1 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" onClick={handleCreate} disabled={!canCreate || isCreating} className="gap-1.5">
                  {isCreating && <Icon name="loader-4" className="h-3.5 w-3.5 animate-spin" />}
                  {isCreating
                    ? t("session.newWorktree.actions.creating")
                    : t("session.newWorktree.actions.createWorktree")}
                </Button>
              </div>
            </DialogFooter>
          </DialogContent>
