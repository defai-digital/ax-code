...
type Props = {
  ...
  prVisualStateByDirectoryBranch: Map<
    string,
    {
...
export function SessionGroupSection(props: Props): React.ReactNode {
  ...
  const sourceGroupNodes = React.useMemo(
    () =>
      [...(shouldFilterGroupContents ? (searchData?.filteredNodes ?? []) : group.sessions)].sort(compareSessionNodes),
  ...
  const shouldVirtualizeArchived =
  ...
  React.useLayoutEffect(() => {
    ...
    if (!scrollEl) return
    const offset = container.getBoundingClientRect().top - scrollEl.getBoundingClientRect().top + scrollEl.scrollTop
  ...
  const archivedVirtualRows =
    shouldVirtualizeArchived && archivedScrollEl !== null ? archivedVirtualizer.getVirtualItems() : []
  ...
  const isGitProject =
    projectId && projectRepoStatus.has(projectId) ? Boolean(projectRepoStatus.get(projectId)) : lastRepoStatus
  const groupDirectoryKey = normalizePath(group.directory ?? null)
  const groupBranchKey = group.branch?.trim() ?? null
  const prIndicator =
    groupDirectoryKey && groupBranchKey
  ...
  const checksSummary =
    prIndicator && prIndicator.state === "open" && prIndicator.checks
  ...
  const checksTail =
    prIndicator && prIndicator.state === "open" && prIndicator.checks
      ? [
          ...
          .join(", ")
      : null
  const mergeabilityLabel =
    prIndicator && prIndicator.state === "open"
      ? prIndicator.mergeableState === "blocked" || prIndicator.mergeableState === "dirty"
        ? t("sessions.sidebar.group.pr.conflictsOrBlocked")
        : prIndicator.mergeableState === "clean" || prIndicator.canMerge === true
          ? t("sessions.sidebar.group.pr.mergeable")
          : null
      : null
  const mergeStateLabel =
    prIndicator && prIndicator.state === "open" && prIndicator.mergeableState
  ...
  const statusLine = (() => {
    ...
    switch (prIndicator.visualState) {
      ...
      case "open":
        return prIndicator.canMerge === true ||
          prIndicator.mergeableState === "clean" ||
          prIndicator.checks?.state === "success"
      ...
      case "blocked":
        return {
          label:
            prIndicator.mergeableState === "dirty"
  ...
  const renderOneFolderItem = (folder: SessionFolder, nodes: SessionNode[], depth: number): React.ReactNode => {
    ...
    const collectFolderSessions = (targetFolderId: string): Session[] => {
      const directNodes =
        allFoldersForGroup.find(({ folder: candidate }) => candidate.id === targetFolderId)?.nodes ?? []
          ...
          <SessionFolderItem
            ...
            onDelete={() => {
              if (group.isArchivedBucket) {
                // Delete sessions in the folder. Archived auto-foldering was
                // retired; any legacy synthetic archived folders are purged on load.
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="top" sideOffset={6} align="start" className="max-w-sm">
                      <div className="space-y-1 text-xs">
                        ...
                        {mergeStateLabel ? (
                          <div className="text-muted-foreground truncate">{mergeStateLabel}</div>
                        ) : null}
          ...
          >
            <Tooltip>
              ...
              <TooltipContent side="bottom" sideOffset={4}>
                <p>{t("sessions.sidebar.group.actions.deleteArchivedSessions")}</p>
              </TooltipContent>
            </Tooltip>
          </div>
        ) : null}
        {group.directory && !group.isMain && group.worktree ? (
          ...
          >
            <Tooltip>
              ...
              <TooltipContent side="bottom" sideOffset={4}>
                <p>{t("sessions.sidebar.group.actions.deleteWorktree")}</p>
              </TooltipContent>
            </Tooltip>
          </div>
        ) : null}
        {group.directory ? (
          ...
          >
            <Tooltip>
              ...
              <TooltipContent side="bottom" sideOffset={4}>
                <p>{t("sessions.sidebar.project.actions.newDraftSession")}</p>
              </TooltipContent>
            </Tooltip>
          </div>
        ) : null}
