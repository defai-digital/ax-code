...
export const useSessionGrouping = (args: Args) => {
  ...
  const buildSessionSearchText = React.useCallback(
  ...
  const buildGroupedSessions = React.useCallback(
    (
      ...
      const sortedProjectSessions = dedupeSessionsById(projectSessions).sort((a, b) =>
        compareSessionsByPinnedAndTime(a, b, args.pinnedSessionIds),
      )
      ...
      const getGroupKey = (session: Session) => {
        ...
        const fallbackDirectory = normalizePath(
      ...
      const groups: SessionGroup[] = [
        {
          id: "root",
          label:
            projectIsRepo && projectRootBranch && projectRootBranch !== "HEAD"
