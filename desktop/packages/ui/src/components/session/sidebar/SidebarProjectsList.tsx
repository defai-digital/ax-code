  ...
  if (props.projectSections.length === 0) {
    return (
      <ScrollableOverlay
        ...
        className="space-y-1 pb-1 pl-2.5 pr-2"
      >
        {props.topContent}
        {props.emptyState}
      </ScrollableOverlay>
    ...
    >
      ...
      {props.showOnlyMainWorkspace ? (
        <div className="space-y-[0.6rem] py-1">
          {(() => {
            const activeSection =
              props.sectionsForRender.find((section) => section.project.id === props.activeProjectId) ??
              props.sectionsForRender[0]
            if (!activeSection) {
                    ...
                    {!isCollapsed ? (
                      <div className="space-y-0 pt-0 pb-0.5 pl-3">
                        {section.groups.length > 0 ? (
                          ...
                          >
                            {rootGroup
                              ? props.renderGroupSessions(rootGroup, `${projectKey}:${rootGroup.id}`, projectKey, true)
                              : null}
                            <SortableContext
                              items={nestedGroups.map((group) => group.id)}
                              strategy={verticalListSortingStrategy}
                            >
                              {nestedGroups.map((group) => {
                                ...
                                return (
                                  <SortableGroupItem key={group.id} id={group.id} disabled={props.isInlineEditing}>
                                    {(dragHandleProps) =>
                                      props.renderGroupSessions(group, groupKey, projectKey, false, dragHandleProps)
                                    }
                                  </SortableGroupItem>
                                )
                              })}
