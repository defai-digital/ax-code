...
export const BulkActionBar: React.FC<Props> = ({
  ...
  const iconButtonClass =
    "inline-flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-interactive-hover/50 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
  const destructiveIconButtonClass =
    "inline-flex h-7 w-7 items-center justify-center rounded-md text-destructive hover:bg-destructive/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive/50"
    ...
    <div className="flex shrink-0 items-center gap-1 border-t border-border px-2.5 py-1.5">
      ...
      <div className="ml-auto flex items-center gap-0.5">
          ...
          <DropdownMenu>
            <Tooltip>
              ...
              <TooltipContent side="top" sideOffset={4}>
                <p>{t("sessions.sidebar.bulkActions.moveToFolder")}</p>
              </TooltipContent>
            </Tooltip>
            <DropdownMenuContent align="end" className="min-w-[180px]">
              ...
              {canRemoveFromFolder ? (
                <DropdownMenuItem onClick={onRemoveFromFolder} className="text-destructive focus:text-destructive">
        ...
        <Tooltip>
          ...
          <TooltipContent side="top" sideOffset={4}>
            <p>{t("sessions.sidebar.header.actions.exitSelection")}</p>
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
