...
type Props = {
  ...
  openContextPanelTab: (
    directory: string,
    options: { mode: "chat"; dedupeKey: string; label: string; readOnly?: boolean },
  ) => void
...
const areEqual = (prev: Props, next: Props): boolean => {
  ...
  const prevMenuInTree = treeContainsMenuKey(
    prev.node,
    prev.openSidebarMenuKey,
    prev.renderContext ?? "project",
    prev.archivedBucket ?? false,
  )
  const nextMenuInTree = treeContainsMenuKey(
    next.node,
    next.openSidebarMenuKey,
  ...
  const prevDirectory =
    normalizePath((prevSession as Session & { directory?: string | null }).directory ?? null) ??
    normalizePath(prev.groupDirectory ?? null)
  const nextDirectory =
    normalizePath((nextSession as Session & { directory?: string | null }).directory ?? null) ??
    normalizePath(next.groupDirectory ?? null)
  if (prevDirectory !== nextDirectory) return false
  if (
    (prevDirectory ? prev.directoryStatus.get(prevDirectory) : null) !==
    (nextDirectory ? next.directoryStatus.get(nextDirectory) : null)
  )
    return false
...
function SessionNodeItemComponent(props: Props): React.ReactNode {
  ...
  const isMultiRunLikeSession = React.useMemo(
    () => parseMultiRunSessionTitle(resolvedSession.title) !== null,
  ...
  const collectChildExports = React.useCallback(
    async (children: SessionNode[]): Promise<{ children: ChildSessionExport[]; skipped: number }> => {
  ...
  const showSkippedSubtasksWarning = React.useCallback(
    (count: number) => {
      if (count <= 0) return
      toast.warning(
        count === 1
  ...
  const leadingIndicators =
    showStatusMarker || isPinnedSession ? (
      <span
        className={cn(
  ...
  const streamingIndicator = isZombie ? <Icon name="error-warning" className="h-4 w-4 text-status-warning" /> : null
    ...
    >
      ...
      {!resolvedSession.share ? (
        ...
        <>
          <DropdownMenuItem
            onClick={() => {
              if (resolvedSession.share?.url) handleCopyShareUrl(resolvedSession.share.url, session.id)
            }}
            className="[&>svg]:mr-1"
          >
            {copiedSessionId === session.id ? (
              <>
                <Icon name="check" className="mr-1 h-4 w-4" style={{ color: "var(--status-success)" }} />
                {t("sessions.sidebar.session.menu.copied")}
              </>
            ) : (
              <>
      ...
      >
        ...
        <span className="shrink-0 typography-micro px-1 rounded leading-none pb-px text-[var(--status-warning)] bg-[var(--status-warning)]/10">
          {t("sessions.sidebar.session.menu.betaBadge")}
        </span>
      </DropdownMenuItem>
    ...
    <React.Fragment key={session.id}>
        ...
        >
                ...
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={cn(
                      ...
                      alwaysShowActions ? alwaysActionPaddingClass : revealPaddingClass,
                    )}
                  >
                    <div
                      className={cn(
                        "flex w-full items-center min-w-0 flex-1 overflow-hidden",
                        isMinimalMode ? "gap-1" : "gap-1",
                      )}
                    >
                      <div
                        className={cn(
                  ...
                  <div className="flex flex-col gap-1 text-left text-xs">
                    <div
                      className={cn(
                        ...
                        secondaryMeta?.projectLabel ? "justify-between" : "justify-start",
                      )}
                    >
                      {secondaryMeta?.projectLabel ? (
                        <div className="min-w-0 truncate">{secondaryMeta.projectLabel}</div>
                      ) : null}
                    ...
                    {secondaryMeta?.branchLabel || sessionDiffStats ? (
                      <div
                        className={cn(
                          "flex items-center gap-3 text-left text-muted-foreground",
                          secondaryMeta?.branchLabel ? "justify-between" : "justify-start",
                        )}
                      >
          ...
          <div
            className={cn(
              ...
              isMenuOpen ? "opacity-100" : alwaysShowActions ? "opacity-100" : cn("opacity-0", revealOnHoverClass),
            )}
          >
            {showQuickArchiveAction ? (
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    <Icon
                      name="archive"
                      className={cn(isMinimalMode && !alwaysShowActions ? "h-2.5 w-2.5" : "h-3.5 w-3.5")}
                    />
                  </button>
                </TooltipTrigger>
      ...
      <Dialog open={exportDialogOpen} onOpenChange={setExportDialogOpen}>
        <DialogContent showCloseButton={false} className="max-w-sm gap-5">
          ...
          <DialogFooter>
            <Button type="button" onClick={() => setExportDialogOpen(false)} variant="outline" size="sm">
