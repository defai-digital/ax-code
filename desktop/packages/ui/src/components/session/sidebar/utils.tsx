...
export const compareSessionsByPinnedAndTime = (a: Session, b: Session, pinnedSessionIds: Set<string>): number => {
...
export const compareSessionsByPinnedAndCreated = (a: Session, b: Session, pinnedSessionIds: Set<string>): number => {
  const aPinned = pinnedSessionIds.has(a.id)
  const bPinned = pinnedSessionIds.has(b.id)
  if (aPinned !== bPinned) {
    return aPinned ? -1 : 1
...
export const resolveSessionDiffStats = (
  summary?: SessionSummaryMeta,
...
export const renderHighlightedText = (text: string, query: string): React.ReactNode => {
  ...
  while (matchIndex !== -1) {
    ...
    parts.push(
      <mark key={`${matchIndex}-${matchText}`} className="bg-primary text-primary-foreground ring-1 ring-primary/90">
        {matchText}
      </mark>,
    )
