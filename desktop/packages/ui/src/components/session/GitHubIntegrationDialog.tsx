...
export function GitHubIntegrationDialog({ open, onOpenChange, onSelect }: GitHubIntegrationDialogProps) {
  const { t } = useI18n()
  ...
  const validateBranch = React.useCallback(
    async (branchName: string) => {
      ...
      try {
        ...
        setValidations((prev) =>
          new Map(prev).set(branchName, {
            isValid: !blockingError,
            error: blockingError
              ? t(
                  blockingError.code === "branch_exists"
    ...
    <>
      {!isGitHubConnected ? (
        <div className="flex-1 flex flex-col items-center justify-center p-8 gap-4">
          ...
          <Button onClick={openGitHubSettings} size="sm">
            {t("session.githubIntegration.connect.action")}
          </Button>
        </div>
      ) : (
        <>
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                {blocked && validation?.error && (
                                  <span className="typography-micro text-destructive">{validation.error}</span>
                                )}
                              </div>
                            </div>
                          </div>
  ...
  // Footer content
  const footerContent = (
    <div className={cn("w-full", isMobile ? "flex flex-col gap-2" : "flex flex-row items-center")}>
      {/* Left side: Selected Item / Checkbox */}
      <div className={cn("flex items-center gap-4", isMobile ? "w-full justify-center order-1" : "flex-1")}>
      ...
      {/* Right side: Buttons */}
      <div className={cn("flex gap-2", isMobile ? "w-full order-2" : "justify-end")}>
        <Button variant="outline" size="sm" onClick={() => onOpenChange(false)} className={cn(isMobile && "flex-1")}>
          {t("session.githubIntegration.actions.cancel")}
        </Button>
        <Button size="sm" onClick={handleConfirm} disabled={!canConfirm} className={cn(isMobile && "flex-1")}>
      ...
      {isMobile ? (
              ...
              <div className="w-full">
                <SortableTabsStrip
                  items={[
                    {
                      id: "issues",
                      label: t("session.githubIntegration.tabs.issues"),
                      icon: <Icon name="git-branch" className="h-3.5 w-3.5" />,
                    },
                    {
                      id: "prs",
                      label: t("session.githubIntegration.tabs.pullRequests"),
                      icon: <Icon name="git-pull-request" className="h-3.5 w-3.5" />,
                    },
                  ]}
                  activeId={activeTab}
                  onSelect={(id) => {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
          <DialogContent className="max-w-2xl max-h-[70vh] flex flex-col">
            ...
            {/* Footer */}
            <DialogFooter className="mt-1">{footerContent}</DialogFooter>
          </DialogContent>
        </Dialog>
      )}
    </>
