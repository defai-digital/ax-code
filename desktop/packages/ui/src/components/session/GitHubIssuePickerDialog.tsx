...
export function GitHubIssuePickerDialog({
  ...
  const title =
    mode === "select" ? t("session.githubIssuePicker.title.select") : t("session.githubIssuePicker.title.createSession")
  const description =
    mode === "select"
    ...
    <>
      ...
      <div className={cn(isMobile ? "min-h-0 mt-2" : "flex-1 overflow-y-auto mt-2")}>
        ...
        {filtered.length === 0 && !isLoading && connected && github && projectDirectory ? (
          <div className="text-center text-muted-foreground py-8">
            {query
              ? t("session.githubIssuePicker.empty.noIssuesFound")
              : t("session.githubIssuePicker.empty.noOpenIssuesFound")}
          </div>
        ) : null}

        {filtered.map((issue) => {
          const safeIssueUrl = getSafeExternalUrl(issue.url)
          return (
            <div
              ...
              <div className="flex-shrink-0 h-5 flex items-center mr-2">
                {startingIssueNumber === issue.number ? (
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      void openExternalUrl(safeIssueUrl)
                    }}
          ...
          <div className="py-2 flex justify-center">
            <button
              ...
              className={cn(
                ...
                (isLoadingMore || Boolean(startingIssueNumber)) &&
      ...
      {mode !== "select" && (
        <div className="mt-4 p-3 bg-muted/30 rounded-lg">
          <p className="typography-meta text-muted-foreground font-medium mb-2">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-2">
            ...
            <div className="flex items-center gap-2">
              {safeRepoUrl ? (
                <Button variant="outline" size="sm" onClick={() => void openExternalUrl(safeRepoUrl)}>
              ...
              <Button
                ...
                size="sm"
                onClick={refresh}
                disabled={isLoading || Boolean(startingIssueNumber)}
              >
