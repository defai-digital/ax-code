...
const FINDER_DEFAULT_ICON_DATA_URL =
...
const TERMINAL_DEFAULT_ICON_DATA_URL =
...
const AppIcon = ({
  ...
  if (src && !failed) {
    return <img src={src} alt="" className="h-4 w-4 rounded-sm" onError={() => setFailed(true)} />
  }
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56 max-h-[70vh] overflow-y-auto">
          ...
          {availableApps.map((app) => {
            ...
            return (
              <DropdownMenuItem key={app.id} className="flex items-center gap-2" onClick={() => void handleSelect(app)}>
                <AppIcon
                  label={app.label}
                ...
                {selectedApp.id === app.id ? <Icon name="check" className="ml-auto h-4 w-4 text-primary" /> : null}
              </DropdownMenuItem>
            )
          })}
          {isCacheStale ? (
            <DropdownMenuItem className="flex items-center gap-2" onClick={() => void loadInstalledApps(true)}>
