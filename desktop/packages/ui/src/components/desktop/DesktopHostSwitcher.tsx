...
const statusLabelKey = (
  status: HostProbeResult["status"] | null,
):
...
export function DesktopHostSwitcherDialog({
  ...
  const persist = React.useCallback(
    async (nextHosts: DesktopHost[], nextDefaultHostId: string | null) => {
    ...
    try {
      const results = await Promise.all(
        hosts.map(async (h) => {
          ...
          const res = await desktopHostProbe(url).catch(
    ...
    async (host: DesktopHost) => {
      ...
      if (host.id !== LOCAL_HOST_ID && isSshHost && isTauriShell()) {
        ...
        try {
          ...
          const readyStatus = await waitForSshReady(
            host.id,
      ...
      if (host.id !== LOCAL_HOST_ID && isTauriShell()) {
        setSwitchingHostId(host.id)
        const probe = await desktopHostProbe(origin).catch(
          (): HostProbeResult => ({ status: "unreachable", latencyMs: 0 }),
        )
        setStatusById((prev) => ({
          ...prev,
  ...
  const deleteHost = React.useCallback(
    async (id: string) => {
  ...
  const connectSshHostInPlace = React.useCallback(
    async (host: DesktopHost) => {
      if (!isTauriShell()) return
      setSwitchingHostId(host.id)
      try {
    ...
    <>
      {embedded ? (
        ...
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription>{t("desktopHostSwitcher.description")}</DialogDescription>
        </DialogHeader>
      )}
      ...
      {!tauriAvailable && (
        <div className="flex-shrink-0 rounded-lg border border-border/50 bg-muted/20 p-3">
          <div className="typography-meta text-muted-foreground">{t("desktopHostSwitcher.state.limitedOnPage")}</div>
        </div>
      )}
        ...
        <div className="space-y-1">
          {isLoading ? (
            ...
            allHosts.map((host) => {
              ...
              const displayLabel =
                host.id === LOCAL_HOST_ID ? t("desktopHostSwitcher.instance.local") : redactSensitiveUrl(host.label)
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="typography-micro text-muted-foreground truncate font-mono">{displayUrl}</div>
                    </div>
                  </button>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {isLocal && <div className="h-8 w-8 opacity-0 pointer-events-none" aria-hidden="true" />}
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent sideOffset={6}>
                        {isDefault
                          ...
                          : t("desktopHostSwitcher.actions.setAsDefault")}
                      </TooltipContent>
                    </Tooltip>
      ...
      {error && <div className="flex-shrink-0 typography-meta text-status-error">{error}</div>}
    </>
  )
      ...
      <DialogContent className="w-[min(28rem,calc(100vw-2rem))] max-w-none">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t("desktopHostSwitcher.ssh.connectingTo", {
              host: sshSwitchModal.hostLabel || t("desktopHostSwitcher.ssh.instanceFallback"),
            })}
          </DialogTitle>
          <DialogDescription>
        ...
        {sshSwitchModal.error ? (
          <div className="flex justify-end gap-2">
            <Button type="button" size="sm" variant="outline" onClick={switchToLocal}>
              {t("desktopHostSwitcher.actions.switchToLocal")}
            </Button>
            <Button type="button" size="sm" onClick={retrySshSwitch} disabled={!sshSwitchModal.hostId}>
  ...
  if (embedded) {
    return (
      <>
        <div className="w-full max-h-[70vh] flex flex-col overflow-hidden gap-2">{content}</div>
        {sshSwitchDialog}
      </>
    )
  }
...
export function DesktopHostSwitcherButton({ headerIconButtonClass }: DesktopHostSwitcherButtonProps) {
  ...
  const connectDefaultSshInstance = React.useCallback(
    async (hostId: string, hostLabel: string, options?: { showProgress?: boolean }): Promise<boolean> => {
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {startupSshModal.connecting
                ? t("desktopHostSwitcher.startup.connectingTo", {
                    host: startupSshModal.hostLabel || t("desktopHostSwitcher.ssh.instanceFallback"),
                  })
