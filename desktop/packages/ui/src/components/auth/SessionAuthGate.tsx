...
const AuthShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    ...
    >
      ...
      />
      <div className="app-region-no-drag relative z-10 flex w-full justify-center px-4 py-12 sm:px-6">{children}</div>
    </div>
  )
}
...
export const SessionAuthGate: React.FC<SessionAuthGateProps> = ({ children }) => {
  ...
  const checkStatus = React.useCallback(async () => {
    ...
    try {
      const [response, latestPasskeyStatus] = await Promise.all([fetchSessionStatus(), refreshPasskeyStatus()])
      ...
      <AuthShell>
        <div className="flex flex-col items-center gap-6 w-full max-w-xs">
          <div className="flex flex-col items-center gap-1 text-center">
            <h1 className="text-xl font-semibold text-foreground">{t("sessionAuth.locked.unlockTitle")}</h1>
            <p className="typography-meta text-muted-foreground">{t("sessionAuth.locked.passwordDescription")}</p>
          </div>
              ...
              >
                ...
                <span>
                  {isPasskeyBusy && activePasskeyAction === "auth"
                    ...
                    : t("sessionAuth.actions.usePasskey")}
                </span>
              </Button>
            )}
