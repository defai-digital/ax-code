...
export function InlineCommentCard({ draft, onEdit, onDelete, className, maxWidth }: InlineCommentCardProps) {
  const { t } = useI18n()
      ...
      <div className="flex items-start justify-between gap-2 p-3">
        <div className="flex-1 min-w-0">
          ...
          <Collapsible open={isOpen || !isLongContent} onOpenChange={setIsOpen}>
            ...
            <CollapsibleContent>{/* Used for animation purposes if we want to animate height */}</CollapsibleContent>
          </Collapsible>
        </div>
