...
import { SKILL_LOCATION_OPTIONS, locationPartsFrom, locationValueFrom, type SkillLocationValue } from "./skillLocations"
...
const SkillsInstalledPage: React.FC = () => {
  ...
  const hasFileChanges = editingFilePath ? newFileContent !== originalFileContent : newFileName.trim() !== ""
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (success) {
        toast.success(
          isNewSkill ? t("settings.skills.page.toast.skillCreated") : t("settings.skills.page.toast.skillUpdated"),
        )
      } else {
        toast.error(
          isNewSkill
            ? t("settings.skills.page.toast.createSkillFailed")
            : t("settings.skills.page.toast.updateSkillFailed"),
        )
      }
    } catch (error) {
  ...
  const handleSaveFile = async () => {
    ...
    if (isNewSkill) {
      if (isEditing) {
        setPendingFiles((prev) =>
          prev.map((f) => (f.path === editingFilePath ? { path: filePath, content: newFileContent } : f)),
        )
    ...
    if (success) {
      toast.success(
        isEditing
          ? t("settings.skills.page.toast.fileUpdated", { path: filePath })
          : t("settings.skills.page.toast.fileCreated", { path: filePath }),
      )
      setIsFileDialogOpen(false)
      setEditingFilePath(null)
      ...
      toast.error(
        isEditing ? t("settings.skills.page.toast.updateFileFailed") : t("settings.skills.page.toast.createFileFailed"),
      )
    }
  }
    ...
    <ScrollableOverlay outerClassName="h-full" className="w-full">
        ...
        <div className="mb-8">
          ...
          <section className="px-2 pb-2 pt-0 space-y-0">
            {isNewSkill && (
              <div className="py-1.5">
                <span className="typography-ui-label text-foreground">
                  {t("settings.skills.page.field.skillNameLocation")}
                </span>
                <span className="typography-meta text-muted-foreground ml-2">
                  {t("settings.skills.page.field.skillNameHint")}
                </span>
                      ...
                      {SKILL_LOCATION_OPTIONS.map((option) => (
                        <SelectItem key={option.value} value={option.value}>
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="typography-micro text-muted-foreground ml-6">
            ...
            <div className="py-1.5">
              ...
              <span className="typography-meta text-muted-foreground ml-2">
                {t("settings.skills.page.field.descriptionHint")}
              </span>
        ...
        <div className="px-2 py-1">
          ...
          >
            {isSaving
              ...
              : isNewSkill
                ? t("settings.skills.page.actions.createSkill")
      ...
      >
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            <Button size="sm" variant="ghost" onClick={() => setDeleteFilePath(null)} disabled={isDeletingFile}>
      ...
      >
        <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>
              {editingFilePath
                ? t("settings.skills.page.fileDialog.titleEdit")
                : t("settings.skills.page.fileDialog.titleAdd")}
            </DialogTitle>
            <DialogDescription>
              {editingFilePath
                ? t("settings.skills.page.fileDialog.descriptionEdit")
                : t("settings.skills.page.fileDialog.descriptionAdd")}
            </DialogDescription>
          </DialogHeader>
          {isLoadingFile ? (
            <div className="flex-1 flex items-center justify-center py-8">
              <span className="typography-meta text-muted-foreground">
                {t("settings.skills.page.loading.fileContent")}
              </span>
            </div>
          ) : (
