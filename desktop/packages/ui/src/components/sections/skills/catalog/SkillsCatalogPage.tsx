    ...
    <ScrollableOverlay outerClassName="h-full" className="w-full">
      <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
        ...
        <div className="mb-4">
          ...
          <h2 className="typography-ui-header font-semibold text-foreground px-1">
        ...
        <div className="mb-8">
          ...
          <section className="px-2 pb-2 pt-0 space-y-0">
            <div className="flex flex-wrap items-center gap-2 py-1.5">
              <Select value={selectedSourceId || ""} onValueChange={(v) => setSelectedSource(v)}>
              ...
              >
                <Icon
                  name="refresh"
                  className={cn("h-3.5 w-3.5", (isLoadingCatalog || isLoadingSource) && "animate-spin")}
                />
              </Button>
              ...
              <Button size="xs" className="!font-normal gap-1" onClick={() => setAddCatalogOpen(true)}>
        ...
        {lastCatalogError && (
          <div className="mb-8 rounded-lg border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-4 py-3">
            <div className="typography-ui-label font-medium text-[var(--status-error)]">
              {t("settings.skills.catalog.page.error.catalogTitle")}
            </div>
                    ...
                    <div key={`${item.sourceId}:${item.skillDir}`} className="py-2">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {installed && (
                              <span className="typography-micro text-[var(--status-success)] bg-[var(--status-success)]/10 px-1.5 py-0.5 rounded flex-shrink-0">
                                {t("settings.skills.catalog.page.badge.installed", {
                                  scope: installedScope || t("settings.skills.catalog.page.badge.unknown"),
                                })}
                              </span>
                            )}
                            {!item.installable && (
                          ...
                          {item.description ? (
                            <div className="typography-meta text-muted-foreground mt-0.5 line-clamp-2">
                              {item.description}
                            </div>
                          ) : (
                          ...
                          {item.clawdhub && (
                            <div className="typography-micro text-muted-foreground mt-1.5 flex items-center gap-3">
                              {item.clawdhub.owner && (
                                <span>
                                  {t("settings.skills.catalog.page.byOwnerPrefix")}{" "}
                                  <span className="font-medium text-foreground/80">{item.clawdhub.owner}</span>
                                </span>
                              )}
                              ...
                              <span className="bg-[var(--surface-muted)] px-1.5 py-0.5 rounded">
                                v{item.clawdhub.version}
                              </span>
                            </div>
                          )}
