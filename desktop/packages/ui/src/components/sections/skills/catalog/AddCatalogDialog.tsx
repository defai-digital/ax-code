    ...
    if (!result.ok) {
      if (result.error?.kind === "authRequired") {
        ...
        if (!gitIdentityId && ids.length > 0) {
          const preferred =
            defaultGitIdentityId && defaultGitIdentityId !== "global" && ids.some((i) => i.id === defaultGitIdentityId)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {t("settings.skills.catalog.add.descriptionPrefix")} <code className="font-mono">SKILL.md</code>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="typography-ui-label text-foreground">
              {t("settings.skills.catalog.add.field.catalogName")}
            </label>
            <Input
              value={label}
          ...
          <div className="space-y-2">
            <label className="typography-ui-label text-foreground">
              {t("settings.skills.catalog.add.field.repository")}
            </label>
            <Input
              value={source}
              onChange={(e) => {
          ...
          <div className="space-y-2">
            <label className="typography-ui-label text-foreground">
              {t("settings.skills.catalog.add.field.optionalSubpath")}
            </label>
            <Input
              value={subpath}
              onChange={(e) => {
          ...
          {identityOptions.length > 0 ? (
            <div className="space-y-2">
              <div>
                <span className="typography-ui-label text-[var(--status-warning)]">
                  {t("settings.skills.catalog.shared.auth.title")}
                </span>
                <span className="typography-meta text-muted-foreground ml-2">
                  {t("settings.skills.catalog.shared.auth.description")}
                </span>
              </div>
              <Select
                value={gitIdentityId || ""}
              ...
              >
                <SelectTrigger className="w-fit">
                  <span>
                    {identityOptions.find((i) => i.id === gitIdentityId)?.name ||
                      t("settings.skills.catalog.shared.auth.chooseIdentity")}
                  </span>
                </SelectTrigger>
