...
export const DefaultsSettings: React.FC = () => {
  ...
  const formatVariantLabel = React.useCallback(
    (variant: string) => {
  ...
  const availableVariants = React.useMemo(() => {
    ...
    const model = provider?.models.find(
      (m: Record<string, unknown>) => (m as { id?: string }).id === parsedModel.modelId,
      ...
      <section className="px-2 pb-2 pt-0 space-y-0">
        <div className="mt-0 mb-1 typography-meta text-muted-foreground">
          ...
          {parsedModel.providerId ? (
            <span className="text-foreground">
              ...
              {supportsVariants
                ? ` (${defaultVariant ?? t("settings.openchamber.defaults.option.defaultLowercase")})`
                : ""}
            </span>
          ) : (
        ...
        <div className="flex flex-col gap-2 py-1 sm:flex-row sm:items-center sm:gap-8">
          ...
          <div className="flex items-center gap-2 sm:w-fit">
            <Select
              value={defaultVariant ?? DEFAULT_VARIANT_VALUE}
            ...
            >
              ...
              <SelectContent>
                <SelectItem value={DEFAULT_VARIANT_VALUE}>
                  {t("settings.openchamber.defaults.option.default")}
                </SelectItem>
        ...
        >
          <Checkbox
            ...
            onChange={setSettingsDefaultFileViewerPreview}
