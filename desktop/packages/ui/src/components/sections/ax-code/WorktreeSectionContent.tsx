...
export const WorktreeSectionContent: React.FC<WorktreeSectionContentProps> = ({
  ...
  const persistSetupCommands = React.useCallback(
  ...
  const handleRemoveCommand = React.useCallback(
    (index: number) => {
      setSetupCommands((prev) => {
  ...
  if (!projectPath) {
    return (
      <p className="typography-meta text-muted-foreground">{t("settings.openchamber.worktrees.state.selectProject")}</p>
    )
  }

  if (isGitRepoLocal === false) {
    return <p className="typography-meta text-muted-foreground">{t("settings.openchamber.worktrees.state.gitOnly")}</p>
  }
      ...
      <div className="space-y-2">
        <div className="mb-1 px-1">
          <div className="flex items-center gap-2">
            <h3 className="typography-ui-header font-normal text-foreground">
        ...
        {isLoadingCommands ? (
          ...
          <div className="space-y-2 px-1">
            ...
            <Button type="button" variant="ghost" size="xs" className="!font-normal" onClick={handleAddCommand}>
        ...
        {isLoadingWorktrees ? (
          ...
          <div className="space-y-1 px-1 max-w-[32.5rem]">
            {availableWorktrees.map((worktree) => (
              <div key={worktree.path} className="group flex w-full items-center gap-2 py-1.5">
                ...
                <button
                  ...
                  aria-label={t("settings.openchamber.worktrees.list.deleteWorktreeAria", {
                    name: worktree.branch || worktree.label || worktree.path,
                  })}
                >
