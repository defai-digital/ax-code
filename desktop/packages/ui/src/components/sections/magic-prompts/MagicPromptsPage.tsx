...
export const MagicPromptsPage: React.FC = () => {
  ...
  const getBaseline = React.useCallback(
    (id: MagicPromptId) => {
  ...
  const savePrompt = React.useCallback(
    async (id: MagicPromptId) => {
      ...
      try {
        const payload =
          value === getDefaultMagicPromptTemplate(id)
    ...
    <div className="h-full overflow-auto">
      <div className="mx-auto w-full max-w-4xl px-6 py-6 space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          >
            {resettingAll
              ? t("settings.magicPrompts.page.actions.resetting")
              : t("settings.magicPrompts.page.actions.resetAllOverrides")}
          </Button>
        </div>
            ...
            <section key={block.id} className={index > 0 ? "space-y-3 pt-5 border-t border-border" : "space-y-3"}>
              ...
              {isInvalidEmptyVisiblePrompt && (
                <div className="typography-micro text-[var(--status-error)]">
                  {t("settings.magicPrompts.page.validation.visiblePromptRequired")}
                </div>
              )}
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {resetting
                      ? t("settings.magicPrompts.page.actions.resetting")
                      : t("settings.magicPrompts.page.actions.resetToDefault")}
                  </Button>
                  <Button
                    size="sm"
                    onClick={() => {
