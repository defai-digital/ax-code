...
export const SnippetsSidebar: React.FC<SnippetsSidebarProps> = ({ onItemSelect }) => {
  ...
  const { selectedSnippetName, snippets, setSelectedSnippet, setSnippetDraft, deleteSnippet, loadSnippets } =
    useSnippetsStore(
      useShallow((s) => ({
  ...
  return (
    <div className={cn("flex h-full flex-col", "bg-background")}>
        ...
        {sortedSnippets.map((snippet) => (
          <div
            ...
            className={cn(
              ...
              selectedSnippetName === snippet.name ? "bg-interactive-selection" : "hover:bg-interactive-hover",
            )}
          >
            <button
              onClick={() => {
            ...
            <DropdownMenu
              open={openMenuName === snippet.name}
              onOpenChange={(open) => setOpenMenuName(open ? snippet.name : null)}
            >
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  aria-label={t("settings.snippets.sidebar.actions.more", { name: snippet.name })}
                >
              ...
              <DropdownMenuContent align="end" className="w-fit min-w-20">
                <DropdownMenuItem
                  onClick={(e) => {
                    e.stopPropagation()
                    setConfirmDeleteSnippet(snippet)
                  }}
      ...
      <Dialog
        open={confirmDeleteSnippet !== null}
        onOpenChange={(open) => {
          if (!open) setConfirmDeleteSnippet(null)
        }}
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("settings.snippets.sidebar.dialog.deleteDescription", { name: confirmDeleteSnippet?.name ?? "" })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button size="sm" variant="ghost" onClick={() => setConfirmDeleteSnippet(null)}>
