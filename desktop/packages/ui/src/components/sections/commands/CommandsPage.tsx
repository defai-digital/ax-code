...
export const CommandsPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (success) {
        toast.success(
          isNewCommand ? t("settings.commands.page.toast.created") : t("settings.commands.page.toast.updated"),
        )
      } else {
        toast.error(
          isNewCommand
            ? t("settings.commands.page.toast.createFailed")
            : t("settings.commands.page.toast.updateFailed"),
        )
      }
    } catch (error) {
        ...
        <div className="mb-8">
          ...
          <section className="px-2 pb-2 pt-0 space-y-0">
            <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
              ...
              <div className="flex min-w-0 flex-1 items-center gap-2 sm:w-fit sm:flex-initial">
                <AgentSelector agentName={agent} onChange={(agentName: string) => setAgent(agentName)} />
              </div>
            </div>
        ...
        <div className="mb-2">
          ...
          <div className="mt-2 px-2">
            <p className="typography-meta text-muted-foreground">
              ...
              &middot; <code className="text-foreground">!`cmd`</code>{" "}
              {t("settings.commands.page.templateHint.shellOutput")} &middot;{" "}
