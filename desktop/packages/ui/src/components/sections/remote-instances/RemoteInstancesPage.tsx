...
const normalizeForSave = (instance: DesktopSshInstance): DesktopSshInstance => {
  ...
  const forwards = instance.portForwards.map((forward) => ({
    ...
    localPort:
      typeof forward.localPort === "number" ? Math.max(1, Math.min(65535, Math.round(forward.localPort))) : undefined,
    remoteHost: forward.remoteHost?.trim(),
    remotePort:
      typeof forward.remotePort === "number" ? Math.max(1, Math.min(65535, Math.round(forward.remotePort))) : undefined,
  }))
              ...
              <div className="space-y-2">
                {importCandidates.map((candidate) => (
                  <div
                    ...
                    className="flex items-center justify-between gap-3 rounded-md border border-[var(--interactive-border)] px-3 py-2"
                  >
        ...
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <Icon
                name="refresh"
                className={`h-3.5 w-3.5 ${isConnecting || (isReconnecting && !reconnectAppearsStuck) ? "animate-spin" : ""}`}
              />
              {retryButtonLabel}
            </Button>
            <Button
        ...
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">
              {t("settings.remoteInstances.page.field.sshCommand")}
            </span>
            <Input
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">
              {t("settings.remoteInstances.page.field.nickname")}
            </span>
            <Input
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">
              {t("settings.remoteInstances.page.field.connectionTimeoutSeconds")}
            </span>
            <NumberInput
            ...
            <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="upload_bundle">
                    {t("settings.remoteInstances.page.field.installMethodUploadBundle")}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        ...
        <section className="px-2 pb-2 pt-0 space-y-2">
          ...
          {draft.portForwards.map((forward, index) => {
            ...
            const typeLabel =
              forward.type === "local"
                ? t("settings.remoteInstances.page.forwardType.local")
                : forward.type === "remote"
                  ? t("settings.remoteInstances.page.forwardType.remote")
                  : t("settings.remoteInstances.page.forwardType.dynamic")
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateForward((item) => ({ ...item, enabled: checked }))}
                      aria-label={t("settings.remoteInstances.page.actions.enableForwardAria")}
                    />
                    <Button
                      type="button"
                      variant="ghost"
                    ...
                    <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateForward((item) => ({
                            ...item,
                            type: (value === "dynamic" || value === "remote"
                              ? value
                              : "local") as DesktopSshPortForwardType,
                          }))
                        }
                      >
                        <SelectTrigger className="h-7 w-fit min-w-[140px]">
      ...
      <Dialog open={logDialogOpen} onOpenChange={setLogDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {draft?.nickname?.trim() ||
                draft?.sshParsed?.destination ||
                draft?.id ||
                t("settings.remoteInstances.page.logsDialog.selectedInstanceFallback")}
            </DialogDescription>
          </DialogHeader>
          <div className="flex items-center justify-end gap-2">
            <Button
              ...
              disabled={logDialogLoading || !logLinesText.trim()}
            >
              <Icon name="file-copy" className="h-3.5 w-3.5" />
