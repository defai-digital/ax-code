...
export const AgentsSidebar: React.FC<AgentsSidebarProps> = ({ onItemSelect }) => {
  ...
  const { selectedAgentName, agents, setSelectedAgent, setAgentDraft, createAgent, deleteAgent, loadAgents } =
    useAgentsStore(
      useShallow((s) => ({
  ...
  const handleDuplicateAgent = (agent: Agent) => {
    ...
    const modelStr =
      agent.model?.providerID && agent.model?.modelID ? `${agent.model.providerID}/${agent.model.modelID}` : null
    ...
    <div className={cn("flex h-full flex-col", bgClass)}>
            ...
            {customAgents.length > 0 && (
              <>
                ...
                {groupedCustomAgents.map(({ name: groupName, agents: groupAgents }) => (
                  <SidebarGroup key={groupName} label={groupName} count={groupAgents.length} storageKey="agents">
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {confirmActionType === "delete"
                ? t("settings.agents.sidebar.dialog.deleteTitle")
                : t("settings.agents.sidebar.dialog.resetTitle")}
            </DialogTitle>
            <DialogDescription>
              {confirmActionType === "delete"
          ...
          <DialogFooter>
            <Button size="sm" variant="ghost" onClick={closeConfirmActionDialog} disabled={isConfirmActionPending}>
      ...
      <Dialog open={renameDialogAgent !== null} onOpenChange={(open) => !open && setRenameDialogAgent(null)}>
        <DialogContent>
          ...
          />
          <DialogFooter>
            <Button size="sm" variant="ghost" onClick={() => setRenameDialogAgent(null)}>
              {t("settings.common.actions.cancel")}
            </Button>
      ...
      <div className="flex min-w-0 flex-1 items-center">
        ...
        >
          <div className="flex items-center gap-1.5">
            <span className="typography-ui-label font-normal truncate text-foreground">{agent.name}</span>
          ...
          {agent.description && (
            <div className="typography-micro text-muted-foreground/60 truncate leading-tight">{agent.description}</div>
          )}
        </button>
