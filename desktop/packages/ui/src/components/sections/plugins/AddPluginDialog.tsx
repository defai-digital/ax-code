...
export const AddPluginDialog: React.FC<AddPluginDialogProps> = ({ open, onOpenChange, defaultScope = "user" }) => {
  const { t } = useI18n()
    ...
    >
      <DialogContent className="max-w-xl">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <span className="typography-ui-label text-foreground">{t("settings.plugins.page.field.scope")}</span>
            <div className="flex items-center gap-4">
              {(["user", "project"] as const).map((value) => {
                const selected = scope === value
                const label = value === "user" ? t("settings.plugins.scope.user") : t("settings.plugins.scope.project")
                return (
                  <button
                    key={value}
                    type="button"
                  ...
                  >
                    <Radio checked={selected} onChange={() => setScope(value)} ariaLabel={label} />
                    <span
                      className={cn(
