...
export const PluginsPage: React.FC = () => {
  ...
  if (selectedEntry && draft && draft.mode === "entry") {
    ...
    const isDirty = draft.spec !== selectedEntry.spec || draft.optionsJson !== stringifyOptions(selectedEntry.options)
      ...
      try {
        ...
        if (result.ok) {
          if (result.reloadFailed) {
            toast.warning(result.message || t("settings.plugins.toast.reloadFailed"), { description: result.warning })
          } else {
    ...
    return (
      <SettingsPageLayout>
        ...
        <div className="space-y-1.5">
          <label className="typography-meta text-muted-foreground">{t("settings.plugins.page.field.spec")}</label>
          <Input
            value={draft.spec}
        ...
        <div className="space-y-1.5">
          <label className="typography-meta text-muted-foreground">{t("settings.plugins.page.field.options")}</label>
          <Textarea
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handleEntryDiscard} disabled={!isDirty || isSaving}>
  ...
  if (selectedFile && draft && draft.mode === "file") {
    ...
    return (
      <SettingsPageLayout>
        ...
        <div className="space-y-1.5">
          <label className="typography-meta text-muted-foreground">{t("settings.plugins.page.field.content")}</label>
          <Textarea
            value={draft.content}
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handleFileDiscard} disabled={isSaving || isLoadingFile}>
            {t("settings.plugins.page.action.discard")}
          </Button>
        </div>
      </SettingsPageLayout>
