...
type DeleteTarget = { kind: "entry"; id: string; label: string } | { kind: "file"; id: string; label: string } | null

const entryIcon = (entry: PluginEntry): IconName => (entry.parsedKind === "npm" ? "code-box" : "folder")

export const PluginsSidebar: React.FC<PluginsSidebarProps> = ({ onItemSelect, onAddClick }) => {
  const { t } = useI18n()

  const { entries, files, selectedId, setSelected, deleteEntry, deleteFile, loadPlugins } = usePluginsStore(
    useShallow((s) => ({
      entries: s.entries,
      files: s.files,
  ...
  const userEntries = React.useMemo(() => entries.filter((e) => e.scope === "user"), [entries])
  const projectEntries = React.useMemo(() => entries.filter((e) => e.scope === "project"), [entries])
  const userFiles = React.useMemo(() => files.filter((f) => f.scope === "user"), [files])
  const projectFiles = React.useMemo(() => files.filter((f) => f.scope === "project"), [files])
  ...
  const handleUpdateToLatest = React.useCallback(
    async (id: string) => {
      ...
      if (result.ok) {
        toast.success(t("settings.plugins.toast.updatedToLatest", { version: latest }))
      } else {
  ...
  const renderEntry = (entry: PluginEntry) => {
    const info = registryInfo[entry.spec]
    const canUpdate = info?.kind === "npm-ok" && info.hasUpdate && !!info.latestVersion
    const actions: Array<{
      label: string
      icon?: IconName
      destructive?: boolean
    ...
    actions.push({
      ...
      icon: "delete-bin",
      destructive: true,
      onClick: () => setDeleteTarget({ kind: "entry", id: entry.id, label: entry.spec }),
    })
    return (
      <SettingsSidebarItem
        key={entry.id}
        ...
        metadata={
          entry.parsedKind === "npm" ? t("settings.plugins.sidebar.kind.npm") : t("settings.plugins.sidebar.kind.path")
        }
      ...
      actions={[
        {
          ...
          icon: "delete-bin",
          destructive: true,
          onClick: () => setDeleteTarget({ kind: "file", id: file.id, label: file.fileName }),
        },
      ]}
    />
  )
          ...
          <div className="border-b px-3 pt-4 pb-3">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  <Icon name="refresh" className={isLoadingRegistry ? "size-4 animate-spin" : "size-4"} />
                </Button>
                <Button
                  type="button"
                  variant="ghost"
      ...
      >
        {isEmpty ? (
          ...
          <>
            ...
            {userFiles.length > 0 &&
              renderGroup(t("settings.plugins.sidebar.group.userFiles"), userFiles.map(renderFile))}
            ...
            {projectFiles.length > 0 &&
              renderGroup(t("settings.plugins.sidebar.group.projectFiles"), projectFiles.map(renderFile))}
          </>
        )}
      </SettingsSidebarLayout>
