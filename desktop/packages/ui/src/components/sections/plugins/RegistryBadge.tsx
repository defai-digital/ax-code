...
export const RegistryBadge: React.FC<RegistryBadgeProps> = ({ spec }) => {
  ...
  const wrap = (trigger: React.ReactNode, tooltipText: string): React.ReactElement => (
    <Tooltip>
      <TooltipTrigger asChild>
        <span className="inline-flex shrink-0 items-center gap-0.5 typography-micro">{trigger}</span>
      </TooltipTrigger>
  ...
  switch (info.kind) {
    ...
    case "npm-missing-version":
      return wrap(
        <Icon name="error-warning" className="h-3 w-3 text-[var(--status-warning)]" />,
