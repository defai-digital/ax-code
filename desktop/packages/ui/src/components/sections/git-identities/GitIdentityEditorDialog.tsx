  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (success) {
        toast.success(
          isNewProfile
            ? t("settings.gitIdentities.editor.toast.profileCreated")
            : t("settings.gitIdentities.editor.toast.profileUpdated"),
        )
        onOpenChange(false)
      } else {
        toast.error(
          isNewProfile
            ? t("settings.gitIdentities.editor.toast.createProfileFailed")
            : t("settings.gitIdentities.editor.toast.updateProfileFailed"),
        )
      }
    } catch (error) {
          ...
          <div className="space-y-5 py-2">
            ...
            {!isGlobalProfile && (
              <div className="space-y-3">
                <div>
                  <label className="typography-ui-label text-foreground block mb-1.5">
                    {t("settings.gitIdentities.editor.field.profileName")}
                  </label>
                  <Input
                    value={name}
                ...
                <div className="flex items-center justify-between gap-4">
                  <span className="typography-ui-label text-foreground">
                    {t("settings.gitIdentities.editor.field.color")}
                  </span>
                ...
                <div className="flex items-center justify-between gap-4">
                  <span className="typography-ui-label text-foreground">
                    {t("settings.gitIdentities.editor.field.icon")}
                  </span>
                  <div className="flex gap-1.5">
                    {PROFILE_ICONS.map((i) => {
            ...
            {!isGlobalProfile && (
              <>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-4">
                    <span className="typography-ui-label text-foreground">
                      {t("settings.gitIdentities.editor.field.authMethod")}
                    </span>
                    <div className="flex items-center gap-1">
                      ...
                      >
                        ...
                        {t("settings.gitIdentities.editor.field.authToken")}
                      </Button>
                    </div>
                  </div>
                  ...
                  {authType === "ssh" && (
                    <div>
                      <div className="flex items-center gap-1.5 mb-1.5">
                        <label className="typography-ui-label text-foreground">
                          {t("settings.gitIdentities.editor.field.sshKeyPath")}
                        </label>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Icon name="information" className="h-3.5 w-3.5 text-muted-foreground/60 cursor-help" />
            ...
            {!isGlobalProfile && (
              <Button size="sm" onClick={handleSave} disabled={isSaving}>
                {isSaving
                  ...
                  : isNewProfile
                    ? t("settings.gitIdentities.editor.actions.create")
                    : t("settings.gitIdentities.editor.actions.save")}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
