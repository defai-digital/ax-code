...
import { useMcpConfigStore, envRecordToArray, type McpDraft, type McpScope } from "@/stores/useMcpConfigStore"
import { useShallow } from "zustand/react/shallow"
import { parseImportedMcpSnippet, applyImportedMcpToDraft } from "./mcpImport"
...
const CommandTextarea: React.FC<CommandTextareaProps> = ({
  ...
  const handlePasteFromClipboard = async () => {
    try {
      ...
      const lines = trimmed.includes("\n") ? trimmed.split("\n").filter((l) => l.trim()) : parseShellCommand(trimmed)
      setText(lines.join("\n"))
      onChange(lines)
...
const EnvEditor: React.FC<EnvEditorProps> = ({
  ...
  return (
    <div className="space-y-2">
      ...
      <Button variant="outline" size="xs" className="!font-normal gap-1.5" onClick={addRow} type="button">
...
const StatusBadge: React.FC<{
  ...
  return <span className={cn("typography-micro font-medium", colors.text)}>● {getStatusLabel(status)}</span>
}
...
const shouldShowFullStatusCard = (
  status: string | undefined,
  authUrl: string | null,
...
const getPendingMcpAuthContext = async (
  stateKey: string,
...
const clearPendingMcpAuthContext = async (stateKey: string | null | undefined): Promise<void> => {
  ...
  await fetch(`${API_ENDPOINTS.mcp.authPending}?state=${encodeURIComponent(stateKey.trim())}`, {
    method: "DELETE",
  }).catch(() => undefined)
}
...
const buildMcpRuntimeActionKey = (name: string | null, directory?: string | null): string => {
  const normalizedDirectory = typeof directory === "string" && directory.trim() ? directory.trim() : "__global__"
...
export const McpPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (selectedServer) {
      ...
      const nextTimeout =
        typeof remoteServer?.timeout === "number" && Number.isFinite(remoteServer.timeout)
  ...
  const handleSave = async () => {
    ...
    if (isNewServer && mcpServers.some((s) => s.name === name)) {
      toast.error(t("settings.mcp.page.toast.serverNameExists"))
      return
    }
  ...
  const handleRefreshRuntimeStatus = React.useCallback(
    async (silent = false) => {
      try {
  ...
  const handleStartAuthorization = React.useCallback(async () => {
    ...
    try {
      const currentStatus = useMcpStore.getState().getStatusForDirectory(currentDirectory ?? null)[
        selectedMcpName
      ]?.status
      authPollStartsFromNeedsAuthRef.current =
        currentStatus === "needs_auth" || currentStatus === "needs_client_registration"
      ...
      if (!oauthRedirectUri.trim()) {
        ...
        if (saved.reloadFailed) {
          throw new Error(
            saved.warning || saved.message || t("settings.mcp.page.toast.axCodeReloadFailedAfterCallbackSave"),
          )
        }
  ...
  const handleCompleteAuthorization = React.useCallback(async () => {
    ...
    try {
      ...
      if (runtimeActionKeyRef.current === actionKey) {
        toast.error(
          normalizeMcpAuthErrorMessage(err, t("settings.mcp.page.toast.authorizationCompleteFailed"), tUnsafe),
        )
      }
    } finally {
      if (runtimeActionKeyRef.current === actionKey) {
  ...
  return (
    <ScrollableOverlay outerClassName="h-full" className="w-full">
      <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <p className="typography-meta text-muted-foreground truncate">
                {isNewServer
                  ...
                  : t("settings.mcp.page.header.transport", {
                      type:
                        mcpType === "local"
                ...
                <>
                  ...
                  {mcpType === "remote" && (
                    <>
                      ...
                      >
                        {isClearingAuth
                          ? t("settings.mcp.page.actions.clearing")
                          : t("settings.mcp.page.actions.clearAuth")}
                      </Button>
                    </>
                  )}
                  {isConnected && (
                    ...
                    >
                      {isTestingConnection
                        ? t("settings.mcp.page.actions.testing")
                        : t("settings.mcp.page.actions.test")}
                    </Button>
                  )}
                </>
              )}
        ...
        {!isNewServer &&
          shouldShowFullStatusCard(effectiveRuntimeStatus?.status, authUrl, needsAuthorization, isAuthPolling) && (
                  ...
                  <div className="min-w-0 space-y-1">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                      <span className="typography-ui-label text-foreground">
                        {t("settings.mcp.page.status.runtimeStatus")}
                      </span>
                      <StatusBadge
                        status={effectiveRuntimeStatus?.status}
                        enabled={enabled}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {!isConnected && (
                      ...
                      >
                        {isTestingConnection
                          ? t("settings.mcp.page.actions.testing")
                          : t("settings.mcp.page.actions.testConnection")}
                      </Button>
                    )}
                  </div>
                    ...
                    <div className="rounded-md border border-[var(--interactive-border)] bg-[var(--surface-background)] px-3 py-2">
                      <div className="space-y-2">
                        <div className="typography-micro text-muted-foreground">
                          {t("settings.mcp.page.auth.authorizationUrl")}
                        </div>
                      ...
                      <div className="space-y-2">
                        <div>
                          <div className="typography-ui-label text-foreground">
                            {t("settings.mcp.page.auth.manualFallbackTitle")}
                          </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          >
                            {isCompletingAuth
                              ? t("settings.mcp.page.actions.completing")
                              : t("settings.mcp.page.actions.completeAuthorization")}
                          </Button>
                        </div>
                      </div>
                    </div>
        ...
        <div className="mb-6">
          ...
          <section className="px-2 pb-2 pt-0 space-y-0">
                ...
                <div className="flex min-w-0 flex-1 items-center gap-2 sm:w-fit sm:flex-initial">
                  ...
                  <Select value={draftScope} onValueChange={(value) => setDraftScope(value as McpScope)}>
                    <SelectTrigger
                      ...
                      title={
                        draftScope === "user" ? t("settings.common.scope.global") : t("settings.common.scope.project")
                      }
                    >
                      {draftScope === "user" ? (
            ...
            >
              <Checkbox checked={enabled} onChange={setEnabled} ariaLabel={t("settings.mcp.page.server.enableAria")} />
            ...
            <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
              <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
                <span className="typography-ui-label text-foreground">
                  {t("settings.mcp.page.server.transportMode")}
                </span>
        ...
        <div className="mb-6">
          <div className="mb-1 px-1">
            <h3 className="typography-ui-header font-medium text-foreground">
              {mcpType === "local"
                ? t("settings.mcp.page.connection.command")
                : t("settings.mcp.page.connection.serverUrl")}
            </h3>
          </div>
        ...
        {mcpType === "remote" && (
          <div className="mb-6">
            <div className="mb-1 px-1">
              <h3 className="typography-ui-header font-medium text-foreground">
                {t("settings.mcp.page.advanced.title")}
              </h3>
            </div>

            <section className="px-2 pb-2 pt-0">
              <Collapsible open={isAdvancedRemoteOptionsOpen} onOpenChange={setIsAdvancedRemoteOptionsOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between py-0.5 group">
                  <div className="flex items-center gap-1.5 text-left">
                    <span className="typography-ui-label font-normal text-foreground">
                      {t("settings.mcp.page.advanced.configure")}
                    </span>
                    <span className="typography-micro text-muted-foreground">
                      (
                      {oauthEnabled
                        ? t("settings.mcp.page.advanced.autoDetect")
                        : t("settings.mcp.page.advanced.custom")}{" "}
                      · {headerEntries.length} {t("settings.mcp.page.advanced.headers")}
                      {timeout ? ` · ${timeout}ms` : ""})
                    </span>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-8">
                        <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
                          <span className="typography-ui-label text-foreground">
                            {t("settings.mcp.page.advanced.timeoutMs")}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Input
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        ...
                        />
                        <span className="typography-ui-label text-foreground">
                          {t("settings.mcp.page.advanced.oauthAutoDetection")}
                        </span>
                      </div>
                      ...
                      {suggestedRedirectUri && (
                        <p className="typography-micro text-muted-foreground">
                          ...
                          <span className="mt-1 block break-all font-mono text-foreground/80">
                            {suggestedRedirectUri}
                          </span>
                        </p>
                      )}
        ...
        <div className="mb-2">
          <div className="mb-1 px-1">
            <h3 className="typography-ui-header font-medium text-foreground">
              ...
              {envEntries.length > 0 && (
                <span className="ml-1.5 typography-micro text-muted-foreground font-normal">({envEntries.length})</span>
              )}
            </h3>
          </div>
        ...
        <div className="flex items-center gap-2 px-2 py-1">
          ...
          {!isNewServer && (
            <Button variant="destructive" size="xs" className="!font-normal" onClick={() => setShowDeleteConfirm(true)}>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{t("settings.mcp.page.importDialog.description")}</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3">
            ...
            {importError && <p className="typography-micro text-[var(--status-error)]">{importError}</p>}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleImportJson} disabled={!importJsonText.trim()} size="sm">
