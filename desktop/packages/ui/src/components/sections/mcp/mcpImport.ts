...
function buildError(message: string, parsed?: unknown): ImportedMcpError {
  return parsed !== undefined ? { ok: false, error: message, parsed } : { ok: false, error: message }
}
 ...
 *   { ...serverConfig }
 */
export function parseImportedMcpSnippet(raw: string, options?: { fallbackName?: string }): ImportedMcpOutcome {
  let parsed: unknown
  try {
  ...
  if (isPlainRecord(mcpServers)) {
    ...
    if (keys.length > 1) {
      return buildError("Paste one server at a time. Found " + keys.length + " servers in mcpServers", parsed)
    }
  ...
  if (single) {
    return buildResult(single.name, inferType(single.entry), single.entry)
  }

  // Treat top-level as a bare server config
  if (isServerConfig(obj)) {
    const type = inferType(obj)
    const name = typeof obj.name === "string" && obj.name.trim() ? obj.name.trim() : options?.fallbackName
    return buildResult(name, type, obj)
  }

  return buildError("No recognizable MCP server configuration found in JSON", parsed)
}
