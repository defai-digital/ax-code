...
async function ensureLocalBranch(repoRoot: string, candidate: string): Promise<string> {
  ...
  const hasLocal = await execCommand(`git show-ref --verify --quiet ${shellQuote(`refs/heads/${raw}`)}`, repoRoot)
  if (isOk(hasLocal)) {
    return raw
  }
...
async function createTempWorktree(repoRoot: string, targetBranch: string): Promise<string> {
  ...
  const tmp = await execCommand('mktemp -d "$HOME/.config/openchamber/tmp/oc-integrate-XXXXXX"', repoRoot)
  const tmpDir = stdoutText(tmp)
  if (!tmpDir) {
    throw new Error(stderrText(tmp) || "Failed to create temp directory")
  }
  const add = await execCommand(`git worktree add --force ${shellQuote(tmpDir)} ${shellQuote(targetBranch)}`, repoRoot)
  if (!isOk(add)) {
