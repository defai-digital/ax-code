...
const readTextFile = async (path: string): Promise<string | null> => {
  ...
  try {
    ...
    const response = await fetch(makeFsUrl(API_ENDPOINTS.fs.read, params), {
...
const sanitizeProjectActions = (value: unknown): OpenChamberProjectAction[] => {
  ...
  for (const entry of value) {
    ...
    const name = trimToMaxLength(
      ...
      AX_CODE_DESKTOP_PROJECT_ACTION_NAME_MAX_LENGTH,
    )
    const command = trimToMaxLength(
      typeof record.command === "string" ? record.command.trim() : "",
      AX_CODE_DESKTOP_PROJECT_ACTION_COMMAND_MAX_LENGTH,
    )
...
const sanitizeProjectActionsState = (
  value:
    | {
        actions?: unknown
    ...
    | null
    | undefined,
): OpenChamberProjectActionsState => {
  ...
  const primaryActionId = primaryRaw && actions.some((entry) => entry.id === primaryRaw) ? primaryRaw : null
...
const sanitizeProjectNotesAndTodos = (
  value:
    | {
        notes?: unknown
        todos?: unknown
...
const sanitizeProjectContextData = (
  value:
    | {
        notes?: unknown
        todos?: unknown
 ...
 */
export async function writeOpenChamberConfig(project: ProjectRef, config: OpenChamberConfig): Promise<boolean> {
  const projectDirectory = typeof project?.path === "string" ? project.path.trim() : ""
  if (!projectDirectory) {
    return false
  }
...
export async function deleteProjectPlanFile(project: ProjectRef, planId: string): Promise<boolean> {
 ...
 */
export function substituteCommandVariables(command: string, variables: { rootWorktreePath: string }): string {
  return (
    command
      // New preferred name
