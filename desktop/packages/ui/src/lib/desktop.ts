...
const getElectronRuntime = (): ElectronRuntimeGlobal | null => {
  ...
  return (
    (window as unknown as { __AX_CODE_DESKTOP_ELECTRON__?: ElectronRuntimeGlobal }).__AX_CODE_DESKTOP_ELECTRON__ ?? null
  )
}
...
export const isWebRuntime = (): boolean => {
  if (typeof window === "undefined") return false
  const apis = (window as { __AX_CODE_DESKTOP_RUNTIME_APIS__?: { runtime?: { platform?: string } } })
...
export const requestFileAccess = async (options?: {
...
export const startAccessingDirectory = async (directoryPath: string): Promise<{ success: boolean; error?: string }> => {
  void directoryPath
  return { success: true }
}

export const stopAccessingDirectory = async (directoryPath: string): Promise<{ success: boolean; error?: string }> => {
  void directoryPath
  return { success: true }
}
...
export const downloadDesktopUpdate = async (onProgress?: (progress: UpdateProgress) => void): Promise<boolean> => {
  if (!hasDesktopInvoke() || !isDesktopLocalOriginActive()) {
    return false
  }
...
export const saveDesktopMarkdownFile = async (defaultFileName: string, content: string): Promise<string | null> => {
  if (!isTauriShell() || !isDesktopLocalOriginActive()) {
    return null
  }
...
export const openDesktopFileInApp = async (filePath: string, appId: string, appName: string): Promise<boolean> => {
  if (!isTauriShell() || !isDesktopLocalOriginActive()) {
    return false
  }
