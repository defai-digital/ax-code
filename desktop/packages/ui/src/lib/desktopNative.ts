...
export const invokeDesktopCommand = async <TValue = unknown>(command: string, args?: InvokeArgs): Promise<TValue> => {
...
export const setDesktopWindowTheme = async (themeMode?: string, themeVariant?: string): Promise<void> => {
  if (!isDesktopShell()) {
    return
  }
...
export const readDesktopFile = async (path: string): Promise<{ mime: string; base64: string; size?: number }> => {
...
export const listenDesktopNativeDragDrop = async (handler: (event: unknown) => void): Promise<(() => void) | null> => {
