...
const getPreferredTerminalWsPath = (state: TerminalTransportGlobalState): string =>
  state.streamCapability?.ws?.path ?? state.inputCapability?.ws?.path ?? DEFAULT_TERMINAL_WS_PATH
  ...
  async sendInput(sessionId: string, data: string): Promise<boolean> {
    ...
    try {
      if (this.boundSessionId !== sessionId) {
        this.requestedSessionId = sessionId
        socket.send(
          encodeControlFrame({ t: "b", s: sessionId, r: this.replayCursorBySession.get(sessionId) ?? 0, v: 2 }),
        )
      }
      socket.send(data)
      return true
