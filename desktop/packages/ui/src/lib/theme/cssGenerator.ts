...
export class CSSVariableGenerator {
  ...
  private generateTailwindVariables(theme: Theme): string[] {
    ...
    if (sidebarBaseRgb) {
      vars.push(`  --sidebar-overlay-strong: rgb(${sidebarBaseRgb} / ${strongAlpha}) !important;`)
      vars.push(`  --sidebar-overlay-soft: rgb(${sidebarBaseRgb} / ${softAlpha}) !important;`)
    } else {
      const base = theme.colors.surface.muted
      vars.push(`  --sidebar-overlay-strong: ${this.opacity(base, strongAlpha)} !important;`)
      vars.push(`  --sidebar-overlay-soft: ${this.opacity(base, softAlpha)} !important;`)
    }
  ...
  private generateMarkdownColors(markdown: Record<string, string>, theme: Theme): string[] {
    ...
    vars.push(
      `  --markdown-link-hover: ${markdown.linkHover || theme.colors.primary.hover || this.darken(primary, 10)};`,
    )
  ...
  private generateToolColors(tools: Theme["colors"]["tools"], theme: Theme): string[] {
    ...
    vars.push(
      `  --tools-description: ${tools?.description || this.opacity(theme.colors.surface.mutedForeground, 0.6)};`,
    )
    ...
    if (tools?.edit) {
      ...
      vars.push(
        `  --tools-edit-line-number: ${tools.edit.lineNumber || this.opacity(theme.colors.surface.mutedForeground, 0.6)};`,
      )
    } else {
  ...
  private generateDefaultToolColors(theme: Theme): string[] {
    ...
    vars.push(`  --tools-edit-added: ${theme.colors.status.success};`)
    vars.push(
      `  --tools-edit-added-bg: ${this.addTransparency(this.removeTransparency(theme.colors.status.successBackground), 0.15)};`,
    )
    vars.push(`  --tools-edit-removed: ${theme.colors.status.error};`)
    vars.push(
      `  --tools-edit-removed-bg: ${this.addTransparency(this.removeTransparency(theme.colors.status.errorBackground), 0.15)};`,
    )
    vars.push(`  --tools-edit-modified: ${theme.colors.status.info};`)
    vars.push(
      `  --tools-edit-modified-bg: ${this.addTransparency(this.removeTransparency(theme.colors.status.infoBackground), 0.15)};`,
    )
    vars.push(`  --tools-edit-line-number: ${this.opacity(theme.colors.surface.mutedForeground, 0.6)};`)
