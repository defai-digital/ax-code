...
const setRootDeviceAttributes = (isTauriShellRuntime: boolean, deviceType: DeviceType, hasTouchInput: boolean) => {
  ...
  root.classList.add(
    deviceType === "mobile" ? "device-mobile" : deviceType === "tablet" ? "device-tablet" : "device-desktop",
  )
...
export function isMobileDeviceViaCSS(): boolean {
  ...
  const root = document.documentElement
  const isMobileValue =
    root.style.getPropertyValue("--is-mobile") || getComputedStyle(root).getPropertyValue("--is-mobile")
