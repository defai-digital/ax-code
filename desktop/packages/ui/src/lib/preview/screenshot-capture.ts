...
export const renderPreviewScreenshot = async (
  ...
  if (typeof tauri?.core?.invoke === "function") {
    try {
      ...
      const capture = await tauri.core.invoke<{ mime: string; base64: string; width: number; height: number }>(
      ...
      context.fillRect(
        target.bounds.x * xScale,
        target.bounds.y * yScale,
        target.bounds.width * xScale,
        target.bounds.height * yScale,
      )
      context.strokeRect(
        target.bounds.x * xScale,
        target.bounds.y * yScale,
...
const tagFixedElementsForClonePinning = (doc: Document, scrollX: number, scrollY: number): (() => void) => {
  ...
  try {
    ...
    /* best-effort: leave fixed elements untagged */
  }
...
const snapdomFixedPinPlugin = {
  ...
  afterClone(context: { clone?: Element | null }): void {
    ...
    for (const el of (clone as Element).querySelectorAll<HTMLElement>(`[${FIXED_PIN_ATTR}]`)) {
      ...
      try {
        spec = JSON.parse(el.getAttribute(FIXED_PIN_ATTR) || "")
      } catch {
        continue
      }
...
const captureNestedScrollState = (
  doc: Document,
): { reapply: () => void; cleanup: () => void; snapshot: ScrolledElementInfo[] } => {
  ...
  try {
    ...
    /* best-effort: no nested scroll preservation */
  }
  const reapply = () => {
  ...
  const cleanup = () => {
    for (const entry of entries) {
      ...
      try {
        entry.el.removeAttribute(NESTED_SCROLL_ATTR)
      } catch {
        /* best-effort */
      }
    }
...
const snapdomNestedScrollPlugin = {
  ...
  afterClone(context: { clone?: Element | null }): void {
    ...
    for (const el of (clone as Element).querySelectorAll<HTMLElement>(`[${NESTED_SCROLL_ATTR}]`)) {
      ...
      try {
        spec = JSON.parse(el.getAttribute(NESTED_SCROLL_ATTR) || "")
      } catch {
...
const preserveSingleLineTextElements = (
  ...
  const controlsSelector =
    'button, a, summary, label, [role="button"], [role="link"], [role="menuitem"], [role="tab"], nav *, header *'
...
const inlineIframeCaptureAssets = async (
  ...
  for (const picture of pictures) {
    ...
    const previous = sources.map((source) => ({
      source,
  ...
  await Promise.all(
    elements.map(async (element) => {
      ...
      restoreCallbacks.push(() => {
        element.style.backgroundImage = previous
      })
...
async function captureIframeSnapdomScreenshot(
  ...
  try {
    ...
    const scrollX = Math.max(
      windowScrollX,
      pageScrollX,
      visualViewportScrollX,
      rootScrollX,
      bodyScrollX,
      scrollingElementScrollX,
    )
    const scrollY = Math.max(
      windowScrollY,
      pageScrollY,
      visualViewportScrollY,
      rootScrollY,
      bodyScrollY,
      scrollingElementScrollY,
    )
    previewCaptureDebug("scroll sources", {
      ...
      resolvedScrollX: scrollX,
      resolvedScrollY: scrollY,
    ...
    let restoreAssets = () => {
      /* no-op until capture preparation mutates DOM */
    }
    let restoreFixedElements = () => {
      /* no-op until fixed elements are tagged */
    }
    try {
      await document.fonts?.ready.catch(() => undefined)
      restoreAssets = await inlineIframeCaptureAssets(document, viewportWidth, viewportHeight, {
        applyLayoutWorkarounds: false,
      })
