...
const requestChatForceScrollBottom = (sessionId: string) => {
  ...
  window.dispatchEvent(
    new CustomEvent("openchamber:chat-force-scroll-bottom", {
      detail: { sessionId },
    }),
  )
}
...
export async function getGitStatus(
  directory: string,
  options?: { mode?: "light" },
): Promise<import("./api/types").GitStatus> {
  const runtime = getRuntimeGit()
...
export async function getGitDiff(
  directory: string,
  options: import("./api/types").GetGitDiffOptions,
...
export async function deleteGitBranch(
  directory: string,
  payload: import("./api/types").GitDeleteBranchPayload,
...
export async function deleteRemoteBranch(
  directory: string,
  payload: import("./api/types").GitDeleteRemoteBranchPayload,
): Promise<{ success: boolean }> {
  const runtime = getRuntimeGit()
...
export async function popGitStash(
  directory: string,
  options: { ref: string },
): Promise<{ success: boolean; ref: string }> {
  const runtime = getRuntimeGit()
...
export async function createGitIdentity(
  profile: import("./api/types").GitIdentityProfile,
): Promise<import("./api/types").GitIdentityProfile> {
  const runtime = getRuntimeGit()
...
export async function updateGitIdentity(
  id: string,
  updates: import("./api/types").GitIdentityProfile,
): Promise<import("./api/types").GitIdentityProfile> {
  const runtime = getRuntimeGit()
...
export async function getCurrentGitIdentity(
  directory: string,
): Promise<import("./api/types").GitIdentitySummary | null> {
  const runtime = getRuntimeGit()
...
export async function cherryPick(directory: string, hash: string): Promise<import("./api/types").CherryPickResponse> {
  const runtime = getRuntimeGit()
