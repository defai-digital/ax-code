...
const parseInstance = (value: unknown): DesktopSshInstance | null => {
  ...
  const preferredLocalPort = readNumber(localRaw, "preferredLocalPort") ?? readNumber(localRaw, "preferred_local_port")
  ...
  return {
    ...
    connectionTimeoutSec:
      readNumber(value, "connectionTimeoutSec") ?? readNumber(value, "connection_timeout_sec") ?? 60,
    remoteOpenchamber: {
      mode,
      ...
      installMethod,
      uploadBundleOverSsh:
        readBoolean(remoteRaw, "uploadBundleOverSsh") ?? readBoolean(remoteRaw, "upload_bundle_over_ssh") ?? false,
    },
    localForward: {
...
const parseStatus = (value: unknown): DesktopSshInstanceStatus | null => {
  ...
  return {
    ...
    requiresUserAction: readBoolean(value, "requiresUserAction") ?? readBoolean(value, "requires_user_action") ?? false,
...
export const desktopSshStatus = async (id?: string): Promise<DesktopSshInstanceStatus[]> => {
  ...
  if (!Array.isArray(raw)) return []
  return raw.map((item) => parseStatus(item)).filter((item): item is DesktopSshInstanceStatus => Boolean(item))
}
