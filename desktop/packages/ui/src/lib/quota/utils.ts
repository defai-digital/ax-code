...
export const formatQuotaValueLabel = (valueLabel: string | null | undefined, percent: number | null): string => {
  return valueLabel ?? formatPercent(percent)
}

export const formatQuotaResetLabel = (resetAt: number | null, fallback?: string | null): string => {
  if (!resetAt) {
    return fallback ?? ""
  }
...
export const calculatePace = (
  ...
  const predictText =
    predictedFinalPercent > 100 ? `+${Math.round(predictedFinalPercent)}%` : `${Math.round(predictedFinalPercent)}%`
