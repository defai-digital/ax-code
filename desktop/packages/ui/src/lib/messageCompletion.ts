...
export function getLatestAssistantMessageId(messages: MessageRecord[]): string | null {
  ...
  return assistantMessages.length > 0 ? assistantMessages[assistantMessages.length - 1].info.id : null
}
...
export function shouldContinueStreaming(messages: MessageRecord[], currentStreamingId: string | null): boolean {
  ...
  const latestMessage = messages.find((msg) => msg.info.id === latestId && !isFullySyntheticMessage(msg.parts))
  if (!latestMessage) {
    return false
  }
