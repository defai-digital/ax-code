...
describe("updateStreamingState", () => {
  ...
  test("does not mark a previous assistant message as streaming during a new user turn", () => {
    updateStreamingState(stateWithMessages([message("msg_user_1", "user"), message("msg_assistant_1", "assistant")]))
  ...
  test("completes the streaming message when the session becomes idle", () => {
    ...
    updateStreamingState(
      stateWithMessages([message("msg_user_1", "user"), message("msg_assistant_1", "assistant")], {
        type: "idle",
      } as SessionStatus),
    )
