...
const fallbackThemeForVariant = (variant: "light" | "dark"): Theme => getDefaultTheme(variant === "dark")
...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const updateBrowserChrome = useCallback((theme: Theme) => {
    ...
    const mediaQuery =
      theme.metadata.variant === "dark" ? "(prefers-color-scheme: dark)" : "(prefers-color-scheme: light)"
    ...
    const handleStorage = (event: StorageEvent) => {
      ...
      setPreferences((prev) => {
        ...
        const nextMode: ThemeMode =
          nextModeRaw === "light" || nextModeRaw === "dark" || nextModeRaw === "system" ? nextModeRaw : prev.themeMode

        const nextLightRaw = localStorage.getItem("lightThemeId")
        const nextLight =
          typeof nextLightRaw === "string" && nextLightRaw.trim().length > 0 ? nextLightRaw.trim() : prev.lightThemeId

        const nextDarkRaw = localStorage.getItem("darkThemeId")
        const nextDark =
          typeof nextDarkRaw === "string" && nextDarkRaw.trim().length > 0 ? nextDarkRaw.trim() : prev.darkThemeId
  ...
  useEffect(() => {
    ...
    const handleSettingsSynced = (event: Event) => {
      ...
      setPreferences((prev) => {
        ...
        const same = nextMode === prev.themeMode && nextLight === prev.lightThemeId && nextDark === prev.darkThemeId
  ...
  const setSystemPreferenceHandler = useCallback(
    (use: boolean) => {
      ...
      const fallbackMode: ThemeMode = currentTheme.metadata.variant === "dark" ? "dark" : "light"
      setPreferences((prev) => {
  ...
  const setLightThemePreference = useCallback(
    (themeId: string) => {
      const theme = availableThemes.find(
        (candidate) => candidate.metadata.id === themeId && candidate.metadata.variant === "light",
      )
      if (!theme) {
        return
      }
  ...
  const setDarkThemePreference = useCallback(
    (themeId: string) => {
      const theme = availableThemes.find(
        (candidate) => candidate.metadata.id === themeId && candidate.metadata.variant === "dark",
      )
      if (!theme) {
        return
      }
