...
const BOTTOM_SPACER_DESKTOP_VH = 0.1
...
const isAtBottomSnapshot = (
  snapshot: NonNullable<SessionMemoryState["scrollPosition"]>,
  isMobile: boolean,
): boolean => {
...
export const useChatAutoFollow = ({
  ...
  const writeScrollTopInstant = React.useCallback(
    (target: number) => {
      const container = scrollRef.current
      if (!container) return
  ...
  const unseenCount = state === "released" ? Math.max(0, sessionMessageCount - unseenBaseRef.current) : 0
