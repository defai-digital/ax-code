...
export const useFileSystemAccess = () => {
  ...
  const requestAccess = useCallback(
    async (directoryPath: string): Promise<{ success: boolean; path?: string; projectId?: string; error?: string }> => {
      if (!isDesktop) {
  ...
  const startAccessing = useCallback(
    async (directoryPath: string): Promise<{ success: boolean; error?: string }> => {
      if (!isDesktop) {
        return { success: true }
      }
  ...
  const stopAccessing = useCallback(
    async (directoryPath: string): Promise<{ success: boolean; error?: string }> => {
      if (!isDesktop) {
        return { success: true }
      }
