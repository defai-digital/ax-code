...
const decodeParsedStatus = (signature: string): ParsedStatusResult => {
  const [activePartType, activeToolName, statusText = "working", isGenericStatus] =
    signature.split(STATUS_SIGNATURE_SEPARATOR)
  return {
    activePartType:
      activePartType === "text" ||
      activePartType === "tool" ||
...
export function useAssistantStatus(): AssistantStatusSnapshot {
  ...
  const sessionRetryAttempt =
    currentSessionStatus?.type === "retry"
      ? (currentSessionStatus as { type: "retry"; attempt?: number }).attempt
      : undefined

  const sessionRetryNext =
    currentSessionStatus?.type === "retry" ? (currentSessionStatus as { type: "retry"; next?: number }).next : undefined
  ...
  const baseWorking = React.useMemo<WorkingSummary>(() => {
    ...
    const retryInfo = isRetry ? { attempt: sessionRetryAttempt, next: sessionRetryNext } : null
