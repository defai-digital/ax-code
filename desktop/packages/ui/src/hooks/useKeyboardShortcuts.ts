...
export const useKeyboardShortcuts = () => {
  ...
  React.useEffect(() => {
    ...
    const isTerminalEventTarget = (target: EventTarget | null) => {
      ...
      return Boolean(
        target.closest(".terminal-viewport-container") || target.getAttribute("data-terminal-hidden-input") === "true",
      )
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const cycleAgentBackwardCombo =
        cycleAgentCombo && !cycleAgentCombo.includes("shift") ? normalizeCombo(`shift+${cycleAgentCombo}`) : ""
      const cycleAgentDirection =
        cycleAgentBackwardCombo && eventMatchesShortcut(e, cycleAgentBackwardCombo)
          ? -1
      ...
      if (cycleAgentDirection !== 0) {
        ...
        const hasOverlay =
          isSettingsDialogOpen || isCommandPaletteOpen || isHelpDialogOpen || isSessionSwitcherOpen || isAboutDialogOpen
      ...
      if (eventMatchesShortcut(e, combo("cycle_thinking_variant"))) {
        ...
        if (sessionId && agentName && providerId && modelId) {
          useSelectionStore
            .getState()
            .saveAgentModelVariantForSession(sessionId, agentName, providerId, modelId, nextVariant)
        }
