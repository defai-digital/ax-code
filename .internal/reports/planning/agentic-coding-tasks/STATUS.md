...
| Active phase | Phase 4 — cancel / concurrency / restart |
| Overall | Phase 3 shipped; next is cascade cancel |
...
```bash
cd packages/ax-code
AX_TEST_FILES=test/cli/tui/subagent-status-view.test.ts,test/tool/task.test.ts,test/session/task-queue.test.ts,test/session/background-subagent-handoff.test.ts,test/control-plane/autonomous-completion-gate.test.ts pnpm exec vitest run
```
