...
| Inventory ID | W4-02 |
| Source files scanned | 50 (10261 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/server` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/server/constants.ts`, `packages/ax-code/src/server/error.ts`, `packages/ax-code/src/server/event.ts`, `packages/ax-code/src/server/ipc-protocol.ts`, `packages/ax-code/src/server/ipc-transport.ts`, `packages/ax-code/src/server/listen-security.ts`, `packages/ax-code/src/server/mdns.ts`, `packages/ax-code/src/server/middleware.ts`, `packages/ax-code/src/server/request-directory.ts`, `packages/ax-code/src/server/routes/app-context-checks.ts`, `packages/ax-code/src/server/routes/app-context-schema.ts`, `packages/ax-code/src/server/routes/app-context-templates.ts` | 50 files |
| Tests | `packages/ax-code/test/control-plane/workspace-server-sse.test.ts`, `packages/ax-code/test/lsp/server-config.test.ts`, `packages/ax-code/test/lsp/server-defs.test.ts`, `packages/ax-code/test/lsp/server-helpers.test.ts`, `packages/ax-code/test/lsp/server-profile.test.ts`, `packages/ax-code/test/server/app-context-routes.test.ts`, `packages/ax-code/test/server/audit-route.test.ts`, `packages/ax-code/test/server/capability.test.ts` | 25 matched |
| Prior art | `.internal/reports/reviews/2026-07-19-code-quality-stability-review.md` | linked |

### Public API
Scanned 50 source files for exports/routes/commands.
...
Static signals: emptyCatch=2, todo=0, asAny=2
...
TODO density: 0. Residual empty-catch candidates: packages/ax-code/src/server/ipc-transport.ts:319; packages/ax-code/src/server/runtime-adapter.ts:132. Not auto-accepted without reachability proof.

## 7. Test coverage map

| Risk path | Existing test | Gap |
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/control-plane/workspace-server-sse.test.ts` | ok |
...
|------|------|------|----------|
| Reviewer | codex-sol | 2026-08-11 | Protocol complete; 50 files scanned |
