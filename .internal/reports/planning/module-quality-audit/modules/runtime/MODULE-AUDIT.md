...
| Inventory ID | W2-02 |
| Source files scanned | 19 (2524 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/runtime` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/runtime/debug-snapshot.ts`, `packages/ax-code/src/runtime/events.ts`, `packages/ax-code/src/runtime/failure-class.ts`, `packages/ax-code/src/runtime/headless/command.ts`, `packages/ax-code/src/runtime/headless/effects.ts`, `packages/ax-code/src/runtime/headless/event-log.ts`, `packages/ax-code/src/runtime/headless/event-sink-node.ts`, `packages/ax-code/src/runtime/headless/event-sink.ts`, `packages/ax-code/src/runtime/headless/event.ts`, `packages/ax-code/src/runtime/headless/index.ts`, `packages/ax-code/src/runtime/headless/projection.ts`, `packages/ax-code/src/runtime/headless/replay.ts` | 19 files |
| Tests | `packages/ax-code/test/cli/runtime-restart.test.ts`, `packages/ax-code/test/cli/tui/sync-runtime-adapter.test.ts`, `packages/ax-code/test/cli/tui/sync-runtime-probe.test.ts`, `packages/ax-code/test/cli/tui/sync-runtime-store.test.ts`, `packages/ax-code/test/cli/tui/sync-runtime-sync.test.ts`, `packages/ax-code/test/debug-engine/runtime-debug.test.ts`, `packages/ax-code/test/harness/agentic-runtime-eval.test.ts`, `packages/ax-code/test/installation/runtime-mode.test.ts` | 25 matched |
| Prior art | `.internal/reports/reviews/2026-07-19-code-quality-stability-review.md` | linked |

### Public API
Scanned 19 source files for exports/routes/commands.
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/cli/runtime-restart.test.ts` | ok |
...
|------|------|------|----------|
| Reviewer | codex-sol | 2026-08-11 | Protocol complete; 19 files scanned |
