...
| Inventory ID | W9-17 |
| Source files scanned | 4 (1359 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `crates/ax-code-diff` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `crates/ax-code-diff/build.rs`, `crates/ax-code-diff/examples/bench.rs`, `crates/ax-code-diff/src/helpers.rs`, `crates/ax-code-diff/src/lib.rs` | 4 files |
| Tests | `packages/ax-code/test/cli/tui/k-diff-viewer-lcs-budget.test.ts`, `packages/ax-code/test/quality/pr-diff.test.ts`, `packages/ax-code/test/session/diff-recovery.test.ts`, `desktop/packages/ui/src/components/views/diffHunkRevert.test.ts` | 4 matched |
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/cli/tui/k-diff-viewer-lcs-budget.test.ts` | ok |
