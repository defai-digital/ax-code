...
| Inventory ID | W3-13 |
| Source files scanned | 2 (218 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/image` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/image/index.ts`, `packages/ax-code/src/image/provider.ts` | 2 files |
| Tests | `packages/ax-code/test/image/provider.test.ts`, `packages/ax-code/test/tool/image-gen.test.ts` | 2 matched |
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/image/provider.test.ts` | ok |
