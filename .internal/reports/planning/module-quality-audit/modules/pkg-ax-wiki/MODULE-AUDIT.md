...
| Inventory ID | W9-08 |
| Source files scanned | 21 (1960 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-wiki` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-wiki/package.json`, `packages/ax-wiki/src/agents.ts`, `packages/ax-wiki/src/artifacts.ts`, `packages/ax-wiki/src/build.ts`, `packages/ax-wiki/src/discovery.ts`, `packages/ax-wiki/src/frontmatter.ts`, `packages/ax-wiki/src/glob.ts`, `packages/ax-wiki/src/hash.ts`, `packages/ax-wiki/src/index.ts`, `packages/ax-wiki/src/paths.ts`, `packages/ax-wiki/src/plan.ts`, `packages/ax-wiki/src/protected.ts` | 21 files |
| Tests | none auto-matched | 0 matched |
| Prior art | `.internal/reports/reviews/2026-07-19-code-quality-stability-review.md` | linked |

### Public API
Scanned 21 source files for exports/routes/commands.
...
|------|------|------|----------|
| Reviewer | ax-code-glm | 2026-08-11 | Protocol complete; 21 files scanned |
