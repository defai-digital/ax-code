...
| Inventory ID | W2-01a |
| Source files scanned | 135 (26283 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/session (prompt/processor)` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/session/agent-optimization-trace.ts`, `packages/ax-code/src/session/blast-radius.ts`, `packages/ax-code/src/session/branch.ts`, `packages/ax-code/src/session/compaction.ts`, `packages/ax-code/src/session/compare.ts`, `packages/ax-code/src/session/context-tier.ts`, `packages/ax-code/src/session/correction/detector.ts`, `packages/ax-code/src/session/correction/index.ts`, `packages/ax-code/src/session/correction/reflection.ts`, `packages/ax-code/src/session/cycle-detection.ts`, `packages/ax-code/src/session/debug.ts`, `packages/ax-code/src/session/decision-hints.ts` | 135 files |
| Tests | `packages/ax-code/test/acp/agent-prompt.test.ts`, `packages/ax-code/test/acp/session-list.test.ts`, `packages/ax-code/test/cli/cmd/tui/prompt-part.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/autonomous-active.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/coalesce.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/footer-view-model.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/format.test.ts`, `packages/ax-code/test/cli/github-agent-prompts.test.ts` | 25 matched |
| Prior art | `.internal/reports/reviews/2026-07-19-code-quality-stability-review.md` | linked |

### Public API
Scanned 135 source files for exports/routes/commands.
...
Static signals: emptyCatch=0, todo=0, asAny=7
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/acp/agent-prompt.test.ts` | ok |

## 8. Finding register

| Finding | Category | Severity | Origin | Status |
|---------|----------|----------|--------|--------|
| AUDIT-session-prompt-processor-001 | stability | Critical | prior-review | verified-fixed |
...
|------|------|------|----------|
| Reviewer | codex-sol | 2026-08-11 | Protocol complete; 135 files scanned |
