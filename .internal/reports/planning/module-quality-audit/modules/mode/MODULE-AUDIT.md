...
| Inventory ID | W5-13 |
| Source files scanned | 15 (1917 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/mode` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/mode/arena.ts`, `packages/ax-code/src/mode/budget.ts`, `packages/ax-code/src/mode/council.ts`, `packages/ax-code/src/mode/debate.ts`, `packages/ax-code/src/mode/ensemble-shared.ts`, `packages/ax-code/src/mode/hybrid.ts`, `packages/ax-code/src/mode/implement-arena.ts`, `packages/ax-code/src/mode/index.ts`, `packages/ax-code/src/mode/json-mode-prompt.ts`, `packages/ax-code/src/mode/memory.ts`, `packages/ax-code/src/mode/policy.ts`, `packages/ax-code/src/mode/preflight.ts` | 15 files |
| Tests | `packages/ax-code/test/cli/cmd/tui/routes/session/footer-view-model.test.ts`, `packages/ax-code/test/cli/prompt-view-model.test.ts`, `packages/ax-code/test/cli/tui/dialog-help-view-model.test.ts`, `packages/ax-code/test/cli/tui/dialog-model-options.test.ts`, `packages/ax-code/test/cli/tui/dialog-select-view-model.test.ts`, `packages/ax-code/test/cli/tui/directory-view-model.test.ts`, `packages/ax-code/test/cli/tui/footer-view-model.test.ts`, `packages/ax-code/test/cli/tui/input-mode.test.ts` | 25 matched |
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/cli/cmd/tui/routes/session/footer-view-model.test.ts` | ok |
...
|------|------|------|----------|
| Reviewer | ax-code-glm | 2026-08-11 | Protocol complete; 15 files scanned |
