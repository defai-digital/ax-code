...
| Inventory ID | W3-05 |
| Source files scanned | 2 (534 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/pty` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/pty/index.ts`, `packages/ax-code/src/pty/schema.ts` | 2 files |
| Tests | `packages/ax-code/test/pty/pty-output-isolation.test.ts`, `packages/ax-code/test/pty/pty-session.test.ts`, `packages/ax-code/test/session/prompt-loop-empty-turn.test.ts` | 3 matched |
...
Static signals: emptyCatch=3, todo=0, asAny=0
...
TODO density: 0. Residual empty-catch candidates: packages/ax-code/src/pty/index.ts:192; packages/ax-code/src/pty/index.ts:278; packages/ax-code/src/pty/index.ts:282. Not auto-accepted without reachability proof.

## 7. Test coverage map

| Risk path | Existing test | Gap |
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/pty/pty-output-isolation.test.ts` | ok |
