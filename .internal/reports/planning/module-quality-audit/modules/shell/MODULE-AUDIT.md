...
Owns `packages/ax-code/src/shell` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/shell/shell.ts` | 1 files |
| Tests | `packages/ax-code/test/runtime/shell-env.test.ts`, `packages/ax-code/test/session/prompt-shell-command.test.ts`, `packages/ax-code/test/shell/shell.test.ts`, `packages/ax-code/test/support/bun-shell.ts`, `packages/ax-code/test/util/shell-args.test.ts` | 5 matched |
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/runtime/shell-env.test.ts` | ok |
