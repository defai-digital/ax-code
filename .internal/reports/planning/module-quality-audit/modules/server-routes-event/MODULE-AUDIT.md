# MODULE-AUDIT: server-routes-event

| Field | Value |
|-------|-------|
| Unit slug | `server-routes-event` |
| Scope | `packages/ax-code/src/server/routes/event.ts` |
...
| Inventory ID | W4-03-10 |
| Source files scanned | 1 (135 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/server/routes/event.ts` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/server/routes/event.ts` | 1 files |
| Tests | `packages/ax-code/test/acp/event-subscription.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/autonomous-active.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/coalesce.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/footer-view-model.test.ts`, `packages/ax-code/test/cli/cmd/tui/routes/session/format.test.ts`, `packages/ax-code/test/cli/tui/coalesce-stream-events.test.ts`, `packages/ax-code/test/cli/tui/event-source-wire-death.test.ts`, `packages/ax-code/test/cli/tui/sync-store-event.test.ts` | 25 matched |
