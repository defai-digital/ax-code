...
| Source files scanned | 1 (585 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `desktop/packages/ui/src/components/auth` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `desktop/packages/ui/src/components/auth/SessionAuthGate.tsx` | 1 files |
| Tests | `packages/ax-code/test/auth/auth.test.ts`, `packages/ax-code/test/auth/encryption.test.ts`, `packages/ax-code/test/cli/plugin-auth-picker.test.ts`, `packages/ax-code/test/cli/tui/c-dialog-provider-auth-errors.test.ts`, `packages/ax-code/test/fixture/workflow/verified-bug-sweep-seeded/src/auth.ts`, `packages/ax-code/test/mcp/auth.test.ts`, `packages/ax-code/test/mcp/oauth-auto-connect.test.ts`, `packages/ax-code/test/mcp/oauth-browser.test.ts` | 25 matched |
