# AUDIT-desktop-web-event-stream-empty-catch

| Field | Value |
|-------|-------|
| Title | 10 empty catch site(s) remain (best-effort/deferred) |
| Category | silent-error |
| Severity | Medium |
| Origin | new |
| Status | deferred |
| Module | desktop-web-event-stream |
| Evidence | desktop/packages/web/server/lib/event-stream/directory-ws-bridge.js:50; desktop/packages/web/server/lib/event-stream/directory-ws-bridge.js:60; desktop/packages/web/server/lib/event-stream/directory-ws-bridge.js:216; desktop/packages/web/server/lib/event-stream/global-ws-bridge.js:66; desktop/packages/web/server/lib/event-stream/global-ws-bridge.js:163; desktop/packages/web/server/lib/event-stream/global-ws-bridge.js:177; desktop/packages/web/server/lib/event-stream/protocol.js:77; desktop/packages/web/server/lib/event-stream/protocol.js:87 |
| Independent verifier | codex-sol |
| Regression test | n/a — deferred with owner review 2026-09-11 |
| Owner | ax-code-glm |
| Expiry | 2026-09-11 |

## Proof
Disposition: best-effort dispose/teardown or intentional ignore; not auto-fixed to avoid noise. Tracked residual risk. High-risk kill paths fixed in terminal/pty/auth.

## Impact
Affects `desktop/packages/web/server/lib/event-stream` (desktop, performance).

## Verification
- Evidence path re-read at commit `8556bab68b2232bf9bbf4509092468efa73611af`
- n/a — deferred with owner review 2026-09-11
