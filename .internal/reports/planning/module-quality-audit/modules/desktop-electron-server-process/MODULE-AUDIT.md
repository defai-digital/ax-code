...
Owns `desktop/packages/electron/src/server-process.js` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `desktop/packages/electron/src/server-process.js` | 1 files |
| Tests | `packages/ax-code/test/cli/tui/desktop-handoff.test.ts`, `packages/ax-code/test/cli/tui/process-wire.test.ts`, `packages/ax-code/test/control-plane/workspace-server-sse.test.ts`, `packages/ax-code/test/desktop/webui.test.ts`, `packages/ax-code/test/lsp/server-config.test.ts`, `packages/ax-code/test/lsp/server-defs.test.ts`, `packages/ax-code/test/lsp/server-helpers.test.ts`, `packages/ax-code/test/lsp/server-profile.test.ts` | 25 matched |
...
TODO density: 0. Residual empty-catch candidates: desktop/packages/electron/src/server-process.js:51. Not auto-accepted without reachability proof.
