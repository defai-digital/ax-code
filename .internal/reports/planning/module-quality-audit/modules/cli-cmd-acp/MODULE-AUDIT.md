# MODULE-AUDIT: cli-cmd-acp

| Field | Value |
|-------|-------|
| Unit slug | `cli-cmd-acp` |
| Scope | `packages/ax-code/src/cli/cmd/acp` |
...
| Inventory ID | W6-26 |
| Source files scanned | 0 (0 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/cli/cmd/acp` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | path absent at review | 0 files |
| Tests | `packages/ax-code/test/acp/agent-adapter.test.ts`, `packages/ax-code/test/acp/agent-interface.test.ts`, `packages/ax-code/test/acp/agent-prompt.test.ts`, `packages/ax-code/test/acp/event-subscription.test.ts`, `packages/ax-code/test/acp/sdk-client-naming.test.ts`, `packages/ax-code/test/acp/session-list.test.ts`, `packages/ax-code/test/acp/todo-plan-entries.test.ts`, `packages/ax-code/test/cli/account.test.ts` | 25 matched |
| Prior art | `.internal/reports/reviews/2026-07-19-code-quality-stability-review.md` | linked |

### Public API
Scanned 0 source files for exports/routes/commands.
...
|------|------|------|----------|
| Reviewer | codex-sol | 2026-08-11 | Protocol complete; 0 files scanned |
