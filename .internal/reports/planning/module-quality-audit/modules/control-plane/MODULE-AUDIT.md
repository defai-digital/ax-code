...
| Inventory ID | W1-09 |
| Source files scanned | 17 (2361 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/control-plane` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/control-plane/abort.ts`, `packages/ax-code/src/control-plane/adaptors.ts`, `packages/ax-code/src/control-plane/agent-control-events.ts`, `packages/ax-code/src/control-plane/agent-control-summary.ts`, `packages/ax-code/src/control-plane/agent-control.ts`, `packages/ax-code/src/control-plane/autonomous-completion-gate.ts`, `packages/ax-code/src/control-plane/execution-controller.ts`, `packages/ax-code/src/control-plane/reasoning-policy.ts`, `packages/ax-code/src/control-plane/safety-policy.ts`, `packages/ax-code/src/control-plane/schema.ts`, `packages/ax-code/src/control-plane/sse.ts`, `packages/ax-code/src/control-plane/types.ts` | 17 files |
| Tests | `packages/ax-code/test/cli/tui/agent-control-activity.test.ts`, `packages/ax-code/test/cli/tui/sync-bootstrap-controller.test.ts`, `packages/ax-code/test/control-plane/abort.test.ts`, `packages/ax-code/test/control-plane/agent-control-events.test.ts`, `packages/ax-code/test/control-plane/agent-control-summary.test.ts`, `packages/ax-code/test/control-plane/agent-control.test.ts`, `packages/ax-code/test/control-plane/autonomous-completion-gate.test.ts`, `packages/ax-code/test/control-plane/execution-controller.test.ts` | 17 matched |
| Prior art | `.internal/reports/reviews/2026-07-19-code-quality-stability-review.md` | linked |

### Public API
Scanned 17 source files for exports/routes/commands.
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/cli/tui/agent-control-activity.test.ts` | ok |
...
|------|------|------|----------|
| Reviewer | ax-code-glm | 2026-08-11 | Protocol complete; 17 files scanned |
