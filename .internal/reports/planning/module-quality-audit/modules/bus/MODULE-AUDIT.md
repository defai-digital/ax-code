...
| Inventory ID | W2-13 |
| Source files scanned | 3 (217 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/bus` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/bus/bus-event.ts`, `packages/ax-code/src/bus/global.ts`, `packages/ax-code/src/bus/index.ts` | 3 files |
| Tests | `packages/ax-code/test/bus/bus.test.ts`, `packages/ax-code/test/bus/publish-callsite.test.ts` | 2 matched |
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/bus/bus.test.ts` | ok |
