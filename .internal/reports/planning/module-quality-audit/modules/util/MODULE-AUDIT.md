...
| Inventory ID | W10-04 |
| Source files scanned | 54 (4059 lines) |

## 1. Scope and map

### Purpose and ownership
Owns `packages/ax-code/src/util` within AX Code CLI/Desktop architecture per PRD inventory.

### Source, tests, and artifacts

| Kind | Paths | Notes |
|------|-------|-------|
| Source | `packages/ax-code/src/util/abort.ts`, `packages/ax-code/src/util/archive.ts`, `packages/ax-code/src/util/color.ts`, `packages/ax-code/src/util/concurrency.ts`, `packages/ax-code/src/util/context.ts`, `packages/ax-code/src/util/data-url.ts`, `packages/ax-code/src/util/defer.ts`, `packages/ax-code/src/util/directory-headers.ts`, `packages/ax-code/src/util/env.ts`, `packages/ax-code/src/util/error-message.ts`, `packages/ax-code/src/util/fan-out.ts`, `packages/ax-code/src/util/feature-flags.ts` | 54 files |
| Tests | `packages/ax-code/test/cli/tui/json-util.test.ts`, `packages/ax-code/test/debug-engine/scanner-utils.test.ts`, `packages/ax-code/test/util/concurrency.test.ts`, `packages/ax-code/test/util/data-url.test.ts`, `packages/ax-code/test/util/directory-headers.test.ts`, `packages/ax-code/test/util/env.test.ts`, `packages/ax-code/test/util/error-message.test.ts`, `packages/ax-code/test/util/fan-out.test.ts` | 25 matched |
| Prior art | `.internal/reports/reviews/2026-07-19-code-quality-stability-review.md` | linked |

### Public API
Scanned 54 source files for exports/routes/commands.
...
|-----------|---------------|-----|
| Primary unit behavior | `packages/ax-code/test/cli/tui/json-util.test.ts` | ok |
...
|------|------|------|----------|
| Reviewer | codex-sol | 2026-08-11 | Protocol complete; 54 files scanned |
