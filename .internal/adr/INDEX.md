...
| [ADR-022](ADR-022-codex-like-desktop-app-from-openchamber-baseline.md) | Build a Codex-Like Desktop App from an OpenChamber Product Baseline | Accepted; staged implementation underway, desktop attach/auth preflight, native menu, project picker, backend attach form, network-mode warnings, explicit session creation, queue edit/restart recovery/lifecycle/status boundary guards, policy-bound auto-accept, terminal association, browser preview tabs, browser verification queue, macOS bundle, renderer smoke, diagnostics version metadata, live-history hydration, release evidence artifacts, worktree branch contracts, and canonical fixture catalog added |
