...
用户随后授权将修复应用到当前安装。已针对 `%USERPROFILE%/.ax-code/lib/index-node-tui.js` 制作定点补丁，保留安装包的其他代码和本地改动，未移动 `node_modules`。候选版本通过实际 CLI 验证后，备份原 bundle，再替换入口并复测。版本号仍为 **7.18.2（本地补丁）**。

- 实际候选 CLI：版本检查、前台与后台拒绝原命令、无前置文件副作用、正确 `/dev/null`、普通文件输出均通过。
- 替换后的实际 CLI：版本检查、拒绝原命令、正确输出行为均通过；日常 `ax-code.cmd --version` 也通过。
- 备份文件模式：`index-node-tui.js.before-issue-460-<时间戳>.bak`，原文件 SHA-256 为 `89bd0949e188c13f665589a40a530de380e611d1674680c3738d094c91daee6f`。
- 应用后 SHA-256：`3cf37d306ea898ce13a74e912047eba4e29c5ff4e287db6230b4148ca1d3a12f`。
- 安装检查还发现 AX Engine 注册器会二次覆盖工具描述，已补充该路径的 Shell 占位符替换和真实注册器断言；相关两套测试 **15 项通过**，类型检查通过。

以上安装均使用隔离配置、临时项目和本地工具执行验证，无模型请求。用户原有 `nul` 文件保持原样。已运行的 CLI 会话需要重启才能加载新代码；后续正式升级可能覆盖本地补丁。
