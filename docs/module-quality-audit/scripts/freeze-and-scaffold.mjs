...
const w1 = [
  ...
  [
    ...
    "desktop/packages/electron/src (security policies)",
    "L",
    ["security", "desktop"],
  ],
  [
    ...
    "desktop/packages/electron/src (IPC policy/handlers)",
    "L",
    ["security", "desktop"],
  ],
...
const w2 = [
  [
    ...
    "packages/ax-code/src/session (prompt/processor)",
    "L",
  ...
  [
    ...
    "packages/ax-code/src/session (messages/parts)",
    "L",
  ...
  [
    ...
    "packages/ax-code/src/session (lifecycle/queue)",
    "L",
  ...
  [
    "W2-01e",
    "session-fork-revert",
    "packages/ax-code/src/session (fork/revert/rollback)",
    "M",
...
const w3 = [
  ...
  [
    "W3-03a",
    "tool-mutation",
    "packages/ax-code/src/tool (edit/write/apply_patch/mutation)",
    "L",
  ...
  [
    "W3-03d",
    "tool-orchestration",
    "packages/ax-code/src/tool (task/arena/council/orchestration)",
    "L",
    ["concurrency"],
  ],
...
routeFamilies.forEach((name, i) => {
  add(
    `W4-03-${String(i + 1).padStart(2, "0")}`,
    `server-routes-${name}`,
    `packages/ax-code/src/server/routes/${name}.ts`,
    4,
    "S",
    ["network", "api"],
  )
...
const w5 = [
  ...
  [
    ...
    "packages/ax-code/src/provider (models-snapshot/model data)",
    "M",
    ["correctness"],
  ],
...
for (const [id, slug, scope, size] of cliCmds)
  add(id, slug, scope.startsWith("packages/") ? scope : `packages/ax-code/src/${scope}`, 6, size, ["cli"])
...
const w7 = [
  ...
  [
    "W7-02",
    "desktop-electron-server-process",
    "desktop/packages/electron/src/server-process.js",
    "L",
    ["desktop", "stability"],
  ],
  ...
  [
    "W7-08",
    "desktop-web-event-stream",
    "desktop/packages/web/server/lib/event-stream",
    "L",
    ["desktop", "performance"],
  ],
...
if (fs.existsSync(uiComponents)) {
  componentKids = fs
    .readdirSync(uiComponents, { withFileTypes: true })
...
if (componentKids.length === 0) {
  ...
  componentKids.forEach((name, i) => {
    add(
      `W8-03-${String(i + 1).padStart(2, "0")}`,
      `ui-components-${name}`,
      `desktop/packages/ui/src/components/${name}`,
      8,
      "S",
      ["desktop", "ui"],
    )
...
const w9 = [
  ...
  ["W9-05", "pkg-ax-code-tui-core", "packages/ax-code-tui", "L", ["ui"]],
  ["W9-06", "pkg-ax-code-tui-solid", "packages/ax-code-tui/solid", "L", ["ui"]],
  ["W9-07", "pkg-ax-code-tui-spinner", "packages/ax-code-tui/spinner", "S", ["ui"]],
...
| Source | ${
    files
      .slice(0, 15)
      .map((f) => `\`${f}\``)
      .join(", ") || "(path not present at freeze)"
  } | ${files.length} files discovered |
| Tests | ${
    tests
      .slice(0, 10)
      .map((f) => `\`${f}\``)
      .join(", ") || "(none auto-matched)"
  } | ${tests.length} matched |
