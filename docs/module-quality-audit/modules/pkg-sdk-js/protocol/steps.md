...
- Client side: `packages/sdk/js/src/protocol.ts:12-42` (`assertLocalAxCodeBaseUrl`) parses the URL, allows same-origin relative paths via a `http://localhost` resolution fallback (`protocol.ts:26-28`), and rejects anything whose hostname is not `localhost`/`::1`/`127.x`/`opencode.internal`/`ax-code.internal` (`protocol.ts:32-41`). The v1 factory calls it unconditionally on a provided `baseUrl` (`src/client.ts:13`); v2 does the same (`src/v2/client.ts:18`).
