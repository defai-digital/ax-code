# Review Protocol — pkg-ax-code-tui-core

Reviewer: ax-code-glm · Model: zai-coding-plan/glm-5.2[1m] · Date: 2026-08-11
Scope: `packages/ax-code-tui` (vendored terminal UI core). Independent second-pass
...
The unit `pkg-ax-code-tui-core` exposes its public contract through `packages/ax-code-tui/index.d.ts:1-24`,
...
`packages/ax-code-tui/index-pcvh9d34.js:15104-15106`, where the top-level
...
Because this is a vendored framework consumed by `packages/ax-code-tui/solid` and the `packages/ax-code`
...
`cli/tui/tui-ffi-coordinate-guard.test.ts`, `cli/tui/tui-ffi-pointer-pin.test.ts`,
`cli/tui/tui-spinner.test.ts`, `script/tui-package-integrity.test.ts`, and
...
A single finding exists: `AUDIT-pkg-ax-code-tui-core-empty-catch` (category `silent-error`,
...
Because `pkg-ax-code-tui-core` is a read-only library consumed by the core runtime, the meaningful
...
`tui-ffi-coordinate-guard`, `tui-ffi-pointer-pin`, `tui-spinner`, and
`tui-package-integrity` via `AX_TEST_FILES=…` under `packages/ax-code`. Reviewer (ax-code-glm)
