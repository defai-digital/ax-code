...
**Idle "Allow once" (ADR-138, opt-in):** even the remaining interactive-only prompts can be given a human-veto countdown. With `experimental.permission_idle_once.enabled: true`, an allowlisted interactive permission (default `bash_destructive`) that asks while running unattended (autonomous) with the sandbox off (`full-access`) auto-replies **once** after a 90-second countdown (configurable `timeout_ms`, 5s–300s) — the dialog shows "Allow once (90s)" ticking down and any human answer cancels the deadline. Only the session's oldest pending ask may auto-allow, so queued asks never mass auto-approve, and `isolation_escalation`, `hook`, `ops_approve`, `webmcp`, `computer`, and `external_directory` are always excluded. The server owns the deadline (a detached TUI cannot wedge the turn) and logs every unattended approval. The opt-in config is read live from the same cached config the other permission gates use, so mid-session changes (e.g. turning the sandbox on) take effect on the very next ask; while a config re-read is in flight after an edit the countdown simply does not arm (fail closed). The `AX_CODE_PERMISSION_IDLE_ONCE_MS` debug override is also read live and is capped at the Node `setTimeout` maximum so a huge value cannot collapse into an immediate auto-approve.
