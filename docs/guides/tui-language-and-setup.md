...
1. Choose the interface language. All thirteen languages are available together; use the arrow keys to scroll the list in short terminals.
...
Interface values: `en`, `zh-TW`, `zh-CN`, `ja`, `ko`, `es`, `pt-BR`, `fr`, `de`, `ru`, `id`, `tr`, `vi`. Conversation also accepts `auto`. Defaults are `en` and `auto`. Choices made through `/language` are saved in the local TUI state and override these file defaults. Use `/language` to change them again. Language choices do not change credentials or server policy.
