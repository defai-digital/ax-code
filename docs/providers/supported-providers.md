...
1. `provider.ax-engine.options.binaryPath` in `ax-code.json` (requires a verified version of at least 7.5.7)
2. the `AX_ENGINE_BIN` environment variable (requires a verified version of at least 7.5.7)
3. `ax-engine` on your `PATH` (requires a verified version of at least 7.5.7)
4. an AX Code-managed overlay install (`ax-code providers ax-engine install`; also requires at least 7.5.7)
