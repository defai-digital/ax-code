...
| `zai`            | Current GLM-5.3 SKUs on the PAYG API                  | AX Code's picker starts at GLM-5.3. Connect the `zai` provider, not `zai-coding-plan`, and check [Z.AI pricing](https://docs.z.ai/guides/overview/pricing) for the current no-cost SKU.              |
...
one provider ID or credential flow for the other. The `zai` and `zhipuai` general APIs start at GLM-5.3; older GLM 3,
GLM 4, GLM-5, and GLM-5.2 SKUs stay filtered out.
