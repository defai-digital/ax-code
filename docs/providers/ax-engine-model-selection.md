...
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Ornith 1.5 35B A3B AXQ 6-bit MTP | [AutomatosX/AX-Ornith-1.5-35B-A3B-MLX-AXQ-6bit-MTP](https://huggingface.co/AutomatosX/AX-Ornith-1.5-35B-A3B-MLX-AXQ-6bit-MTP) | Pinned `AutomatosX/AX-Ornith-1.5-35B-A3B-MLX-AXQ-6bit-MTP@<commit>` |
...
Copy the exact model ID from the catalog. For Ornith 1.5 35B A3B, retain its 40-character commit:

```bash
ax-code providers ax-engine prepare \
  --model 'AutomatosX/AX-Ornith-1.5-35B-A3B-MLX-AXQ-6bit-MTP@<40-character-commit>' \
...
Excluded models fail new prepare, download, and managed activation requests before starting work. Existing model records remain available for status and cleanup; the old Ornith 1.0 35B alias and Ornith 1.5 9B IDs are never redirected to Ornith 1.5 35B A3B. Removing an option does not delete its weights or stop an existing server.
...
Session compaction uses the active model's context/output budgets and reserves input headroom. It does not transfer the historical Ornith 1.0 35B model's 256K policy to Ornith 1.5 35B A3B.
