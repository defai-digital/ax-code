...
ax-code is being built by importing features from ax-cli into the OpenCode base. Four medium/low priority features remain from the ax-cli import tracker:
...
These features existed in ax-cli and need to be adapted for ax-code's architecture (Effect, SolidJS TUI, Hono server).
...
| Memory Warmup | System prompt (injector), config (.ax-code/memory.json), CLI |
| Context Stats | Session/prompt (collector), CLI (output), session messages |
...
| Day | Deliverable |
|---|---|
| Day 1 | Context Stats (collector, breakdown, cost, CLI command) |
| Day 2 | Memory Warmup (generator, store, injector, CLI commands) |
| Day 3 | i18n (types, loader, English JSON, 10 language JSONs) |
| Day 4 | Design Check (5 rules, scanner, fixer, CLI command) |

---

*This ADR documents the architectural decisions for implementing features 5-8 in ax-code.*
