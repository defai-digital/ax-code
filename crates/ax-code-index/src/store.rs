...
mod tests {
    ...
    fn test_insert_and_find_nodes() {
        ...
        let result = store
            .find_nodes_by_name("proj1".into(), "alpha".into(), opts)
            .unwrap();
    ...
    fn test_find_nodes_by_prefix() {
        ...
        let opts = r#"{}"#.to_string();
        let result = store
            .find_nodes_by_prefix("proj1".into(), "get".into(), opts)
            .unwrap();
        let found: Vec<NodeRow> = serde_json::from_str(&result).unwrap();
        assert_eq!(found.len(), 2);
    ...
    fn test_nodes_in_file() {
        ...
        let result = store
            .nodes_in_file("proj1".into(), "/src/a.ts".into())
            .unwrap();
        let found: Vec<NodeRow> = serde_json::from_str(&result).unwrap();
        assert_eq!(found.len(), 2);
    ...
    fn test_insert_and_query_edges() {
        ...
        store
            .insert_nodes(serde_json::to_string(&nodes).unwrap())
            .unwrap();
        ...
        store
            .insert_edges(serde_json::to_string(&edges).unwrap())
            .unwrap();
    ...
    fn test_delete_edges_touching_file() {
        ...
        let edges = vec![make_edge("e1", "proj1", "n1", "n2", "/file1.ts")];
        store
            .insert_edges(serde_json::to_string(&edges).unwrap())
            .unwrap();
        ...
        store
            .delete_edges_touching_file("proj1".into(), "/file1.ts".into())
            .unwrap();
    ...
    fn test_upsert_file() {
        ...
        store
            .upsert_file(serde_json::to_string(&f1).unwrap())
            .unwrap();

        let f1_v2 = make_file("f1_v2", "proj1", "/src/main.ts", "sha_v2");
        store
            .upsert_file(serde_json::to_string(&f1_v2).unwrap())
            .unwrap();
    ...
    fn test_get_file() {
        ...
        store
            .upsert_file(serde_json::to_string(&f).unwrap())
            .unwrap();

        let result = store
            .get_file("proj1".into(), "/lib/utils.ts".into())
            .unwrap();
    ...
    fn test_prune_orphan_files() {
        ...
        let result = store
            .prune_orphan_files("proj1".into(), live, "/src/".into())
            .unwrap();
    ...
    fn test_ingest_file_atomic() {
        ...
        store
            .insert_nodes(serde_json::to_string(&old_nodes).unwrap())
            .unwrap();
        let old_edges = vec![make_edge("old_e1", "proj1", "old_n1", "old_n1", "/app.ts")];
        store
            .insert_edges(serde_json::to_string(&old_edges).unwrap())
            .unwrap();
        ...
        let new_edges = vec![make_edge("new_e1", "proj1", "new_n1", "new_n2", "/app.ts")];
        ...
        let result = store
            .nodes_in_file("proj1".into(), "/app.ts".into())
            .unwrap();
        let found: Vec<NodeRow> = serde_json::from_str(&result).unwrap();
        assert_eq!(found.len(), 2);
    ...
    fn test_cursor_upsert_and_get() {
        ...
        store
            .upsert_cursor("proj1".into(), Some("abc123".into()), 42, 17)
            .unwrap();
        ...
        store
            .upsert_cursor("proj1".into(), Some("def456".into()), 100, 50)
            .unwrap();
        let result = store.get_cursor("proj1".into()).unwrap();
        let cursor: CursorRow = serde_json::from_str(&result.unwrap()).unwrap();
    ...
    fn test_clear_project() {
        ...
        store
            ...
            .unwrap();
        store
            .upsert_cursor("proj1".into(), Some("abc".into()), 1, 1)
            .unwrap();
