...
impl IndexStore {
    ...
    pub fn clear_project(&self, project_id: String) -> napi::Result<()> {
        self.with_conn_mut(|conn| {
            ...
            tx.execute(
                "DELETE FROM code_edge WHERE project_id = ?1",
                params![project_id],
            )?;
            tx.execute(
                "DELETE FROM code_node WHERE project_id = ?1",
                params![project_id],
            )?;
            tx.execute(
                "DELETE FROM code_file WHERE project_id = ?1",
                params![project_id],
            )?;
            tx.execute(
                "DELETE FROM code_index_cursor WHERE project_id = ?1",
                params![project_id],
            )?;
            tx.commit()?;
            Ok(())
