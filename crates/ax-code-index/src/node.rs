...
impl IndexStore {
    ...
    pub fn get_node(&self, project_id: String, id: String) -> napi::Result<Option<String>> {
        self.with_conn(|conn| {
            ...
            let result = stmt
                .query_row(params![project_id, id], row_to_node)
                .optional()?;
            match result {
    ...
    pub fn find_nodes_by_name(
        ...
        let opts: FindOpts = serde_json::from_str(&opts_json).unwrap_or(FindOpts {
            kind: None,
            file: None,
            limit: None,
        });
        ...
        self.with_conn(|conn| {
            let mut sql =
                format!("SELECT {SELECT_COLS} FROM code_node WHERE project_id = ?1 AND name = ?2");
            let mut param_values: Vec<Box<dyn rusqlite::types::ToSql>> =
                vec![Box::new(project_id.clone()), Box::new(name.clone())];
    ...
    pub fn count_nodes(&self, project_id: String) -> napi::Result<u32> {
        self.with_conn(|conn| {
            let mut stmt =
                conn.prepare_cached("SELECT count(*) FROM code_node WHERE project_id = ?1")?;
