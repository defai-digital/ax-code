...
fn main() {
    ...
    println!(
        "  JS IntervalTree:      ~500ns/lookup O(n) (vs Rust ~50ns/lookup O(log n) → 10x faster)"
    );
}
...
fn bench_sqlite_bulk_insert() {
    ...
    println!(
        "SQLite bulk insert:  {total} nodes in {:?} ({:.1}us/node)",
        elapsed,
...
fn bench_sqlite_query() {
    ...
    for i in 0..iterations {
      ...
      "SELECT id, name, kind, file FROM code_node WHERE project_id = ?1 AND name = ?2 ORDER BY file LIMIT 50"
        ...
        let _rows: Vec<String> = stmt
            .query_map(rusqlite::params!["proj1", name], |row| row.get(0))
            .unwrap()
            .filter_map(|r| r.ok())
            .collect();
    }
    let elapsed = start.elapsed();
    println!(
        "SQLite findByName:   {iterations} queries in {:?} ({:.1}us/query)",
        elapsed,
...
fn bench_sqlite_count() {
    ...
    let elapsed = start.elapsed();
    println!(
        "SQLite countNodes:   {iterations} queries in {:?} ({:.1}us/query)",
        elapsed,
        elapsed.as_micros() as f64 / iterations as f64
    );
}
...
fn bench_sqlite_ingest_cycle() {
    ...
    let elapsed = start.elapsed();
    println!(
        "SQLite ingest cycle: {iterations} ops in {:?} ({:.1}us/op)",
        elapsed,
        elapsed.as_micros() as f64 / iterations as f64
    );
}
