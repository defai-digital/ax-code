...
fn bench_simple_replace() {
    ...
    for _ in 0..iterations {
        let _ = content
            .find(old)
            .map(|pos| format!("{}{}{}", &content[..pos], new, &content[pos + old.len()..]));
    }
    let elapsed = start.elapsed();
    println!(
        "Simple replace:      {iterations} ops in {:?} ({:.0}ns/op)",
        elapsed,
...
fn bench_seek_sequence() {
    let lines: Vec<String> = (0..500)
        .map(|i| format!("  line {i}: let x{i} = compute({i})"))
        .collect();
    ...
    let elapsed = start.elapsed();
    println!(
        "seekSequence exact:  {iterations} ops in {:?} ({:.0}ns/op)",
        elapsed,
        elapsed.as_nanos() as f64 / iterations as f64
    );
    ...
    let elapsed = start.elapsed();
    println!(
        "seekSequence trim:   {iterations} ops in {:?} ({:.0}ns/op)",
        elapsed,
        elapsed.as_nanos() as f64 / iterations as f64
    );
}
...
fn bench_unified_diff() {
    ...
    let new = old
        .replace("compute(50)", "compute_v2(50)")
    ...
    for _ in 0..iterations {
        ...
        let _ = diff
            .unified_diff()
            .header("a/file.ts", "b/file.ts")
            .to_string();
    }
    let elapsed = start.elapsed();
    println!(
        "Unified diff (200L): {iterations} ops in {:?} ({:.1}us/op)",
        elapsed,
...
fn bench_levenshtein() {
    ...
    let elapsed = start.elapsed();
    println!(
        "Levenshtein:         {iterations} ops in {:?} ({:.0}ns/op)",
        elapsed,
        elapsed.as_nanos() as f64 / iterations as f64
    );
}
...
fn generate_content(lines: usize) -> String {
    (0..lines)
        .map(|i| format!("    let value{i} = compute({i})"))
        .collect::<Vec<_>>()
        .join("\n")
}
