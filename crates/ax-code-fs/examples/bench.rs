...
fn bench_is_ignored() {
    ...
    println!(
        "is_ignored check:    {iterations} ops in {:?} ({:.0}ns/op)",
        elapsed,
...
fn bench_glob_matching() {
    let patterns = ["**/*.ts", "**/*.rs", "src/**/*.tsx", "*.json"];
    let test_paths = [
        "src/index.ts",
        "src/utils/helper.ts",
        "lib/main.rs",
    ...
    let elapsed = start.elapsed();
    println!(
        "Glob match:          {iterations} ops in {:?} ({:.0}ns/op)",
        elapsed,
        elapsed.as_nanos() as f64 / iterations as f64
    );
}
...
fn bench_walk_self() {
    ...
    let elapsed = start.elapsed();
    println!(
        "Walk crates/ dir:    {iterations} walks ({total_files} files each) in {:?} ({:.1}ms/walk)",
        elapsed,
