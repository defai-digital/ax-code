...
fn scan_security(
    ...
    if enabled.contains("path_traversal") {
        for (i, line) in lines.iter().enumerate() {
            ...
            if !patterns.path_join.is_match(line) {
                continue;
            }
            let nearby = nearby_window(&lines, i, 5, 5);
            if patterns.containment.is_match(&nearby) {
                continue;
            }
            let args_start = line.find("path.").unwrap_or(0);
            if patterns.all_literals.is_match(&line[args_start..]) {
                continue;
            }
            findings.push(SecurityFinding {
                file: file.to_string(),
                line: i + 1,
                pattern: "path_traversal".into(),
                severity: "high".into(),
                description: format!(
    ...
    if enabled.contains("command_injection") {
        for (i, line) in lines.iter().enumerate() {
            ...
            findings.push(SecurityFinding {
                file: file.to_string(),
                line: i + 1,
                pattern: "command_injection".into(),
                severity: "high".into(),
                description: format!(
                    "{} with string interpolation/concatenation at line {}",
    ...
    if enabled.contains("env_leak") {
        for (i, line) in lines.iter().enumerate() {
            ...
            if !patterns.env_spread.is_match(line) {
                continue;
            }
            let nearby = nearby_window(&lines, i, 3, 3);
            if patterns.env_sanitize.is_match(&nearby) {
                continue;
            }
            findings.push(SecurityFinding {
                file: file.to_string(),
                line: i + 1,
                pattern: "env_leak".into(),
    ...
    if enabled.contains("missing_validation") {
        for (i, line) in lines.iter().enumerate() {
            ...
            if !patterns.route.is_match(line) {
                continue;
            }
            let after = nearby_window(&lines, i, 0, 5);
            if patterns.validator.is_match(&after) {
                continue;
            }
            findings.push(SecurityFinding {
                file: file.to_string(),
                line: i + 1,
                pattern: "missing_validation".into(),
                severity: "medium".into(),
                description: format!(
    ...
    if enabled.contains("ssrf") {
        for (i, line) in lines.iter().enumerate() {
            ...
            if !patterns.fetch_var.is_match(line) {
                continue;
            }
            let nearby = nearby_window(&lines, i, 10, 2);
            if patterns.ssrf_guard.is_match(&nearby) {
                continue;
            }
            findings.push(SecurityFinding {
                file: file.to_string(),
...
fn lifecycle_rules() -> Vec<ResourceRule> {
    vec![
        ResourceRule {
            ...
            create_re: Regex::new(r#"(\w+)\.(?:on|addEventListener)\s*\(\s*["'`](\w+)["'`]"#)
                .unwrap(),
            cleanup_patterns: vec![
                Regex::new(r"\.(?:off|removeEventListener|removeListener|removeAllListeners)\s*\(")
                    .unwrap(),
            ],
            severity: "medium",
        ...
        ResourceRule {
            ...
            cleanup_patterns: vec![
                Regex::new(r"clearInterval\s*\(").unwrap(),
                Regex::new(r"clearTimeout\s*\(").unwrap(),
            ],
            severity: "high",
    ...
    for scope in &scopes {
        for rule in &rules {
            ...
            for mat in rule.create_re.find_iter(&scope.content) {
                ...
                let has_cleanup = rule
                    .cleanup_patterns
                    .iter()
                    .any(|p| p.is_match(&scope.content));
                if has_cleanup {
                    continue;
                }
    ...
    if enabled.contains("map_growth") && findings.len() < max {
        ...
        for (name, line) in &set_names {
            ...
            findings.push(LifecycleFinding {
                ...
                severity: "low".into(),
                description: format!(
                    "Map '{}' has .set() calls without .delete() or .size guard",
                    name
                ),
                cleanup_location: None,
            });
...
fn has_char_class_diversity(s: &str) -> bool {
    ...
    for ch in s.chars() {
        if !has_upper && ch.is_ascii_uppercase() {
            has_upper = true;
            classes += 1;
        }
        if !has_lower && ch.is_ascii_lowercase() {
            has_lower = true;
            classes += 1;
        }
        ...
        if !has_special && !ch.is_ascii_alphanumeric() {
            has_special = true;
            classes += 1;
        }
        if classes >= 3 {
    ...
    for (i, &line) in lines.iter().enumerate() {
        ...
        if enabled.contains("magic_number") && !is_const_assignment(line) && !enum_re.is_match(line)
        {
            for mat in magic_re.find_iter(trimmed) {
                ...
                if bytes.get(mat.end()).is_some_and(|byte| is_word_byte(*byte)) {
                    continue;
                }
                let val = mat.as_str();
                ...
                if let Ok(n) = val.parse::<f64>() {
                    if (1900.0..=2100.0).contains(&n) && !val.contains('.') {
                        continue;
                    }
                }
                ...
                let sev = if let Ok(n) = val.parse::<f64>() {
                    if n.abs() >= 1000.0 { "medium" } else { "low" }
                } else {
                    "low"
                };
                findings.push(HardcodeFinding {
                    ...
                    line: i + 1,
                    column: mat.start() + 1,
                    kind: "magic_number".into(),
                    value: val.to_string(),
                    suggestion: "Extract to a named constant".into(),
                    severity: sev.into(),
                });
            }
        }
        ...
        if enabled.contains("inline_url") && !is_const_assignment(line) {
            for mat in url_re.find_iter(trimmed) {
                ...
                let val = mat.as_str();
                let sev = if val.contains("localhost")
                    || val.contains("127.0.0.1")
                    || val.contains("0.0.0.0")
                {
                    "low"
                } else {
                    "medium"
        ...
        if enabled.contains("inline_path") {
            for caps in path_re.captures_iter(trimmed) {
                ...
                if caps.get(1).map(|m| m.as_str()) != caps.get(3).map(|m| m.as_str()) {
                    continue;
                }
...
pub fn detect_lifecycle_native(input_json: &str) -> Result<String, String> {
    ...
    let scanned = AtomicUsize::new(0);
    let all_findings: Vec<Vec<LifecycleFinding>> = files
        .par_iter()
        .map(|(rel, full)| {
            scanned.fetch_add(1, Ordering::Relaxed);
            let content = match std::fs::read_to_string(full) {
...
pub fn detect_hardcodes_native(input_json: &str) -> Result<String, String> {
    ...
    let scanned = AtomicUsize::new(0);
    let all_findings: Vec<Vec<HardcodeFinding>> = files
        .par_iter()
        .map(|(rel, full)| {
            scanned.fetch_add(1, Ordering::Relaxed);
            let content = match std::fs::read_to_string(full) {
