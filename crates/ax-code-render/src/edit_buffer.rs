...
use crate::segment::{Segment, TextChunk, FLAG_ASCII_ONLY, MARKER_LINESTART};
use crate::text_buffer::TextBuffer;
use crate::unicode::{calculate_text_width, find_pos_by_width, is_ascii_only, is_word_codepoint, WidthMethod};
...
fn make_chunk(mem_id: u8, byte_start: u32, byte_end: u32, bytes: &[u8], tab: u8, method: WidthMethod) -> TextChunk {
    let text = std::str::from_utf8(bytes).unwrap_or("");
    let ascii = is_ascii_only(text);
    let flags = if !bytes.is_empty() && ascii { FLAG_ASCII_ONLY } else { 0 };
...
impl EditBuffer {
    pub fn new(width_method: WidthMethod) -> EditBuffer {
        ...
        let add_mem_id = tb.registry.register(MemBuffer::Owned(Vec::with_capacity(65536))).unwrap_or(0);
        EditBuffer { tb: Box::new(tb), tb_handle: 0, add_mem_id, add_len: 0, cursor: Cursor::default() }
    }
    ...
    pub fn set_cursor(&mut self, row: u32, col: u32) {
        ...
        self.cursor = Cursor { row: clamped_row, col: clamped_col, desired_col: clamped_col, offset };
    }
    ...
    fn add_buffer_append(&mut self, bytes: &[u8]) -> (u8, u32) {
        ...
        self.tb.registry.replace(self.add_mem_id, MemBuffer::Owned(data));
        (self.add_mem_id, start)
    }

    fn chunk_splitter(tb_tab: u8, method: WidthMethod, bytes_of: impl Fn(&TextChunk) -> Vec<u8> + 'static) -> Box<dyn Fn(&Segment, u32) -> Option<(Segment, Segment)>> {
        Box::new(move |seg: &Segment, weight: u32| {
            let chunk = seg.as_text()?;
            let chunk_weight = chunk.width as u32;
            if weight == 0 {
                return Some((Segment::Text(TextChunk::empty()), Segment::Text(chunk.clone())));
            }
            if weight >= chunk_weight {
                return Some((Segment::Text(chunk.clone()), Segment::Text(TextChunk::empty())));
            }
            let bytes = bytes_of(chunk);
            let text = std::str::from_utf8(&bytes).unwrap_or("");
            let is_ascii = (chunk.flags & FLAG_ASCII_ONLY) != 0;
            let split = find_pos_by_width(text, weight, tb_tab, is_ascii, false, method).byte_offset;
            let left = make_chunk(chunk.mem_id, chunk.byte_start, chunk.byte_start + split, &bytes[..split as usize], tb_tab, method);
    ...
    pub fn insert_text(&mut self, bytes: &[u8]) {
        ...
        if !segments.is_empty() {
            ...
            self.tb.rope.insert_slice_by_weight(insert_offset, &segments, &splitter);
        }
        ...
        if num_breaks > 0 {
            ...
            self.cursor = Cursor { row: new_row, col: new_col, desired_col: new_col, offset: new_offset };
        } else {
            let new_col = cursor.col + inserted_width;
            let new_offset = self.coords_to_offset(cursor.row, new_col);
            self.cursor = Cursor { row: cursor.row, col: new_col, desired_col: new_col, offset: new_offset };
        }
    }
    ...
    pub fn delete_range(&mut self, start: Cursor, end: Cursor) {
        ...
        let splitter = self.make_splitter();
        self.tb.rope.delete_range_by_weight(start_offset, end_offset, &splitter);

        let line_count = self.tb.get_line_count();
        let clamped_row = if start.row >= line_count { line_count.saturating_sub(1) } else { start.row };
        let line_width = if line_count > 0 { self.line_width(clamped_row) } else { 0 };
    ...
    pub fn backspace(&mut self) {
        ...
        if cursor.col == 0 {
            if cursor.row > 0 {
                ...
                self.delete_range(
                    Cursor { row: cursor.row - 1, col: prev_line_width, ..Default::default() },
                    Cursor { row: cursor.row, col: 0, ..Default::default() },
                );
            }
        } else {
            ...
            self.delete_range(
                Cursor { row: cursor.row, col: target_col, ..Default::default() },
                Cursor { row: cursor.row, col: cursor.col, ..Default::default() },
            );
        }
    }
    ...
    pub fn delete_forward(&mut self) {
        ...
        if cursor.col < line_width {
            ...
            self.delete_range(
                Cursor { row: cursor.row, col: cursor.col, ..Default::default() },
                Cursor { row: cursor.row, col: cursor.col + w, ..Default::default() },
            );
        } else if cursor.row + 1 < line_count {
            self.delete_range(
                Cursor { row: cursor.row, col: line_width, ..Default::default() },
                Cursor { row: cursor.row + 1, col: 0, ..Default::default() },
            );
        }
    }
    ...
    pub fn set_text(&mut self, text: &[u8]) {
        ...
        self.add_len = 0;
        let mem_id = self.tb.registry.register(MemBuffer::Owned(text.to_vec())).unwrap_or(0);
        self.tb.set_text_from_mem(mem_id);
        self.set_cursor(0, 0);
    }
    ...
    pub fn delete_line(&mut self) {
        ...
        if cursor.row + 1 < line_count {
            ...
            self.delete_range(
                Cursor { row: cursor.row - 1, col: prev_line_width, ..Default::default() },
                Cursor { row: cursor.row, col: curr_line_width, ..Default::default() },
            );
        } else {
            let curr_line_width = self.line_width(cursor.row);
            self.delete_range(
    ...
    pub fn get_eol(&mut self) -> Cursor {
        ...
        Cursor { row: cursor.row, col: line_width, desired_col: line_width, offset }
    }
    ...
    pub fn next_word_boundary(&mut self) -> Cursor {
        ...
        for (chunk_ref, width, bytes, graphemes, wrap_offsets) in &chunks {
            ...
            if cursor.col < next_cols || passed_cursor {
                ...
                for wb in wrap_offsets.iter() {
                    let (break_col, break_width) =
                        crate::unicode::char_offset_to_column(wb.char_offset, graphemes, &mut grapheme_idx, &mut col_delta);
                    if passed_cursor || break_col > local_cursor_col {
                        ...
                        if target_col <= line_width {
                            ...
                            return Cursor { row: cursor.row, col: target_col, desired_col: target_col, offset };
                        }
                    }
                    if !passed_cursor && break_col == local_cursor_col {
        ...
        if cursor.row + 1 < line_count {
            ...
            return Cursor { row: cursor.row + 1, col: 0, desired_col: 0, offset };
        }
        let offset = self.coords_to_offset(cursor.row, line_width);
        Cursor { row: cursor.row, col: line_width, desired_col: line_width, offset }
    }
    ...
    pub fn prev_word_boundary(&mut self) -> Cursor {
        ...
        if let Some(boundary_col) = last_boundary {
            ...
            return Cursor { row: cursor.row, col: boundary_col, desired_col: boundary_col, offset };
        }
        if cursor.row > 0 {
            let prev_line_width = self.line_width(cursor.row - 1);
            let offset = self.coords_to_offset(cursor.row - 1, prev_line_width);
            return Cursor { row: cursor.row - 1, col: prev_line_width, desired_col: prev_line_width, offset };
        }
        Cursor::default()
    }
