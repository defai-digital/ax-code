...
fn bench_is_ignored() {
    ...
    let matchers: Vec<GlobMatcher> = IGNORE_FILE_PATTERNS
        .iter()
    ...
    report(
        "is_ignored (10 paths/iter)",
        ops * paths.len() as u64,
        t.elapsed(),
    );
    ...
    report(
        "folder-only check (10 paths/iter)",
        ops * paths.len() as u64,
        t.elapsed(),
    );
}
...
fn bench_walk_files() {
    ...
    let repo_root = std::env::current_dir()
        ...
        .and_then(|p| {
            ...
            loop {
                if dir.join("crates").join("Cargo.toml").exists() {
...
fn bench_glob_matching() {
    ...
    let test_paths = [
        ...
        "src/main.rs",
        "lib/utils.js",
        "packages/ax-code/src/tool.ts",
        "README.md",
        "Cargo.toml",
        "src/deep/nested/file.ts",
        "test/bench.rs",
        "packages/ui/src/button.tsx",
        "crates/ax-code-fs/src/lib.rs",
    ];
