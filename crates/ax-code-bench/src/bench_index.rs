...
fn gen_id() -> String {
    ...
    let ts = std::time::SystemTime::now()
        ...
        .as_millis() as u64;
    let hex: String = (0..6)
        .map(|i| format!("{:02x}", ((ts >> (40 - 8 * i)) & 0xFF) as u8))
        .collect();
    let rand_part: String = (0..14)
        .map(|_| BASE62[(rng.random::<u8>() % 62) as usize] as char)
        .collect();
...
fn main() {
    ...
    let t = Instant::now();
    for _ in 0..ops {
        std::hint::black_box(sha256(&d10));
    }
    report("sha256 10 KB", ops, t.elapsed());
    let t = Instant::now();
    for _ in 0..ops {
        std::hint::black_box(sha256(&d100));
    }
    ...
    let t = Instant::now();
    let mut stmt = conn
        .prepare("SELECT id FROM code_node WHERE project_id=?1 AND name=?2")
        .unwrap();
    for i in 0..ops {
        let r: Vec<String> = stmt
            .query_map(
                rusqlite::params!["proj1", format!("func_{}", i % 50)],
                |r| r.get(0),
            )
            .unwrap()
            .filter_map(|r| r.ok())
    ...
    let t = Instant::now();
    let mut stmt = conn
        .prepare("SELECT COUNT(*) FROM code_node WHERE project_id=?1")
        .unwrap();
    for _ in 0..ops {
        std::hint::black_box(
            stmt.query_row(rusqlite::params!["proj1"], |r| r.get::<_, i64>(0))
                .unwrap(),
        );
    }
    ...
    for i in 0..iters {
        ...
        conn.execute(
            "DELETE FROM code_node WHERE project_id='proj1' AND file='/src/f_0.ts'",
            [],
        )
        .unwrap();
        for j in 0..200u64 {
    ...
    println!("\n[Interval Tree]");
    let mut rng = rand::rng();
    let mut ivs: Vec<Interval> = (0..5_000u32)
        .map(|i| {
            let s: u32 = rng.random_range(0..10_000);
            Interval {
                start: s,
                end: s + rng.random_range(1..200u32),
                id: i,
            }
        })
        .collect();
    ivs.sort_by_key(|iv| iv.start);
    let qs: Vec<u32> = (0..100_000u64)
        .map(|_| rng.random_range(0..10_000u32))
        .collect();

    let t = Instant::now();
    for &q in &qs {
        std::hint::black_box(linear_scan(&ivs, q));
    }
    let lin = t.elapsed();
    ...
    let t = Instant::now();
    for &q in &qs {
        std::hint::black_box(sorted_scan(&ivs, q));
    }
    let srt = t.elapsed();
    report("sorted scan (5k intervals)", 100_000, srt);
    println!(
        "  Sorted scan speedup: {:.1}x vs linear",
        lin.as_nanos() as f64 / srt.as_nanos() as f64
    );
