    ...
    if sock.exists() {
        if UnixStream::connect(&sock).is_ok() {
            return Err(napi::Error::from_reason(
                "daemon already running for this project",
            ));
        }
...
pub fn daemon_send(socket_path: String, command: String) -> napi::Result<String> {
    let json = if command.starts_with('{') {
        ...
        match parts[0] {
            "scan" | "status" | "stop" => serde_json::to_string(&Command {
                cmd: parts[0].into(),
                pattern: None,
                path: None,
            })
            .unwrap_or_default(),
            "glob" => serde_json::to_string(&Command {
                cmd: "glob".into(),
                pattern: Some(parts.get(1).unwrap_or(&"**/*").to_string()),
                path: None,
            })
            .unwrap_or_default(),
            "reindex" => serde_json::to_string(&Command {
                cmd: "scan".into(),
                pattern: None,
                path: Some(parts.get(1).unwrap_or(&"").to_string()),
            })
            .unwrap_or_default(),
