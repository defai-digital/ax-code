...
All notable changes to the AX Code runtime, CLI, TUI, server, and SDK are documented in this file. Entries before the
2026-09-02 source-ownership cutover may also describe the former in-repository Desktop application; current Desktop
changes belong to AX Coder.
...
- Default the left navigation rail to 30 columns (24/30/36 still selectable), slim the right session sidebar by four columns, and use `/navigation to hide` and `/sidebar to hide` for the same hide action.
- Add Clear on the left session navigation rail (`/navigation-clear`) to hide historical rows from that list without deleting sessions; `/sessions` still resumes them.
...
- Keep provider error bodies out of the local ensemble ledger, and enforce its retention cap in UTF-8 bytes while discarding incomplete oversized records.
...
- Restore standard Windows PowerShell modules during self-upgrade and ZIP extraction when AX Code was launched from PowerShell 7 through an intermediate process.
...
- Retry short-lived Windows file locks when activating or restoring the installed runtime.
- Verify published SDK files and provenance after publication, including recovery from a dropped provenance request.
...
- Honor skill invocation policies consistently across model discovery, direct loading, and explicit user commands; expose the optional policy fields through SDK 2.5.11.
...
- Fix packaged renderer scripts broken by Chromium argv switches.
...
- Narrow plan/embedding repositories + injected env concerns.
- Update repository-policy allowlist count for tracked PRDs.
...
- Drop prd/planning references after relocation to .internal.
- Relocate PRDs and planning/audit working trees to .internal.
- Complete tool integrity acceptance audit.
- Ci: bump google/osv-scanner-action/osv-scanner-action.
...
- Harden refactor apply path and drop dead engine code.
