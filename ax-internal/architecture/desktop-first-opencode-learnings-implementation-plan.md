...
### Phase 3.0: Permission Lifecycle Readiness

Status: Implemented

Scope:

- Fix the stream idle watchdog so local tool execution and human permission waits do not count as provider stalls.
- Publish a reject reply event when an in-flight permission ask is aborted, so subscribed Desktop/TUI prompts unmount instead of lingering.
- Avoid stale permission prompts when abort happens after listener registration but before the ask event is published.
- Keep the TUI change maintenance-only: reset prompt latches and locally dismiss accepted orphan replies without adding new TUI UX.
- Deduplicate Desktop permission ancestor session fetches across overlapping pending-permission checks.

Validation:

- `pnpm --dir packages/ax-code exec vitest run test/permission/next.test.ts test/session/llm.test.ts`
- `pnpm --dir packages/ax-code typecheck`
- `pnpm --dir desktop/packages/ui type-check`

Commits:

- `6812312f fix: keep permission prompts and local tool waits alive`
- `b6a91e5e fix: avoid stale permission asks after abort`
