 ...
 * packages/ax-code-tui/vendor/<target>/ so the TUI renderer no longer
...
const VENDOR_DIR = join(ROOT, "packages", "ax-code-tui", "vendor")
...
async function updateTarget(target: Target): Promise<Record<string, unknown>> {
  ...
  const tmp = mkdtempSync(join(tmpdir(), "ax-code-tui-vendor-"))
  try {
...
async function main() {
  ...
  if (check) {
    ...
    if (problems.length > 0) {
      ...
      fail(`vendor tree drifted from manifest (${problems.length} problem(s)); run pnpm vendor:tui-native`)
    }
    console.log(`✓ vendored AX Code TUI native libraries match manifest (${TARGETS.length} targets, v${VERSION})`)
    return
  }
