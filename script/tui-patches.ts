...
/**
 * Idempotent applier / checker for required AX Code TUI renderer patches.
 *
 * Usage:
...
const CORE_DIR = join(ROOT, "packages", "ax-code-tui")
...
export function findFfiModule(coreDir = CORE_DIR) {
  ...
  if (!match) throw new Error("Could not locate the @ax-code/tui FFI render module")
  return match
}
...
export function findDefaultParserModule(coreDir = CORE_DIR) {
  ...
  if (!match) throw new Error("Could not locate the AX Code TUI default parser module")
  return match
}
...
function stripUnusedCatalogueImports(source: string) {
  return source.replace(/import\s*\{([^}]+)\}\s*from\s*["']@ax-code\/tui["']/g, (full, inner: string) => {
    ...
    if (!multiline) return `import { ${names.join(", ")} } from "@ax-code/tui"`
    return `import {\n  ${names.join(",\n  ")}\n} from "@ax-code/tui"`
  })
}
...
function main() {
  ...
  if (failed.length > 0)
    fail(`${failed.length} required AX Code TUI patch(es) missing: ${failed.map((item) => item.id).join(", ")}`)
  if (check || apply) console.log("✓ AX Code TUI required patches present")
}
