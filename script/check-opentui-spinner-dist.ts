...
const spinnerRoot = path.join(repoRoot, "packages", "opentui-spinner")
...
run("pnpm", ["--dir", "packages/opentui-spinner", "run", "typecheck"])

const generatedDist = await mkdtemp(path.join(os.tmpdir(), "ax-code-opentui-spinner-dist-"))
try {
  run("pnpm", [
    "--dir",
    "packages/opentui-spinner",
    "exec",
    "tsc",
    "-p",
    "tsconfig.build.json",
  ...
  if (mismatches.length > 0) {
    console.error(`OpenTUI spinner dist is stale:\n${mismatches.map((file) => `  - ${file}`).join("\n")}`)
    console.error("Run: pnpm --dir packages/opentui-spinner run build")
    process.exitCode = 1
  } else {
    console.log(`✓ OpenTUI spinner dist matches source (${generatedFiles.length} files)`)
  }
} finally {
