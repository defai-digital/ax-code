...
[![Release v7.6.1](https://img.shields.io/badge/Release-v7.6.1-2F6FED)](https://github.com/defai-digital/ax-code/releases/tag/v7.6.1)
...
This updates the Windows CLI. AX Code Desktop updates through the Desktop auto-updater, or by downloading the latest `AX-Code-<version>-win-x64.exe` / `AX-Code-<version>-win-arm64.exe` installer from GitHub Releases. For unattended Desktop installs, run the NSIS installer with `/S` (for example `.\AX-Code-7.6.1-win-x64.exe /S`).
