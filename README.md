...
- **Session Trust** (v3.2) — in-TUI trust panel with human-readable quality signals (test status, confidence, execution stats), semantic diff with change risk, risk drivers, and plan summary; slash commands: `/trust`, `/graph` (execution graph with topology), `/branches` (branch ranking with advisor), `/compare` (side-by-side session comparison), `/rollback` (step-level rollback points), `/dre-dashboard` (browser-based DRE visualization); auto-route intelligently classifies ambiguous messages to the right specialist agent using a fast LLM pass that also drives complexity-based model selection
