...
nix run nixpkgs#opencode           # 或用 github:defai-digital/ax-code 获取最新 dev 分支
```
...
ax-code 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/defai-digital/ax-code/releases) 或 [opencode.ai/download](https://opencode.ai/download) 下载。
...
如果你在项目名中使用了 “opencode”（如 “opencode-dashboard” 或 “opencode-mobile”），请在 README 里注明该项目不是 ax-code 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [ax-code Zen](https://opencode.ai/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
