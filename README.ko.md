...
nix run nixpkgs#opencode           # 또는 github:defai-digital/ax-code 로 최신 dev 브랜치
```
...
ax-code 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/defai-digital/ax-code/releases) 에서 직접 다운로드하거나 [opencode.ai/download](https://opencode.ai/download) 를 이용하세요.
...
ax-code 설정에 대한 자세한 내용은 [**문서**](https://opencode.ai/docs) 를 참고하세요.
...
ax-code 와 관련된 프로젝트를 진행하면서 이름에 "ax-code"(예: "opencode-dashboard" 또는 "opencode-mobile") 를 포함한다면, README 에 해당 프로젝트가 ax-code 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.
...
- 100% 오픈 소스
- 특정 제공자에 묶여 있지 않습니다. [ax-code Zen](https://opencode.ai/zen) 을 통해 제공하는 모델을 권장하지만, ax-code 는 Claude, OpenAI, Google 또는 로컬 모델과도 사용할 수 있습니다. 모델이 발전하면서 격차는 줄고 가격은 내려가므로 provider-agnostic 인 것이 중요합니다.
- 기본으로 제공되는 LSP 지원
